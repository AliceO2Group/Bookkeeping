...
class EndRunUseCase {
     ...
     */
    async execute(dto) {
        const { body: runPatch, params: { runNumber } } = dto;

        const { lhcPeriod } = runPatch;
        delete runPatch.lhcPeriod;

        try {
            return { result: await runService.update({ runNumber }, { runPatch, relations: { lhcPeriodName: lhcPeriod } }) };
        } catch (e) {
            return {
                error: {
                    status: '400',
