 ...
 */
exports.updateRunDetector = async (runNumber, detectorId, runDetectorPatch, transaction) => await TransactionHelper.provide(
    async () => {
        const runDetectorModel = await getRunDetectorOrFail(runNumber, detectorId, (queryBuilder) => queryBuilder.include('detector'));
        return RunDetectorsRepository.update(runDetectorModel, runDetectorPatch);
    },
    { transaction },
);
