...
{
GrpcQcFlagServiceClient::GrpcQcFlagServiceClient(const std::shared_ptr<::grpc::ChannelInterface>& channel, const std::shared_ptr<::grpc::ClientContext>& clientContext)
{
  ...
  const std::vector<QcFlag>& qcFlags)
{
  ...
  auto status = mStub->CreateForDataPass(mClientContext.get(), request, &response);
  if (!status.ok()) {
  ...
  const std::vector<QcFlag>& qcFlags)
{
  ...
  auto status = mStub->CreateForSimulationPass(mClientContext.get(), request, &response);
  if (!status.ok()) {
    throw std::runtime_error(status.error_message());
  }
  ...
  const std::vector<QcFlag>& qcFlags)
{
  ...
  auto status = mStub->CreateSynchronous(mClientContext.get(), request, &response);
  if (!status.ok()) {
    throw std::runtime_error(status.error_message());
  }
