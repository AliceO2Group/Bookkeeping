...
{
GrpcCtpTriggerCountersServiceClient::GrpcCtpTriggerCountersServiceClient(const std::shared_ptr<::grpc::ChannelInterface>& channel, const std::shared_ptr<::grpc::ClientContext>& clientContext)
{
...
void GrpcCtpTriggerCountersServiceClient::createOrUpdateForRun(uint32_t runNumber, const std::string& className, int64_t timestamp, uint64_t lmb, uint64_t lma, uint64_t l0b, uint64_t l0a, uint64_t l1b, uint64_t l1a)
{
  ...
  auto status = mStub->CreateOrUpdateForRun(mClientContext.get(), request, &response);
  if (!status.ok()) {
