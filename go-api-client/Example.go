...
func main() {
	...
	// Create a run
	apiClient.CreateRun("go-api-Timestamp", 5, 5, 1, 80, "COSMICS", false, false, true, "normal",
		(sw.CPV_Detectors + "," + sw.ZDC_Detectors + "," + sw.EMC_Detectors))

	// Update a run
	apiClient.UpdateRun(80, sw.BAD_RunQuality, -1, testTime, -1, testTime, "LTU", "Repository Hash", "production/production.desc", "processing", "lhc22b")
