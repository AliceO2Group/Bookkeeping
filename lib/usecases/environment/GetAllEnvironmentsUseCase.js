    ...
    async execute(dto = {}) {
        ...
        if (filter?.statusHistory) {
            ...
            for (let i = 0; i < filters.length; i++) {
                // D has a special case, it needs to read the next character
                if (filters[i] === 'D') {
                    if (filters[i + 1] === 'L') {
                        statusFilters.push(acronymToStatus[statusAcronyms.DEPLOYED]);
                        // Skip next iteration
                        i++;
                    } else if (filters[i + 1] === 'S') {
                        statusFilters.push(acronymToStatus[statusAcronyms.DESTROYED]);
                        i++;
                    }
                } else if (Object.values(statusAcronyms).includes(filters[i])) {
                    /*
