...
class CreateEnvironmentUseCase {
     ...
     */
    async execute(dto) {
        const { body: { envId, rawConfiguration, createdAt, status, statusMessage } } = dto;

        try {
            return { result: await environmentService.create({ id: envId, rawConfiguration, createdAt }, { status, statusMessage }) };
        } catch (error) {
            let response = {
                status: '400',
                title: error.message,
