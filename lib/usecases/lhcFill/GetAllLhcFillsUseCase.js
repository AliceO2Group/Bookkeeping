...
class GetAllLhcFillsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            const { beamsMode, fillNumbers, schemeName, beamDurationOperator, beamDuration, runDurationOperator, runDuration, beamsType } = filter;
            if (beamsMode) {
                switch (beamsMode) {
                    case 'STABLE BEAMS':
            ...
            if (beamsType) {
                ...
                queryBuilder.where('beamType').oneOfSubstrings(beamTypes);
            }
        }
