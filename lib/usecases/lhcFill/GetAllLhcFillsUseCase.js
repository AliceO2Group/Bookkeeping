...
class GetAllLhcFillsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            const { hasStableBeams, fillNumbers, schemeName, beamDurationOperator,
                beamDuration, runDurationOperator, runDuration, beamsType, sbStart, sbEnd } = filter;
            if (hasStableBeams) {
            ...
            // Stable beam start and end filters.
            if (sbStart) {
                const from = sbStart.from !== undefined ? sbStart.from : 0;
                const to = sbStart.to !== undefined ? sbStart.to : new Date().getTime();
                queryBuilder.where('stableBeamsStart').between(from, to);
            }

            if (sbEnd) {
                const from = sbEnd.from !== undefined ? sbEnd.from : 0;
                const to = sbEnd.to !== undefined ? sbEnd.to : new Date().getTime();
