...
const { lhcFillSummary } = require('../../server/views/lhcFills/summary.js');
...
class GetAllLhcFillsUseCase {
    ...
    async execute(dto = {}) {
        ...
        const { count, rows } = await TransactionHelper.provide(async () => {
            queryBuilder.selectAttributes([...lhcFillSummary.attributes]);
            queryBuilder.includeAll([...lhcFillSummary.include]);
            queryBuilder.whereAssociation('runs', 'definition', false).is(RunDefinition.PHYSICS);
        ...
        return {
            count,
            lhcFills: rows.map(lhcFillAdapter.toSummary),
        };
    }
}
