 ...
 */
class GetLhcFillsByRunnumberUseCase {
    /**
    ...
    async execute(dto) {
        ...
        const runs = await TransactionHelper.provide(() => RunRepository.findAll(queryBuilder));
        return runs ? runs.map((run) => runAdapter.toEntity(run)) : null;
    }
}

module.exports = GetLhcFillsByRunnumberUseCase;
