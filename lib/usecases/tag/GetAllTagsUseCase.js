...
class GetAllTagsUseCase {
    ...
    async execute(dto = {}) {
        ...
        queryBuilder.orderBy((sequelize) => sequelize.literal('archived_at IS NULL'), 'DESC');
        ...
        const {
            ...
            if (filter) {
                const { ids, texts, partialText, emails, mattermosts } = filter;
                if (ids) {
                ...
                if (texts) {
                    ...
                    queryBuilder.where('text').substring(partialText);
                }
                if (emails) {
