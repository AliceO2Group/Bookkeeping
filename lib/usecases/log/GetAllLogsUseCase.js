...
class GetAllLogsUseCase {
    ...
    async execute(dto = {}) {
        ...
        const {
            ...
            if (filter && filter.runIdentifier) {
                const runQueryBuilder = new QueryBuilder();
                runQueryBuilder.include({
                    association: 'run',
                    where: { runNumber: { [Op.in]: filter.runIdentifier.values } },
                }).orderBy('logId', 'asc');

                let logRuns;
                switch (filter.runIdentifier.operation) {
                    case 'and':
                        logRuns = await LogRunsRepository
                            .findAllAndGroup(runQueryBuilder);
                        logRuns = logRuns
                            .filter((logRun) => filter.runIdentifier.values.every((runNumber) => logRun.runNumbers.includes(runNumber)));
                        break;
                    case 'or':
                        logRuns = await LogRunsRepository
