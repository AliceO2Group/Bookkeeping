...
const applyFilter = async (dataSource, queryBuilder, filter) => {
    ...
    if (author) {
        ...
        const filters = author.split(',').map((filter) => filter.trim());

        filters.forEach((filter) => {
            const { excl, strippedFilter } = checkForFilterExclusion(filter);

            if (strippedFilter !== '') {
                excl
                    ? userQueryBuilder.where('name').not().substring(strippedFilter)
