...
const { logSummary } = require('../../server/views/logs/summary.js');
...
const applyFilter = async (dataSource, queryBuilder, filter) => {
    ...
    if (rootLog) {
        ...
        queryBuilder.where('rootLogId').is(null);
    }
...
class GetAllLogsUseCase {
    ...
    async execute(dto = {}) {
        ...
        queryBuilder.selectAttributes([...logSummary.attributes]);
        queryBuilder.includeAll([...logSummary.include]);
        ...
        const { count, rows } = await dataSource.transaction(async () => {
            ...
            // Separate the find all and count query builder because count is not optimized when using LEFT OUTER JOIN
            const queryBuilderImplementation = queryBuilder.toImplementation('Log');
            const [rows, count] = await Promise.all([
                LogRepository.findAll(queryBuilder),
                LogRepository.count({ where: queryBuilderImplementation.where, replacements: queryBuilderImplementation.replacements }),
            ]);
            return { rows, count };
        });
        ...
        return {
            count,
            logs: rowsWithReplies.map(logAdapter.toSummary),
        };
    }
}
