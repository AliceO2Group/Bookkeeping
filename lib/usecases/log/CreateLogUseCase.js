...
class CreateLogUseCase {
    ...
    async execute(dto) {
        ...
        let tags = null;
        const { log, error } = await TransactionHelper.provide(async () => {
            body.userId = dto?.session?.id || null;

            try {
                body.rootLogId = await this._getRootLogId(parentLogId);
                tags = await this._getAllTagsFromTextOrFail(tagsTexts);
        ...
        if (!error && tags) {
            await TransactionHelper.provide(() => LogTagsRepository.bulkInsert(tags.map((tag) => ({ logId: log.id, tagId: tag.id }))));
        }

        if (!error && files && files.length > 0) {
            await TransactionHelper.provide(() => AttachmentRepository.bulkInsert(files.map((file) => ({ ...file, logId: log.id }))));
        }

        if (!error && body.runIds && body.runIds.length > 0) {
            await TransactionHelper.provide(() => LogRunNumberRepository.bulkInsert(body.runIds.map((runId) => ({ runId, logId: log.id }))));
        }
     ...
     * @return {Promise<object[]>} the list of tags
     * @private
     */
    async _getAllTagsFromTextOrFail(tagsTexts) {
        if (!tagsTexts) {
            return null;
        }

        const tags = await getTagsByTextsUseCase(tagsTexts);
