...
class CreateLogUseCase {
    ...
    async execute(dto) {
        ...
        const { parentLogId, tags: tagsTexts, runNumbers } = body;
            ...
            if (tagsTexts) {
                ...
                if (missingTags.length > 0) {
                    return {
                        error: {
                            status: '400',
                            title: `Tags ${missingTags.join(', ')} could not be found`,
                        },
                    };
                }
            }
        ...
        if (!log.error && tags) {
            await TransactionHelper
                .provide(() => LogTagsRepository.bulkInsert(tags.map((tag) => ({ logId: log.id, tagId: tag.id }))));
        }
