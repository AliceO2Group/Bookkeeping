...
class CreateLogUseCase {
    ...
    async execute(dto) {
        ...
        const log = await TransactionHelper.provide(async () => {
            ...
            if (runNumbers) {
                // Parse comma-seperated string to array of numbers
                ...
                if (numbers.some(Number.isNaN)) {
                    return {
                        error: {
                            status: '400',
                            title: `Run numbers should be comma-seperated, and should only contain numbers`
                        }
                    };
                }

                // Check if, for each runNumber, there is a run present in the DB with that runNumber, and add the run's id to runIds if there is.
                const runIds = [];
        ...
        if (!log.error && body.runIds && body.runIds.length > 0) {
            await TransactionHelper
                .provide(() => LogRunNumberRepository.bulkInsert(body.runIds.map(runId => ({ runId, logId: log.id }))));
        }
