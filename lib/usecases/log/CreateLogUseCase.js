...
class CreateLogUseCase {
    ...
    async execute(dto) {
        ...
        const { tags: tagsTexts, runNumbers: serializedRunNumbers, environments, lhcFills } = body;
        ...
        const lhcFillNumbers = lhcFills ? [...new Set(lhcFills)] : [];

        body.userId = dto?.session?.id || null;

        try {
            return { result: await logService.create(body, runNumbers, tagsTexts || [], environments || [], lhcFillNumbers || [], files || []) };
        } catch (e) {
            return {
                error: {
                    status: '400',
