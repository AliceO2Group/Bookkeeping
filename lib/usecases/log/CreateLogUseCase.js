...
class CreateLogUseCase {
    ...
    async execute(dto, transaction) {
        ...
        /**
         * @type {{log: [Object], error: [Object]}}
         */
        const { /** @type {Object|undefined} */ log, /** @type {Object|undefined} */ error } = await TransactionHelper.provide(
            async () => {
        ...
        if (!transaction) {
            ...
            transaction.afterCommit(async () => {
                await this._notifyCreation(await new GetLogUseCase().execute({ params: { logId: log.id } }));
            });
     ...
     *
     * @param {Object|null} log the log to notify
     * @return {Promise<void>} notification sent
     * @private
     */
    async _notifyCreation(log) {
