...
const GetAllTagsUseCase = require('../tag/GetAllTagsUseCase.js');
...
class UpdateRunUseCase {
    ...
    async execute(dto) {
        ...
        const run = await TransactionHelper.provide(async () => this._updateRun(runId, runNumber, body));
        if (run.error) {
            return {
                error: {
                    status: 500,
                    title: 'ServiceUnavailable',
                    detail: run.error.message || `Unable to update run with id ${runId ? runId : runNumber}`,
                },
            };
        }
    ...
    async _updateRun(runId, runNumber, runUpdates) {
        try {
            ...
            if (!runObject) {
                throw new Error(`Run with this ${runId ? `id (${runId})` : `runNumber (${runNumber})`} could not be found`);
            } else {
                if (runQuality) {
                ...
                if (aliceL3Polarity) {
                    runObject.aliceL3Polarity = aliceL3Polarity;
                }
                if (aliceDipolePolarity) {
                    runObject.aliceDipolePolarity = aliceDipolePolarity;
                }
                if (fillNumber) {
                ...
                if (tags) {
                    ...
                    if (missingTags.length > 0) {
                        throw new Error(`Tags with these ids (${missingTags.join(', ')}) could not be found`);
                    }

                    // Remove existing tags of the run and insert new ones
                    await RunTagsRepository.removeById(runId)
                        .then(() => RunTagsRepository.insertMany(tags.map((tag) => ({ runId, tagId: tag }))));
                }
