...
class UpdateRunUseCase {
    ...
    async execute(dto) {
        ...
        const { runQuality, envId, lhcBeamEnergy, lhcBeamMode, lhcBetaStar, aliceL3Current, aliceDipoleCurrent, lhcFillId } = body;
            ...
            if (runObject) {
                ...
                if (lhcFillId) {
                    const lhc = await new GetLhcFillUseCase()
                        .execute({ params: { lhcFillId } });
                    if (!lhc) {
                        return {
                            error: {
                                status: '400',
                                title: `LhcFill with id ('${lhcFillId}') could not be found`,
                            },
                        };
                    }
                    runObject.lhcFillId = lhcFillId;
                }
