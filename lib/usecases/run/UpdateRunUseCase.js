...
class UpdateRunUseCase {
    ...
    async execute(dto) {
        ...
        try {
            const { eorReasons, tags: tagsTexts, detectorsQualities: detectorsQualities } = body;
            ...
            if (detectorsQualities) {
                for (const i in detectorsQualities) {
                    await runDetectorService.update(
                        run.result.runNumber,
                        detectorsQualities[i].detectorId,
                        { quality: detectorsQualities[i].quality },
                    );
                }
            }
            return run;
