...
class GetFlpsByRunUseCase {
    ...
    async execute(dto) {
        ...
        const flps = await TransactionHelper.provide(async () => {
            const run = await RunRepository.findOne(queryBuilder);
            return run ? FlpRepository.findAllByRunId(run.id) : null;
        });

        if (flps) {
            const flpsWithReplies = await FlpRepository.addChildrenCountByRootFlp(flps);
            return flpsWithReplies.map(FlpAdapter.toEntity);
        } else {
            return null;
        }
    }
