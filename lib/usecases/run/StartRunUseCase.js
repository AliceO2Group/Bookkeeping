...
const { RunTypeRepository } = require('../../database/repositories');
...
class StartRunUseCase {
    ...
    async _createRun(run) {
        ...
        const runType = await this._getRunType();
        run.runType = null;
    ...
    /**
     * Creates a run type if not exists
     * @param {String} runType name of run type
     *
     * @returns {RunType} entity of run type
     */
    async _getRunType(runType) {
        const queryBuilder = new QueryBuilder()
            .where('name').is(runType);
        const existingRunType = await RunTypeRepository.findOne(queryBuilder);
        if (existingRunType) {
            return RunTypeRepository.toEntity(existingRunType);
        }
    }
}
