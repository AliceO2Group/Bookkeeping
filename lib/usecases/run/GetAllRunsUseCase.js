...
const { qcFlagService } = require('../../server/services/qualityControlFlag/QcFlagService.js');
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (detectorsQc) {
                ...
                const { [true]: dplDetectorIdsWithMcAsNotBad, [false]: dplDetectorIdsWithMcAsBad } = detectorsQc.reduce(
                    (acc, { dplDetectorId, mcReproducibleAsNotBad }) => {
                        acc[mcReproducibleAsNotBad].push(dplDetectorId); return acc;
                    },
                    { [true]: [], [false]: [] },
                );
                const qcSummary1 = await qcFlagSummaryService.getSummary(
                    { dataPassId, simulationPassId, lhcPeriodId, dplDetectorIds: dplDetectorIdsWithMcAsBad },
                    { mcReproducibleAsNotBad: false },
                );
                const qcSummary2 = await qcFlagSummaryService.getSummary(
                    { dataPassId, simulationPassId, lhcPeriodId, dplDetectorIds: dplDetectorIdsWithMcAsNotBad },
                    { mcReproducibleAsNotBad: true },
                );
                const qcSummary = { ...qcSummary1, ...qcSummary2 };

                for (detectorQc of detectorsQc) {
                    const { operator, limit: value } = detectorsQc.notBadFraction;
