...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (dataPassIds) {
                const runNumbers = (await RunRepository.findAll(dataSource.createQueryBuilder()
                    .set('attributes', ['runNumber'])
                    .set('raw', true)
                    .setModel('Run')
                    .whereAssociation('dataPass', 'id').oneOf(...dataPassIds))).map(({ runNumber }) => runNumber);
