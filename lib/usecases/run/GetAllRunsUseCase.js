...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            const {runNumber, environmentId, o2start, o2end, trgStart, trgEnd, nDetectors, nFlps, ddflp, epnTopology, detectors } = filter;

            if (runNumber) {
                queryBuilder.where('runNumber').substring(runNumber);
            }
            if (o2start) {
            ...
            if (trgStart) {
                const from = trgStart.from !== undefined ? trgStart.from : 0;
                const to = trgStart.to !== undefined ? trgStart.to : new Date().getTime();
                queryBuilder.where('timeTrgStart').between(from, to);
            }
            if (trgEnd) {
                const from = trgEnd.from !== undefined ? trgEnd.from : 0;
                const to = trgEnd.to !== undefined ? trgEnd.to : new Date().getTime();
                queryBuilder.where('timeTrgEnd').between(from, to);
            }
            if (environmentId) {
                queryBuilder.where('environmentId').substring(environmentId);
            }
            if (nDetectors) {
                queryBuilder.where('nDetectors').substring(nDetectors);
            }
            if (nFlps) {
                queryBuilder.where('nFlps').substring(nFlps);
            }
            if(ddflp){
