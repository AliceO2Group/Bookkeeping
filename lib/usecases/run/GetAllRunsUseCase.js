...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (detectorsQcNotBadFraction) {
                const [dataPassId] = dataPassIds ?? [];
                const [simulationPassId] = simulationPassIds ?? [];
                const [lhcPeriodId] = lhcPeriodIds ?? [];
                const { mcReproducibleAsNotBad } = detectorsQcNotBadFraction;
                delete detectorsQcNotBadFraction.mcReproducibleAsNotBad;

                const detectorsId = Object.keys(detectorsQcNotBadFraction).map((id) => parseInt(id.slice(1), 10));
                if (detectorsId.length > 0) {
                    const scope = { dataPassId, simulationPassId, lhcPeriodId, detectorsId };
                    const qcSummary = await qcFlagSummaryService.getSummary(scope, { mcReproducibleAsNotBad });

                    const runNumbers = Object.entries(qcSummary)
                        .filter(([_, runSummary]) => {
                            const mask = Object.entries(detectorsQcNotBadFraction).map(([prefixedDetectorId, { operator, limit }]) => {
                                const detectorId = parseInt(prefixedDetectorId.slice(1), 10);
                                if (!(detectorId in runSummary)) {
                                    return false;
                                }
                                return badEffectiveRunCoverageComparison(runSummary[detectorId].badEffectiveRunCoverage, operator, limit);
                            });
        ...
        if (effectiveQcFlags) {
            ...
            const detectorTypesOfNoneExportableAnonymousFlagsEscaped = [DetectorType.AOT_GLO, DetectorType.AOT_EVENT, DetectorType.MUON_GLO]
                .map((n) => `'${n}'`)
                .join(',');
            ...
            if (dataPassId) {
                qcFlagsAssociationDef.include.push({ association: 'dataPasses', required: true, where: { id: dataPassId } });
                fetchQueryBuilder.include(qcFlagsAssociationDef);
            } else if (simulationPassId) {
                qcFlagsAssociationDef.include.push({ association: 'simulationPasses', required: true, where: { id: simulationPassId } });
                fetchQueryBuilder.include(qcFlagsAssociationDef);
            } else {
