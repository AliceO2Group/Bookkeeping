...
const { EorReasonRepository, QcFlagRepository } = require('../../database/repositories');
...
const { qcFlagService } = require('../../server/services/qualityControlFlag/QcFlagService');
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (aliceL3Current !== undefined) {
                 ...
                 */
                const computedColumn = ({ literal }) => literal("ROUND(alice_l3_current * IF(`alice_l3_polarity` = 'NEGATIVE', -1, 1) / 1000)");
            ...
            if (gaq) {
                ...
                const { notBadFraction, mcReproducibleAsNotBad = false } = gaq;
                const [dataPassId] = dataPassIds;
                const gaqSummary = await qcFlagService.getGaqSummary(dataPassId, { mcReproducibleAsNotBad });
                const runNumbers = Object.entries(gaqSummary)
                    .filter(([_, { badEffectiveRunCoverage }]) => badEffectiveRunCoverage === notBadFraction)
                    .map(([runNumber]) => runNumber);
                filteringQueryBuilder.where('runNumber').oneOf(...runNumbers);
            }
