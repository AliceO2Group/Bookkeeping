...
const { ReasonTypeRepository, EorReasonRepository } = require('../../database/repositories');
...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (eor) {
                let eorReasonTypeWhere = {category: { [Op.eq]: eor.category }}
                if (eor.title) eorReasonTypeWhere.title = { [Op.eq]: eor.title }
