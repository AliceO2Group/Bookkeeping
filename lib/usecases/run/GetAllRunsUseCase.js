...
const { COSMICS_OR_PHYSICS_TF_BUILDER_MODE, RunDefinition, CALIBRATION_RUN_TYPES_STARTS_WITH } = require('../../services/getRunDefinition.js');
...
class GetAllRunsUseCase {
    ...
    async fetchAndSetRunDetectors(runs) {
        ...
        const detectorQueryBuilder = new QueryBuilder()
            .where('$runs.run_number$').oneOf(runs.map(({ runNumber }) => runNumber))
            .include('runs');
