...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            // Added filtering logic for find by pattern.
            if (runNumberPattern) {
                const runsWithPattern = await RunRepository.findAll({
                    where: { runNumber: { [Op.like]: `%${runNumberPattern}%` } },
                    raw: true,
                });
                filteringQueryBuilder.where('runNumber').oneOf(...runsWithPattern.map(({ id }) => id));
            }
        }
