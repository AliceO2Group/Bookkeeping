    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (aliceL3Current) {
                 ...
                 */
                const computedColumn = ({ literal }) => literal("alice_l3_current * IF(`alice_l3_polarity` = 'NEGATIVE', -1, 1)");
                for (const [operator, limit] of Object.entries(aliceL3Current)) {
                    filteringQueryBuilder.where(computedColumn).applyOperator(operator, limit);
                }
            }
            if (aliceDipoleCurrent) {
                 ...
                 * @return {Object} the object representing the column
                 */
                const computedColumn = ({ literal }) => literal("alice_dipole_current * IF(`alice_dipole_polarity` = 'NEGATIVE', -1, 1)");
                for (const [operator, limit] of Object.entries(aliceDipoleCurrent)) {
                    filteringQueryBuilder.where(computedColumn).applyOperator(operator, limit);
                }
            }
            if (runDuration) {
