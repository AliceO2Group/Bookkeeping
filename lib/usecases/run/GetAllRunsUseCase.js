...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (ctfFileCount) {
                const { operator, limit: ctfFileCountLimit } = ctfFileCount;
                filteringQueryBuilder.where('ctfFileCount').applyOperator(operator, ctfFileCountLimit);
            }
            if (tfFileCount) {
                const { operator, limit: tfFileCountLimit } = tfFileCount;
                filteringQueryBuilder.where('tfFileCount').applyOperator(operator, tfFileCountLimit);
            }
            if (otherFileCount) {
                const { operator, limit: otherFileCountLimit } = otherFileCount;
                filteringQueryBuilder.where('otherFileCount').applyOperator(operator, otherFileCountLimit);
            }
            if (nEpns) {
