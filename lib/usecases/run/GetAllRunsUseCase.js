...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            const { runNumbers, environmentId, runQuality, o2start, o2end, nDetectors, nFlps, ddflp, dcs, epn, epnTopology, detectors } = filter;

            if (runNumbers) {
                const runNumberList = runNumbers.split(',').filter((runNumber) => parseInt(runNumber, 10));
            ...
            if (dcs === false) {
                queryBuilder.where('dcs').isOrNull(false);
            } else if (dcs === true) {
