    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (detectorsQc) {
                ...
                const { mcReproducibleAsNotBad, operator = 'and' } = detectorsQc;
                ...
                const qcSummary = await qcFlagSummaryService.getSummary(
                    { dataPassId, simulationPassId, lhcPeriodId, dplDetectorIds: detectorsQc.keys() },
                ...
                const runNumbers = Object.entries(qcSummary)
                    .filter(([_, runSummary]) => {
                        const mask = Object.entries(runSummary).map((dplDetecotrId, { badEffectiveRunCoverage }) => {
                            if (!(dplDetecotrId in detectorsQc)) {
                                return false;
                            }
                            const { operator: forDetectorOperator, limit: value } = detectorsQc[dplDetecotrId].notBadFraction;
                            return badEffectiveRunCoverageComparison(badEffectiveRunCoverage, forDetectorOperator, value);
                        });
