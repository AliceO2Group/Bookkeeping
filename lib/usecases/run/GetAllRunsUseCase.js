    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (lhcPeriods) {
                ...
                const runsWithExpectedLHCPeriod = await RunRepository.findAll({
                    attributes: ['id'],
                    include: [
                        {
                ...
                console.log('TOBECERTAIN', runsWithExpectedLHCPeriod)
                filteringQueryBuilder.where('id').oneOf(...runsWithExpectedLHCPeriod.map(({ id }) => id));
            }
