...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (aliceL3Current) {
                const { operator, limit: aliceL3CurrentLimit } = aliceL3Current;

                /**
                 * The alice l3 current computed with its absolute value and polarity
                ...
                filteringQueryBuilder.where(computedColumn).applyOperator(operator, aliceL3CurrentLimit);
            }
            if (runDuration) {
