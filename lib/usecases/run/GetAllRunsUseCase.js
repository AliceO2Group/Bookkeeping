    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (endOfRunReason) {
                ...
                if (endOfRunReason.category) {
                    eorReasonTypeWhere.category = { [Op.eq]: endOfRunReason.category };
                }
                if (endOfRunReason.title) {
                    eorReasonTypeWhere.title = { [Op.eq]: endOfRunReason.title };
                }
                if (!endOfRunReason.description) {
                    endOfRunReason.description = '';
                }

                const eorReasonsWithCategory = await EorReasonRepository.findAll({
                    attributes: ['runId'],
                    ...endOfRunReason.description && { where: { description: { [Op.substring]: endOfRunReason.description } } },
                    include: {
