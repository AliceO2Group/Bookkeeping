...
const {
    ...
    RunDefinition,
    CALIBRATION_RUN_TYPES_STARTS_WITH,
} = require('../../server/services/run/getRunDefinition.js');
...
class GetAllRunsUseCase {
    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (runTypes) {
                const { operator: runTypesOperator, values: runTypesValues } = runTypes;
                switch (runTypesOperator) {
                    case 'and':
                        queryBuilder.where('runTypeId').allOf(...runTypesValues);
                        break;
                    case 'or':
                        queryBuilder.where('runTypeId').oneOf(...runTypesValues);
                        break;
                }
            }

            if (definitions) {
                queryBuilder.andWhere((sequelize) => this.getRunDefinitionCriteria(sequelize, definitions));
            }
            ...
            if (detectorFilter) {
                ...
                switch (detectorsOperator) {
                    case 'none':
                        queryBuilder.where('concatenatedDetectors').isOrNull('');
                        break;
                    case 'and':
                        queryBuilder.where('concatenatedDetectors')
        ...
        /*
         * Because duplicating = false, we need to fetch tags and detectors in another request
    ...
    async fetchAndSetRunDetectors(runs) {
        ...
        const detectorQueryBuilder = new QueryBuilder()
            ...
            .oneOf(runs.map(({ runNumber }) => runNumber))
            .include('runs')
            .orderBy('name', 'ASC');
     ...
     * @param {string[]} definitions the list of run definitions to include
     * @return {Object} the criteria to add to the query builder
     */
    getRunDefinitionCriteria({ col, fn }, definitions) {
        /*
         * Run definitions can be defined by the following criteria:
         * Run match physics or cosmics criteria common criteria ("criteriaPoC")
         *      - match physics specific criteria ("criteriaPKnowingPoC") then it is PHYSICS
         *      - DO NOT match physics specific criteria (NOT "criteriaPKnowingPoC") and it match cosmics criteria (criteriaCKnowingPoCAndNotP),
         *            then it is COSMIC
         * Run DO NOT match cosmics or physics criteria (NOT "criteriaPoC")
         ...
         * Run DO NOT match cosmics, physic, technical or synthetic
         *      - run type starts with CALIBRATION_
         * Else, run has no definition
         */

        // Common criteria between physics and cosmics runs
        const criteriaPoC = {
            dcs: true,
            dd_flp: true,
            epn: true,
            triggerValue: 'CTP',
            tfbDdMode: { [Op.or]: COSMICS_OR_PHYSICS_TF_BUILDER_MODE },
        ...
        const criteriaPKnowingPoC = {
            ...
            [Op.or]: [
                { timeO2Start: { [Op.not]: null } },
                { timeTrgStart: { [Op.not]: null } },
            ],
            '$lhcFill.stable_beams_start$': {
                [Op.not]: null,
                [Op.lte]: fn('COALESCE', col('time_trg_end'), col('time_o2_end'), fn('NOW')),
            },
            '$lhcFill.stable_beams_end$': {
                [Op.or]: [
                    { [Op.eq]: null }, // Not ended yet, run start must be before now if it is defined
                    { [Op.gte]: fn('COALESCE', col('time_trg_start'), col('time_o2_start')) },
                ],
            },
        };

        // Criteria to match any run that is cosmics if we already know that it match common criteria between physics or comic but is not physics
        const criteriaCKnowingPoCAndNotP = {
            '$runType.name$': {
                [Op.and]: [
                    { [Op.not]: null },
                    { [Op.in]: ['cosmics', 'cosmics'] },
                ],
            },
            lhcBeamMode: {
                [Op.or]: [
        ...
        // Criteria to match any run that is not physics or cosmics
        ...
        // Criteria to match any run that is technical if we already know that it is not physics either cosmics
        ...
        // Criteria to match any run that is synthetic if we already know that it is not technical, physics or cosmics
        ...
        const criteriaCAKnowingNotPoCoToS = {
            '$runType.name$': {
                [Op.and]: [
                    { [Op.not]: null },
                    {
                        [Op.or]: CALIBRATION_RUN_TYPES_STARTS_WITH.map((prefix) => ({ [Op.like]: `${prefix}%` })),
                    },
                ],
            },
        };
        ...
        if (definitions.includes(RunDefinition.Physics)) {
            orDefinitionCriteria({
                [Op.and]: [criteriaPoC, criteriaPKnowingPoC],
            });
        }

        if (definitions.includes(RunDefinition.Cosmics)) {
            orDefinitionCriteria({
                [Op.and]: [
                    criteriaPoC,
                    { [Op.not]: criteriaPKnowingPoC },
        ...
        if (definitions.includes(RunDefinition.Technical)) {
            orDefinitionCriteria({
                [Op.and]: [
                    criteriaNotPoC,
        ...
        if (definitions.includes(RunDefinition.Synthetic)) {
            /*
             * Apply NOT physics or cosmics criteria AND NOT technical knowing not physics or cosmics criteria AND synthetic knowing not
        ...
        if (definitions.includes(RunDefinition.Calibration)) {
            orDefinitionCriteria({
                [Op.and]: [
                    criteriaNotPoC,
                    { [Op.not]: criteriaTKnowingNotPoC },
        ...
        // Any other case is commissioning
        if (definitions.includes(RunDefinition.Commissioning)) {
            orDefinitionCriteria({
                [Op.and]: [
                    criteriaNotPoC,
                    { [Op.not]: criteriaTKnowingNotPoC },
                    { [Op.not]: criteriaSKnowingNotPoCoT },
                    { [Op.not]: criteriaCAKnowingNotPoCoToS },
                ],
            });
        }
