    ...
    async execute(dto = {}) {
        ...
        if (filter) {
            ...
            if (runNumbers) {
                ...
                // If there is a comma oneOf is used to search runNumbers
                if (runNumbers.includes(',')) {
                    filteringQueryBuilder.where('runNumber').oneOf(...runNumberList);
                }

                // If there is no comma we use LIKE to search runNumbers
                if (!runNumbers.includes(',')) {
                    filteringQueryBuilder.where('runNumber').oneOfSubstrings(runNumberList);
                }
            }
