...
const { CONFIGURATION: { TEST } } = window;
...
export class OverviewPageModel extends Observable {
    ...
    parseApiRemoteData(remoteData) {
        ...
        /**
         * Reasons to add delay in test mode:
         * Notoriously, puppetear cannot catch event of switching
         * table display into loading state (when spinner is displayed)
         * because transition into next state (success or failure) is to fast
         * @param {function} func function
         * @returns {function} if test mode, the function provided as argument but delayed, identical function otherwise
         */
        const addTestDelay = (func) => {
            if (TEST) {
        ...
        remoteData.match({
            ...
            Success: addTestDelay(({ items, totalCount }) => {
            ...
            Failure: addTestDelay((error) => {
                this._observableItems.setCurrent(RemoteData.failure(error));
            }),
        });
    }
