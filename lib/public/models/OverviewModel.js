...
export class OverviewPageModel extends Observable {
    /**
     * Constructor
     * @param {boolean} hasStableBeams - Flag indicating whether to fetch items with stable beams.
     */
    constructor(hasStableBeams = false) {
        super();

        this._pagination = new PaginationModel();
        this._pagination.observe(() => this.load(hasStableBeams));
     ...
     */
    async load(hasStableBeams = false) {
        const params = {
            ...hasStableBeams && {
