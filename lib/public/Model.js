...
export default class Model extends Observable {
    ...
    constructor(window, document) {
        ...
        this._usersDplDetectors = ObservableData.builder()
            .source(dplDetectorsProvider.physical$)
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => {
                    const { access = [] } = this.session;
                    return detectors.filter(({ name }) => access.includes(BkpRoles.ADMIN) || access.includes(getRoleForDetector(name)));
                },
            }))
            .build();
        this.userRoleSelectionModel.observe(() => this._usersDplDetectors.setCurrent(dplDetectorsProvider.physical$.getCurrent()));
    ...
    /**
     * Get dpl detector which for user can manage QC flags
     *
     * @return {RemoteData<DplDetector[]>} dpl detectors remote data
     */
    get usersDplDetectors() {
        return this._usersDplDetectors.getCurrent();
    }
}
