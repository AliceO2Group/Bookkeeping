...
export default class Model extends Observable {
    ...
    async handleLocationChange() {
        ...
        const tempRowCount = Math.floor((window.innerHeight - PAGE_USED_HEIGHT) / TABLEROW_HEIGHT);
        const rowCount = tempRowCount < 5 ? 5 : tempRowCount;

        switch (this.router.params.page) {
            case 'home':
                // Setting the rows per page also collects all the runs and logs
                this.logs.setLogsPerPage(rowCount);
                this.runs.setRunsPerPage(rowCount);
                break;
            case 'log-overview':
            ...
            case 'tag-overview':
                if (!this.runs.isInfiniteScrollEnabled()) {
                    // Setting the tags per page also fetches the tags
                    this.tags.setTagsPerPage(rowCount);
                }
                break;
            case 'tag-detail':
                if (this.router.params.panel) {
