...
export default (model) => {
    ...
    const quickAccessStack = quickAccessService.getUrlStack(model.router.getUrl().href);

    let body;
    if (quickAccessStack) {
        const tabAndLinks =
        body = tabbedPanelComponent(
            { currentPanelKey: 'test', currentPanelData: '', defaultPanelKey: 'test' },
            { test: 'Hello' },
            {
                test: () => content(model, pages),
            },
        );
    } else {
        body = content(model, pages);
    }
