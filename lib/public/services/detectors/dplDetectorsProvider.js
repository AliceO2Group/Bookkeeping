    ...
    constructor() {
        ...
        this._physical$ = ObservableData.builder()
            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => {
                    const filteredDetectors = [];
                    let glo = null;
                    for (const detector of detectors) {
                        const detectorName = detector.name.toUpperCase();
                        if (detectorName === VIRTUAL_DETECTOR_NAME.TST) {
                            continue;
                        } else if (detectorName === VIRTUAL_DETECTOR_NAME.GLO) {
                            glo = detector;
                        } else if (detectorName) {
                            filteredDetectors.push(detector);
                        }
                    }
                    filteredDetectors.sort(({ name: nameA }, { name: nameB }) => nameA < nameB ? -1 : 1);
                    if (glo !== null) {
                        return [glo, ...filteredDetectors];
                    }
