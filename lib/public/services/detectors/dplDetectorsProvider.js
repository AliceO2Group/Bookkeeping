    ...
    constructor() {
        ...
        this._physical$ = ObservableData.builder()
            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => {
                    const filteredDetectors = detectors.filter(({ name }) => name && name !== TST_DETECTOR_NAME);
                    const gloIndex = filteredDetectors.findIndex(({ name }) => name === GLO_DETECTORS_NAME);
                    if (gloIndex !== -1) {
