...
import { DETECTOR_NAME } from './detectorsNames.js';
    ...
    constructor() {
        ...
        this._physical$ = ObservableData.builder()
            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => {
                    const filteredDetectors = detectors.filter(({ name }) => name && name.toUpperCase() !== DETECTOR_NAME.TST);
                    const gloIndex = filteredDetectors.findIndex(({ name }) => name.toUpperCase() === DETECTOR_NAME.GLO);
                    if (gloIndex !== -1) {
