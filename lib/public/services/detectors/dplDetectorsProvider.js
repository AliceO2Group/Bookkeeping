            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => {
                    const filteredDetectors = detectors
                        .filter(({ name }) => name
                            && name.toUpperCase() !== VIRTUAL_DETECTOR_NAME.TST
                            && name.toUpperCase() !== VIRTUAL_DETECTOR_NAME.GLO)
                        .sort(({ name: nameA }, { name: nameB }) => nameA < nameB);

                    const gloDetector = detectors.find(({ name }) => name.toUpperCase() === VIRTUAL_DETECTOR_NAME.GLO);
                    if (gloDetector !== undefined) {
                        return [gloDetector, ...filteredDetectors];
                    }
