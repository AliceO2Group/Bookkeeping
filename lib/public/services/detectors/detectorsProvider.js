...
export class DetectorsProvider extends Observable {
        ...
        this._runAssignable$ = ObservableData.builder()
            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => detectors
                    .filter(({ type }) => [DetectorType.Physical, DetectorType.AbstractForRuns].includes(type)),
            }))
            .build();

        this._physicalAndChecks$ = ObservableData.builder()
            .source(this._all$)
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => detectors
                    .filter(({ type }) => [DetectorType.Physical, DetectorType.Check].includes(type))
                    .sort((a, b) => {
    ...
    get physicalAndChecks$() {
        ...
        return this._physicalAndChecks$;
    }
