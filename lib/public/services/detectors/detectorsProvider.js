...
import { DATA_TAKING_DETECTOR_TYPES } from '../../../domain/enums/DetectorTypes.js';
 ...
 */
const getPhysicalDetectorsFromAllDetectors = (allDetectors) => allDetectors.filter(({ name }) => name && name !== VIRTUAL_DETECTOR_NAME.TST);
        ...
        this._dataTaking$ = ObservableData.builder()
            ...
            .apply((remoteDetectors) => remoteDetectors.apply({
                Success: (detectors) => detectors
                    .filter(({ type }) => DATA_TAKING_DETECTOR_TYPES.includes(type))
