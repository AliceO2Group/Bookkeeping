...
export class QcFlagTypesProvider extends Observable {
    ...
    constructor() {
        ...
        this._notArchived$ = ObservableData.builder()
            .initialValue(RemoteData.notAsked())
            .source(this._all$)
            .apply((remoteData) => remoteData.apply({
                Success: (qcFlagTypes) => qcFlagTypes.filter((qcFlagType) => !qcFlagType.archived),
            }))
            .build();
    }

    /**
     * Return all QC flag types observable data
     *
    ...
    _load() {
        ...
        getRemoteData('/api/qcFlagTypes').then(
            ({ data: qcFlagTypes }) => this._all$.setCurrent(RemoteData.success(qcFlagTypes)),
