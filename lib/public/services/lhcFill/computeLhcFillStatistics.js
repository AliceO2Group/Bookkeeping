 ...
 *
 * @return {Object} the LHC fill statistics
 */
export const computeLhcFillStatistics = (runs, stableBeamsDuration, stableBeamsStart, stableBeamsEnd) => {
    ...
    for (const run of runs) {
        ...
        // If the run do not overlap with LHC fill at all, do not consider it
        if (runEnd < stableBeamsStart || runStart > stableBeamsEnd) {
            continue;
        }

        const start = Math.max(runStart, stableBeamsStart);
        const end = Math.min(runEnd, stableBeamsEnd);

        if (!(runQuality === 'good' && run.definition !== 'physics') || end - start < 0 || !stableBeamsDuration) {
            continue;
        }
    ...
    const firstRunStartOrFillEnd = firstSegment ? firstSegment.start : stableBeamsStart + stableBeamsDuration;
    const durationBeforeFirstRun = firstSegment ? firstSegment.start - stableBeamsStart : null;
    const efficiencyLossAtStart = (firstRunStartOrFillEnd - stableBeamsStart) / stableBeamsDuration * 100;

    const lastRunEndOrFillStart = lastSegment ? lastSegment.end : stableBeamsStart;
    const durationAfterLastRun = lastSegment ? stableBeamsStart + stableBeamsDuration - lastSegment.end : null;
    const efficiencyLossAtEnd = (stableBeamsStart + stableBeamsDuration - lastRunEndOrFillStart) / stableBeamsDuration * 100;
