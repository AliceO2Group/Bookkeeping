...
export class LhcFillStatisticsExtractor {
    ...
    constructor({ stableBeamsStart, stableBeamsEnd, stableBeamsDuration, collidingBunchesCount }, runs = []) {
        ...
        this._perDetectorsTimeSegmentsMergers = new Map();
    ...
    addRun(run) {
        ...
        for (const detector of (detectors || '').split(',')) {
            ...
            if (!this._perDetectorsTimeSegmentsMergers.has(key)) {
                this._perDetectorsTimeSegmentsMergers.set(key, new NonOverlappingSegmentsMerger());
            }
            this._perDetectorsTimeSegmentsMergers.get(key).merge(segment);
        }
    ...
    get statistics() {
        ...
        const firstRunStartOrFillEnd = firstSegment
            ? firstSegment.start
            : this._stableBeamsEnd;
        const timeLossAtStart = firstSegment ? firstSegment.start - this._stableBeamsStart : null;
        const efficiencyLossAtStart = (firstRunStartOrFillEnd - this._stableBeamsStart) / this._stableBeamsDuration;

        const lastRunEndOrFillStart = lastSegment ? lastSegment.end : this._stableBeamsStart;
        const timeLossAtEnd = lastSegment
            ? this._stableBeamsEnd - lastSegment.end
            : null;
        const efficiencyLossAtEnd =
            (this._stableBeamsEnd - lastRunEndOrFillStart) / this._stableBeamsDuration;

        const totalRunsDuration = this._rawTimeSegments.reduce((sum, segment) => sum + segment.end - segment.start, 0);

        const getEfficiencyFromCoverage = (coverage) => coverage / this._stableBeamsDuration;
        const perDetectorsEfficiency = new Map();
        for (const [detector, merger] of this._perDetectorsTimeSegmentsMergers.entries()) {
