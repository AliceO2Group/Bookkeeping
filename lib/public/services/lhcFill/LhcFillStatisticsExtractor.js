...
export class LhcFillStatisticsExtractor {
     ...
     */
    addRun(run) {
        const { runQuality, detectors } = run;
    ...
    get statistics() {
        ...
        const firstRunStartOrFillEnd = firstSegment ? firstSegment.start : this._stableBeamsStart
                                                                           + this._stableBeamsDuration;
        const durationBeforeFirstRun = firstSegment ? firstSegment.start - this._stableBeamsStart : null;
        const efficiencyLossAtStart = (firstRunStartOrFillEnd - this._stableBeamsStart)
                                      / this._stableBeamsDuration
                                      * 100;
        ...
        const efficiencyLossAtEnd =
            (this._stableBeamsStart + this._stableBeamsDuration - lastRunEndOrFillStart)
            / this._stableBeamsDuration
            * 100;
