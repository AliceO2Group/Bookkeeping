...
export const getRunQcGuiLinkUrl = (run) => {
    ...
    const { definition, detectors: concatenatedDetectors, pdpBeamType, runNumber, runType } = run;
    const urlParams = {
    ...
    const detectors = concatenatedDetectors?.split(',');
    if (concatenatedDetectors && detectors?.length === 1) {
    ...
    if (runType) {
        urlParams.runType = runType.name;
    }
