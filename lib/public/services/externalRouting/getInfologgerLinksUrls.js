...
export const getInfologgerLinksUrls = ({ environmentId, runNumbers }) => {
     ...
     */
    const appConfiguration = configurationService.configuration$.getCurrent().match({
    ...
    const formatUrl = (origin) => {
        ...
        if (!queryObject.partition && !queryObject.run) {
            return origin;
        }
    ...
    return {
        flp: formatUrl(appConfiguration.FlpInfologgerUrl),
        epn: formatUrl(appConfiguration.EpnInfologgerUrl),
    };
};
