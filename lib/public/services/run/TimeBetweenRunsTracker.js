 ...
 */
import { getRunStartAndEnd } from './getRunStartAndEnd.js';
...
export class TimeBetweenRunsTracker {
    ...
    trackRun(run) {
        ...
        const { start, end } = getRunStartAndEnd(run);
        if (start === null || this._previousEnd === null) {
            elapsed = null;
            // There is a missing edge
            this._totalIsUnderEvaluated = true;
        } else if (this._previousEnd !== undefined) {
            elapsed = Math.max(0, start - this._previousEnd);
        }
