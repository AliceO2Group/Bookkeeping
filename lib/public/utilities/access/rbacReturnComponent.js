...
export const linkOrDisabledOrNull = (roles, onPage, content, toPage = '', parameters = {}, attributes = {}) => {
    // Extract the component name from the content parameter or from attributes.title
    const component = typeof content === 'string' ? content : attributes.title;
    // Decide how to show the component, based on the roles
    const showType = rbacShowComponent(roles, onPage, component);

    if (showType === ShowType.SHOW) {
        // If a component isn't a link (e.g. open a dropdown menu), display as a button not a link
        const displayAsButton = attributes?.isLink === false ? true : false;
