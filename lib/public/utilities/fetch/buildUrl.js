...
export const buildUrl = (baseURL, parameters = {}) => {
    ...
    const [originAndPathname, existingParameters] = baseURL.split('?');

    if (existingParameters) {
        for (const formattedParameter of existingParameters.split('&')) {
            const parameterElements = formattedParameter.split(/\[|\]\[|\]/);
            const nestedKeys = parameterElements.slice(0, parameterElements.length - 1);
            const [lastKey, value] = parameterElements[parameterElements.length - 1].split('=');

            if (lastKey === '') {
                buildParameterFromNestedKeys(parameters, nestedKeys, value, null);
            } else { // List nestedKeys is empty
                buildParameterFromNestedKeys(parameters, [lastKey], value, null);
            }
        }
    }
    ...
    return `${originAndPathname}?${serializedQueryParameters.join('&')}`;
};
