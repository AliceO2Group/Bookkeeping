...
export class PaginatableRemoteDataSource extends RemoteDataSource {
     ...
     * @param {object} [options] options modifying behaviour
     * @param {Function<U, V>} [options.dataTransformer = null] function to transform response of server
     * @param {Function<V, V, V>} [options.dataMerger = null] function to merging newly acuired data to existing one,
    ...
    async acquire(endpoint) {
        ...
        try {
            ...
            const response = await getRemoteData(endpoint, { signal: this._abortController.signal });
            const { dataTransformer, onSuccess, dataMerger = (newData, _) => newData } = this._options;
            await onSuccess?.(response);
            this._observableData.data =
                RemoteData.success(dataMerger?.(dataTransformer?.(response) ?? response, this._observableData.data.payload));
        } catch (error) {
            // Use local variable because the class member (this._abortController) may already have been override in another call
            if (!abortController.signal.aborted) {
                this._observableData.data = RemoteData.failure(error);
    ...
    paginationForMissingData(endpoint) {
        ...
        const sliceLowerBound = Math.max(nextOffset, previousOffset);
        const sliceHigherBound = Math.min(nextOffset + nextLimit, previousOffset + previousLimit);
        return {
            subPagination: [
                {
                    offset: nextOffset,
                    limit: sliceLowerBound - nextOffset,
                },
                {
                    offset: sliceHigherBound,
                    limit: nextOffset + nextLimit - sliceHigherBound,
                },
            ],
            currentDataSlice: {
                lower: sliceLowerBound,
                higher: sliceHigherBound,
            },
        };
    }
}
