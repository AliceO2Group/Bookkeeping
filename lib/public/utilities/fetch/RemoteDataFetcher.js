...
class DataSource extends Observable {
    ...
    /**
     * Abstract method for data source to acquire data
 ...
 * @template U, V where U is type of data from server response, V is type of data being server by this class
 */
export class RemoteDataSource extends DataSource {
    ...
    async acquire(endpoint) {
        ...
        try {
            ...
            this._observableData.data = RemoteData.success(this._dataTransformer ? this._dataTransformer(response) : response);
        } catch (error) {
