...
export const modalContainer = (modalModel) => {
    ...
    return h(
        '.modal-container',
        {
            oncreate: () => documentClickTaggedEventRegistry.addListenerForAnyExceptTagged(dismissCurrent, CLICK_ON_MODAL_EVENT_TAG),
            onremove: () => documentClickTaggedEventRegistry.removeListener(dismissCurrent),
        },
        h(
            `.${classes}`,
            {
                onclick: (e) => documentClickTaggedEventRegistry.tagEvent(e, CLICK_ON_MODAL_EVENT_TAG),
            },
            [
