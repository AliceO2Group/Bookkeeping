...
import { DetectorsProviderWrp } from '../../services/detectors/detectorsProvider.js';
...
export class DetectorSelectionDropdownModel extends SelectionDropdownModel {
     ...
     */
    _initialize() {
        this._detectorsProvider = DetectorsProviderWrp.getInstance();
        this._detectorsProvider.bubbleTo(this);
        this._detectorsProvider.fetch()
            .then(() => {
                const { detectors } = this._detectorsProvider;
                this.availableOptions = detectors.isSuccess() ?
                    RemoteData.success(detectors.payload.map(({ name }) => ({ value: name })))
                    : detectors;
            });
    }
}
