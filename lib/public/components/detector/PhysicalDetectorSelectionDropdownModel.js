 ...
 */
export class PhysicalDetectorSelectionDropdownModel extends SelectionDropdownModel {
    /**
     * Constructor
     */
    constructor() {
        super({ multiple: true, availableOptions: RemoteData.loading() });
        detectorsProvider.physical$.observe(() => {
            this.setAvailableOptions(detectorsProvider.physical$.getCurrent().apply({
                Success: (detectors) => detectors.map(({ id, name }) => ({ label: name, value: id })),
            }));
        });
    }
}
