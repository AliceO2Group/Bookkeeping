 ...
 *
 * @param {object|object[]} fields object describing the list of fields that must be displayed and how they must be formatted
...
export const detailsList = (fields, subject, configuration) => {
    ...
    return h(
        ...
        Object.entries(fields).map(([key, { name, format, visible }]) => {
            ...
            if (key.startsWith('_') || (typeof visible === 'function' ? visible(fieldValue, subject) : visible) === false) {
                // The key is configuration, not a real property
                return null;
            }

            let content = emptyString;
            if (format) {
                content = format(fieldValue, subject);
