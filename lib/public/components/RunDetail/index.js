 ...
 *
 * @returns {string} The name of the runType or a '-'
 */
function _formatRunType(model) {
    const { runType } = model.runs;
    return runType.match({
        NotAsked: () => {
            model.runs.fetchRunType();
            return '-';
        },
        Loading: () => '-',
