...
const activeFields = (model) => ({
    ...
    runQuality: {
        ...
        format: (runQuality) => model.runs.isEditModeEnabled ? [
            h('select.form-control.w-15#runQualitySelect', {
                ...
                h('option#runQualityGood', { value: 'good', selected: model.runs.run.payload.runQuality === 'good' }, 'good'),
                h('option#runQualityBad', { value: 'bad', selected: model.runs.run.payload.runQuality === 'bad' }, 'bad'),
                h('option#runQualityTest', { value: 'test', selected: model.runs.run.payload.runQuality === 'test' }, 'test'),
            ]),
        ] : runQuality ? h(
            '.badge.white',
    ...
    runType: {
        ...
        visible: true,
        size: 'cell-m',
        format: (type) => formatRunType(type),
    },
    nDetectors: {
