 ...
 */
const perPageAmountInputComponent = (onblur) => {
    const state = { value: null };

    return h('input[type=number][placeholder=Custom].form-control', {
        style: {
            marginBottom: '4px',
            border: state.value === null || isPerPageAmountValid(state.value) ? '' : '1px solid red',
        },
        ...
        onblur: ({ target }) => {
            if (isPerPageAmountValid(target.value)) {
                onblur(target.value);
            }
        },
    });
...
const mapAmounts = (onclick, amounts) => [
    ...
    h('.ph3', perPageAmountInputComponent(onclick)),
];
