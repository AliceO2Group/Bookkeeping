...
const perPageAmountInputComponent = (onCustomChoiceConfirm, paginationModel) => h('input[type=number][placeholder=Custom]', {
    ...
    class: `form-control ${paginationModel.customItemsPerPage
                           && !isPerPageAmountValid(paginationModel.customItemsPerPage)
    ...
    return [
        ...
        h(
            '.ph3',
            {
                ...
                oncreate: function () {
                    taggedEventRegistry.addListenerForAnyExceptTagged(dismissAmountSelector, CLICK_ON_AMOUNT_SELECTOR_EVENT_TAG);

                    /**
                     * Confirm the usage of custom amount when "enter" key is pressed
                ...
                // eslint-disable-next-line require-jsdoc
                onremove: function () {
                    taggedEventRegistry.removeListener(dismissAmountSelector);
