...
const checkboxFilter = (model, tags) => {
    ...
    if (checkboxes.length <= FILTERS_LIMIT) {
        ...
        }, ...model.logs.moreFilters ? [iconMinus(), ' Less filters'] : [iconPlus(), ' More filters']);

        const slicedCheckboxes = model.logs.moreFilters ? checkboxes : checkboxes.slice(0, FILTERS_LIMIT);
