 ...
 *
 * @param {BeamsModeFilterModel} beamsModeFilterModel beamsModeFilterModel
 ...
 */
export const stableBeamFilter = (beamsModeFilterModel, radioButtonMode = false) => {
    const isStableBeamsOnly = beamsModeFilterModel.isStableBeamsOnly();
    ...
    if (radioButtonMode) {
        return h('.form-group-header.flex-row.w-100', [
            radioButton({
                ...
                action: () => beamsModeFilterModel.setStableBeamsOnly(false),
                name: name,
            }),
            radioButton({
                label: label2,
                isChecked: isStableBeamsOnly === true,
                action: () => beamsModeFilterModel.setStableBeamsOnly(true),
                name: name,
            }),
        ]);
    } else {
        return switchInput(isStableBeamsOnly, (newState) => {
            beamsModeFilterModel.setStableBeamsOnly(newState);
