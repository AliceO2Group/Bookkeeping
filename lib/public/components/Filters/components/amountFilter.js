 ...
 *
 * @param {{operator: string, limit: number}} currentValue the operator to use to compare the limit to the actual
 *     values
...
export const amountFilter = (currentValue, onChange, options) => {
    const { operator, limit } = currentValue || { operator: '=' };
    ...
    return comparisonOperatorFilter(h('input.flex-grow', {
        ...
        oninput: (e) => {
            ...
            updateFilter({ newLimit: !isNaN(value) && value > 0 ? value : null });
        },
        ...limitAttributes,
