...
const dcsOperationRadioButtons = (model) => {
    ...
    return h('.form-group-header.flex-row.w-100', [
        radioButton('ANY', state === '', () => model.runs.removeDcs()),
        radioButton('OFF', state === false, () => model.runs.setDcsFilterOperation(false)),
        radioButton('ON', state === true, () => model.runs.setDcsFilterOperation(true)),
    ]);
};
