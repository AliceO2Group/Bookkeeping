        ...
        h('input.w-50.mv1', {
            ...
            oninput: (e) => {
                if (!o2from) {
                    ...
                    model.runs.setO2endFilterWithDebounce('FromTime', e.target.value, e.target.value <= now);
                } else {
                    model.runs.setO2endFilterWithDebounce('FromTime', e.target.value, e.target.validity.valid);
                }
            } }, ''),
        h('.f6', 'Ended Till:'),
        h('input.w-50.mv1', {
        ...
        h('input.w-50.mv1', {
            ...
            oninput: (e) => {
                if (!o2to) {
                    ...
                    model.runs.setO2endFilterWithDebounce('ToTime', e.target.value, e.target.value <= now);
                } else {
                    model.runs.setO2endFilterWithDebounce('ToTime', e.target.value, e.target.validity.valid);
                }
            },
        }, ''),
    ]);
