...
const tagFilterOperationRadioButtons = (model) => h('.form-group-header.flex-row', ['AND', 'OR'].map((operation) =>
    h('.form-check.mr2', [
        h('input.form-check-input', {
            onclick: () => model.runs.setTagFilterOperation(operation),
            id: `tagFilterOperationRadioButton${operation}`,
            checked: operation === model.runs.getTagFilterOperation(),
            type: 'radio',
...
const tagsFilter = (model) => {
    ...
    if (tags) {
        const checkboxes = tags.map((tag, index) => {
            const isChecked = model.runs.isTagInFilter(tag.id);
            return h('.form-check', [
                h('input.form-check-input', {
                    onclick: () => isChecked ? model.runs
                        .removeTagFromFilter(tag.id) : model.runs.addTagToFilter(tag.id),
        ...
        if (checkboxes.length == 0) {
            ...
            const showMoreFilters = model.runs.shouldShowMoreTags();
            const toggleFilters = h('button.btn.btn-primary.mv1#toggleMoreTags', {
                onclick: () => model.runs.toggleMoreTags(),
