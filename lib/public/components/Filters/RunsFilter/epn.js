...
const epnOperationRadioButtons = (model) => {
    ...
    return h('.form-group-header.flex-row.w-100', [
        radioButton('ANY', state === '', () => model.runs.removeEpn()),
        radioButton('OFF', state === false, () => model.runs.setEpnFilterOperation(false)),
        radioButton('ON', state === true, () => model.runs.setEpnFilterOperation(true)),
    ]);
};
