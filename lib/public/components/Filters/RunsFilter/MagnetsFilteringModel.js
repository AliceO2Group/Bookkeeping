 ...
 */
const magnetsCurrentLevelsToOptionValue = ({ l3Level, dipoleLevel }) => `${l3Level}kA/${dipoleLevel}kA`;
        ...
        magnetsCurrentLevels$.observe(() => {
            magnetsCurrentLevels$.getCurrent().match({
                ...
                Success: (currentLevels) => {
                    this._valueToFilteringParamsMap = new Map(currentLevels.map(({ l3Level, dipoleLevel }) => [
                        magnetsCurrentLevelsToOptionValue({ l3Level, dipoleLevel }),
                        { l3: l3Level, dipole: dipoleLevel },
                    ]));
                },
                Other: () => {
                    this._valueToFilteringParamsMap = new Map();
