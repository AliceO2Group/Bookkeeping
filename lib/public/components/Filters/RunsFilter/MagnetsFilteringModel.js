...
/**
 * Maps a magnet configuration to option value
 *
 * @param {number|string} l3 l3 magnets value
 * @param {number|string} dipole dipole magnets value
 * @return {string} the option value
 */
const magnetsConfigurationToOption = ({ l3, dipole }) => `${l3}kA/${dipole}kA`;
...
export class MagnetsFilteringModel extends FilterModel {
    ...
    async hydrate(selected) {
        const { l3, dipole } = selected;

        try {
            await this._selectionDropdownModel.setSelectedOptions([magnetsConfigurationToOption({ l3, dipole })]);
        } catch (e) {
        }
    }
