...
export class NumericalComparisonFilterModel extends FilterModel {
    ...
    hydrate(normalizedValue) {
        ...
        if (this._useOperatorAsNormalizationKey) {
            const [[key, value]] = Object.entries(normalizedValue);
            operator = key;
            operand = value;
        } else {
            const { operator: key, limit } = normalizedValue;
            operator = key;
            operand = limit;
        }

        this._operatorSelectionModel.select(operator);
        this._operandInputModel.update(parseInt(operand, 10) / this._scale, true);
