...
export class TimeRangeInputModel extends FilterModel {
     ...
     * @inheritDoc
     * @override
     * @return {Partial<Period>} the normalized value of the filter
     */
    get normalized() {
        return {
   ...
   set normalized({ from, to}) {
        this._fromTimeInputModel.update(from);
        
        this._toTimeInputModel.update(to);
        values.split(',').forEach(value=> this._selectionModel.select(value));
    }
