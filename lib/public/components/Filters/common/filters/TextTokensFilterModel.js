...
export class TextTokensFilterModel extends Observable {
     ...
     * @param {RegExp|string} [splitPattern = r' *'] regex for splitting input into tokens
     */
    ...
    update(value) {
        ...
        } else if (JSON.stringify(this.normalized) === JSON.stringify(this._transformInput(value))) {
            this._raw = value;
            this._visualChange$.notify();
            if (this.isValid) {
                this.notify();
     ...
     */
    get normalized() {
        return this._transformInput(this._raw);
    }
