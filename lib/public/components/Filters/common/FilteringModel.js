...
export class FilteringModel extends Observable {
    ...
    /**
     * Hydrate a filter from a normalized value
     *
     * @param {string} key the key of the filter to hydrate
     * @param {NormalizedFilterValue} value the normalized value of the filter
     * @return {Promise<void>} Resolve once hydratation succeeded
     */
    async hydrate(key, value) {
        const filter = this._filters[key];
        await filter?.hydrate(value);
    }
}
