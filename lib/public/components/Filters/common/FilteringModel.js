...
export class FilteringModel extends Observable {
   ...
   constructor(router, filters) {
     ...
     *
     * @param {string[]} _pageIdentifiers Strings that identify the pages as shown in the router params. Used to prevent unneeded reads/writes from/to the url
     * @returns {void}
     */
    ...
    setFilterToURL() {
        ...
        if (!this._pageIdentifiers.includes(params.page)) return;
    ...
    async setFilterFromURL() {
        ...
        if (!this._pageIdentifiers.includes(page)) return;

        for (let [key, value] of Object.entries(filter)) {
            const filterModel = this._filters[key];

            console.log(page, key, value, !!filterModel);
