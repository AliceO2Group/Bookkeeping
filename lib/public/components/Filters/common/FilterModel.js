/**
 * @license
 * Copyright CERN and copyright holders of ALICE O2. This software is
 * distributed under the terms of the GNU General Public License v3 (GPL
 * Version 3), copied verbatim in the file "COPYING".
 *
 * See http://alice-o2.web.cern.ch/license for full licensing information.
 *
 * In applying this license CERN does not waive the privileges and immunities
 * granted to it by virtue of its status as an Intergovernmental Organization
 * or submit itself to any jurisdiction.
 */
import { Observable } from '/js/src/index.js';

/**
 * @typedef {string|number|object|string[]|number[]|null} NormalizedFilterValue
 */

/**
 * Model storing the state of a given filter
 *
 * @abstract
 */
export class FilterModel extends Observable {
    /**
     * Constructor
     */
    constructor() {
        super();

        this._visualChange$ = new Observable();
    }

    /**
     * Reset the filter to its initial state
     *
     * @return {void}
     * @abstract
     */
    reset() {
        throw new Error('Abstract function call');
    }

    /**
     * States if the filter has been filled with a valid value
     *
     * @return {boolean} true if the filter is filled
     * @abstract
     */
    get isEmpty() {
        throw new Error('Abstract function call');
    }

    /**
     * Returns the normalized value of the filter, that can be used as URL parameter
     *
     * @return {NormalizedFilterValue} the normalized value
     * @abstract
     */
    get normalized() {
        throw new Error('Abstract function call');
    }

    /**
     * Hydrate the filter model from a normalized value
     *
     * @param {NormalizedFilterValue} _value the normalized value
     * @return {Promise<void>} resolve once the filter is hydrated
     * @abstract
     */
    hydrate(_value) {
        throw new Error('Abstract function call');
    }

    /**
     * Returns the observable notified any time there is a visual change which has no impact on the actual filter value
     *
     * @return {Observable} the observable
     */
    get visualChange$() {
        return this._visualChange$;
    }

    /**
     * Utility function to register a filter model as sub-filter model
     *
     * TODO for now this function simply propagate observable notifications, in the future this should register submodels with a key and allow
     *      automated normalization (recursive filter model)
     *
     * @param {object} subModel the submodel to register
     * @return {void}
     * @protected
     */
    _addSubmodel(subModel) {
        subModel.bubbleTo(this);
        subModel.visualChange$?.bubbleTo(this._visualChange$);
    }
}
