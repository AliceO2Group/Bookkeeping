...
/**
 * Adapt column profiles for applyProfile function
 *
 * @param {string|string[]|object<string, *>} profilesToBeAdapted profiles
 * @return {string[]|object<string, *>} adapted profile
 */
const adaptColumnProfiles = (profilesToBeAdapted) => {
    let columnProfiles = profilesToBeAdapted ?? [profiles.none];
...
const filtersToggleContent = (
    ...
    h('.flex-column.g2', [
        Object.entries(columns)
            .filter(([_, column]) =>
                column.filter
                && (applyProfile(column, appliedProfile, adaptColumnProfiles(column.profiles))?.visible
                 || (column.filterProfiles ?? []).includes(appliedProfile)))
 ...
 * @param {string} [configuration.profile] specify by which columns filtering is enabled using active columns profiles
