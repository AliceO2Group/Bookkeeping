 ...
 */
const pasteButtonOption = (filteringModel) => {
    ...
    if (filteringModel.filteringModel) {
        filteringModel = filteringModel.filteringModel;
    }

    return h('.btn.btn-primary', { onclick: async () => {
        const url = await navigator.clipboard.readText();
        filteringModel.setFilterFromURL(url);

    }, disabled: !clipboardSupported}, 'Paste filters');
}
