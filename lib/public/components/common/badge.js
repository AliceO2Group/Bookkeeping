 ...
 * @param {string|null} [configuration.color] the color of the badge, default to light gray, hex or css var
...
export const badge = (label, configuration) => {
    ...
    if (!rawColor) {
        ...
        let color = null;
        if (/var\(.+\)/.test(rawColor)) {
            color = rawColor;
        } else {
            color = rawColor.length === 6 ? `#${rawColor}` : rawColor;
        }
