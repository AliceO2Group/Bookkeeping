 ...
 */
const aliceMagnetsConfigurationSnapshotForm = (magnetsConfigurationSnapshot) => h('.flex-row.flex-wrap.flex-grow.items-center.g2', [
    h(TimestampInputComponent, {
        seconds: true,
        onChange: (timestamp) => {
            magnetsConfigurationSnapshot.timestamp = timestamp;
        },
        value: magnetsConfigurationSnapshot.timestamp,
    }),
    h('', '-'),
...
export const aliceMagnetsConfigurationForm = (magnetsConfiguration) => [
    h('.flex-column.magnet-dynamic-width', [
        h('', 'Dipole'),
        h(
