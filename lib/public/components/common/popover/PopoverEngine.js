...
class PopoverEngine {
     ...
     * @param {boolean} [configuration.resetSize=false] if true, size of the popover will be removed before re-computing size and position. This
     *     is useful when the popover content size is based on the popover size (displaying an image for example) but this should not be used
     *     with scrollable content, because it will reset scroll on every render
     */
    constructor(
        trigger,
        popover,
        ...
        const { resetSize = false } = configuration ?? {};
        this._resetSize = resetSize;
    }
    ...
    resizeAlongMainAxis() {
        if (
            this._triggerStartMainAxis <= this._popoverContentSizeMainAxis
            && this._availableSpaceInMainAxis - this._triggerEndMainAxis <= this._popoverContentSizeMainAxis
        ) {
            let mainAxisSize;
            ...
            this._mainAxisProjector.setSize(this._popover.style, null);
        }
    ...
    resizeAlongCrossAxis() {
        if (this._popoverContentSizeCrossAxis > this._availableSpaceInCrossAxis) {
            this._crossAxisProjector.setSize(this._popover.style, `${this._availableSpaceInCrossAxis}px`);
        } else {
            this._crossAxisProjector.setSize(this._popover.style, null);
        }
        this._refreshPopoverBoundingBox();
    }
    ...
    positionAlongMainAxis() {
        ...
        const doesFitBeforeAlongMainAxis = this._triggerStartMainAxis >= this._popoverSizeMainAxis;
        const doesFitAfterAlongMainAxis = this._availableSpaceInMainAxis - this._triggerEndMainAxis >= this._popoverSizeMainAxis;
