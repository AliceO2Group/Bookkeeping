 ...
 * @property {(subject: {width: T, height: T}, size: T) => void} setMaxSize set the maximum size of the subject to a given value
 */
 ...
 * @param {object} [configuration] additional configuration
 * @param {boolean} [configuration.imperativeSize=false] if true, size of the popover will be removed before re-computing size and position,
 * and size will be imperative (width instead of max-width). This is useful when the popover content size is based on the popover size
 *     (displaying an image for example) but this should not be used with scrollable content, because it will reset scroll on every render,
 *     or with popover for which height might be reduced dynamically (dropdown options being filtered)
