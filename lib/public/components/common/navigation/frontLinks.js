...
import { showFilePreview } from '../../../views/Logs/FilePreview/filePreviewComponent.js';
...
export const frontLinks = (items, configurationExtractor, configuration) => {
    const { separator = h('span.mr2', ','), empty = '-', attributes = null, group = true, bypassRouter = false, filePreviewModel = null } = configuration || {};
    ...
    const linksWithSeparator = items.map((item) => {
        ...
        if (page) {
            ...
            if (filePreviewModel) {
                ...
                return [showFilePreview(filePreviewModel, {
                    type: item.mimeType,
                    name: item.originalName,
                }, separator, isLastItem, href, attributes)]
            } else {
    ...
    // Remove separator from last link except if a preview is shown (that has its own seperator)
    if (!filePreviewModel) {
