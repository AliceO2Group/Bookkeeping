...
export class FilterableRemoteSelectionModel extends Observable {
    ...
    /**
     * Wait for available options to be ready, and then apply a selection
     *
     * @param {SelectionChoice[]} selection the new selected options
     * @return {Promise<void>} resolves once selection has been set
     */
    async setSelectedOptions(selection) {
        return new Promise((resolve, reject) => {
            /**
             * Apply the selection if the selectionModel is successful
             * @return {void}
             */
            const applySelection = () => {
                this._selectionModel.match({
                    ...
                    /**
                     * Success clause
                     * @param {SelectionModel} selectionModel the actual selection model
                     * @return {void}
                     */
            ...
            this.visualChange$.observe(applySelection);
            // Check now, if the selection model is already successful
            applySelection();
        });
    }
