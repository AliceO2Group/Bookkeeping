...
/**
 * @typedef {SelectionOption|number|string|boolean} SelectionChoice
 */
...
export class SelectionModel extends Observable {
     ...
     *
     * @return {string[]|number[]|boolean} the values of the selected options
     */
    get selected() {
     ...
     *
     * @param {SelectionChoice} option the option to select
     * @return {void}
     */
    select(option) {
        let selectOption;
     ...
     *
     * @param {SelectionChoice[]} selected the list of selected options
     * @return {void}
     */
    setSelected(selected) {
        this._selectedOptions = selected
            .map((option) => {
                if (typeof option === 'string' || typeof option === 'number' || typeof option === 'boolean' || option === null) {
                    return this._availableOptions.find(({ value }) => value === option);
                }
                return option;
            })
