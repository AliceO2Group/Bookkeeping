...
export class SelectionModel extends Observable {
    ...
    select(option) {
        ...
        if (typeof option === 'string' || typeof option === 'number') {
            if (this._availableOptions instanceof RemoteData) {
                selectOption = this._availableOptions.match({
                    // String comparison in needed since select may use url parameters which are always strings, causing false negatives
                    Success: (options) => options.find(({ value }) => String(value) === String(option)), 
                    Other: () => null,
                });
            } else {
        ...
        // if (this.multiple) console.log('selectedOption', selectOption);
        ...
        // if (this.multiple) console.log('selectedOptions: ', JSON.stringify(this._selectedOptions));
    }
    ...
    get options() {
        ...
        return prepareOptions(this._availableOptions);
    }
    ...
    setAvailableOptions(availableOptions) {
        ...
        for (let option of this.selectionBacklog) {
            // console.log(option);
    ...
    set normalized(value) {
        ...
        if (postponeSelection) {
            this.selectionBacklog = options;
        } else {
            options.forEach((option) => this.select(option))
        }
    }

    /**
     * Return comma-seperated string containing the selected options
     *
     * @return {string}
     */
    get normalized() {
