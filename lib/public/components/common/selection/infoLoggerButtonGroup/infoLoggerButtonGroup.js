...
export const infoLoggerButtonGroup = (parameters) => {
    ...
    const { environmentId, runNumbers, fillNumber } = parameters;

    if (fillNumber) {
        return identifierColumnAndActionsDropdown('lhc-fill-details', fillNumber, parameters, [
    ...
    if (runNumbers) {
        return identifierColumnAndActionsDropdown('run-detail', runNumbers, { runNumber: runNumbers }, [
            h(CopyToClipboardComponent, { value: runNumbers, id: runNumbers }, 'Copy Run Number'),
            flpInfologgerLink,
            epnInfologgerLink,
        ]);
    }
