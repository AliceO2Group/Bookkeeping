...
const filteredDropdownOptions = (selectionModel, filter, selectorPrefix) => {
     ...
     */
    const displayOption = (option, checked, onChange, selectorPrefix) => {
    ...
    const optionsList = (availableOptions, onChange) => {
        ...
        return availableOptions.map((option) => displayOption(
            option,
            selectionModel.isSelected(option),
            onChange,
            selectorPrefix,
        ));
    };

    return h('.dropdown-options', optionsList(
        filterSelectionOptions(selectionModel.options, filter),
        (option, state) => {
            state ? selectionModel.select(option) : selectionModel.deselect(option);
        },
    ));
};
