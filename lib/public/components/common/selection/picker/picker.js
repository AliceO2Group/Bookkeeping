 ...
 * @property {object|null} [attributes=null] if specified, picker items will be wrapped within a component with these attributes. If a limit is
 ...
 * @param {object|null} optionsAttributes attributes applied to picker elements
 * @return {Component[]} the list of inputs
 */
const mapOptionsToInput = (pickerModel, pickerOptions, selector, optionsAttributes) => pickerOptions.map((pickerOption, index) => h(
...
export const picker = (pickerOptions, pickerModel, configuration) => {
    const { selector = 'picker', attributes = null, optionsAttributes = null, placeHolder = null, searchEnabled = true } = configuration || {};

    const checkboxes = mapOptionsToInput(pickerModel, pickerOptions, selector, optionsAttributes);
    ...
    const searchInput = h(`input.form-control.${selector}-search-input`, {
        type: 'search',
