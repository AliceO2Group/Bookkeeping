 ...
 * @property {((data: any, row: Object) => any) | null} [format] function to apply on cell's data to obtain cell's content (the function
...
/**
 * @typedef TableConfiguration
 * @property {string} profile the profile to apply to the table, to filter out columns or apply
 ...
 * @param {TableConfiguration|null} [tableConfiguration] the global table configuration
...
export const table = (
    ...
    return h(`.table-max-height${horizontalScrollEnabled ? '.scroll-auto.shadow-level1' : ''}`, h(
        `table.sticky-top.table.table-hover.shadow-level1.no-z-index${freezeFirstColumn ? '.freeze-first-column' : ''}`,
        {
