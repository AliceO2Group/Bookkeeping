 ...
 * @param {boolean} [tableConfiguration.xScrollable = false] is true use x scroll in case of overflow, fixed layout otherwise
 * @param {boolean} [tableConfiguration.leftSticky = false] has effect only if xScrollable is set to true,
 * if true first left column become frozen, otherwise it is like the other columns
...
export const table = (
    ...
    const { profile: currentProfile = profiles.none, xScrollable = false, leftSticky = false } = tableConfiguration || {};
    ...
    return h(`${xScrollable ? '.x-scrollable' : ''}`, h(
        `table.table.table-hover.shadow-level1.no-z-index${leftSticky ? '.left-sticky' : ''}`,
        {
            style: `table-layout: ${xScrollable ? 'auto' : 'fixed'}`,
        },
        [
