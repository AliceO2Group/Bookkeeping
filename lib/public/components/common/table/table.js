 ...
 * @property {boolean|function} [balloon] true to display a balloon if the cell's content overflow, or use a function
 *     that will be used to extract the balloon text from the cell's data
    ...
    return h(
        ...
        [
            ...
            remoteData.match({
                ...
                Success: (payload) => rows(payload, idKey, displayedColumns, rowsConfiguration),
                Failure: (errors) => h('tr', h('td', { colSpan: displayedColumns.length }, errors.map(errorAlert))),
            }),
        ],
    );
};
