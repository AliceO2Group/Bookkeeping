 ...
 * @property {boolean|function(object[], boolean)} [visible] false to hide the column
 *      if visible is function, it called with all data provided as argument
 ...
 * @param {RemoteData} remoteData the array data to be displayed in the table, eventually wrapped in a
 *     RemoteData
 * @return {ParsedColumnsConfiguration} the parsed configuration
 */
const parseColumnsConfiguration = (columns, currentProfile, remoteData) => {
    const displayedColumns = [];
    // As a default, the id column is the first one
    let [{ key: idKey }] = columns;
    for (const column of columns) {
        const { key, primary } = column;
        if (primary) {
            idKey = key;
        }
        ...
        const { visible = false } = columnUnderProfile ?? {};
        if (typeof visible === 'function' ? remoteData?.match({ Success: (data) => visible(data), Other: () => false }) : visible) {
...
export const table = (
    ...
    const { idKey, displayedColumns } = parseColumnsConfiguration(columns, currentProfile, remoteData);
