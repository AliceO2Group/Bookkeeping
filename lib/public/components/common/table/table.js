...
export const table = (
    ...
    // return h(`.fitting-table${horizontalScrollEnabled ? '.scroll-auto.shadow-level1' : ''}`, h(
    return h(`.fitting-table.scroll-y`, h(
        // `table.table.table-hover.shadow-level1.no-z-index${freezeFirstColumn ? '.freeze-first-column' : ''}`,
        `table.table.table-hover.shadow-level1.scroll-y`,
        {
