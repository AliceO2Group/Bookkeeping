 ...
 */
const cell = (rowId, column, rowData, index) => {
    ...
    return h(`td.${column.size} ${column.classes} .${index === 0 ? '.first-headcol' : null}`, {
        id: columnId,
        key: columnId,
...
export const rows = (data, idKey, columns, configuration) => {
    ...
    if (!data || data.length === 0) {
        ...
        body = data.map((rowData) => {
            ...
            }, columns.map((column, index) => cell(rowId, column, rowData, index)));
        });
    }
