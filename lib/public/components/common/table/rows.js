...
const cell = (rowId, column, rowData) => {
    ...
    const { key, title: isTitle, balloon = false } = column;
    ...
    return h(`td.${column.size} ${column.classes}`, {
        id: columnId,
        key: columnId,
    }, isBalloon ? wrapWithBalloon(cellContent) : h('.w-wrapped', allParams, cellContent));
};

/**
 * @typedef RowsConfiguration
 * @property {function((T|undefined)):Object} [callback] a callback called on each item of data, and whose result will
 *     be added to the row component's attributes
...
export const rows = (data, idKey, columns, configuration) => {
    ...
    return h('tbody', data.map((rowData) => {
