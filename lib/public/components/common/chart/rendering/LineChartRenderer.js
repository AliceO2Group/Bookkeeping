...
import { ChartRenderer, X_AXIS_NAME } from './ChartRenderer.js';
...
export class LineChartRenderer extends ChartRenderer {
    ...
    constructor(configuration, points) {
        ...
        if (!this.isXIndexAxis) {
            throw new Error(`LineChartRendered only supports '${X_AXIS_NAME}' as index axis`);
        }
    ...
    renderDataset(datasetIndex, svg) {
        ...
        const pointLocator = {
            ...
            getY: this._stackedValues ? ({ y }) => this.yScale(y[datasetIndex]) : ({ y }) => this.yScale(y),
        };
