 ...
 * @property {formatAxisTick} [ticks.filter] function applied on tick's domain value to filter out ticks
...
export class AxisRenderer {
    ...
    constructor(svg, key, configuration) {
        ...
        const { gap: ticksGap = DEFAULT_AXIS_TICK_GAP, format: tickFormat, filter: tickFilter } = ticks || {};
        this.configuration = {
            label,
            gap,
            ticks: {
    ...
    render(scale, chartDrawingZone) {
        ...
        const tickFilter = this.configuration.ticks.filter;
        if (tickFilter) {
            const axisTicks = tickFilter ? scale.ticks().filter(Number.isInteger) : undefined;
            lineAndTicksRenderer.tickValues(axisTicks);
        }
