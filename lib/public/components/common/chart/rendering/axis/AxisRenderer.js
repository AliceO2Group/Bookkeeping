 ...
 * @property {'decimate'|'rotate'} [ticks.overlapping='decimate'] specify what to do in the case ticks overlaps (overlapping might not be solved)
...
export class AxisRenderer {
    ...
    constructor(svg, key, configuration) {
        ...
        const {
            ...
            format: tickFormat,
            filter: tickFilter,
            overlapping: tickOverlapping = 'decimate',
        } = ticks || {};
        this.configuration = {
            label,
            gap,
    ...
    render(scale, chartDrawingZone) {
        ...
        if (factor > 1) {
            if (this.configuration.ticks.overlapping === 'rotate') {
