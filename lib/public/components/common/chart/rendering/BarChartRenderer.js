...
export class BarChartRenderer extends ChartRenderer {
    ...
    constructor(configuration, points) {
        ...
        /**
         * Calculate bar nodes
         * @param {Point} p point - bar definition
         * @return {[number, number]} polygon
         */
        this._barToPolygon = (p) => {
            const x = this._barPropertiesProvider.getX(p);
            const y = this._barPropertiesProvider.getY(p);
            const w = this._barPropertiesProvider.getWidth(p);
            const h = this._barPropertiesProvider.getHeight(p);
            return [
                [x, y],
                [x + w, y],
                [x + w, y + h],
     ...
     */
    getHoveredPoint({ x, y }) {
        // TODO caching polygon info
        const bar = this._points.find((bar) => polygonContains(this._barToPolygon(bar), [x, y]));
        return { coordinates: bar, relativePosition: { x, y } };
    }
