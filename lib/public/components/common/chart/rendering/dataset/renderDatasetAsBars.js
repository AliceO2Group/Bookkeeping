...
export const renderDatasetAsBars = (
     ...
     * @return {string} fill
     */
    const getFill = (p) => pointDataProvide.getFill(p) ?? fill ?? 'none';
     ...
     */
    const getStroke = (p) => pointDataProvide.getStroke(p) ?? stroke ?? 'none';
    ...
    if (independentVariable === 'x') {
        select(svg)
            ...
            .attr('height', pointDataProvide.getBarLength)
