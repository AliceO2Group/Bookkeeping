...
export class ChartRenderer {
    ...
    constructor(configuration, points) {
        ...
        this._points = points;
        console.log(this._independentVariable)
        ...
        } else if (this._independentVariable === 'y') {
            const { x: { min: xMin, max: xMax } } = this.getAxisConfiguration();
            this._yAxisScaleFactory = new EnumerableBasedScaleFactory({ point: this.isPoint });
            const xAxisScaleFactoryConfiguration = { forceRange: { min: xMin, max: xMax } };
            if (this._forceZero) {
                xAxisScaleFactoryConfiguration.minimalRange = { min: 0, max: 0 };
            }
            this._xAxisScaleFactory = new RangeBasedScaleFactory(xAxisScaleFactoryConfiguration);

            for (const point of points) {
                this._yAxisScaleFactory.processValue?.(point.y);
                if (this._stackedX && Array.isArray(point.x)) {
