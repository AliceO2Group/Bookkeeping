...
import { select, scaleLinear, scaleBand, scalePoint, zoom } from '/assets/d3.min.js';
...
export class ChartRenderer {
     ...
     * - and if independent variable is 'y', 'x' must contain an array
     */
    constructor(configuration, data) {
        ...
        this._refreshScales('constructor');
    }
    ...
    render(svg) {
        ...
        // const zoomedHandler = (event) => {
            // Rescale the axes
            // const newXScale = event.transform.rescaleX(this.xScale);
            // console.log(newXScale)
            // this._xScale = newXScale;
        
            // Update the bar widths based on the new scale
            // svg.selectAll('.bar')
            // .attr('width', d => new_x(d.value));
        
            // Optional: Prevent bars from getting negative width
            // svg.selectAll('.bar')
            // .attr('width', d => Math.max(0, new_x(d.value)));  // Ensures no negative width bars
        // };
        ...
        // d3Svg.call(zoomCall).on('wheel.zoom', null);
        ...
        // Render the horizontal axis
        {
            ...
            this.updateChartDrawingZone({ height: this._chartDrawingZone.height - height }, 'render X axis');
        }
     ...
     * @return {void}
     */
    updateChartDrawingZone(patch, reason = null) {
        this._chartDrawingZone = {
            ...this._chartDrawingZone,
            ...patch,
        };
        console.log(`chart drawing zone Height, (R: ${reason})`, this._chartDrawingZone.height.toFixed(2))
        this._refreshScales('update chart drawing zone');
    }
     ...
     * @protected
     */
    _refreshScales(reason = null) {
        // console.log(`refresh X scale ${reason}: `, [
        ...
        console.log(`refresh Y scale (R:${reason}): `, [
            (this._chartDrawingZone.height + this._chartDrawingZone.top).toFixed(2),
            this._chartDrawingZone.top.toFixed(2),
        ]);
