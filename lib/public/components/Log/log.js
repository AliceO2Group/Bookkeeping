 ...
 * @param {Object} [opts.logMarkdownDisplayOpts] markdown display opts @see logText opts argument
 * @param {Partial<MarkdownBoxSize>} [opts.logMarkdownDisplayOpts.boxSize] size of markdown display
...
export const logComponent = (
    ...
    const { title = '', id = '' } = log;
    const fieldFormatting = logFields(opts?.authenticationToken);
    ...
    return h(
        ...
        h('div.flex-row.justify-between.', [
            ...
            h('div.flex-row.flex-shrink-0.flex-wrap.items-start.g1', [
                ...
                (opts.replayButton ?? true) && logReplyButton(log),
                (opts.goToDetailsButton ?? false) && logDetailsButton(log),
                logViewButton,
            ]),
        ]),
        h('.bt1.b-gray-light'),
        logText(log, isCollapsed, opts?.logMarkdownDisplayOpts),
    );
};
