...
import { filePreviewComponent } from '../../views/Logs/FilePreview/filePreviewComponent.js';
 ...
 * @param {FilePreviewModel} filePreviewModel the file preview model
 * @return {Object} an object determining how each log field wil lbe displayed on the page
 */
const logFields = (authenticationToken, filePreviewModel) => ({
    author: {
        name: 'Source',
    ...
    attachments: {
        name: 'Attachments',
        visible: true,
        format: (attachments) => h('span.flex-row.flex-wrap.gc1', attachments.map((item) => {
            const itemSeperator = item.id === attachments[attachments.length - 1].id ? null : h('span.mr2', ',');
            return filePreviewComponent(
                filePreviewModel,
                {type: item.mimeType, name: item.originalName},
                itemSeperator,
                `/api/attachments/${item.id}?token=${authenticationToken}`,
                { download: item.fileName, class: 'break-all' }
            )
        })),
    },
});
...
export const logComponent = (
    ...
    const { title = '', id = '' } = log;
    const fieldFormatting = logFields(options?.authenticationToken, options?.filePreview);
