...
export const logCollapsed = (log, highlight, showLogTitle, onExpand, ) => {
    ...
    return h(`#log-${log.id}.w-100.m1.p3.shadow-level1`, 
        ...
        h('div.flex-row.justify-between.w-100', [
            h('div.flex-row', [ 
                h('em.p1', `${logFields.author.format(log.author)}, (${logFields.createdAt.format(log.createdAt)})`),
                (log.tags.length != 0) && (h('div.p1', `${logFields.tags.format(log.tags)}`)),
                (log.runs.length != 0) && h('div.p1', logFields.runs.format(log.runs)),
                (log.attachments.length != 0) && h('div.p1', logFields.attachments.format(log.attachments)),
            ]),
            h('div.flex-row', {style: `align-items: center; justify-content: space-between`}, [
                replyButton(log),
