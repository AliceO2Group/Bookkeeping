...
export const logExpanded = (log, highlight, showLogTitle, onCollapse, authenticationToken) => {
    ...
    return h(
        `#log-${log.id}.w-100.m1.p3.shadow-level1`,
        h(
            ...
            h('div.flex-row.justify-between.w-100', [
                h('div.flex-column', [
                    ...
                    log.tags.length != 0 && h('div', `${logFields.tags.format(log.tags)}`),
                    log.runs.length != 0 && h('div', logFields.runs.format(log.runs)),
                    log.attachments.length != 0 && h('div', logFields.attachments.format(log.attachments)),
                ]),
