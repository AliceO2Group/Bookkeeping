...
const activeFields = (model) => ({
    ...
    runs: {
        name: 'Runs',
        visible: true,
        format: (runs) => runs.length > 0 ? h('.flex-row', runs.map((run, index) => {
            const href = `?page=run&id=${run}`;
            return [
                h('a', { onclick: (e) => model.router.handleLinkEvent(e), href }, run),
                index + 1 < runs.length ? h('', { style: 'white-space: pre;' }, ', ') : null,
            ];
        })) : 'None',
    },
});
...
const entry = (model, post, index, highlight) => {
    ...
    return h('.w-100.flex-row.shadow-level1', {
        ...
        h(`.w-20.shadow-level1${highlight ? '.bg-gray-light' : ''}`, {
            ...
            h('table', [
                h('tbody', Object.entries(postFields).map(([key, { name, format, visible }]) => visible && h('tr', [
