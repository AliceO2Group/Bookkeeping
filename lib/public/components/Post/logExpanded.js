 ...
 *
 * @TODO remove the model dependency, find a way to inject the token in another way
 *
 * @param {Model} model global model used to get security token
 * @param {Log} log all data related to the log
 * @param {boolean} highlight indicator if this log should be highlighted
 * @param {function} onCollapse function called when the post should be collapsed
 * @return {Component} the log's display component
 */
export const logExpanded = (model, log, highlight, onCollapse) => {
    const postFields = activeFields(model);

    return h(`#post${log.id}.w-100.flex-row.m1.shadow-level1`, [
        h(`.w-20.p2.shadow-level1${highlight ? '.bg-gray-light' : ''}`, [
            h('table', [
                h('tbody', Object.entries(postFields)
                    ...
                    ]) => visible && h(`tr#post${log.id}-${key}`, [
                        h('td.text-right', { style: 'font-weight: bold' }, `${name}:`),
                        h('td', format ? format(log[key]) : log[key]),
                    ]))),
            ]),
        ]),
        h('.w-80.p2.shadow-level1', h('div.flex-wrap.justify-between.mv1', h(`h4#post${log.id}title`, log.title), h('div', [
            replyButton(log),
