...
export const getRunNotSubjectToQcReason = ({ runQuality, tags }) => {
    ...
    if (runQuality === RunQualities.BAD) {
        reason += `Quality of the run was changed to ${RunQualities.BAD} so it is no more subject to QC`;
    }
    if (tags.find(({ text }) => RUN_TAGS_DISABLING_QC.has(text))) {
        reason += 'Run was tags as not for physics so it is no more subject to QC';
    }
    return reason;
};
