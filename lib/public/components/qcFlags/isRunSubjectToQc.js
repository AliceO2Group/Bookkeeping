...
export const getRunNotSubjectToQcReason = ({ runQuality, tags }) => {
    ...
    if (runQuality === RunQualities.BAD) {
        reasons.push(`Quality of the run was changed to ${RunQualities.BAD} so it is no more subject to QC`);
    }
    if (tags.find(({ text }) => RUN_TAGS_DISABLING_QC.includes(text))) {
        reasons.push('Run was tags as not for physics so it is no more subject to QC');
    }
    return reasons.join('; ');
};
