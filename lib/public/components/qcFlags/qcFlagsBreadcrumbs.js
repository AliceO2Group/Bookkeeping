...
export const qcFlagsBreadcrumbs = ({ remoteDataPass, remoteSimulationPass, remoteRun, remoteDplDetector }, header = 'QC') => {
    ...
    console.log(remoteRun, remoteDataPass);
    ...
    return breadcrumbs([
        ...
        remoteRun && remoteRun.match({
            Success: (runs) => runs[0].runQuality === RunQualities.BAD ?
                h('h2.danger', frontLink(runs[0].runNumber, 'run-detail', { runNumber: runs[0].runNumber })) :
                h('h2', frontLink(runs[0].runNumber, 'run-detail', { runNumber: runs[0].runNumber })),
        ...
        remoteDplDetector && remoteDplDetector.match({
            Success: (detectorsMap) => h('h2', Object.values(detectorsMap)[0][0].name),
