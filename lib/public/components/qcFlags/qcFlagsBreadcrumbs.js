...
export const qcFlagsBreadcrumbs = ({ remoteDataPass, remoteSimulationPass, remoteRun, remoteDplDetector }, header = 'QC') => {
    ...
    return breadcrumbs([
        ...
        remoteRun && remoteRun.match({
            Success: ({ runNumbers, runQuality }) => runQuality === RunQualities.BAD
                ? h('h2.danger', frontLink(runNumbers, 'run-detail', { runNumbers }))
                : h('h2', frontLink(runNumbers, 'run-detail', { runNumbers })),
