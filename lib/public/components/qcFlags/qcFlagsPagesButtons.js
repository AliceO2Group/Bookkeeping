 ...
 * @param {string} scope.runQuality run quality
 * @param {number} scope.dplDetectorId DPL detector id
 * @return {Component} link
 */
export const qcFlagCreationPanelLink = ({ dataPassId, simulationPassId, runNumber, runQuality, dplDetectorId }) => {
    ...
    return dplDetectorsProvider.physical$.getCurrent().match({
        NotAsked: () => null,
        Loading: () => null,
        Failure: () => tooltip(disabledButton, `An error has occurred. Unable to determine permissions on detector with id ${dplDetectorId}`),
        Success: (detectors) => {
            ...
            if (sessionService.session?.access?.includes(BkpRoles.ADMIN)
                ...
                return runQuality === RunQualities.BAD
                    ? tooltip(
