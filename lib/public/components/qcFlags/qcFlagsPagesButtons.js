 ...
 * @param {number} detectorId DPL detector id
...
export const qcFlagCreationPanelLink = (
    ...
    return remoteDetectorsUserHasAccessTo.match({
        NotAsked: () => null,
        Loading: () => null,
        Failure: () => tooltip(
            h('button.btn.btn-primary', { disabled: true }, content),
            `An error has occurred. Unable to determine permissions on detector with id ${detectorId}`,
        ),
        Success: (detectors) => {
            const userHasAccessToDetector = detectors.find(({ id }) => id === detectorId);
                ...
                return remoteRun.match({
                    ...
                    Success: (run) => isRunNotSubjectToQc(run)
                        ...
                        : frontLink(
                            content,
                            pageName,
                            { runNumber: run.runNumber, detectorId, ...options },
                            {
 ...
 * @param {number} scope.detectorId DPL detector id
