...
export const qcFlagCreationPanelLink = ({ dataPassId, simulationPassId, runNumber, dplDetectorId }) => {
    ...
    return dplDetectorsProvider.physical$.getCurrent().match({
        NotAsked: () => tooltip(disabledButton, `Cannot say whether you have permissions for detector with id ${dplDetectorId}`),
        Loading: () => tooltip(disabledButton, 'Loading...'),
        Failure: () => tooltip(disabledButton, `Cannot say whether you have permissions for detector with id ${dplDetectorId} because of error`),
        Success: (detectors) => {
