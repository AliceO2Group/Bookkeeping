...
export const qcFlagCreationPanelLink = (
    ...
    return remoteDplDetectorsUserHasAccessTo.match({
        ...
        Success: (detectors) => {
            ...
            const detector_name = detectors.find(({ id }) => id === dplDetectorId).name;
                ...
                return remoteRun.match({
                    ...
                    Success: (run) => isRunNotSubjectToQc(run)
                        ...
                        : [frontLink(
                            content,
                            pageName,
                            { runNumber: run.runNumber, dplDetectorId, ...options },
                            {
                                class: ['btn', 'btn-primary', ...linkClasses].join(' '),
                                title: 'Create a QC flag',
                            },
                            ),
                            h('label', { for: 'qc-flag-checkbox' }, 
                            h('input.select-multi-flag', { type: 'checkbox', id: run.runNumber.toString() + '-' + detector_name })),
                        ]
                });
            } else {
