...
import { colorInputComponent } from '../common/form/inputs/colorInputComponent.js';
...
const activeFields = (detailsModel) => ({
    ...
    color: {
        ...
        format: (color) => detailsModel.isEditModeEnabled ?
            colorInputComponent(detailsModel.tagChanges['color'], (e) => {
                detailsModel.setTagChange('color', (e.target.value || null).replace(/^#/, ''));
            }) :
            h('.br2', { style: { backgroundColor: `#${color}` } }, color ? `#${color}` : ''),
    },
});
