...
const activeFields = (detailsModel) => ({
    ...
    description: {
        ...
        format: (description) => detailsModel.isEditModeEnabled
            ...
            : h('.break-word.text-right', description) || '-',
    },
    mattermost: {
        name: 'Mattermost',
        visible: true,
    ...
    color: {
        ...
        format: (color) => detailsModel.isEditModeEnabled ?
            h('input#color.form-control.w-15.mb2', {
                placeholder: 'Enter the color#...',
                value: detailsModel.tagChanges['color'],
                minlength: 6,
                maxlength: 6,
                oninput: (e) => {
                    if (e.target.value !== '') {
                        detailsModel.setTagChange('color', e.target.value);
                    } else {
                        detailsModel.setTagChange('color', null);
                    }
                },
                style: { backgroundColor: `#${detailsModel.tagChanges['color']}` },
            }) :
            h('.rounded-1', { style: { backgroundColor: `#${color}` } }, color ? `#${color}` : ''),
    },
});
