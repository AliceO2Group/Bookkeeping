...
const activeFields = (model) => ({
    ...
    mattermost: {
        ...
        visible: true,
        size: 'cell-l',
        format: (mattermost) => model.tags.detailsModel.isEditModeEnabled
            ? h('input.form-control.w-60', {
                placeholder: 'Enter the mattermost channels... ("," separation)',
                value: model.tags.detailsModel.tagChanges['mattermost'],
                oninput: (e) => {
                    model.tags.detailsModel.setTagChange('mattermost', e.target.value);
                },
            })
            : mattermost || '-',
    },
    email: {
        ...
        visible: true,
        size: 'cell-l',
        format: (emails) => model.tags.detailsModel.isEditModeEnabled
            ? h('input.form-control.w-60', {
                placeholder: 'Enter email groups... ("," separation)',
                value: model.tags.detailsModel.tagChanges['email'],
                oninput: (e) => {
                    model.tags.detailsModel.setTagChange('email', e.target.value);
                },
            })
    ...
    archived: {
        ...
        visible: true,
        size: 'cell-l',
        format: (_, tag) => model.tags.detailsModel.isEditModeEnabled
            ? h(
                ...
                [
                    ...
                    h('.switch#tag-archive-toggle', [
                        h(
                            'input',
                            {
                                checked: Boolean(model.tags.detailsModel.tagChanges['archivedAt']),
                                onchange: (e) => {
                                    const archivedAt = e.target.checked ? tag.archivedAt ?? Date.now() : null;
                                    model.tags.detailsModel.setTagChange('archivedAt', archivedAt);
                                },
                                type: 'checkbox',
                            },
                        ),
