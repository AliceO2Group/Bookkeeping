...
export class BkpRolesSelectionModel extends SelectionDropdownModel {
     ...
     */
    constructor() {
        super({ defaultSelection: [{ value: 'admin' }] });
    }
    ...
    _initialize() {
        ...
        const update = () => this.setAvailableOptions(dplDetectorsProvider.physical$.getCurrent().match({
            ...
            Other: () => [{ value: 'admin', rawLabel: 'admin' }],
        }));
...
const bkpRolesSelectionModel = new BkpRolesSelectionModel();
...
function navBar(model) {
    ...
    if (model.session.personid === 0) { // Anonymous user has id 0
        bkpRolesSelectionModel.observe(() => {
            const options = bkpRolesSelectionModel.options.map(({ value }) => value);
            model.session.access = [
                ...(model.session.access ?? [])
                    .filter((roleName) => !options.includes(roleName)),
                ...bkpRolesSelectionModel.selected,
            ];
        });
    }
        ...
        h('.flex-row.g2', [
            ...
            dropdownSubMenu(iconPerson(), model.isDropdownMenuOpened('account'), () => model.toggleDropdownMenu('account'), [
                ...
                model.session.personid === 0 // Anonymous user has id 0
                    ? [
                        h('p', 'privileges selection dropdown'),
                        selectionDropdown(
                            bkpRolesSelectionModel,
