...
const navBar = (model) => {
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        ...
        h('.btn-group', [
            ...
            pageTab(model, 'lhcFill-overview', 'Lhc Fills'),
        ...
        h('.flex-row', [
            ...
            h('.navbar-dropdown', {
                title: 'User Actions', class: model.dropdownMenu === 'account' ? 'dropdown-open' : '',
            }, [
                        ...
                        ? [
                            ...
                            h('label.switch', [
                                h('input', {
                                    onclick: () => {
                                        isChecked = !isChecked;
                                        isChecked ? model.session.access = ['admin'] : model.session.access = [];
                                    },
                                    type: 'checkbox',
                                    checked: isChecked,
                                }),
