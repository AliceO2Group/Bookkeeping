...
const pageTab = (model, id, text) => frontLink(text, id, null, {
    ...
    onclick: () => model.toggleDropdownMenu(),
});
...
const pageLink = (model, href, text) => absoluteFrontLink(
    ...
    {
        class: 'menu-item',
        id: `${text.toLowerCase().replace(/ /g, '-')}-link`,
        onclick: () => model.toggleDropdownMenu(),
    },
);
 ...
 * @param {object} [options.menuStyle] style for dropdown
 * @param {Function<void, boolean>} [options.isSelected] function to return whether
 * dropdown menu button should be styled as selected
 * @return {Component} dropdown menu
 */
const dropdownSubMenu = (model, key, dropdownButtonContent, entries, { menuStyle, isSelected } = {}) =>
    h(`.navbar-dropdown ${model.dropdownMenu === key ? '.dropdown-open' : ''}`, [
        h(`button#${key}-dropdown`, {
            class: `btn btn-tab ${isSelected?.call() ? 'selected' : ''}`,
            onclick: () => model.toggleDropdownMenu(key),
        }, dropdownButtonContent),
        h('.dropdown-menu', menuStyle, entries),
    ]);
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        ...
        h('.btn-group', [
            ...
            dropdownSubMenu(model, 'LHC', 'LHC', [
                pageLink(model, '?page=lhc-fill-overview', 'LHC Fills'),
                pageLink(model, '?page=lhc-period-overview', 'LHC Periods'),
            ], {
                menuStyle: { style: 'width: max-content;' },
