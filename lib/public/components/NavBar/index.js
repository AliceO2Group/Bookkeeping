 ...
 */
const menuLink = (content, href, onclick) => absoluteFrontLink(
    content,
    href,
    {
        class: 'menu-item',
 ...
 * @param {string} key by which the model references dropdown menu
 ...
 */
const dropdownSubMenu = (model, key, dropdownButtonContent, entries, { menuAttributes, isSelected } = {}) =>
    h(`.navbar-dropdown ${model.dropdownMenu === key ? '.dropdown-open' : ''}`, [
        h(`button#${key}-dropdown`, {
            class: `btn btn-tab ${isSelected ? 'selected' : ''}`,
            onclick: () => model.toggleDropdownMenu(key),
...
function navBar(model) {
    ...
    const pageTab = (page, content) => basePageTab(page, content, () => model.clearDropdownMenu());
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        ...
        h('.btn-group', [
            ...
            dropdownSubMenu(model, 'LHC', 'LHC', [
                menuLink('LHC Fills', '?page=lhc-fill-overview'),
                menuLink('LHC Periods', '?page=lhc-period-overview'),
            ], {
                menuAttributes: { style: 'width: max-content;' },
                isSelected: ['lhc-fill-overview', 'lhc-period-overview'].includes(model.router.params.page),
            }),

            pageTab('run-overview', 'Runs'),
            dropdownSubMenu(model, 'overviews', 'Overview', [
                menuLink('Tag Overview', '?page=tag-overview'),
                menuLink('Subsystem Overview', '?page=subsystem-overview'),
            ], {
                menuAttributes: { style: 'width: max-content;' },
                isSelected: ['tag-overview', 'subsystem-overview'].includes(model.router.params.page),
            }),
        ...
        h('.flex-row.g2', [
            ...
            dropdownSubMenu(model, 'account', iconPerson(), [
                h('p.mh3.text-ellipsis', `Welcome ${model.session.name}`),
                h('hr'),
                menuLink('Jira', 'https://alice.its.cern.ch/jira/projects/O2B/summary'),
                menuLink('GitHub', 'https://github.com/AliceO2Group/Bookkeeping'),
                menuLink('Support Forum', 'https://alice-talk.web.cern.ch/c/o2/o2-bookkeeping/'),
                h('hr'),
                ...
                model.session.personid === 0 // Anonymous user has id 0
                    ...
                    : menuLink('Logout', 'https://auth.cern.ch/auth/realms/cern/protocol/openid-connect/logout'),
            ]),
        ]),
    ]);
}
