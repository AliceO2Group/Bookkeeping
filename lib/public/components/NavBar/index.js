...
/**
 * Return link-button (opens new tab) with given href
 * @param {Object} configuration configuration of the component
 * @param {String} [conf.aliFlpIndexUrl] ali flp index url
 * @returns {vnode} returns tab
 */
const aliFlpLinkTab = (configuration) => absoluteFrontLink(
    'ALI FLP',
    configuration.aliFlpIndexUrl,
    {
        class: 'btn btn-tab',
        target: '_blank',
    },
...
const btnGroupsDelimiter = h('.mh1', { style: { 'border-left': '2px solid var(--color-gray-dark)' } });
...
const navBar = (model) => {
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        ...
        h('.btn-group', [
            aliFlpLinkTab(getAliFlpReferencesConfiguration()),
            btnGroupsDelimiter,
