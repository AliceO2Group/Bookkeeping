...
const dropdownSubMenu = (model, key, title, entires, { menuStyle, isSelectedCondition } = {}) =>
    h(`.navbar-dropdown ${model.dropdownMenu === key ? '.dropdown-open' : ''}`, [
        h(`button#${key}-dropdown`, {
            class: `btn btn-tab ${isSelectedCondition?.call() ? 'selected' : ''}`,
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        ...
        h('.btn-group', [
            ...
            dropdownSubMenu(model, 'LHC', 'LHC', [
                ...
                isSelectedCondition: () => ['lhc-fill-overview', 'lhc-period-overview'].includes(model.router.params.page),
            }),
            ...
            dropdownSubMenu(model, 'overview', 'Overview', [
                ...
                menuStyle: { style: 'width: max-content;' },
                isSelectedCondition: () => ['tag-overview', 'subsystem-overview'].includes(model.router.params.page),
            }),
