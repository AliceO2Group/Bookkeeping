...
/**
 * Model storing state of selected detector expert roles (for TEST only)
 */
export class DetectorExpertRolesSelectionModel extends SelectionDropdownModel {
    ...
    _initialize() {
        ...
        const update = () => this.setAvailableOptions(dplDetectorsProvider.physical$.getCurrent().match({
            Success: (detectors) => detectors
                .map(({ name }) => getDetectorExpertRoleName(name))
                .map((roleName) => ({
                    value: roleName,
                    rawLabel: roleName,
                })),
...
const detectorExpertRolesSelectionModel = new DetectorExpertRolesSelectionModel();
...
function navBar(model) {
    ...
    if (model.session.personid === 0) { // Anonymous user has id 0
        detectorExpertRolesSelectionModel.observe(() => {
            model.session.access = [
                ...(model.session.access ?? [])
                    .filter((roleName) => !detectorExpertRolesSelectionModel.options.map(({ value }) => value).includes(roleName)),
                ...detectorExpertRolesSelectionModel.selected,
            ];
        });
    }
        ...
        h('.flex-row.g2', [
            ...
            dropdownSubMenu(iconPerson(), model.isDropdownMenuOpened('account'), () => model.toggleDropdownMenu('account'), [
                ...
                model.session.personid === 0 // Anonymous user has id 0
                    ? [
                        h('p', `user/admin slider \n privileges: ${isChecked ? 'admin' : 'user'}`),
                        h('label.switch', [
                            h('input', {
                                onclick: () => {
                                    isChecked = !isChecked;
                                    model.session.access = [
                                        ...model.session.access.filter((role) => role !== 'admin'),
                                        ...isChecked ? ['admin'] : [],
                                    ];
                                    model.notify();
                                },
                        ...
                        h('p', 'detector experts privileges dropdown'),
                        selectionDropdown(
                            detectorExpertRolesSelectionModel,
                            { selectorPrefix: 'detector-roles' },
                        ),
