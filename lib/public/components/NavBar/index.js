...
import { h, iconList } from '/js/src/index.js';
...
import { getAliFlpReferencesConfiguration } from '../../services/externalRouting/getFlpIndexLinks.js';
 ...
 */
const pageLink = (model, href, text, attr = {}) => absoluteFrontLink(
    text,
    href,
    {
        class: 'menu-item',
...
/**
 * Return link-button (opens new tab) with given main href
 * alongside with dropdown menu containing other links if provided,
 * if not provided the dropdown menu works exactly like main link.
 * @param {Model} model model
 * @param {Object} conf configuration of the component
 * @param {String} [conf.main.name] name of main
 * @param {String} [conf.main.href] href of main
 * @param {undefined|Array<{name: String, href: String}>} [conf.other]
 * optional list for other links, each described the same way as main one
 * @returns {vnode} returns tab
 */
const aliFlpReferencesTab = (model, conf) => {
    const componentsHorizontalHalfSitance = '.15rem';
    const iconLeftPadding = '1.5rem';
    const iconStyle = {
        'padding-right': componentsHorizontalHalfSitance,
        'padding-left': iconLeftPadding,
    };
    const commonClasses = 'btn btn-tab';

    const mainPageAnchor = absoluteFrontLink(
        conf.main.name,
        conf.main.href,
        {
            class: commonClasses,
            target: '_blank',
            style: {
                'padding-left': componentsHorizontalHalfSitance,
            },
        },
    );

    // eslint-disable-next-line require-jsdoc
    const subpagesDropdown = () => h(`.navbar-dropdown${model.dropdownMenu === 'ali-flp-overview' ? '.dropdown-open' : ''}`, [
        h('button#ali-flp-overview', {
            class: commonClasses,
            style: iconStyle,
            onclick: () => model.toggleAliFlpDropdown(),
        }, iconList()),
        h(
            '.dropdown-menu',
            { style: { width: 'max-content' } },
            conf.other
                .map((subSConf) => pageLink(model, subSConf.href, subSConf.name, { target: '_blank' })),
        ),
    ]);

    // eslint-disable-next-line require-jsdoc
    const linkToMainIfNoSubpages = () => absoluteFrontLink(
        iconList(),
        conf.main.href,
        {
            class: commonClasses,
...
const navBar = (model) => {
    ...
    return h('.flex-row.justify-between.items-center.p2.shadow-level2.level2.bg-gray-light', [
        h('.flex-row', [
            ...
            aliFlpReferencesTab(model, getAliFlpReferencesConfiguration()),
        ]),
        h('.btn-group', [
