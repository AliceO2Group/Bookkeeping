...
const content = (key, value, text, rowId, model, entry) => {
    ...
    let testText = formattedText;
    let displayToolTip = false;

    if (key == 'title' && formattedText.length > 47) {
        testText = `${formattedText.substring(0, 47)}...`;
        displayToolTip = true;
    } else if (key == 'tags' && formattedText.length > 31) {
        testText = `${formattedText.substring(0, 25)}...`;
        displayToolTip = true;
    }
    ...
    return h(`td.${value.size}`, {
        ...
        h('.flex-row.items-center', [
            h('div.show-overflow', {
                id: `${columnId}-text`,
            }, !displayToolTip ? testText : h('div.tooltip', testText, h('span.tooltiptext', {
