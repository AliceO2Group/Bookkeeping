 ...
 */
const content = (key, value, text, rowId, entry) => {
    value = {
        format: (text) => text,
        balloon: false,
        ...value,
 ...
 */
const rows = (data, columnsDescriptions, rowConfigurationCallback, row = '') => {
    ...
    return h('tbody', data.map((entry) => {
        ...
        return h(`tr#${rowId}.${row}`, {
            ...
            if (value.visible) {
                accumulator.push(content(key, value, entry[key], rowId, entry));
            }
            return accumulator;
        }, []));
    }));
