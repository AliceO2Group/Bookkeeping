...
const content = (key, value, text, rowId, model, entry) => {
    let formattedText = value && value.format ? value.format(text, entry) : text;
    const displayToolTip = formattedText.length >= 47;
    formattedText = key == 'title' ? formattedText.substring(0, 47) : formattedText;
    ...
    return h(`td.${value.size}`, {
        ...
        h('.flex-row.items-center', [
            h('div.show-overflow', {
                id: `${columnId}-text`,
            }, !displayToolTip ? formattedText : h('div.tooltip', formattedText, h('span.tooltiptext', {
...
const rows = (data, keys, model, params) => {
    ...
    return h('tbody', data.map((entry) => {
        ...
        return h(`tr#${rowId}.clickable`, params(entry), Object.entries(keys).reduce((accumulator, [key, value]) => {
            if (value.visible) {
