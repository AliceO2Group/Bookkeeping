 ...
 */
const headers = (keys, model) => h('thead', h('tr', Object.entries(keys).map(([key, value]) => {
    if (value.visible) {
        const classes = [value.size || null, value.classes || null].filter((item) => Boolean(item)).join(' ');
        if (value.sortable) {
            ...
            return h(`th#${key}.${classes}.clickable`, {
                scope: 'col',
