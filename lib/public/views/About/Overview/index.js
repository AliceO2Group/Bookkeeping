...
const detailsFields = {
    ...
    status: {
        name: 'Status',
        format: (statuses) => h(
            'div.flex-row.flex-wrap.justify-start',
 ...
 */
const serviceDetailsPanelTitle = (serviceName, version) =>
 ...
 * @returns {vnode} a grid of panels, one for each service
 */
const servicesInfo = (appInfo, dbInfo) => {
    ...
    return services.map((s) => h(`div#${s.name}.flex-column.shadow-level1.flex-grow`, [
        serviceDetailsPanelTitle(
            ...
            s.infoRemoteData.isSuccess() || s.infoRemoteData.isFailure() ? s.infoRemoteData.payload.status : '',
        ),
        h('div.p2', s.infoRemoteData.match({
            NotAsked: () => 'Not Asked',
            Loading: () => 'Fetching Details...',
            Success: (payload) => h('div', detailsFields.status.format(payload.status)),
            Failure: (payload) => h('.flex-column.g2', [
                h('', detailsFields.status.format(payload.status)),
