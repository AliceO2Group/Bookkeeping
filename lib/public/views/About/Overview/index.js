 ...
 * @param {Object} service the service to display
 * @param {string} service.name the name of the service
 * @param {RemoteData} service.infoRemoteData the remote data corresponding to the service
...
const servicePanel = ({ name, infoRemoteData }) => {
    const panelBody = infoRemoteData.match({
        ...
        Success: (payload) => successPanelBody(payload.status),
        Failure: (payload) => failurePanelBody(payload.status),
    });
 ...
 * @param {AboutModel} model.aboutModel the about model
