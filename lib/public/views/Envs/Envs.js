...
export default class Overview extends Observable {
     ...
     */
    async fetchAllEnvs() {
        this._envs = RemoteData.success([
            {
                environmentId: 'hello',
                createdAt: null,
                toredownAt: null,
                status: 'ERROR',
                statusMessage: 'Welp this is broken',
                runs: null,
            },
            {
                environmentId: 'Bye',
         ...
         *     },
         *     'page[offset]': this.envs.payload && this.envsPerPage === Infinity ?
         *         this.envs.payload.length : (this.selectedPage - 1) * this.envsPerPage,
         *     'page[limit]': this.envsPerPage === Infinity ? this.model.INFINITE_SCROLL_CHUNK_SIZE : this.envsPerPage,
         * };
         */

        /*
         * Const endpoint = `/api/envs?${new URLSearchParams(params).toString()}`;
         * const response = await fetchClient(endpoint, { method: 'GET' });
         * const result = await response.json();
         * if (result.data) {
         *     if (this.isInfiniteScrollEnabled()) {
         *         const payload = this.envs && this.envs.payload ? [...this.envs.payload, ...result.data] : result.data;
         *         this.envs = RemoteData.success(payload);
         *     } else {
         *         this.envs = RemoteData.success(result.data);
         *     }
         */
         ...
         * } else {
         *     this.envs = RemoteData.failure(result.errors || [
         *         {
         *             title: result.error,
    ...
    /**
     * Getter for for all the environment objects in the class.
     * @returns {Array} Array of envrionments
     */
    get envs() {
        return this._envs;
    }
    ...
    /**
     * Setter for envs
     * @param {Array} envs array of environment objects
     */
    set envs(envs) {
        this._envs = envs;
        this.notify();
     ...
     * @param {integer} selectedPage new value
     */
