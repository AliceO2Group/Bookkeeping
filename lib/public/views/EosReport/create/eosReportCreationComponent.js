...
import { eosFormEcsSpecificPart } from './ecs/eosFormEcsSpecificPart.js';
    ...
    h('.flex-column.g3', [
        h('.flex-row.g3', [
            h('#shifter-name.panel.flex-grow', [
                ...
                h('input.form-control', { disabled: true, value: sessionService.get().name }),
            ]),
...
export const eosReportCreationComponent = (eosReportCreationModel) => eosReportCreationModel.currentShiftData.match({
    ...
    Success: (shiftData) => {
        ...
        return [
            h('h1', `End of shift report - ECS - ${formattedShiftDate} ${shiftData.shift.period}`),
            creationFormComponent(eosReportCreationModel, eosReportCreationForm(eosReportCreationModel.data, shiftData)),
        ];
    },
