 ...
 *
 * @param {object} formData the intermediates configuration
 * @return {Component} the form
 */
const intermediatesMagnetConfigurationForm = (formData) => {
    const entries = Object.entries(formData.typeSpecific.magnets.intermediates)
    ...
    return entries.map(([key, magnetConfigurationSnapshot]) => h(
        ...
        [
            ...
            // eslint-disable-next-line no-return-assign
            h('.btn.btn-danger', { onclick: () => formData.removeIntermidiateMagnet(key) }, iconTrash()),
        ],
    ));
};
    ...
    return h('#type-specific.flex-column.g3', [
        h('.panel', [
            ...
            h('.flex-column.p2.pv3.g3', [
                ...
                intermediatesMagnetConfigurationForm(formData),
                h('.flex-row.items-center.g2', [
                    h(
                        '#magnets-add.button.btn.btn-primary',
                        {
                            onclick: () => {
                                formData.pushIntermidiateMagnet({
                                    timestamp: null,
