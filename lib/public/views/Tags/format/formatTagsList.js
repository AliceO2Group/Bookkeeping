 ...
 * @param {boolean} [configuration.description] if true, tag will have the description tooltip enabled
 * @param {Component} [configuration.placeholder] return used as placeholder if no tags are present
 * @return {Component|string} Tag with its correct color or empty value
 */
export const formatTagsList = (tags, configuration) => {
    const { flex, description: enableDescription, placeholder = '-' } = configuration || {};

    /**
     * Wrap the tag badge component to include the description
     *
     * @param {tag} tag the tag to display
     * @return {Component} the badge component
     */
    const tagToBadge = ({ text, archived, color, description }) => {
        const badgeProperties = { text, archived, color };
        if (enableDescription) {
            badgeProperties.description = description;
        }
        return tagBadge(badgeProperties);
    };
