...
class TagModel extends Observable {
    ...
    async createTag() {
        ...
        this.notify();
        const isAdmin = this.model.session.access.includes('admin');
        const response = await fetchClient('/api/tags', {
            ...
            body: JSON.stringify({
                text: this.getText(),
                mattermost: isAdmin && this.mattermost ? this.getMattermost() : null,
                email: isAdmin && this.email ? this.getEmail() : null,
            }),
        });
        const result = await response.json();
