...
class TagModel extends Observable {
    ...
    /**
     * Return the remote data representation of all the not archived tags
     *
     * @return {RemoteData} the active tags
     */
    getActiveTags() {
        return this.tags.isSuccess() ? RemoteData.success(this.tags.payload.filter(({ archived }) => !archived)) : this.tags;
    }
    ...
    async fetchAllTags() {
        ...
        try {
            ...
            this.tags = RemoteData.success(items);
        } catch (errors) {
            this.tags = RemoteData.failure(errors);
        }
