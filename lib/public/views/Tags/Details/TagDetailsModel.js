...
export class TagDetailsModel extends Observable {
     ...
     * Constructor
     *
     * @param {number|null} [tagId=null] the id of the tag this model represents
    ...
    /**
     * Defines the current tag by its id
     *
     * @param {number|null} tagId the new tag id
     */
    set tagId(tagId) {
        this._tagId = tagId;
        if (tagId === null) {
    ...
    set currentPanelKey(panelKey) {
        this._currentPanelKey = this._availablePanelKeys.includes(panelKey) ? panelKey : this.defaultPanelKey;
