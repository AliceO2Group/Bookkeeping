...
export const TagDetailsPage = (model) => h('', {
    ...
    Success: (tag) => {
        ...
        return [
                        ...
                        model.tags.detailsModel.isEditModeEnabled ?
                            ...
                            : [
                                h('#edit-tag.button.btn.btn-primary', {
                                    onclick: () => {
                                        model.tags.detailsModel.setTagChange('description', tag.description);
            ...
            h('.flex-row.w-100', [h('.w-40.ph1', tagDetail(model.tags.detailsModel, tag))]),
            h('.w-100', [
                h(
                    'ul.nav.nav-tabs',
                    Object.entries(panelsTitles).map(([key, title]) => h(
                        'li.nav-item',
                        tabLink(title, { panel: key }, { id: `${key}-tab` }, key === model.tags.detailsModel.defaultPanelKey),
                    )),
                ),
                h(
                    '.tab-content.p2',
