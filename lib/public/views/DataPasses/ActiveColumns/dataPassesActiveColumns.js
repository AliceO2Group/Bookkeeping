...
const dataPassDeletedFromMlIcon = tooltip(iconDelete(), 'The production was removed from MonALISA');
...
export const dataPassesActiveColumns = {
    name: {
        ...
        visible: true,
        sortable: true,
        format: (name, { versions }) => versions?.every(({ deletedFromMl }) => deletedFromMl)
            ? badge(h('.flex-row.g1', [dataPassDeletedFromMlIcon, name]), { color: '#4caf50' })
            : name,
    ...
    description: {
        ...
        visible: true,
        sortable: false,
        format: (_, { versions }) => versions?.map(({ description, deletedFromMl }) =>
            h(`${deletedFromMl ? '.success' : ''}`, h('.flex-row.g1', [deletedFromMl ? dataPassDeletedFromMlIcon : null, description]))),
        balloon: true,
    },
