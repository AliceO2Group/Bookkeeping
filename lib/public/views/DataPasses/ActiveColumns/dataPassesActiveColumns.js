...
import { h, iconDelete, switchCase } from '/js/src/index.js';
...
export const dataPassesActiveColumns = {
    ...
    description: {
        ...
        format: (_, { versions }) => versions?.map(({ description, statusHistory }) =>
            statusHistory[statusHistory.length - 1]?.status === DataPassVersionStatus.DELETED
                ? h('.flex-row.g1', [iconDelete(), description])
                : description),
        balloon: true,
    },
    ...
    statusHistory: {
        name: 'Status history',
        format: (_, { versions }) => versions?.map(({ statusHistory }) => h(
            '.flex-row.g1',
            statusHistory.map(({ status, createdAt }) => switchCase(status, {
