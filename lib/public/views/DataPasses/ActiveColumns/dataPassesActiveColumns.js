...
const dataPassDeletedFromMlIcon = tooltip(
    iconDelete(),
...
export const dataPassesActiveColumns = {
    ...
    name: {
        ...
        visible: true,
        sortable: true,
        format: (name, { versions }) => versions?.every(({ deletedFromMonAlisa }) => deletedFromMonAlisa)
    ...
    description: {
        ...
        visible: true,
        sortable: false,
        format: (_, { versions }) => versions?.map(({ description, deletedFromMonAlisa }) =>
            h(deletedFromMonAlisa ? '.success' : '', h('.flex-row.g1', [deletedFromMonAlisa ? iconDelete() : null, description]))),
        balloon: true,
    },
