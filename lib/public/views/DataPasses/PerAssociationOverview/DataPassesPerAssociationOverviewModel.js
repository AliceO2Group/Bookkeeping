 ...
 */
export class DataPassesPerAssociationOverviewModel extends OverviewPageModel {
    /**
     * Constructor
     */
    constructor() {
     ...
     * @inheritdoc
     */
    async load({ lhcPeriodId, simulationPassId } = {}) {
        if (lhcPeriodId && !simulationPassId) {
            this._lhcPeriodId = lhcPeriodId ?? this._lhcPeriodId;
        } else if (!lhcPeriodId && simulationPassId) {
            this._simulationPassId = simulationPassId ?? this._simulationPassId;
        } else if (lhcPeriodId && simulationPassId) {
            throw new Error('Cannot load Data Pass with respect to LHC Period and Simulation Pass at the same time');
        }
        super.load();
    }
    ...
    getRootEndpoint() {
        ...
        if (this._simulationPassId) {
            params.filter.simulationPassIds = [this._simulationPassId];
        }
    ...
    /**
     * Get id of current simulation pass which data passes are fetched
     */
