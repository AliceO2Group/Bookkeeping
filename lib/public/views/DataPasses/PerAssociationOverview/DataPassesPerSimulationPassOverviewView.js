...
export const DataPassesPerSimulationPassOverviewPage = ({ dataPasses: {
    ...
    return h('', {
        ...
        h('.flex-row.header-container.pv2', filtersPanelPopover(dataPassesPerSimulationPassOverviewModel, dataPassesActiveColumns)),
        h('.w-100.flex-column', [
            table(
                dataPassesPerSimulationPassOverviewModel.items,
                dataPassesActiveColumns,
                { classes: '.table-sm' },
                null,
                { sort: dataPassesPerSimulationPassOverviewModel.overviewSortModel },
            ),
            paginationComponent(dataPassesPerSimulationPassOverviewModel.pagination),
        ]),
    ]);
};
