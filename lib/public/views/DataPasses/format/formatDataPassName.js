...
export const formatDataPassName = ({ name, versions }) => {
    ...
    const someDeleted = versions?.some(({ statusHistory }) => statusHistory[statusHistory.length - 1].status === DataPassVersionStatus.DELETED);

    if (allDeleted) {
        return h('.flex-row.g1', [
            tooltip(h('.gray-darker', iconDelete()), 'The production was removed from MonALISA'),
            name,
        ]);
    } else if (someDeleted) {
        return h('.flex-row.g1', [
            tooltip(h('.gray    ', iconDelete()), 'The production was partially removed from MonALISA'),
            name,
        ]);
    } else {
        return name;
