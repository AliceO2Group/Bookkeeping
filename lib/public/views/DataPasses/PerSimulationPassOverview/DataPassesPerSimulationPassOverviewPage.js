...
/**
 * Return specific classes to style row of data pass which was deleted
 *
 * @param {DataPass} dataPass a data pass
 * @return {string} css classes
 */
const getRowClasses = ({ versions }) => {
    const allDeleted = versions
        ?.every(({ statusHistory }) => statusHistory[statusHistory.length - 1].status === DataPassVersionStatus.DELETED);
    return allDeleted ? '.gray-darker' : '';
};
