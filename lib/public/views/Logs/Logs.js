...
export default class Overview extends Observable {
    ...
    /**
     * Add eligble titles for collapse to the array in the model
     ...
     * @returns {undefined}
     */
    addCollapsableTitle(rowId, name, length) {
        this.titlesCanCollapse.push({ rowId, name });
        const currentRow = parseInt(rowId.substring(3), 10);
        if (!this.hasUpdated && currentRow === length) {
            this.notify();
    ...
    /**
     * Toggle the collapse of a title
     * @param {String} rowId The rowId being collapsed
     * @param {String} name The name of the column to be collapsed
     * @returns {undefined}
     */
    toggleCollapse(rowId, name) {
        if (this.titleShouldCollapse(rowId, name)) {
            this.collapsedTitles = this.collapsedTitles
                .filter((element) => !(element.rowId === rowId && element.name === name));
        } else {
            this.collapsedTitles.push({ rowId, name });
        }
    ...
    /**
     * Returns wether the title should collapse or not
     * @param {String} rowId The rowId to be checked
     * @param {String} name The name of the column to be collapsed
     * @returns {Boolean} Returns wether the title in the row should collapse
     */
    titleShouldCollapse(rowId, name) {
        let shouldCollapse = false;
        this.collapsedTitles.forEach((entry) => {
            if (entry.rowId === rowId && entry.name === name) {
                shouldCollapse = true;
            }
        });
     ...
     * @returns {Boolean} Returns wether the title in the row should collapse
     */
    titleCanCollapse(rowId, name) {
        let canCollapse = false;
        this.titlesCanCollapse.forEach((entry) => {
            if (entry.rowId === rowId && entry.name === name) {
                canCollapse = true;
            }
        });
