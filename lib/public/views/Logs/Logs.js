...
export default class Overview extends Observable {
     ...
     */
    getDetailedEntries() {
        return this.detailed_column_ids;
    }
    collapseAllPosts() {
        this.detailed_column_ids = [];
        this.notify();
    ...
    setDetailedEntries(entries) {
        this.detailed_column_ids = entries;
        this.notify();
    }
    /**
     * Show log entry detailed
     * @param {Integer} id of log to show detailed
     * @returns {undefined}
     */
    addDetailedEntry(id) {
        this.detailed_column_ids.push(id);
        this.notify();
    }
     ...
     * @returns {undefined}
     */
    removeDetailedEntry(id) {
        const index = this.detailed_column_ids.indexOf(id, 0);
        if (index > -1) {
            this.detailed_column_ids.splice(index, 1);
        }
        this.notify();
    }
