...
import { fetchJson } from '../../../utilities/fetch/fetchJson.js';
...
export class LogCreationModel extends Observable {
    ...
    constructor(onCreation, runNumber, parentLogId) {
        ...
        this._title = '';
        this._text = '';
        this._creationTagsPickerModel = new PickerModel();
        ...
        if (parentLogId) {
            ...
            fetchJson(`/api/logs/${parentLogId}`).then(
                ({ data: parentLog }) => {
    ...
    async submit() {
        ...
        const tags = this.tagsPickerModel.selected;
        ...
        // TODO check the failure case
        const response = await fetchClient('/api/logs', options);
        const result = await response.json();

        if (result.data) {
            this._createdLog = RemoteData.success(result.data);
            this._onCreation(result.data.id);
        } else {
            this._createdLog = RemoteData.failure(result.errors || [
                {
                    title: result.error,
    ...
    set parentLog(parentLog) {
        ...
        if (parentLog.isSuccess()) {
            ...
            if (this.tagsPickerModel.hasOnlyDefaultSelection()) {
                this.tagsPickerModel.selected = tags;
            }
            if (!this.runNumbers) {
