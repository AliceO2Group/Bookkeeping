...
export class LogCreationModel extends Observable {
    ...
    constructor(onCreation, relations) {
        ...
        const { runNumbers = [], environmentIds = [], lhcFillNumbers = [], template = '', description = '', system = '' } = relations;
    ...
    async submit() {
        ...
        const { title, text, parentLogId, runNumbers, environments, lhcFills, attachments, template, description, system } = this;
        ...
        // The field system and description should be allowed only if the log template is On-call
        if (template) {
            options.body.template = template;

            if (template === 'on-call') {
                options.body.description = description;
                options.body.system = system;
            }
        }
    ...
    /**
     * Returns the template of the log
