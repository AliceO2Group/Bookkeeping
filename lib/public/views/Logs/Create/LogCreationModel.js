...
export class LogCreationModel extends Observable {
    ...
    constructor(onCreation, runNumber, parentLogId) {
        ...
        this._logType = 'New log';
        this._titleLabelText = 'Title*';
        this._title = '';
        this._text = '';
        ...
        if (parentLogId) {
            ...
            this._parentLog = RemoteData.notAsked();
        }
    ...
    async submit() {
        ...
        const { title, text, parentLogId, runNumbers, attachments } = this;
        ...
        if (attachments.length > 0 || tagsTexts.length > 0) {
    ...
    /**
     * Returns the current logtype text
     *
     * @returns {string} the logtype text
     */
    get logType() {
    ...
    /**
     * Set the logType text to: Reply to log: ${title} when the log is a reply
     *
     * @param {string} value of the new logType
     */
    set logType(value) {
        this._logType = value;
    }
    ...
    /**
     * Returns the current titleLableText
     *
     * @returns {string} the title lable text
     */
    get titleLableText() {
        return this._titleLabelText;
    }

    /**
     * Set the titleLableText to Reply title when the log is a reply
     *
     * @param {string} value the new titleLableText
     */
    set titleLabelText(value) {
        this._titleLabelText = value;
    }
    ...
    /**
     * Returns the run numbers to which the log will be linked to, as a coma separated string
     *
    ...
    set parentLog(parentLog) {
        ...
        if (parentLog.isSuccess()) {
            ...
            if (!this.title) {
                this.title = `${title}`;
                this.titleLabelText = 'Reply title';
                this.logType = `Reply to log: ${title}`;
            }
     ...
     */
    get isReady() {
        return this._parentLog.isSuccess() || this._parentLog.isNotAsked();
    }
