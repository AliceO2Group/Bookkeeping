...
/**
 * Transform list of values to common well defined format:
 * single string of @see DATA_DISPLAY_DELIMITER delimited values in ascending order
 * @param {any[]|unedfined} values values to be formatted
 * @return {string|undefined} formatted output
 */
const transformToCommonFormat = (values) => values?.sort().join(DATA_DISPLAY_DELIMITER);
...
export class LogCreationModel extends Observable {
     ...
     */
    constructor(onCreation, parentLogId, { runNumbers, lhcFillNumbers, environmentIds }) {
        super();
        ...
        this._runNumbers = transformToCommonFormat(runNumbers);
        this._lhcFills = transformToCommonFormat(lhcFillNumbers);
        this._environments = transformToCommonFormat(environmentIds);
    ...
    /**
     * Return wheter model with some input is reusable - can be modifed after exting create-log page
     * @param {Number} parentLogId parent log identifier to compare with one currently set in model
     * @param {string[]} preFilledRelations.runNumbers the run numbers to link to the log being created
     * @param {string[]} preFilledRelations.lhcFillNumbers the fill numbers to link to the log being created
     * @param {string[]} preFilledRelations.environmentIds the environment ids to link to the log being created
     * @returns {Boolean} true if model with preserved modifications can be resued
     */
    isReusable(parentLogId, { runNumbers, lhcFillNumbers, environmentIds }) {
        const parentLogRunNumber = this._parentLog?.payload.runs?.map(({ runNumber }) => Number(runNumber));
        const areRunNumbersFromAPITheSame = this._runNumbers === transformToCommonFormat(runNumbers);
        const areRunNumbersFromParentLogTheSame = this._runNumbers === transformToCommonFormat(parentLogRunNumber);

        const areRunNumberTheSame = areRunNumbersFromAPITheSame || areRunNumbersFromParentLogTheSame;
        const areLhcFillsTheSame = this._lhcFills === transformToCommonFormat(lhcFillNumbers);
        const areenvironmentsTheSame = this._environments === transformToCommonFormat(environmentIds);
        const haveModelBeenUsedAlready = this._createdLog?.match({
            NotAsked: () => true,
            Loading: () => false,
            Success: () => false,
            Failure: () => false,
        });
        return this._parentLogId === parentLogId
            && areRunNumberTheSame
            && areLhcFillsTheSame
            && areenvironmentsTheSame
            && !haveModelBeenUsedAlready;
    }
}
