...
const TemplateClasses = Object.freeze({
...
const logTemplatesFactory = (key, templateData) => {
    const templateClass = TemplateClasses[key] ?? null;
    if (templateClass) {
...
export class TemplatedLogCreationModel extends LogCreationModel {
    ...
    useTemplate(key, templateData = {}) {
        ...
        const applyTemplateTags = () => {
            this._creationTagsPickerModel.selectionModel.match({
                Success: (selectionModel) => {
                    for (const tag of templateModel?.tags$?.getCurrent() ?? []) {
                        selectionModel.select(tag);
                    }
                },
                Other: () => {
                    // Do nothing, tags are not available yet
                },
            });
        };
