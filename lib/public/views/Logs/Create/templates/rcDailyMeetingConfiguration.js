...
/**
 * Display a list of logs grouped by tags clusters
 *
 * @param {RemoteData<Map<string, Log[]>, ApiError>} logsMappedByTagsClusters the logs grouped by tags clusters
 * @return {Component} the logs display component
 */
const displayRemoteLogsClusters = (logsMappedByTagsClusters) => logsMappedByTagsClusters.match({
    NotAsked: () => null,

    /**
     * Display logs clusters created between noon last day and noon today
     *
     * @param {Map<string, Log[]>} logs logs to display
     * @return {Component} the logs display
     */
    Success: (logs) => h('ul', [...logs.entries()].map(([label, logs]) => h('li', { key: label }, [
        label,
        h('ul', [
            logs.map(({ id, title }) => h(
                'li',
                { key: id },
                frontLink(title, 'log-detail', { id }, { onclick: () => true, target: '_blank' }),
            )),
        ]),
    ]))),
...
export const rcDailyMeetingConfiguration = (creationModel, template) => {
    ...
    return {
        mainPane: [
            ...
            template.runsToCheck.match({
                ...
                Success: (runsToCheck) => runsToCheck.length
                    ? table(
                        ...
                        {
                            ...runsActiveColumns,
                            ...createRunDetectorsActiveColumns(template.physicalDetectors.match({
            ...
            h(PanelComponent, [
                h(LabelPanelHeaderComponent, 'Last 24h'),
                displayRemoteLogsClusters(template.systemLogs),
            ]),
            h('h4', 'Detectors'),
            h(PanelComponent, [
                h(LabelPanelHeaderComponent, 'Last 24h'),
                displayRemoteLogsClusters(template.detectorsLogs),
            ]),
