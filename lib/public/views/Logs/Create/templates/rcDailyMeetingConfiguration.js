        ...
        mainPane: [
                ...
                template.lhcFills.match({
                    ...
                    Success: (lhcFills) => lhcFills.length
                        ? h('ul', lhcFills.map((lhcFill) => h('li', [
                            ...
                            `Duration: ${formatDuration(lhcFill.stableBeamsDuration ? lhcFill.stableBeamsDuration * 1000 : null)}`,
                        ].join(' - '))))
            ...
            h(PanelComponent, [
                template.runsToCheck.match({
                    NotAsked: () => null,
                    Success: (runs) => markdownInput(
                        runs,
                        {
                            id: 'runs-to-check',
                            onchange: (e) => {
                                template.runsToCheck = RemoteData.success(e.target.value);
                            },
                            height: '10em',
                        },
                    ),
