...
export class RcDailyMeetingTemplate extends Observable {
    ...
    constructor() {
        ...
        const dataFetchingPeriod = {
            from: new Date(startOfToday - 20 * MILLISECONDS_IN_ONE_DAY / 2).setHours(12),
        ...
        getRemoteData(buildUrl('/api/lhcFills/stable-beams-ended-within', dataFetchingPeriod)).then(
            ({ data: lhcFills }) => {
        ...
        this._getRunsToCheckMarkdownText(dataFetchingPeriod).then(
            (runs) => {
        ...
        if (lhcFills.length) {
            ...
            for (const lhcFill of lhcFills) {
                textParts.push([
                    ...
                    `Duration: ${formatDuration(lhcFill.stableBeamsDuration)}`,
                ].join(' - '));
            }
        } else {
    ...
    async _getRunsToCheckMarkdownText(dataFetchingPeriod) {
        const { data: runs } = await getRemoteData(buildUrl(
            '/api/runs',
            { filter: { o2end: dataFetchingPeriod, definitions: RunDefinition.Physics } },
        ));
