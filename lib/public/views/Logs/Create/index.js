...
const createLogScreen = (model) => {
    ...
    const allRuns = model.runs.getRuns();
    const selectedTags = model.logs.getSelectedTags();
    const selectedRuns = model.logs.getSelectedRuns();
    const attachments = model.logs.getAttachments();
    const runNumbers = model.logs.getRunNumbers();
    ...
    return h('div#create-log', [
        ...
        h('', {
            ...
            // h('select#runs.form-control', {
            //     multiple: true,
            //     onchange: (e) => model.logs.setSelectedRuns(e.target.selectedOptions),
            // }, allRuns.isSuccess() ? [
            //     ...allRuns.payload.map((run) => h('option', {
            //         value: run.id,
            //         selected: selectedRuns.includes(run.id),
            //     }, run.id)),
            // ] : h('option', { disabled: true }, 'Loading runs...')),
