...
const createLogScreen = (model) => {
    ...
    const runNumber = model.logs.getRunNumber();
    ...
    return h('div#create-log', [
        ...
        h('', {
            ...
            h('label.form-check-label.f4.mt2', { for: 'run-number' }, 'Run number (optional)'),
            h('input#run-number.form-control.w-100', {
                type: 'number',
                placeholder: 'Enter the run number...',
                value: runNumber,
                oninput: (e) => model.logs.setRunNumber(e.target.value),
            }),
