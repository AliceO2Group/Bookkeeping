...
const createScreen = (model) => {
    ...
    const errorMessages = model.logs.getErrorMessages()
        || [{ detail: 'There was a problem with posting the log, try again later' }];

    return h('div#create-log', [
        error && h('div#errors', errorMessages.map((error) =>
            h('.danger', [h('span', error.detail), h('br', '')]))),
        h('', [
