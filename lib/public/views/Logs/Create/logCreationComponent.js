...
const attachmentsPanel = (logCreationModel) => {
    ...
    const filePreviewSeperator = h('span.mr2', ',');
    ...
    return labeledInput(
        ...
        h('.flex-column.p2.g2', [
            h('.flex-row.justify-between.items-center', [
                h('label.flex-row.g2.items-center', {
                    for: 'attachments',
                }, [
                    h('.btn', 'Choose files'),
                    h('', fileText),
            ...
            attachments.length > 0 && h(
                '#attachmentNames.flex-column',
                [...attachments].map((attachment) => {
                    const itemSeperator = attachment === attachments[attachments.length - 1] ? null : filePreviewSeperator;
                    return [filePreviewComponent(filePreviewModel, attachment, itemSeperator)];
                }),
            ),
        ]),
    );
