...
import { showFilePreview } from '../FilePreview/filePreviewComponent.js';
...
const attachmentsPanel = (logCreationModel) => {
    const { attachments, filePreviewModel } = logCreationModel;
    const filePreviewSeperator = [h('span.mr2', ','), h('br')];
    ...
    return labeledInput(
        ...
        h('.flex-column.p2.g2', [
            h('.flex-row.justify-between.items-center', [
                h('label.attachment-label', {
                    for: 'attachments',
                }, [
                    h('.btn', 'Choose files'),
                    h('.ml1', fileText),
                ]),
                h('input#attachments.w-33.attachment-input', {
                    type: 'file',
                    multiple: true,
                    onchange: (e) => {
            ...
            attachments.length > 0 && h(
                ...
                [...attachments].map((attachment) => {
                    const isLastItem = attachment === attachments[attachments.length - 1];
                    return [showFilePreview(filePreviewModel, attachment, filePreviewSeperator, isLastItem)];
                }),
            ),
        ]),
    );
