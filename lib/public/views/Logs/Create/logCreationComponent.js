    ...
    return labeledInput(
        ...
        h('.flex-column.p2.g2', [
            ...
            attachments.length > 1 && h('#attachmentNames', {
                ...
                showFilePreview(attachment, index)
            ])),
        ]),
    );
};

const showFilePreview = (attachment, index) => {
    ...
    const previewFileStyle = 'margin: 1%; overflow: scroll; max-height: 99%;'
    ...
    } else if (previewFileTypes.pdf) {
        return previewPopover('PDF');
    } else if (previewFileTypes.doc) {
        return previewPopover('DOC');
    } else if (previewFileTypes.json) {
        ...
        const fileContent = h('pre', {
            ...
            oncreate: (vnode) => {
                reader.onload = () => {
                    ...
                    vnode.dom.innerText = JSON.stringify(jsonContent, null, 2);
                };
