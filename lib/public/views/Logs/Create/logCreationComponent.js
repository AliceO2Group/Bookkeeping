...
import { filePreviewComponent, isPreviewScrollable } from '../FilePreview/filePreviewComponent.js';
...
import { onCallLogCreationConfiguration } from './templates/onCallLogCreationConfiguration.js';
import { rcDailyMeetingConfiguration } from './templates/rcDailyMeetingConfiguration.js';
...
export const logCreationForm = (creationModel, configuration) => {
    ...
    return [
        ...
        h('.flex-row.g3.flex-grow', [
            mainPane && h(`.flex-column.g3.w-${sidePane ? 60 : 100}`, mainPane),
            sidePane && h(`.flex-column.g3.w-${mainPane ? 40 : 100}`, sidePane),
        ]),
    ];
};
...
const titlePanel = (logCreationModel) => h(PanelComponent, [
    h(LabelPanelHeaderComponent, { for: 'title', required: true }, 'Title (between 3 and 140 characters)'),
...
const attachmentsPanel = (logCreationModel) => {
    ...
    return h(PanelComponent, [
        ...
        attachments?.length > 0 && h('#attachments-list.flex-row.flex-wrap.p2', attachments.flatMap((attachment) => {
            ...
            return [
                ...
                preview
                    ? popover(
                        ...
                        h(
                            '.p2.w-100.h-100.flex-row.items-center',
                        ...
                        ),
                        {
                            ...
                            scroll: isPreviewScrollable(attachment.type),
                        },
                    )
                    : h('', attachment.name),
            ];
...
const tagsPanel = (logCreationModel) => {
    ...
    return h(
        ...
        [
            ...
            h(
                '.flex-column.flex-control-overflow.g2.p2',
...
const templates = {
    ...
    ['rc-daily-meeting']: {
        label: 'Rc daily meeting',
        configuration: rcDailyMeetingConfiguration,
    },
};
...
export const logCreationComponent = (creationModel) => {
    ...
    console.log(creationModel.templateModel, creationModel.templateModel)
                    ...
                    [
                        ...
                        ...Object.entries(templates)
                            .map(([template, configuration]) => h(
                                'option',
                                { key: template, value: template, selected: template === creationModel.templateKey },
