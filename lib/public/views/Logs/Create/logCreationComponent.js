...
import { markdownInput, markdownDisplay } from '../../../components/common/markdown/markdown.js';
 ...
 */
export const logCreationComponent = (creationModel, opts) => {
    ...
    return [
        ...
        creationModel.parentLog.match({
            ...
            Loading: () => spinner(),
            Success: (parentLog) => logCreationForm(creationModel, parentLog, opts),
 ...
 */
const logCreationForm = (creationModel, parentLog, opts) => [
    ...
    h('.w-100.flex-row.g3.flex-grow', [
        h('.w-60.flex-column.g3', [
            parentLog === null ? titlePanel(creationModel)
                // : h('.flex-row', [
                //     creationModel.isCollapsed ?
                //         h(`.text-ellipsis.w-100#log-text-${parentLog.id}`, parentLog.text)
                //         : markdownDisplay(
                //             parentLog.text,
                //             {
                //                 class: 'flex-column v-resize w-100',
                //                 resize: 'vertical',
                //             },
                //             { height: '10rem' },
                //         ),
                //     [
                //         creationModel.isCollapsed
                //             ? primaryButton('Show', () => creationModel.isCollapsed = false, `show-collapse-${parentLog.id}`)
                //             : primaryButton('Collapse', () => creationModel.isCollapsed = true, `collapse-${parentLog.id}`),
                //     ],
                // ]),
                : logComponent(
                    parentLog,
                    ...
                    () => creationModel.isCollapsed = true,
                    () => creationModel.isCollapsed = false,
                    () => creationModel.onCopyUrlSuccess(),
                    { ...opts, goToDetailsButton: true, replayButton: false },
                ),
