...
export const logCreationComponent = (creationModel) => {
    ...
    return [
        ...
        creationModel.parentLog.match({
            NotAsked: () => logCreationForm(creationModel),
            Loading: () => spinner(),
...
const titlePanel = (logCreationModel) => labeledInput(
    { for: 'title' },
    `${logCreationModel.titleLableText}`,
    h('input#titlefield.form-control.panel-body.bg', {
        placeholder: 'Enter the title of the log entry...',
        minlength: 3,
        maxlength: 140,
        value: logCreationModel.title,
        disabled: logCreationModel.parentLog.isSuccess(),
        oninput: (e) => {
