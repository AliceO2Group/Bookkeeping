...
const logDetailScreen = (model) => {
    ...
    if (data.isSuccess()) {
        ...
        const treeItem = (item, level, is_collapsed) => h(
            '.mw-100',
            {
                style: {
            ...
            is_collapsed ? PostBoxCollapsed(model, item, item.id, item.id === id) : PostBox(model, item, item.id, item.id === id),
        );
        ...
        return h('', [
            ...
            h('.w-100.flex-column', {
                oncreate: () => {
                    scrollTo(`#post${id}`, 150);
                    if (!data.payload[0].children || data.payload[0].children.length === 0) {
                        model.logs.showPostDetailed(data.payload[0].id);
                    }
                },
            }, tree(data.payload[0])),
        ]);
