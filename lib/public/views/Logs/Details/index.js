...
const logDetailScreen = (model) => {
    ...
    const detailed_columns = model.logs.getDetailedEntries();
    ...
    if (data.isSuccess()) {
        ...
        const tree = (obj, level = 0, items = []) => {
            if (level === 0) {
                let collapsed = true;
                if (detailed_columns.includes(obj.id)) {
                    collapsed = false;
                }
                items.push(treeItem(obj, level, collapsed));
                return tree(obj, level + 1, items);
            }

            for (const child of obj.children || []) {
                let collapsed = true;
                if (detailed_columns.includes(child.id)) {
                    collapsed = false;
                }
                items.push(treeItem(child, level, collapsed));
