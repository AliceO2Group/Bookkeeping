...
export class LogTreeViewModel extends Observable {
    ...
    setShowCopyUrlSuccessContent(id, value) {
        ...
        console.log(this._logTreeLeafs.payload.find(leaf => leaf.log.id == id))
    }

    /**
     * Given to the copy url component so that it can rerender after a timeout
     * @return {void}
     */
    onCopyUrlSuccess(id) {
    ...
    async _fetchLogTree(selectedLogId) {
        ...
        try {
            ...
            const parseTreeNode = (node, level) => {
                ...
                logTreeLeafs.push({ log: log, level: level, showCopyUrlSuccessContent: false });
