    ...
    return logTreeLeafsRemoteData.match({
        ...
        Success: (logTreeLeafs) => {
            ...
            const logTreeLeafDisplay = (log, level, isCollapsed) => h(
                ...
                isCollapsed
                    ? logCollapsed(log, log.id === selectedLogId, () => treeViewModel.expandLog(log.id))
                    : logExpanded(log, log.id === selectedLogId, () => treeViewModel.collapseLog(log.id), authenticationToken),
            );
