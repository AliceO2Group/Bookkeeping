    ...
    return logTreeLeafsRemoteData.match({
        ...
        Success: (logTreeLeafs) => {
            ...
            const logTreeLeafDisplay = (log, level, isCollapsed) => h(
                ...
                isCollapsed
                    ? logCollapsed(log, log.id === selectedLogId, !(log.title === parentLogTitle), () => treeViewModel.expandLog(log.id))
                    : logExpanded(log, log.id === selectedLogId, !(log.title === parentLogTitle), () => treeViewModel.collapseLog(log.id), authenticationToken),
            );
