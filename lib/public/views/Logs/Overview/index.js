...
const logOverviewScreen = (model) => {
    ...
    return h('', {
        ...
        Success: (logs) => showLogsTable(model, logs),
 ...
 * @param {Array<JSON>} logs list of logs retrieved from server
...
const showLogsTable = (model, logs) => {
    ...
    return [
        ...
        h('.w-100.flex-column', [
            table(logs, logsColumns, model, (_) => ({}), ''),
            pagination(model),
        ]),
    ];
};
 ...
 * @param {Logs.js} logs Model to access Logs functions
 ...
 * @param {JSON} logsColumns - JSON containing columns displayed and their properties
