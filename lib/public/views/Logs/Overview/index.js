...
const logOverviewScreen = (model) => {
    ...
    return h('', { onremove: () => model.logs.clearLogs(),
        onclick: () => {
            if (model.logs.getShowFilters()) {
                const elementFilters = document.getElementById('filters');
                const elementButton = document.getElementById('openFilterToggle');
                elementFilters.classList.remove('display-block');
                elementFilters.classList.add('display-none');
                elementButton.innerText = 'Open filters';
            }
        } }, [
        ...
        h('.flex-row.header-container', [
            ...
            h(`.w-15.filters
            ${(model.logs.isAnyFilterActive() && !model.logs.getShowFilters()) ? '.display-block' : '.display-none'}`,
                h(`.f5`), "Active filters: " + model.logs.getActiveFilters().join(', ')),
