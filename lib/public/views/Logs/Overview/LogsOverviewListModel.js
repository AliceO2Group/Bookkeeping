 ...
 */
export class LogsOverviewListModel extends Observable {
    /**
    ...
    async fetchReplies(parentLogNode) {
        const { log: parentLog } = parentLogNode;
        ...
        try {
            ...
            RemoteData.success(logs.map((log) => ({ ...log, children: RemoteData.NotAsked() })));
        } catch (errors) {
