...
export const filePreviewComponent = (filePreviewModel, attachment, seperator, url = undefined, downloadAttributes = null) => {
    ...
    /**
     * Creates a default button for downloading a file
     *
     * @returns {vnode} a clickable link for downloading the file if possible
     */
    ...
    /**
     * Handles the remote data for the preview popover, for each RemoteData type
     *
     * @param {vnode} showSuccessPopover the preview popover to show
     * @returns {vnode|null} the vnode to return, based on the condition
     */
    const defaultRemoteDataHandler = (showSuccessPopover) => filePreviewModel.getAttachmentPreview(attachment.name).match({
        NotAsked: () => null,
        Loading: () => h('span', attachment.name),
        Success: () => showSuccessPopover,
        Failure: () => defaultDownloadButton(),
    });
    ...
    if (previewFileTypes.image) {
        ...
        const fileContent = h('img.preview-file-content', {
            alt: attachment.name,
            src: filePreviewModel.getAttachmentPreview(attachment.name).payload,
        });

        return defaultRemoteDataHandler(previewPopover(fileContent));
        ...
        const fileContent = h(
            ...
            filePreviewModel.getAttachmentPreview(attachment.name).payload,
        );
