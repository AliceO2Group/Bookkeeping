 ...
 */
export const showFilePreview = (filePreviewModel, attachment, seperator, url = undefined, downloadAttributes = null) => {
    ...
    const previewPopover = (content) => popover(
        h('a.attachment', {
            key: attachment.name,
        }, [iconEye(), attachment.name, seperator]),
        h('', {
    ...
    if (previewFileTypes.image) {
        ...
        if (downloadAttributes) return h('a', { ...downloadAttributes, href: url }, [iconCloudDownload(), attachment.name]);
        else return h('span.attachment', {
            key: attachment.name,
        }, [attachment.name, seperator]);
    }
};
