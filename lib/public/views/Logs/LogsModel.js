...
export class LogsModel extends Observable {
    ...
    loadCreation({ runNumbers, lhcFillNumbers, environmentIds }) {
        ...
        if (runNumbers) {
            relations.runNumbers = (Array.isArray(runNumbers) ? runNumbers : [runNumbers]).map(parseInt);
        }
        if (lhcFillNumbers) {
            relations.lhcFillNumbers = (Array.isArray(lhcFillNumbers) ? lhcFillNumbers : [lhcFillNumbers]).map(parseInt);
        }
        if (environmentIds) {
            relations.environmentIds = (Array.isArray(environmentIds) ? environmentIds : [environmentIds]).map(parseInt);
        }

        this._creationModel = new LogCreationModel(this.handleLogCreation.bind(this), relations);
