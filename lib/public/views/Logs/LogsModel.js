 ...
 */
export default class LogsOverviewModel extends Observable {
    /**
    ...
    constructor(model) {
        ...
        this._creationModel = new LogCreationModel((logId) => this.model.router.go(`/?page=log-detail&id=${logId}`));
    ...
    loadOverview() {
        if (! this._overviewModel.pagination.isInfiniteScrollEnabled) {
            this._overviewModel.fetchAllLogs();
        }
    }
