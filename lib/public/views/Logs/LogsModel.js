...
export class LogsModel extends Observable {
     ...
     * @param {string} commaSeparatedString The comma-separated string of numbers to parse
     * @return {number[]} The parsed numbers
     */
    _parseCommaSeparatedNumbers(commaSeparatedString) {
        return commaSeparatedString.split(',').map((numberString) => parseInt(numberString.trim(), 10));
    }
