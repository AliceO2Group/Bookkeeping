...
export class LogsModel extends Observable {
    ...
    /**
     * Parse the input to an array of numbers
     * @param {string|string[]} input the input to parse
    ...
    _parseNumbers(input) {
        ...
        // If it's a string, split by comma, convert each part to an integer, and return the array
    ...
    loadCreation({ runNumbers, lhcFillNumbers, environmentIds }) {
        ...
        if (runNumbers) {
            relations.runNumbers = this._parseNumbers(runNumbers);
        }
        if (lhcFillNumbers) {
            relations.lhcFillNumbers = this._parseNumbers(lhcFillNumbers);
        }
        if (environmentIds) {
