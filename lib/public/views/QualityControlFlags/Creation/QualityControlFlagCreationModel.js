...
export class QualityControlFlagCreationModel extends Observable {
    ...
    getFlagToDisply() {
        const p = {
            ...
            timeStart: this._timeStartModel?.value,
            timeEnd: this._timeEndModel?.value,
        };
    ...
    get flagReason() {
        return this._flagReasonsSelectionModel.selectedOptions[0]?.original ?? { name: 'NEW*' };
    }
    ...
    get timeStart() {
        return this._timeStartModel?.value;
    }

    get timeEnd() {
        return this._timeEndModel?.value;
    }
