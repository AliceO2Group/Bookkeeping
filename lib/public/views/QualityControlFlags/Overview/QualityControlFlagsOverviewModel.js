...
export class QualityControlFlagsOverviewModel extends OverviewPageModel {
    ...
    async _fetchRun() {
        ...
        try {
            ...
            this._observableRun.setCurrent(RemoteData.filure(error));
        }
    }
    ...
    async _fetchDataPass() {
        ...
        try {
            ...
            this._observableRun.setCurrent(RemoteData.success(targetDetector));
        } catch (error) {
            this._observableRun.setCurrent(RemoteData.filure(error));
        }
    }
    ...
    async _fetchDetector() {
        ...
        try {
            const { data: [detector] } = await getRemoteData(`/api/dataPasses/filter[ids][]=${this._dataPassId}`);
            this._observableRun.setCurrent(RemoteData.success(detector));
        } catch (error) {
            this._observableRun.setCurrent(RemoteData.filure(error));
        }
    }
