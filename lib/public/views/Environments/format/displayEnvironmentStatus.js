...
export const displayEnvironmentStatus = ({ status: currentStatus, historyItems }) => {
    const coloredStatusDisplay = coloredEnvironmentStatusComponent(currentStatus);
    if (historyItems.some(({ status }) => status === 'ERROR')) {
        return h('.flex-row', h(
            '.flex-row.g2.justify-center.items-center',
            [coloredStatusDisplay, tooltip(iconWarning(), h('.black', 'Environment has been in ERROR state'))],
        ));
    }

    return coloredStatusDisplay;
};
