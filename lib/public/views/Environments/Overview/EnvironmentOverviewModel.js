...
export class EnvironmentOverviewModel extends Observable {
    ...
    /**
     * Returns whether sucessfuly copied parent identifier to clipboard
     *
     * @return {boolean} true if identifier was copied successfuly
     */
    get showCopySuccessContent() {
        return this._showCopySuccessContent;
    }

    /**
     * A callback given to the copyIdentifierButton component so that it can rerender after a timeout
     *
     * @returns {void}
     */
    onCopySuccess() {
        this._showCopySuccessContent = true;
        this.notify();
        setTimeout(() => {
            this._showCopySuccessContent = false;
            this.notify();
        }, 2000);
    }
}
