...
export const HomePage = ({ home: { logsOverviewModel, runsOverviewModel, lhcFillsOverviewModel } }) => {
    const rowCount = Math.round(estimateDisplayableRowsCount(TABLEROW_HEIGHT, PAGE_USED_HEIGHT) / 2) - 2;

    logsOverviewModel.pagination.provideDefaultItemsPerPage(rowCount, MAX_ROWS);
    lhcFillsOverviewModel.pagination.provideDefaultItemsPerPage(rowCount, MAX_ROWS);
    runsOverviewModel.pagination.provideDefaultItemsPerPage(rowCount, MAX_ROWS);
    ...
    return (
        h('div', [
            h('.flex-row.w-100', [
                ...
                h('.flex-column.w-50', [
                    h('h3', 'LHC Fills'),
                    table(lhcFillsOverviewModel.lhcFills, lhcFillsActiveColumns, null, { profile: 'home' }),
                ]),
            ]),
