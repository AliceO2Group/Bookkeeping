...
const HomeOverviewScreen = (model) => {
    ...
    model.logs.pagination.provideDefaultItemsPerPage(rowCount, MAX_ROWS);
    model.runs.pagination.provideDefaultItemsPerPage(rowCount);
    model.lhcFills.overviewModel.pagination.provideDefaultItemsPerPage(rowCount);

    const logs = model.logs.getLogs();
    const runs = model.runs.getRuns();
    const fills = model.lhcFills.overviewModel.lhcFills;
    ...
    return (
        h('div', [
            h('.flex-row.w-100', [
                ...
                h('.flex-column.w-50', [
                    h('h3', 'Runs'),
                    table(runs, runsActiveColumns(model.runs), null, { profile: 'home' }),
                ]),
            ]),
            h('.flex-row.w-100.mv3', [
                h('.flex-column.w-100', {
                    ...
                    h('h3', 'LHC Fills'),
                    table(fills, lhcFillsActiveColumns(model.lhcFills), null),
                ]),
            ]),
        ]));
};
