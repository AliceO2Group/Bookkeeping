...
export const runDetailsComponent = (runDetailsModel, router) => runDetailsModel.runDetails.match({
    ...
    Success: (run) => {
        ...
        const { flp: flpInfologger, epn: epnInfologger } = getInfologgerLink({ environmentId: run.environmentId, runNumber: run.runNumber });
        return [
            h('.flex-column.w-100', [
                h('.w-100.flex-row', [
            ...
            h('', tabbedPanelComponent(
                ...
                {
                    [RUN_DETAILS_PANELS_KEYS.LOGS]: (dataLogs) => activeColumnsTable(dataLogs, logsActiveColumns, null, { profile: 'embeded' }),
                    [RUN_DETAILS_PANELS_KEYS.FLPS]: (dataFlps) => activeColumnsTable(dataFlps, flpsActiveColumns, {
                        callback: (entry) => ({
                            style: {
                                cursor: 'pointer',
                            },
