...
export const runDetailsComponent = (runDetailsModel, router) => runDetailsModel.runDetails.match({
    ...
    Success: (run) => {
        ...
        const { environmentId } = run;

        const flpInfologgerLink = flpInfologgerLinkComponent({ environmentId });
        const epnInfologgerLink = epnInfologgerLinkComponent({ environmentId });
