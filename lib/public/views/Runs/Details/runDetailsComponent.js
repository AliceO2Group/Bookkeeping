...
export const runDetailsComponent = (runDetailsModel, router) => runDetailsModel.runDetails.match({
    ...
    Success: (run) => {
        ...
        // Link options with runNumber as default, and environmentId or lhcFillNumber if available
        const linkOptions = {
            runNumbers: [run.runNumber],
            ...run.environmentId ? { environmentIds: [run.environmentId] } : {},
            ...!run.environmentId && run.lhcFill && run.lhcFill.fillNumber ? { lhcFillNumbers: [run.lhcFill.fillNumber] } : {},
        };
