        ...
        return [
            ...
            h('', tabbedPanelComponent(
                ...
                },
                {
                    ...
                    [RUN_DETAILS_PANELS_KEYS.DPL_PROCESSES]: (detectorsRemoteData) => detectorsRemoteData.match({
                        NotAsked: () => null,
                        Loading: () => spinner({ size: 5, absolute: false }),

                        /**
                         * Display a detector table
                         *
                         * @param {DplDetectorTreeNodeModel[]} detectorNodes the detectors nodes to display
                         * @return {vnode} the table
                         */
                        Success: (detectorNodes) => {
                            if (detectorNodes.length === 0) {
                                return h('p', 'NO DETECTOR');
                            }
                            return h('.flex-column.g3', detectorNodes.map((detectorNode) => collapsibleTreeNode(
                                detectorNode,
