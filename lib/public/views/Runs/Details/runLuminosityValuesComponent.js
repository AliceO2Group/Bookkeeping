...
export const runLuminosityValuesComponent = (run, ctpTriggerCounters) => {
    ...
    if (pdpBeamType === PdpBeamType.PROTON_PROTON) {
        ...
        triggers = ctpTriggerCounter.lmb;
        ...
        triggers = ctpTriggerCounter.l1a;
    }
    const triggerRate = runDuration && triggers !== null ? triggers / runDuration : null;
    ...
    return [
        h('.flex-column.items-center.flex-grow', [
            ...
            h('', formatFloat(integratedLuminosity)),
        ]),
        h('.flex-column.items-center.flex-grow', [
            h('strong', 'Pile up'),
