...
export class RunDetailsModel extends Observable {
    ...
    constructor(runId, panelKey) {
        ...
        this.allDetectors = RemoteData.notAsked();
    ...
    async _fetchAllDetectors() {
        this.allDetectors = RemoteData.loading();
        this.notify();

        try {
            const allDetectors = await detectorsProvider.getAll();
            this.allDetectors = RemoteData.success(allDetectors);
        } catch (error) {
            this.allDetectors = RemoteData.failure(error);
        }
