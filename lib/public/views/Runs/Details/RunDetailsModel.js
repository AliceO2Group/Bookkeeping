...
export class RunDetailsModel extends Observable {
     ...
     * Constructor
     *
     * @param {number} runNumber the id of the run to display
     * @param {string|null|undefined} [panelKey=null] the key of the current panel
     */
    constructor({ runNumber, runId }, panelKey) {
        super();
        ...
        this._tabbedPanelModel = new RunDetailsTabbedPanelModel(runNumber, panelKey);
    ...
    set runNumber(runNumber) {
        if (runNumber && runNumber !== this._runNumber) { // TODO Explain thy it is fired by this#_handleFetchRemoteRun
            this._runNumber = runNumber;
    ...
    async _fetchOneRun() {
        ...
        this.notify();
        if (this._runNumber) {
            return this._handleFetchRemoteRun(() => getRemoteData(`/api/v2/runs/${this._runNumber}`));
        } else if (this._runId) {
            return this._handleFetchRemoteRun(() => getRemoteData(`/api/runs/${this._runId}`));
        } else {
            throw new Error('No run identifier during run details loading attempt');
        }
    }
    ...
    async updateOneRun() {
        ...
        await this._handleFetchRemoteRun(() => jsonFetch(`/api/v2/runs/${runNumber}`, options));
    }
    ...
    async _handleFetchRemoteRun(fetchRuns) {
        try {
            ...
            this._runNumber = run.runNumber; // Do not understand why but this line fires RunDetailsModel#setter<$runNumber>
        } catch (error) {
...
class RunDetailsTabbedPanelModel extends TabbedPanelModel {
     ...
     * Constructor
     *
     * @param {number} runNumber the run number of the current run
     * @param {string|null|undefined} [panelKey=null] the current panel key (null will use default)
     */
    constructor(runNumber, panelKey) {
    ...
    async _fetchLogsPanelData() {
        ...
        if (this._runNumber) {
            try {
                const { data: logsOfRun } = await getRemoteData(`/api/v2/runs/${this._runNumber}/logs`);
    ...
    async _fetchFlpsPanelData() {
        ...
        if (this._runNumber) {
            try {
                const { data: flpsOfRun } = await getRemoteData(`/api/v2/runs/${this._runNumber}/flps`);
