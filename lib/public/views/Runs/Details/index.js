...
const runDetailScreen = (model) => {
    ...
    if (data.isSuccess()) {
        ...
        return h('', [
            h('h2.mv2', {
                onremove: () => {
                    model.runs.clearRuns(),
                    model.runs.flushModel();
                },
            }, 'Run Tree'),
            h('.w-100.flex-column', {
...
export default (model) => [runDetailScreen(model)];
