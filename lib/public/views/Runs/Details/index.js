...
const runDetails = (model) => {
    ...
    if (!runs.isLoading() && runs.isNotAsked()) {
        model.runs.fetchAllRuns();
    }
    if (runs.isSuccess()) {
        const i = runs.payload.findIndex((x)=> x.id == model.router.params.id);
        if (i > 0) {
            nextId = runs.payload[i - 1].id;
        }
        i < runs.payload.length - 1 && i != -1 ?
            prevId = runs.payload[i + 1].id :
            model.runs.getSelectedPage() < model.runs.getTotalPages() &&
                model.runs.setSelectedPage(model.runs.getSelectedPage() + 1, true);
    }
        ...
        return [
            h('.flex-column.w-100', [
                    ...
                    h('.w-60.flex-row.items-center', {
                        ...
                        !model.runs.isEditModeEnabled ?
                            [
                                ...
                                model.isAdmin() && h('button.btn#edit-run', {
                                    onclick: () => {
                                        model.runs.isEditModeEnabled = true;
                                        model.runs.runChanges = { key: 'eorReasons', value: data.payload.eorReasons };
                                    },
                                }, 'Edit Run'),
                            ]
                            : [
                ...
                h('.flex-row.w-100', [
                    ...
                    h('.w-30.ph3', lhcFillPanel(model)),
                    model.runs.isEditModeEnabled && editTagsPanel(model),
                ]),
                h('.flex-row.w-100.justify-center', [
                    prevId && h('button.btn.btn-primary.m2#prev-run', {
                        onclick: () => model.router.go(`/?page=run-detail&id=${prevId}`),
                    }, 'Prev'),
                    nextId && h('button.btn.btn-primary.m2#next-run', {
                        onclick: () => model.router.go(`/?page=run-detail&id=${nextId}`),
                    }, 'Next'),
                ]),
            ]),
            h('.w-100', [
