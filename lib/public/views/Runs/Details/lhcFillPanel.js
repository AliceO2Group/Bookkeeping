...
const lhcFillPanel = (model) => {
    const { lhcFill, run } = model.runs;
    return [
        h('strong', 'LHC Data'),
        lhcFill.match({
            NotAsked: () => {
                model.runs.fetchLhcFill();
                return null;
            },
            Loading: () => spinner({ size: 5, absolute: false }),
            Success: (payload) => detailsList(lhcFillDisplayConfiguration, payload || {}, 'lhc-fill', '-'),
            Failure: (error) => error.map(errorAlert),
        }),
        run.match({
            NotAsked: () => null,
            Loading: () => spinner({ size: 5, absolute: false }),
            Success: (payload) => detailsList(runLhcFillDisplayConfiguration, payload, 'run-lhc-fill', '-'),
            Failure: (error) => error.map(errorAlert),
        }),
    ];
};
