 ...
 * @param {RunDetailsModel} runDetailsModel run details model
 * @returns {Component} run's LHC fill information
 */
const lhcFillPanel = (runDetailsModel) => {
    const { run } = runDetailsModel;
    return [
        h('strong', 'LHC Data'),
        run.match({
            NotAsked: () => null,
            ...
            Success: (payload) => [
                detailsList(commonLhcFillDisplayConfiguration, payload.lhcFill || {}, 'lhc-fill', '-'),
                detailsList(runLhcFillDisplayConfiguration, payload, 'run-lhc-fill', '-'),
            ],
