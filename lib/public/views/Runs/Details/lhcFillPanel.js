...
export const lhcFillPanel = (run) => [
    ...
    run.match({
        ...
        Success: (runPayload) => runPayload.lhcBeamMode === BeamModes.STABLE_BEAMS ? [
            detailsList(commonLhcFillDisplayConfiguration, runPayload.lhcFill || {}, { selector: 'lhc-fill' }),
            detailsList(runLhcFillDisplayConfiguration, runPayload, { selector: 'run-lhc-fill' }),
        ] : h('div', h('em#NoLHCDataNotStable', contextualInfo(`No LHC Fill information, beam mode was: ${runPayload.lhcBeamMode}`))),
