...
export class RunPatch extends Observable {
    ...
    constructor(run) {
         ...
         */
        this._run = run;
        this._requireCalibrationStatusChangeReason = run
            && run.definition === RunDefinition.Calibration
            && run.calibrationStatus === RunCalibrationStatus.FAILED;
     ...
     */
    isValid() {
        // Patch is valid if we do not require calibration change reason
        return !this._requireCalibrationStatusChangeReason
            // Or the calibration status have not changed
            || this._calibrationStatus === RunCalibrationStatus.FAILED
            // Or we have a reason filled
    ...
    toPojo() {
        ...
        if (this._requireCalibrationStatusChangeReason) {
