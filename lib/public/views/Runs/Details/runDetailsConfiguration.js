...
import { formatEditableNumericValue } from '../format/formatEditableNumericValue.js';
...
export const runDetailsConfiguration = (runDetailsModel) => ({
    ...
    tags: {
        name: 'Tags',
        visible: true,
        format: (tags) => formatTagsList(tags, { flex: true }),
    },
    timeO2Start: {
        name: 'O2 Start',
        visible: true,
    ...
    muInelasticInteractionRate: {
        ...
        visible: (_, { pdpBeamType }) => pdpBeamType === 'pp',
        format: formatFloat,
    },
    avgInelasticInteractionRate: {
        name: ['INEL', h('sub', 'avg')],
        visible: (_, { pdpBeamType }) => pdpBeamType === 'PbPb' || pdpBeamType === 'pp',
        format: (_, run) => formatEditableNumericValue(run, 'avgInelasticInteractionRate', runDetailsModel, { unit: 'Hz' }),
    },
    inelasticInteractionRateAtStart: {
        name: ['INEL', h('sub', 'start')],
        visible: (_, { pdpBeamType }) => pdpBeamType === 'PbPb',
        format: (_, run) => formatEditableNumericValue(run, 'inelasticInteractionRateAtStart', runDetailsModel, { unit: 'Hz' }),
    },
    inelasticInteractionRateAtMid: {
        name: ['INEL', h('sub', 'mid')],
        visible: (_, { pdpBeamType }) => pdpBeamType === 'PbPb',
        format: (_, run) => formatEditableNumericValue(run, 'inelasticInteractionRateAtMid', runDetailsModel, { unit: 'Hz' }),
    },
    inelasticInteractionRateAtEnd: {
        name: ['INEL', h('sub', 'end')],
        visible: (_, { pdpBeamType }) => pdpBeamType === 'PbPb',
        format: (_, run) => formatEditableNumericValue(run, 'inelasticInteractionRateAtEnd', runDetailsModel, { unit: 'Hz' }),
    },
    tfbDdMode: {
        name: 'TFB DD Mode',
        visible: true,
