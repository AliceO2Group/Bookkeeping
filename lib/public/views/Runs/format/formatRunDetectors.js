    ...
    return detailsModel.allDetectors.match({
        ...
        Success: (allDetectors) => {
            const detectorsBadges = h(
                ...
                allDetectors.map(({ id, name }) => {
                    const quality = detectorsQualities.find((x) => x.id == id) ? detectorsQualities.find((x) => x.id == id).quality : null;
            ...
            if (detailsModel.isEditModeEnabled && (run.timeTrgEnd || run.timeO2End)) {
                detectorsBadges.children = detectorsBadges.children.filter((elem) => !elem.attrs.className.includes('b-gray'));
                return h(
                    '',
                    dropdown(
