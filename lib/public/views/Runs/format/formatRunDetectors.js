...
import { editableDetectors } from '../../../utilities/access/rbacGetEditableDetectors.js';
 ...
 */
export const formatRunDetectors = (run, detailsModel, roles) => {
    const { detectorsQualities } = run;
    if (!detectorsQualities || detectorsQualities.length === 0) {
        return '-';
    }
    const { allDetectors: areAllDetectorsAllowed, detectors: allowedDetectorsArray } = editableDetectors(roles);

    /**
     * Creates a component containing the detector badges
     *
     * @param {RunDetectorQuality[]} detectors list of detectors
     * @param {Boolean} checkEditRights whether to check the user's edit rights for each detector
    ...
    return detailsModel.allDetectors.match({
        ...
        Success: (allDetectors) => {
            if (detailsModel.isEditModeEnabled && (run.timeTrgEnd || run.timeO2End)) {
                const validDetectors = !areAllDetectorsAllowed
                    ? detectorsQualities.filter((detector) => allowedDetectorsArray.includes(detector.name))
                    : detectorsQualities;
                return h(
                    '',
                    dropdown(
                        detailsModel.editionDetectorsDropdownModel,
                        detectorsBadges(validDetectors),
                        h(
                            '.scroll-y',
                            validDetectors.map(({ id, name, quality }) => h(
