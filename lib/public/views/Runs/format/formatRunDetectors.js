...
import { iconBan, iconCheck, iconX } from '/js/src/icons.js';
...
export const formatRunDetectors = (run, detailsModel) => {
    ...
    return detailsModel.allDataTakingDetectors.match({
        NotAsked: () => [],
        Loading: () => [],
        Success: (allDataTakingDetectors) => {
            if (detailsModel.isEditModeEnabled && (run.timeTrgEnd || run.timeO2End)) {
                ...
                return detectorsBadges(allDataTakingDetectors);
            }
        },
        Failure: (error) => error,
    });
