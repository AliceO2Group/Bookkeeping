...
export const formatRunDuration = (run, raw = false) => {
    ...
    if (timeTrgEnd || timeO2End) {
        ...
        if (missingTimeTrgStart && missingTimeTrgEnd) {
            ...
            warningPopover = 'Duration based on o2 start because of missing start trigger information';
        } else if (missingTimeTrgEnd) {
            warningPopover = 'Duration based on o2 stop because of missing end trigger information';
        }

        return raw
            ? formatDuration(runDuration)
            : h('.flex-row.g2', [formatDuration(runDuration), warningPopover && popover(iconWarning(), warningPopover)]);
    }
