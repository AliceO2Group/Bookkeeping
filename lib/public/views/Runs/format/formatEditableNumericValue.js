/**
 * @license
 * Copyright CERN and copyright holders of ALICE O2. This software is
 * distributed under the terms of the GNU General Public License v3 (GPL
 * Version 3), copied verbatim in the file "COPYING".
 *
 * See http://alice-o2.web.cern.ch/license for full licensing information.
 *
 * In applying this license CERN does not waive the privileges and immunities
 * granted to it by virtue of its status as an Intergovernmental Organization
 * or submit itself to any jurisdiction.
 */

import { formatFloat } from '../../../utilities/formatting/formatFloat.js';
import { h } from '/js/src/index.js';

/**
 * Format a editable numeric property of run
 *
 * @param {Run} run run
 * @param {string} propertyName updateable property name
 * @param {RunDetailsModel} runDetailsModel details model
 * @param {number} [options.inputPrecision = 0.001] precision of input value
 * @param {number} [options.displayPrecision = 3] precision of displayed value
 * @return {Component} display or input
 */
export const formatEditableNumericValue = (
    run,
    propertyName,
    runDetailsModel,
    { inputPrecision = 0.001, displayPrecision = 3 } = {},
) => {
    if (runDetailsModel.isEditModeEnabled) {
        return h('input.form-control', {
            type: 'number',
            step: inputPrecision,
            value: runDetailsModel.runPatch.formData[propertyName],
            onchange: (e) => runDetailsModel.runPatch.patchFormData({ [propertyName]: parseFloat(e.target.value) || null }),
        });
    } else {
        const value = run[propertyName];
        return value !== null && value !== undefined ? h('', formatFloat(run[propertyName], { precision: displayPrecision })) : '-';
    }
};
