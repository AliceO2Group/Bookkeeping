 ...
 */
export const RunsPerLhcPeriodOverviewPage = ({ runs: { perSimulationPassOverviewModel: runsPerSimulationPassOverviewModel }, modalModel }) => {
    runsPerSimulationPassOverviewModel.pagination.provideDefaultItemsPerPage(estimateDisplayableRowsCount(
        TABLEROW_HEIGHT,
        PAGE_USED_HEIGHT,
    ));

    const { items, detectors } = runsPerSimulationPassOverviewModel;
    ...
    return h('', [
        h('.flex-row.header-container.pv2', exportRunsTriggerAndModal(runsPerSimulationPassOverviewModel, modalModel)),
        h('.flex-column.w-100', [
            table(detectors.isSuccess() ? items : detectors, activeColumns, null, { profile: 'runsPerSimulationPassPeriod' }),
