 ...
 */
export const RunsPerSimulationPassOverviewPage = ({ runs: {
    perSimulationPassOverviewModel: runsPerSimulationPassOverviewModel }, modalModel }) => {
    runsPerSimulationPassOverviewModel.pagination.provideDefaultItemsPerPage(estimateDisplayableRowsCount(
        TABLEROW_HEIGHT,
        PAGE_USED_HEIGHT,
    ));

    const { items: runs, detectors } = runsPerSimulationPassOverviewModel;
