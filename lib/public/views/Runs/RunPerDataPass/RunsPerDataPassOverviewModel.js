...
export class RunsPerDataPassOverviewModel extends FixedPdpBeamTypeRunsOverviewModel {
    ...
    async _fetchSkimmableRuns() {
        ...
        try {
            ...
            this._skimmableRuns$.setCurrent(RemoteData.failure(runToReadyForSkimmingFlag));
        } catch (error) {
    ...
    /**
     * Update skimmable runs for given data pass
     *
     * @param {number} runNumber run number
     * @return {Promise<void>} resolves once data are fetched
     */
    async updateSkimmableRuns(runNumber) {
            ...
            Success: async (currentSkimmableRuns) => {
                ...
                try {
                    const { data: skimmableRuns } = await jsonPut(
                        ...
                        { data: [{ runNumber, readyForSkimming: !currentSkimmableRuns[runNumber] }] },
                    );
                    const runToReadyForSkimmingFlag = Object.fromEntries(skimmableRuns
                        .map(({ runNumber, readyForSkimming }) => [runNumber, readyForSkimming]));

                    this._skimmableRuns$.setCurrent(RemoteData.success({ ...currentSkimmableRuns, ...runToReadyForSkimmingFlag }));
                } catch (error) {
                    this._skimmableRuns$.setCurrent(RemoteData.failure(error));
                }
            },
    ...
    getRootEndpoint() {
        ...
        if (!this._gaqNotBadFractionFilterModel.isEmpty) {
            const { operator, operand } = this._gaqNotBadFractionFilterModel.value;
            filter.gaq = {
                notBadFraction: {
                    [encodeURIComponent(operator)]: operand / 100, // API does not use percentage but fraction
                },
                mcReproducibleAsNotBad: this._mcReproducibleAsNotBad,
            };
        }
