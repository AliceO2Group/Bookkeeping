...
export class RunsPerDataPassOverviewModel extends FixedPdpBeamTypeRunsOverviewModel {
    ...
    constructor(model) {
        ...
        this._recalculateGaqActionState$ = new ObservableData(RemoteData.notAsked());
        this._recalculateGaqActionState$.bubbleTo(this);
    ...
    /**
     * Recalculate GAQ summary for current data pass and optionally given run numbers
     *
     * @param {number[]} [runNumbers] optional run numbers to recalculate GAQ summary for
     * if not provided, GAQ summary will be recalculated for all runs of the data pass
     * @return {Promise<void>} resolves once request is handled
     */
    async recalculateGaqSummary(runNumbers) {
        this._recalculateGaqActionState$.setCurrent(RemoteData.loading());
        try {
            const queryParams = { dataPassId: this._dataPassId, onlyInvalidated: false };
            if (runNumbers?.length) {
                queryParams.runNumbers = runNumbers.join(',');
            }
            await jsonFetch(
                buildUrl('/api/qcFlags/summary/gaq/recalculate', queryParams),
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                },
            );
            this._recalculateGaqActionState$.setCurrent(RemoteData.success(null));
            if (runNumbers?.length) {
                for (const runNumber of runNumbers) {
                    this._fetchGaqSummary(runNumber);
                }
            } else {
                this._fetchGaqSummaryForCurrentRuns();
            }
        } catch (error) {
            this._recalculateGaqActionState$.setCurrent(RemoteData.failure(error));
        }
    }

    /**
     * Return the state of the recalculate GAQ action
     *
     * @return {RemoteData<null, ApiError>} the recalculate GAQ action state
     */
    get recalculateGaqActionState() {
        return this._recalculateGaqActionState$.getCurrent();
    }
    ...
    async _fetchGaqSummary(runNumber) {
        ...
        await this._gaqSummarySources[runNumber].fetch(url, { headers: { 'Cache-Control': 'no-cache' } });
    }
    ...
    _fetchGaqSummaryForCurrentRuns() {
        ...
        this._item$.getCurrent().match({
            Success: (runs) => {
                ...
                const gaqSummary = Object.fromEntries(runNumbers.map((rn) => [rn, RemoteData.notAsked()]));
    ...
    async _fetchSkimmableRuns() {
        this._skimmableRuns$.setCurrent(RemoteData.loading());
        try {
            const url = buildUrl('/api/dataPasses/skimming/runs', { dataPassId: this._dataPassId });
            const { data: skimmableRuns } = await getRemoteData(url, { headers: { 'Cache-Control': 'no-cache' } });
