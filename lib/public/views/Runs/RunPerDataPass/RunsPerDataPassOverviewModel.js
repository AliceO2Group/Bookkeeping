...
export class RunsPerDataPassOverviewModel extends FixedPdpBeamTypeRunsOverviewModel {
     ...
     * @return {void}
     */
    async _fetchGaqSummaryForCurrentRuns() {
        ...
        this._item$.getCurrent().match({
            Success: async (runs) => {
                ...
                const runNumberMap = runNumbers.map((runNumber) => [runNumber, RemoteData.loading()]);
                this._gaqSummary$.setCurrent(Object.fromEntries(runNumberMap));

                // Fetch all in parallel so redraws can be prevented
                const results = await Promise.all(runNumbers.map(async (runNumber) => {
                    if (signal.aborted) {
                        return [runNumber, RemoteData.notAsked()];
                    }
                    const url = buildUrl('/api/qcFlags/summary/gaq', {
                        dataPassId: this._dataPassId,
                        mcReproducibleAsNotBad: this._mcReproducibleAsNotBad,
                    ...
                    try {
                        ...
                        return [runNumber, RemoteData.success(data.data)];
                    } catch (e) {
                        if (signal.aborted) {
                            return [runNumber, RemoteData.notAsked()];
                        }
                        return [runNumber, RemoteData.failure(e)];
                    }
                }));

                if (!signal.aborted) {
                    this._gaqSummary$.setCurrent(Object.fromEntries(results));
                }
            },
            Other: () => {},
        });
