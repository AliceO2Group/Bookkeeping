...
export const setQcFlagsTriggerAndModal = (runsOverviewModel, modalModel, { autoMarginLeft = true } = {}) =>
    h(`button.btn.btn-primary.w-15.h2${autoMarginLeft ? '.mlauto' : ''}#set-qc-flags-trigger`, {
        disabled: runsOverviewModel.items.match({
            Success: (payload) => payload.length === 0,
            NotAsked: () => true,
            Failure: () => true,
            Loading: () => true,
        }),
        onclick: () => modalModel.display({ content: (modalModel) => setQcFlagsModal(runsOverviewModel, modalModel), size: 'medium' }),
    }, 'Set QC Flags');
