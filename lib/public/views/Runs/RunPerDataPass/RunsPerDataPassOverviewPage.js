...
import { getGAQSummaryDisplay } from '../ActiveColumns/getGAQSummaryDisplay.js';
...
export const RunsPerDataPassOverviewPage = ({
    ...
    const activeColumns = {
        ...
        globalAggregatedQuality: {
            ...
            format: (_, { runNumber }) => getGAQSummaryDisplay(
                remoteGaqSummary[runNumber],
                dataPassId,
                runNumber,
                () => perDataPassOverviewModel.recalculateGaqSummary([runNumber]),
            ),
            filter: ({ filteringModel }) =>
                numericalComparisonFilter(filteringModel.get('gaq').notBadFraction, { step: 0.1, selectorPrefix: 'gaqNotBadFraction' }),
            ...
            h('#actions-dropdown-button', DropdownComponent(
                ...
                h('.flex-column.p2.g2', [
                    ...
                    sessionService.hasAccess([BkpRoles.DPG_ASYNC_QC_ADMIN]) && [
                        h(
                            'button.btn.btn-primary.w-100.h2',
                            {
                                ...recalculateGaqActionState.match({
                                    Loading: () => ({
                                        disabled: true,
                                        title: 'Loading',
                                    }),
                                ...
                                onclick: () => {
                                    if (confirm('Recalculate the GAQ summary for the entire data pass?')) {
                                        perDataPassOverviewModel.recalculateGaqSummary();
                                    }
                                },
                                id: 'recalculate-gaq-summary-trigger',
                            },
                            'Recalculate GAQ summary',
                        ),
                        h(
