...
export const RunsPerDataPassOverviewPage = ({ runs: { perDataPassOverviewModel }, modalModel }) => {
    ...
    const { items, detectors, dataPass } = perDataPassOverviewModel;
    ...
    return h('', [
        h('.flex-row.header-container.pv2', [
            h('.f4.gray-darker.bg-gray-lighter.br4', `Good, physics runs of ${dataPass?.name}`),
            exportRunsTriggerAndModal(perDataPassOverviewModel, modalModel),
        ]),
        h('.flex-column.w-100', [
            table(detectors.isSuccess() ? items : detectors, activeColumns, null, { profile: 'runsPerDataPass' }),
