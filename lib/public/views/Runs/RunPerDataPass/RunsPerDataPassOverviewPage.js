 ...
 *
 * @param {RunsPerDataPassOverviewModel} runsModel the model of the runs overview
 * @param {QueryRouter} router router
 * @returns {Component} the toggle switch
 */
export const toggleMcReproducibleAsNotBad = (runsModel, router) => h('#mcReproducibleAsNotBadToggle', switchInput(
    runsModel.mcReproducibleAsNotBad,
    () => router.go(`?page=runs-per-data-pass&dataPassId=${runsModel.dataPassId}&mcReproducibleAsNotBad=${!runsModel.mcReproducibleAsNotBad}`),
...
export const RunsPerDataPassOverviewPage = ({
    ...
    const activeColumns = {
        ...runsActiveColumns,
        ...cachedPdpBeamTypes.has('PbPb')
            ? inelasticInteractionRateActiveColumnsForPbPb
            : {},
        ...cachedPdpBeamTypes.has('pp')
    ...
    return h('', { onremove: () => perDataPassOverviewModel.reset(false) }, [
        h('.flex-row.justify-between.items-center.g2', [
            ...
            toggleMcReproducibleAsNotBad(perDataPassOverviewModel, router),
