...
export const RunsPerDataPassOverviewPage = ({ runs: { perDataPassOverviewModel }, modalModel }) => {
    ...
    const titlePrefix = h('h2', { style: 'white-space: nowrap;' }, 'Physics Runs');
    ...
    return h('', [
        h('.flex-row.justify-between.items-center.g2', [
            h(
                ...
                dataPass.match({
                    Success: (payload) => breadcrumbs(titlePrefix, h('h2', payload.name)),
                    Failure: (payload) => [titlePrefix, tooltip(h('.f3', iconWarning()), errorAlert(payload))],
                    Loading: () => [titlePrefix, spinner({ size: 2, absolute: false })],
                    NotAsked: () => [titlePrefix, tooltip(h('.f3', iconWarning()), errorAlert([{ detail: 'No data was asked for' }]))],
                }),
            ),
