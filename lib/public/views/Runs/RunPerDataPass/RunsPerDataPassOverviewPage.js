...
export const RunsPerDataPassOverviewPage = ({
    ...
    const selectedDetectors = Array.from(document.getElementsByClassName('select-multi-flag'))
        .filter((detector) => detector.checked).map((detector) =>
            [detector.getAttribute('runNumber'), detector.getAttribute('dplDetectorId')]);
    ...
    return h('', { onremove: () => perDataPassOverviewModel.reset(false) }, [
        h('.flex-row.justify-between.items-center.g2', [
            ...
            frontLink(
                ...
                {
                    'runNumbers': selectedDetectors.map((item) => item[0]),
                    'dplDetectorIds': selectedDetectors.map((item) => item[1]),
