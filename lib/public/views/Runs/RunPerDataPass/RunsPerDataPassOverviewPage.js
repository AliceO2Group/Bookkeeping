...
const skimmableControl = (dataPass, onclick, requestResult) => {
    ...
    if (validSkimmableProductionNameRegex.test(name) && pdpBeamType === PdpBeamType.PROTON_PROTON) {
        ...
        return requestResult.match({
            ...
            Loading: () => h('button.btn.primary', { onclick }, h('.flex-row', ['Sending request...'])),
...
export const RunsPerDataPassOverviewPage = ({
                ...
                ? {
                    readyForSkimming: {
                        ...
                        format: (_, { runNumber }) => remoteSkimmableRuns.match({
                            Success: (skimmableRuns) => h(
                                `button.btn.primary${skimmableRuns[runNumber] ? '.success' : '.warning'}`,
                                { onclick: () => perDataPassOverviewModel.updateSkimmableRuns(runNumber) },
                                skimmableRuns[runNumber] ? 'ready' : 'not ready',
                            ),
                            Loading: () => '... ...',
    ...
    return h('', { onremove: () => perDataPassOverviewModel.reset(false) }, [
        ...
        markAsSkimmableRequestResult.match({ // TODO handle other errors
