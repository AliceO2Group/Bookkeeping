        ...
        ...remoteGaqSummary.match({
            Success: (gaqSummary) => ({
                globalAggregatedQuality: {
                    ...
                    /**
                     * GAQ not bad fraction filter component
                     *
                     * @param {RunsOverviewModel} runsOverviewModel the runs overview model
                     * @return {Component} the gaq filter
                     */
                    filter: (runsOverviewModel) => numericalComparisonFilter(
                        runsOverviewModel.filteringModel.get('gaq').notBadFractionFilterModel,
        ...
        h('.flex-row.justify-between.items-center.g2', [
            ...
            frontLink(
                h('button.btn.btn-primary.w-100.h2}#set-qc-flags-trigger', {
                    disabled: perDataPassOverviewModel.runDetectorsSelectionModel.selectedQueryString.length < 1,
                }, 'Set QC Flags'),
