...
let _activeColumnsCache = null;
let _activeColumnsCacheKeys = null;

const getActiveColumns = (detectors, dataPass, qcSummary, remoteDplDetectorsUserHasAccessTo, perDataPassOverviewModel) => {
    const {
        dataPassId,
    ...
    const cacheKeys = [detectors, dataPass, qcSummary, mcReproducibleAsNotBad, remoteDplDetectorsUserHasAccessTo];

    if (_activeColumnsCache && cacheKeys.every((key, i) => key === _activeColumnsCacheKeys[i])) {
        return _activeColumnsCache;
    }
    ...
    const columns = {
        ...
        globalAggregatedQuality: {
            ...
            format: (_, { runNumber }) => {
                const runGaqSummary = perDataPassOverviewModel.gaqSummary[runNumber];
                ...
                return runGaqSummary.match({
                    ...
                    Loading: () => tooltip(spinnerEl),
                    NotAsked: () => tooltip(spinnerEl),
                    Failure: () =>
                        frontLink(
    ...
    detectors && dataPass && Object.assign(
        ...
        createRunDetectorsAsyncQcActiveColumns(
            ...
            { dataPass },
            { profile: 'runsPerDataPass', qcSummary, mcReproducibleAsNotBad },
        ),
    );

    _activeColumnsCache = columns;
    _activeColumnsCacheKeys = cacheKeys;
    return columns;
};
...
export const RunsPerDataPassOverviewPage = ({
    ...
    const activeColumns = getActiveColumns(detectors, dataPass, qcSummary, remoteDplDetectorsUserHasAccessTo, perDataPassOverviewModel);
    ...
    return [
        h('.flex-row.justify-between.items-center.g2', [
            ...
            h('#actions-dropdown-button', DropdownComponent(
                ...
                h('.flex-column.p2.g2', [
                    exportTriggerAndModal(perDataPassOverviewModel.exportModel, modalModel, { autoMarginLeft: false }),
                    frontLink(
