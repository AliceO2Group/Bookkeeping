...
const exportForm = (model, items, modalHandler, activeColumns) => {
    ...
    const exportFields = model.getExportFields() || [];
    const exportFormat = model.getExportFormat() || exportTypes[0];
    const selectableFields = Object.keys(activeColumns);
    const enabled = exportFields.length > 0 && items;
    ...
    return [
        ...
        h('select#fields.form-control', {
            ...
            multiple: true,
            onchange: ({ target }) => model.setExportFields(target.selectedOptions),
        }, [
            ...selectableFields
        ...
        h('.flex-row.justify-between', [
                ...
                h('.flex-row.g3', exportTypes.map((exportType) => {
                    ...
                    return h('.form-check', [
                        h('input.form-check-input', {
                            ...
                            type: 'radio',
                            value: exportType,
                            checked: exportFormat.length ? exportFormat.includes(exportType) : false,
                            name: 'runs-export-type',
                            onclick: () => model.setExportFormat(exportType),
                        }),
            ...
            h('', [
                ...
                h('input', {
                    ...
                    onchange: (e) => model.exportName = e.target.value,
                }),
            ]),
        ]),
