    ...
    return h('', { onremove: () => model.runs.clearRuns() }, [
        ...
        h('.flex-row.justify-between.items-center', [
            ...
            h('button.btn.btn-primary.w-15.h2#create', {
                onclick: () => model.router.go('/?page=create-run-entry'),
            }, 'Add Entry'),
        ]),
        h('.flex-row.mv1', [
            h('.w-15', filters(model, runsColumns)),
            h('.w-85', data.isSuccess() && h('.w-100.flex-column.mh3', [
                table(data.isSuccess() ? data.payload : [], runsColumns, (entry) => ({
...
export default (model) => [runOverviewScreen(model)];
