...
const showRunsTable = (model, runs) => {
    ...
    return [
        h('.flex-row.header-container.pv2', [
            filtersPanelPopover(model.runs, runsActiveColumns),
...
const exportRunsButton = (model) => {
    const { runs: { overviewModel: runsOverviewModel } } = model;
    return h('button.btn.btn-primary.w-15.h2.mlauto#export-runs-trigger', {
        disabled: runsOverviewModel.runs.isSuccess() && runsOverviewModel.runs.payload.length === 0,
