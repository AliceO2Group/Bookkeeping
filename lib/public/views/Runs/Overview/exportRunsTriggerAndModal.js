...
const exportForm = (runsOverviewModel, runs, modalHandler) => {
    ...
    // const selectedExportType = runsOverviewModel.getSelectedExportType() || EXPORT_TYPES[0];

    const selectedRunsFields = exportModel.selectedFields || [];
    const selectedExportType = exportModel.exportType || EXPORT_TYPES[0];

    const runsFields = Object.keys(runsActiveColumns);
    const loadingData = !runs;
    const exportEnabled = selectedRunsFields.length > 0 && !loadingData;

    return [
        loadingData
            ? spinner({ size: 2, absolute: false })
            : runsOverviewModel.areExportItemsTruncated
                ? h(
        ...
        h('select#fields.form-control', {
            ...
            multiple: true,
            onchange: ({ target }) => {
                runsOverviewModel.setSelectedRunsFields(target.selectedOptions);
        ...
        h('.flex-row.g3', EXPORT_TYPES.map((exportType) => {
            ...
            return h('.form-check', [
                h('input.form-check-input', {
                    ...
                    type: 'radio',
                    value: exportType,
                    checked: exportModel.exportType === exportType, //selectedExportType.length ? selectedExportType.includes(exportType) : false,
                    name: 'runs-export-type',
                    onclick: () => {
                        runsOverviewModel.setSelectedExportType(exportType);
