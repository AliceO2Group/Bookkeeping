...
export class RunsWithQcModel extends RunsOverviewModel {
     ...
     */
    get filterQuery() {
        const filter = this.filteringModel.normalized; // comes from superclass. 
        filter.detectorsQc = {
            mcReproducibleAsNotBad: this._mcReproducibleAsNotBad,
        };

        return { filter, include: { effectiveQcFlags: true } }
    }
    ...
    getRootEndpoint(filterQuery = this.filterQuery) {
        return buildUrl(super.getRootEndpoint(super.filterQuery), filterQuery);
    }
