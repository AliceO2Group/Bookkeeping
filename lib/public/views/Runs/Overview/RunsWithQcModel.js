...
export class RunsWithQcModel extends RunsOverviewModel {
    ...
    constructor(model) {
        ...
        this.mcReproducibleAsNotBad.observe(() => {
            this.load();
        });
        ...
        this._filteringModel.put('detectorsQc', new MultiCompositionFilterModel({ mcReproducibleAsNotBad: this._mcReproducibleAsNotBad }));
    }
     ...
     */
    getRootEndpoint() {
        return buildUrl(super.getRootEndpoint(), { include: { effectiveQcFlags: true } });
    }

    /**
     * Get mcReproducibleAsNotBad
     *
     * @return {ToggleFilterModel} mcReproducibleAsNotBad
     */
    get mcReproducibleAsNotBad() {
    ...
    registerDetectorsNotBadFractionFilterModels(detectors$) {
        const detectorsQc = this._filteringModel.get('detectorsQc');

        detectors$.observe((observableData) => observableData.getCurrent().apply({
            Success: (detectors) =>
                detectors.forEach(({ id }) =>
                    detectorsQc.putFilter(
                        `_${id}`, // This should probably be changed to a flat name in the future
                        new MultiCompositionFilterModel({ notBadFraction: new NumericalComparisonFilterModel({ scale: 0.01, integer: false }) }),
                    )),
        }));
    }
