...
export class RunsOverviewModel extends OverviewPageModel {
    ...
    constructor(model) {
        ...
        // Content
        this._exportableItems = RemoteData.notAsked();
    ...
    async loadExportableRuns() {
        ...
        if (this._exportableItemsEndpoint === endpoint && this._exportableItems.isSuccess() || this._exportableItems.isLoading()) {
            return;
        } else {
            this._exportableItemsEndpoint = endpoint;
        }

        this._exportableItems = RemoteData.loading();
        this.notify();

        try {
            const { items } = await getRemoteDataSlice(endpoint);
            this._exportableItems = RemoteData.success(items);
        } catch (errors) {
            this._exportableItems = RemoteData.failure(errors);
        }
