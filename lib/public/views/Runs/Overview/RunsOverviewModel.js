...
export class RunsOverviewModel extends OverviewPageModel {
    ...
    constructor(model) {
        ...
        // Content
        this._exportableRuns = RemoteData.notAsked();

        this.reset(false);
        // eslint-disable-next-line no-return-assign,require-jsdoc
        const updateDebounceTime = () => this._debouncedFetchAllRuns = debounce(this.load.bind(this), model.inputDebounceTime);
     ...
     */
    get allRuns() {
        if (this._loadExportableRuns.isNotAsked()) {
            this._loadExportableRuns();
        }

        return this._loadExportableRuns;
    }
     ...
     * @private
     */
    async _loadExportableRuns() {
        if (this.items.isSuccess() && this.items.payload.length === this._pagination.itemsCount) {
            this._exportableRuns = RemoteData.success([...this.runs.payload]);
            this.notify();
            return;
        }
        this._exportableRuns = RemoteData.loading();
        this.notify();
        ...
        try {
            ...
            this._exportableRuns = RemoteData.success(items);
        } catch (errors) {
            this._exportableRuns = RemoteData.failure(errors);
        }
    ...
    _applyFilters(now = false) {
        ...
        now ? this.load() : this._debouncedFetchAllRuns(true);
    }
}
