...
import { NumericComparisonOperatorSelectionModel }
    from '../../../components/Filters/common/filters/NumericComparisonOperatorSelectionModel.js';
import { NumericFilterModel } from '../../../components/Filters/common/filters/NumericFilterModel.js';
...
export class RunsOverviewModel extends OverviewPageModel {
    ...
    constructor(model) {
        ...
        this._runDurationOperatorSelectionModel = new NumericComparisonOperatorSelectionModel();
        this._runDurationOperatorSelectionModel.observe(() => this._applyFilters());

        this._nEpnsFilterModel = new NumericFilterModel();
        this._nEpnsFilterModel.observe(() => this._applyFilters());

        this._nDetectorsFilterModel = new NumericFilterModel();
        this._nDetectorsFilterModel.observe(() => this._applyFilters());

        this._nFlpsFilterModel = new NumericFilterModel();
        this._nFlpsFilterModel.observe(() => this._applyFilters());
    ...
    reset(fetch = true) {
        ...
        this._nDetectorsFilterModel.clear(true);

        this._nEpnsFilterModel.clear(true);

        this._nFlpsFilterModel.clear(true);
    ...
    isAnyFilterActive() {
        return (
            ...
            || this._nDetectorsFilterModel.value !== null
            || this._nEpnsFilterModel.value !== null
            || this._nFlpsFilterModel.value !== null
            || this.ddflpFilter !== ''
            || this.dcsFilter !== ''
            || this.epnFilter !== ''
            || this._odcTopologyFullNameFilter !== ''
    ...
    getActiveFilters() {
        ...
        if (this._nDetectorsFilterModel.value !== null) {
            this.activeFilters.push('# of detectors');
        }
        if (this._nEpnsFilterModel.value !== null) {
            this.activeFilters.push('# of epns');
        }
        if (this._nFlpsFilterModel.value !== null) {
     ...
     * @return {NumericComparisonOperatorSelectionModel} selection model
     */
    ...
    /**
     * Returns the detectors amount filter model
     * @return {NumericFilterModel} the amount filter moment
     */
    get nDetectorsFilterModel() {
        return this._nDetectorsFilterModel;
    }

    /**
     * Returns the EPNs amount filter model
     * @return {NumericFilterModel} The current amount of epns filters
     */
    get nEpnsFilterModel() {
        return this._nEpnsFilterModel;
    }

    /**
     * Returns the FLPs amount filter model
     * @return {NumericFilterModel} the amount filter moment
     */
    get nFlpsFilterModel() {
        return this._nFlpsFilterModel;
    }
    ...
    _getFilterQueryParams() {
        return {
            ...
            ...this._nDetectorsFilterModel.value !== null && {
                'filter[nDetectors][operator]': this._nDetectorsFilterModel.operatorSelectionModel.selected,
                'filter[nDetectors][limit]': this._nDetectorsFilterModel.value,
            },
            ...this.nFlpsFilterModel.value && {
                'filter[nFlps][operator]': this._nFlpsFilterModel.operatorSelectionModel.selected,
                'filter[nFlps][limit]': this._nFlpsFilterModel.value,
            },
            ...this._nEpnsFilterModel.value && {
                'filter[nEpns][operator]': this._nEpnsFilterModel.operatorSelectionModel.selected,
                'filter[nEpns][limit]': this._nEpnsFilterModel.value,
            },
