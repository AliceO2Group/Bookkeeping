...
export class RunsOverviewModel extends OverviewPageModel {
     ...
     */
    getRootEndpoint() {
        return `/api/runs?${new URLSearchParams(this._getFilterQueryParams()).toString()}`;
    }
    ...
    isAnyFilterActive() {
        return (
            ...
            || this.nDetectorsFilter?.operator && this.nDetectorsFilter.limit
            || this._nEpnsFilter?.operator && this._nEpnsFilter?.limit
            || this.nFlpsFilter?.operator && this.nFlpsFilter?.limit
            || this.ddflpFilter !== ''
            || this.dcsFilter !== ''
            || this.epnFilter !== ''
            || this._odcTopologyFullNameFilter !== ''
     ...
     */
    get exportableItems() {
        return this._exportableItems.getCurrent();
    }
     ...
     * @private
     */
    async loadExportableRuns() {
        const endpoint = this.getRootEndpoint();
        if (this._exportableItemsEndpoint === endpoint &&
            (this.exportableItems.isSuccess() || this.exportableItems.isLoading())) {
            return;
        } else {
            this._exportableItemsEndpoint = endpoint;
            this._exportableDataSource.fetch(endpoint);
        }
    }
     ...
     */
    get areExportableItemsTruncated() {
        const { exportableItems } = this;
        if (!exportableItems.isSuccess()) {
            return null;
        }
        return exportableItems.payload.length < this._pagination.itemsCount;
    }
