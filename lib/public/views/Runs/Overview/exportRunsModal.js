                ...
                await runsOverviewModel.createRunsExport(
                    runs.map((selectedRun) => {
                        ...
                        const formattedEntries = entries.map(([key, value]) => {
                            const formatExport = runsActiveColumns[key].exportFormat || ((identity) => identity);
 ...
 * @param {Model} runsOverviewModel Pass the model to access the defined functions
 * @param {ModalHandler} modalHandler The modal handler, used to dismiss modal after export
 * @return {vnode} Return the view of the inputs
 */
export const exportRunsModal = (runsOverviewModel, modalHandler) => {
