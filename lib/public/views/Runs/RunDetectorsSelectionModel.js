...
export class RunDetectorsSelectionModel extends Observable {
    ...
    unselectRunDetector(runNumber, detectorId) {
        const originalRunDetectors = this._selectedRunDetectors.get(runNumber) ?? [];
        const runDetectors = originalRunDetectors.filter((selectedDetectorId) => selectedDetectorId !== detectorId);
        ...
        if (runDetectors.length !== originalRunDetectors.length) {
            this.notify();
        }
    }
