...
const activeColumns = (model) => ({
    ...
    detectors: {
        ...
        visible: true,
        size: 'w-20 f6',
        format: (detectors, model) => {
            if (detectors && detectors.length > 0) {
                return [h('.badge.bg-gray-light.mh2.nDetectors-badge', model.nDetectors), `${detectors.toString()}`];
            }
            return '-';
        },
    ...
    tags: {
        ...
        filter: remoteDataTagFilter(model.tags.getTags(), model.runs.listingTagsFilterModel),
    ...
    runDuration: {
        ...
        format: (duration, run) => {
            if (duration === null) {
                return '-';
            } else if (run.timeTrgEnd) {
                return duration !== undefined ? `${Math.floor(duration / 60000)} min` : '-';
            } else {
                return h('strong.success', 'RUNNING');
            }
        },
        exportFormat: (duration, run) => {
            if (duration === null) {
                return '-';
            } else if (run.timeTrgEnd) {
                return duration !== undefined ? `${Math.floor(duration / 60000)} min` : '-';
 ...
 * @return {Object} A collection of columns with parameters for the Run table
 */
export const activeHomeOverviewColumns = (_model) => ({
    runNumber: {
        name: 'Number',
        visible: true,
        size: 'w-20',
