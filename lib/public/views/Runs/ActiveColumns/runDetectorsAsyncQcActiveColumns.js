...
export const createRunDetectorsAsyncQcActiveColumns = (
    ...
    let activeColumnEntries = dplDetectors?.map(({ name: detectorName, id: detectorId, type: dplDetectorType }) => [
        detectorName,
        {
            name: detectorName.toUpperCase(),
            visible: true,
            format: (_, run) => remoteDplDetectorUserHasAccessTo(detectorId, remoteDplDetectorsUserHasAccessTo).match({
                NotAsked: () => null,
    ...
    const filtersEntries = dplDetectors?.map(({ name: detectorName, id: detectorId }) => [
        detectorName,
        {
            name: detectorName.toUpperCase(),
            visible: false,
