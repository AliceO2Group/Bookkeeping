...
const runDetectorAsyncQualityDisplay = ({ dataPassId, simulationPassId }, run, dplDetectorId, qcSummary) => {
    ...
    const missingVerificationDisplay = h('.d-inline-block.va-t-bottom', missingVerificationsCount
        ? tooltip(
            h('.f7', iconWarning()),
    ...
    if (!badEffectiveRunCoverage) {
        if (explicitlyNotBadEffectiveRunCoverage === 1) {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(qcSummaryContent, { color: QC_SUMMARY_COLORS.ALL_GOOD });
        } else {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(qcSummaryContent, { color: QC_SUMMARY_COLORS.PARTIALLY_GOOD_NOT_BAD });
        }
    } else {
        if (!explicitlyNotBadEffectiveRunCoverage && mcReproducible) {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(
                [
                    notBadPercentageFormat,
                    h('.d-inline-block.va-top.f7.italic', 'MC.R'),
                    missingVerificationDisplay,
                ],
            ...
            if (badEffectiveRunCoverage === 1) {
                qcSummaryDisplay = getQcSummaryDisplayWithColor(qcSummaryContent, { color: QC_SUMMARY_COLORS.ALL_BAD });
            } else {
                qcSummaryDisplay = getQcSummaryDisplayWithColor(qcSummaryContent, { color: QC_SUMMARY_COLORS.PARTIALLY_BAD });
            }
        }
    }
