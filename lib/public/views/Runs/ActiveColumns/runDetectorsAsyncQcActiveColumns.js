...
const runDetectorAsyncQualityDisplay = (runNumber, dplDetectorId, qcSummary) => {
    ...
    const {
        bad: {
            effectiveRunCoverage: badEffectiveRunCoverage = null,
            missingVerificationsCount: badMissingVerificationsCount = 0,
        } = {},
        notBad: {
            effectiveRunCoverage: explicitlyNotBadEffectiveRunCoverage = null,
            missingVerificationsCount: notBadMissingVerificationsCount = 0,
        } = {},
    } = qcSummary[runNumber][dplDetectorId];

    const missingVerificationsCount = badMissingVerificationsCount + notBadMissingVerificationsCount;
    const notBadPercentageFormat = h('', [
        formatFloat((1 - badEffectiveRunCoverage) * 100),
