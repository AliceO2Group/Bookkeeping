...
const UNCALCULATABLE_COVERAGE = -1;
...
const runDetectorAsyncQualityDisplay = ({ dataPassId, simulationPassId }, run, dplDetectorId, qcSummary) => {
    ...
    if ([badEffectiveRunCoverage, explicitlyNotBadEffectiveRunCoverage].includes(UNCALCULATABLE_COVERAGE)) {
        qcSummaryDisplay = getQcSummaryDisplayWithColor(
            [
                h('.d-inline-block', tooltip(
                    iconBolt(),
                    'Run start or stop is missing and time-based flag was assigned, cannot calculate coverage',
                )),
                missingVerificationDisplay,
            ],
            {
                color: QC_SUMMARY_COLORS.PARTIALLY_GOOD_NOT_BAD,
            },
        );
