...
const specialDetectorsColumnsConfiguration = {
    VTX: { information: 'Vertexing' },
    MTE: { information: 'ITS/TPC Matching Efficiency' },
    EVS: { information: 'Event Selection' },
    CEN: { information: 'Centrality' },
    EVP: { information: 'Event Plane' },
    MUD: { information: 'Moun Detectors: MCH/MID' },
    GMU: { information: 'Global Moun Detectors: MFT/MCH/MID' },
};
...
export const createRunDetectorsAsyncQcActiveColumns = (
    ...
    let activeColumnEntries = dplDetectors?.map(({ name: detectorName, id: dplDetectorId, type: dplDetectorType }) => [
        detectorName,
        {
            ...
            profiles: profile,
            ...specialDetectorsColumnsConfiguration[detectorName.toUpperCase()],
        },
    ]) ?? [];
    ...
    const filtersEntries = dplDetectors?.map(({ name: detectorName, id: dplDetectorId }) => [
        detectorName,
        {
            ...
            filter: (filteringModel) => {
                const filterModel = filteringModel.get('detectorsQc').getFilter(`_${dplDetectorId}`).getFilter('notBadFraction');
                return filterModel
