 ...
 */
const runDetectorAsyncQualityDisplay = ({ runNumber, runQuality }, dplDetectorId, { dataPassId, simulationPassId }, qcSummary) => {
    const runQualityWasChangedToBad = runQuality === RunQualities.BAD;
    const tooltipTextWhenRunQualityWasChangedToBad = `Quality of the run was changed to ${RunQualities.BAD} so it is no more subject for QC`;
    if (!qcSummary?.[runNumber]?.[dplDetectorId]) { // If there is no QC flag assigned
        return runQualityWasChangedToBad
            ? tooltip(iconX(), tooltipTextWhenRunQualityWasChangedToBad)
            : qcFlagCreationPanelLink({ dataPassId, simulationPassId, runNumber, dplDetectorId });
    }
    ...
    return runQualityWasChangedToBad
        ? qcFlagOverviewPanelLink(tooltip(qcSummaryDisplay, tooltipTextWhenRunQualityWasChangedToBad), { dataPassId, simulationPassId, runNumber, dplDetectorId })
        : qcFlagOverviewPanelLink(qcSummaryDisplay, { dataPassId, simulationPassId, runNumber, dplDetectorId });
};
