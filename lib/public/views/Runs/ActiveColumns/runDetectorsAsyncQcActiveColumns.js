...
const runDetectorAsyncQualityDisplay = (runNumber, dplDetectorId, qcSummary) => {
    ...
    if (!badEffectiveRunCoverage) {
        if (explicitlyNotBadEffectiveRunCoverage === 1) {
            return getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Green.dark });
        } else {
            return getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Gray.dark });
        }
    } else {
        if (badEffectiveRunCoverage === 1) {
            return getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Red.dark });
        } else {
            return getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Orange.dark });
        }
    }
};
