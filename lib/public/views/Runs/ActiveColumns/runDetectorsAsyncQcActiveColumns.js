 ...
 * @param {boolean} mcReproducibleAsNotBad mcReproducibleAsNotBad
 * @return {Component} component
 */
const runDetectorAsyncQualityDisplay = ({ dataPassId, simulationPassId }, run, dplDetectorId, qcSummary, mcReproducibleAsNotBad) => {
    const { runNumber } = run;

    let qcSummaryDisplay = getQcSummaryDisplay(qcSummary[runNumber][dplDetectorId], { mcReproducibleAsNotBad });
...
export const createRunDetectorsAsyncQcActiveColumns = (
    ...
    { profiles, qcSummary, mcReproducibleAsNotBad } = {},
) =>
    Object.fromEntries(dplDetectors?.map(({ name: detectorName, id: dplDetectorId }) => [
        detectorName,
        {
            ...
            format: (_, run) => {
                ...
                if (!detectorWasActiveDuringRun && detectorName !== NonPhysicalDetector.GLO) {
                    return null;
                }
