...
const runDetectorAsyncQualityDisplay = (runNumber, dplDetectorId, { dataPassId, simulationPassId }, qcSummary) => {
    ...
    if (!badEffectiveRunCoverage) {
        if (explicitlyNotBadEffectiveRunCoverage === 1) {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Green.dark });
        } else {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Gray.dark });
        }
    } else {
        if (badEffectiveRunCoverage === 1) {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Red.dark });
        } else {
            qcSummaryDisplay = getQcSummaryDisplayWithColor(notBadPercentageFormat, { color: ChartColors.Orange.dark });
        }
    }
