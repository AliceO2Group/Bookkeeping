 ...
 * @param {RemoteData<DplDetector[]>} remoteUsersDplDetectors dpl detectors list remote data
 ...
 */
const runDetectorAsyncQualityDisplay = (runNumber, dplDetectorId, remoteUsersDplDetectors, { dataPassId, simulationPassId }, qcSummary) => {
    if (!qcSummary?.[runNumber]?.[dplDetectorId]) { // If there is no QC flag assigned
        return qcFlagCreationPanelLink(
            ...
            runNumber,
            dplDetectorId,
            remoteUsersDplDetectors,
            {
                noPermissionContent: tooltip(
                    h('.gray', iconBan()),
 ...
 * @param {QcSummary} [options.qcSummary] QC summary for data/simulation pass
