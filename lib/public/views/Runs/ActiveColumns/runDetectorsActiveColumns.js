        ...
        detectorName, {
            ...
            format: (_, run) => {
                if (dataPassId && simulationPassId) {
                    throw new Error('`dataPassId` ans `simulationPassId` are exclusive options');
                }
                const detectorQualityDisplay = formatDetectorQuality(run.detectorsQualities.find(({ name }) => name === detectorName)?.quality);
                if (dataPassId) {
                    return frontLink(
