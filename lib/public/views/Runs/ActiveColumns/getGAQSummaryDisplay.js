...
const invalidatedDisplay = () => h(
    ...
    tooltip(h(
        '.f7',
        { id: 'clock-icon' },
        h('span', { style: 'color: var(--color-orange)' }, iconClock()),
 ...
 *
 * @param {ApiError[]|ApiError} errors the JSON:API conform error object(s)
 * @return {Component} failure warning display
 */
const failureWarningDisplay = (errors) => h(
    '.d-inline-block.va-t-bottom',
    tooltip(h('.f7', { id: 'warning-icon' }, iconWarning()), errorAlert(errors)),
);
...
const getSuccessGaqDisplay = (gaqSummary, isFirstInGroup) => {
    const { undefinedQualityPeriodsCount, invalidatedAt } = gaqSummary;
    ...
    if (undefinedQualityPeriodsCount === 0 && !invalidatedAt) {
        return getQcSummaryDisplay(gaqSummary, isFirstInGroup ? { classes: 'first-item' } : {});
    }

    return h(`button.btn.btn-primary.w-100${isFirstInGroup ? '.first-item' : ''}`, [
        'GAQ',
        invalidatedAt ? invalidatedDisplay() : null,
    ]);
};
...
export const getGAQSummaryDisplay = (remoteSummary, dataPassId, runNumber, onRecalculate) => {
    ...
    return remoteSummary.match({
        Success: (gaqSummary) => h(
            `.flex-row.w-100${hasRecalculateAccess ? '.btn-group' : ''}`,
            [
        ...
        Failure: (response) => gaqFlagLink(h(
            'button.btn.btn-primary.w-100',
            ['GAQ', failureWarningDisplay(response)],
        )),
    });
};
