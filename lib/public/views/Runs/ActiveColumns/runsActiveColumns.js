...
export const runsActiveColumns = {
    ...
    beamModes: {
        ...
        classes: 'w-5 f6',
        exportFormat: (beamModes) => beamModes?.length ? beamModes.map(({ text }) => text).join('-') : '-',

        /**
         * Beam Mode filter component
         *
         * @param {RunsOverviewModel} runModel the runs overview model
         * @return {Component} the filter component
         */
        filter: (runModel) => selectionDropdown(runModel.filteringModel.get('beamModes').dropDownModel, { selectorPrefix: 'beamMode' }),
        balloon: (beamMode) => beamMode && beamMode.length > 0,
    },
    fillNumber: {
        name: 'Fill No.',
        visible: true,
