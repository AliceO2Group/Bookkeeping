...
import { infoLoggerButtonGroup } from '../../../components/common/selection/infoLoggerButtonGroup.js';
import { BeamModes } from '../../../domain/enums/BeamModes.js';

/**
 * List of active columns for a generic runs table
 *
 * @param {RunModel} runModel the run's model
 * @return {Component} the runsActiveColumns component
 */
export const runsActiveColumns = (runModel) => ({
    id: {
        name: 'ID',
        visible: false,
        primary: true,
    ...
    runNumber: {
        ...
        format: (_, run) => infoLoggerButtonGroup(runModel, run),
        profiles: {
            lhcFill: true,
            environment: true,
            home: {
