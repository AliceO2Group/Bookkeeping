...
const formatAliceCurrent = (polarity, current) => (
    switchCase(polarity, { NEGATIVE: -1, POSITIVE: 1 }) *
        current
...
export const getDetectorsActiveColumns = (detectors) => Object.fromEntries(detectors?.map(({ name: detectorName }) => [
    detectorName, {
        ...
        format: (_, run) => {
            ...
            return h('', {
                class: detectorsQualities[detectorName] ? `btn run-quality ${detectorsQualities[detectorName]}` : '',
            }, detectorsQualities[detectorName] || '');
        },
    },
]) ?? []);
