...
export const runDetectorsSyncQcActiveColumns = (
    detectors,
    { profiles } = {},
) => Object.fromEntries(detectors?.map(({ name: detectorName, id }) => [
    detectorName, {
        name: detectorName.toUpperCase(),
        visible: true,
        format: (_, run) => {
            const detectorWasActiveDuringRun = run.detectorsQualities.find(({ name }) => name === detectorName);
            if (!detectorWasActiveDuringRun) {
                return '';
            }
            const b = (Math.random() * ((id) / (id + 5)) > 0.5) * Math.random();
            return getQcSummaryDisplay({
                badEffectiveRunCoverage: b,
                explicitlyNotBadEffectiveRunCoverage: 1 - b,
                missingVerificationsCount: 1,
                mcReproducible: b ? Math.random() < 0.5 : false,
            });
        },
        profiles,
    },
