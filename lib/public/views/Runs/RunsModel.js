...
export class RunsModel extends Observable {
     ...
     *
     * @param {object} parameters overview page parameters
     * @param {object} [parameters.filter] normalized filters
     * @return {void}
     */
    loadOverview({ filter }) {
        if (!this._overviewModel.pagination.isInfiniteScrollEnabled) {
            this._overviewModel.load(filter || {});
        }
    }
     ...
     * @return {void}
     */
    loadPerLhcPeriodOverview({ lhcPeriodName, panel, filter }) {
        ...
        if (!this._perLhcPeriodOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            this._perLhcPeriodOverviewModel.load(filter || {});
        }
    }
     ...
     * @return {void}
     */
    loadPerDataPassOverview({ dataPassId, pdpBeamType, filter }) {
        if (!this._perDataPassOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            this._perDataPassOverviewModel.load(filter || {});
        }
    }
     ...
     * @return {void}
     */
    loadPerSimulationPassOverview({ simulationPassId, filter }) {
        if (!this._perSimulationPassOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            this._perSimulationPassOverviewModel.load(filter || {});
        }
    }
