...
export class RunsModel extends Observable {
    ...
    constructor(model) {
        ...
        this._overviewModel = new RunsOverviewModel(model, 'run-overview');
        this._overviewModel.bubbleTo(this);
        this._perLhcPeriodOverviewModel = new RunsPerLhcPeriodOverviewModel(model, 'runs-per-lhc-period');
        this._perLhcPeriodOverviewModel.bubbleTo(this);
        this._perDataPassOverviewModel = new RunsPerDataPassOverviewModel(model, 'runs-per-data-pass');
        this._perDataPassOverviewModel.bubbleTo(this);
        this._perSimulationPassOverviewModel = new RunsPerSimulationPassOverviewModel(model, 'runs-per-simulation-pass');
    ...
    loadOverview() {
        if (! this._overviewModel.pagination.isInfiniteScrollEnabled) {
            this._overviewModel.setFilterFromUrl(null, false);
    ...
    loadPerLhcPeriodOverview({ lhcPeriodId, panel }) {
        ...
        if (!this._perLhcPeriodOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            this._perLhcPeriodOverviewModel.setFilterFromUrl(null, false);
    ...
    loadPerDataPassOverview({ dataPassId }) {
        if (!this._perDataPassOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            if (this._perDataPassOverviewModel.pagination._defaultItemsPerPage) {
                 ...
                 */
                this._perDataPassOverviewModel.setFilterFromUrl(null, false);
    ...
    loadPerSimulationPassOverview({ simulationPassId }) {
        if (!this._perSimulationPassOverviewModel.pagination.isInfiniteScrollEnabled) {
            ...
            if (this._perSimulationPassOverviewModel.pagination._defaultItemsPerPage) {
                 ...
                 * Thus, we need to trigger the load here.
                 */
                this._perSimulationPassOverviewModel.setFilterFromUrl(null, false);
