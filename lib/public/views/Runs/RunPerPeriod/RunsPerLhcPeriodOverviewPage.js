...
import { runDetectorsOnlineGeneralQcActiveColumns } from '../ActiveColumns/runDetectorsOnlineGeneralQcActiveColumns.js';
...
export const RunsPerLhcPeriodOverviewPage = ({ runs: { perLhcPeriodOverviewModel }, modalModel }) => {
    ...
    return h('', [
        ...
        tabbedPanelComponent(
            ...
            {
                [RUNS_PER_LHC_PERIOD_PANELS_KEYS.DETECTOR_QUALITIES]: () => h('.flex-column.w-100', [
                    table(
                        remoteDetectors.isSuccess() ? remoteRuns : remoteDetectors,
                        {
                            ...activeColumns,
                            ...runDetectorsOnlineGeneralQcActiveColumns(remoteDetectors.match({
                ...
                [RUNS_PER_LHC_PERIOD_PANELS_KEYS.SYNCHRONOUS_FLAGS]: (remoteSynchronousQcSummary) => h('.flex-column.w-100', [
                    table(
                        remoteDetectors.isSuccess() ? remoteRuns : remoteDetectors,
                        {
                            ...activeColumns,
                            ...runDetectorsSyncQcActiveColumns(remoteDetectors.match({
                                Success: (payload) => payload,
                                Other: () => [],
                            }), {
                                profiles: 'runsPerLhcPeriod',
                                qcSummary: remoteSynchronousQcSummary.match({
