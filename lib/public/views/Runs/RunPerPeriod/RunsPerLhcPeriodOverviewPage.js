...
import { createInelasticInteractionRateActiveColumns } from '../ActiveColumns/createInelasticInteractionRateActiveColumns.js';
...
export const RunsPerLhcPeriodOverviewPage = ({ runs: { perLhcPeriodOverviewModel }, modalModel }) => {
    ...
    const { items: remoteRuns, detectors: remoteDetectors, lhcPeriodName } = perLhcPeriodOverviewModel;

    const activeColumns = {
        ...runsActiveColumns,
        ...createInelasticInteractionRateActiveColumns(remoteRuns.match({ Success: (runs) => runs, Other: () => [] })),
