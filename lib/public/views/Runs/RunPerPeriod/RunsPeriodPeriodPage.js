...
import { exportRunsButton } from '../Overview/exportRunsButton.js';
 ...
 */
export const RunsPerPeriodPage = ({ runs: { perPeriodOverviewModel }, modalModel }) => {
    ...
    const activeColumns = {
        ...
        ...createDetectorsActiveColumns(perPeriodOverviewModel.detectors.payload ?? []),
    };

    return h('', [
        h('.flex-row.header-container.pv2', [
            h('.f4.gray-darker.bg-gray-lighter.br4', `Good, physics runs of ${perPeriodOverviewModel.lhcPeriodName}`),
            exportRunsButton(perPeriodOverviewModel, modalModel),
        ]),
        h('.flex-column.w-100', [
