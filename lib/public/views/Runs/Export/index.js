...
const exportRunsScreen = (model) => {
    let runNumbers = model.runs.getRunNumbers();
    const runsFields = Object.keys(activeColumns());
    let selectedRunsFields = model.runs.getSelectedFields;
    ...
    return h('div#export-runs', [
        ...
        h('', {
            ...
            h('input#run-number.form-control.w-100', {
                ...
                value: runNumbers,
                oninput: (e) => model.runs.setRunNumbers(e.target.value),
            ...
            h('select#fields.form-control', {
                multiple: true,
                onchange: (e) => { console.log("selected fields", e); model.runs.setSelectedFields(e.target.value); },
            }, [
                ...runsFields.map((name) => h('option', {
                    value: name,
                    selected: selectedRunsFields
                }, name)),
            ] ),
            ...
            h('button.shadow-level1.btn.btn-success.mt2#send', {
                disabled,
                onclick: () => {
                    const filteredData = model.runs.getFilteredRuns(data.payload, runNumbers, selectedRunsFields)
                },
