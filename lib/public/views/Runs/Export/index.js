...
const exportRunsScreen = (model) => {
    ...
    return h('div#export-runs', [
        ...
        h('', {
            ...
            h('label.form-check-label.f4', { for: 'run-number' }, 'Run numbers'),
            h('label.form-check-label.f6', { for: 'run-number' }, 'Select which runs with one or multiple ' +
                'run numbers, separated by commas. (max 100)'),
            ...
            h('label.form-check-label.f6', { for: 'run-number' }, 'Select which fields to be exported. (CTRL + click for multiple selection)'),
            h('select#fields.form-control', {
                style: 'min-height: 28.5rem;',
                multiple: true,
            ...
            h('button.shadow-level1.btn.btn-success.mt2#send', {
                disabled,
                onclick: async () => {
                    const filteredRuns = await model.runs.getFilteredRuns();
