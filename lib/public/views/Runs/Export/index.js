...
const exportRunsScreen = (model) => {
    let runNumbers = model.runs.getSelectedRunNumbers();
    const runsFields = Object.keys(activeColumns());
    let selectedRunsFields = model.runs.getSelectedRunsFields;
    const exportTypes = [".json", ".csv"];
    let selectedExportType = model.runs.getSelectedExportType;
    ...
    return h('div#export-runs', [
        ...
        h('', {
            ...
            h('label.form-check-label.f6', { for: 'run-number' }, `Select which runs with one or multiple run numbers, seperated by commas.'`),
