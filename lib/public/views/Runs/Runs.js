...
export default class Overview extends Observable {
     ...
     * @param {Number} The selected run numbers
     * @param {Number} The selected fields
     * @returns {undefined} Filtered runs data
     */
    async getFilteredRuns(runs, selectedRunNumbers, selectedRunsFields) {
        console.log("selectedRunsFields", selectedRunsFields);
        const testSelectedRunsFields = ["runNumber", "runQuality"];
        const selectedRuns = runs.filter(run => selectedRunNumbers.includes(run.runNumber));
        console.log("selectedRuns", selectedRuns)
        const filteredRuns = selectedRuns.map(selectedRun => pick(selectedRun, testSelectedRunsFields));
