...
import { PickerModel } from '../../components/pickers/tag/PickerModel.js';
...
export default class RunModel extends Observable {
    ...
    constructor(model) {
        ...
        this.editionTagPickerModel = new PickerModel('tag');

        this._listingRunTypesFilterModel = new PickerModel('runType');
        this._listingRunTypesFilterModel.observe(()=> this._applyFilters());
        // Content
    ...
    async fetchOneRun(id) {
        ...
        if (result.data) {
            ...
            this.editionTagPickerModel = new PickerModel('tag', this._run.payload.tags);
        } else {
     ...
     */
    async fetchLhcFill() {
        const id = this._run.isSuccess() && this._run.payload.fillNumber
            ? this._run.payload.fillNumber : null;
        if (id) {
    ...
    getActiveFilters() {
        ...
        if (this._triggerValuesFilters.length !== 0) {
    ...
    /**
     * Getter for runtypes filtering
     * @returns {string[]} list of run type ids
     */
    get runTypesFilter() {
        return this._runTypesFilter;
    }

    /**
     * Setter for runtypes filtering
     * @param {string[]} values list of runType ids
     */
    set runTypesFilter(values) {
        this._runTypesFilter = values;
        this._applyFilters();
    }
     ...
     * @returns {TagFilter} A tag filter model used to get
     */
    ...
    _getFilterQueryParams() {
        return {
            ...
            ...this._listingRunTypesFilterModel.selected.length > 0 && {
                'filter[runTypes]': this._listingRunTypesFilterModel.selected.map((run) => run.id),
