...
export default class RunModel extends Observable {
    ...
    constructor(model) {
        ...
        documentClickTaggedEventRegistry.addListenerForAnyExceptTagged(() => this.setShowFilters(false), FILTER_PANEL_CLICK_TAG);
     ...
     *
     * @param {number|string|undefined} [runIdentifier.id]  the id of the run to display
     * @param {number|string|undefined} [runIdentifier.runNumber]  the id of the run to display
     * @param {string|null|undefined} [panelKey=null] the key of the panel to display
     * @return {void}
     */
    loadDetails({ id: runId, runNumber }, panelKey = null) {
        runId = runId ? Number(runId) : runId;
        runNumber = runNumber ? Number(runNumber) : runNumber;
        if (this._detailsModel === null) {
            this._detailsModel = new RunDetailsModel({ runId, runNumber }, panelKey);
            this._detailsModel.bubbleTo(this);
        } else {
            if (!this.model.router.params.runNumber && this._detailsModel._runNumber) {
                this.model.router.go(`?page=run-detail&runNumber=${this._detailsModel._runNumber}`);
            }
            if (runNumber) {
                this._detailsModel.runNumber = runNumber;
            } else if (runId) {
                this._detailsModel.runId = runId;
            } else {
                throw new Error('No run identifier to fetch its details');
            }
     ...
     * @return {Boolean} return whether the filter should be shown
     */
