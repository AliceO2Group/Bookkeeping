...
export default class Overview extends Observable {
    ...
    async getFilteredRuns() {
        ...
        for (let i = 0; i < runIds.length; i++) {
            response = await fetchClient(`/api/runs/${runIds[i]}`, { method: 'GET' });
            ...
            foundRuns.push(result.data);
        }

        this.runs = RemoteData.success();
        const filteredRuns = foundRuns.map((selectedRun) => pick(selectedRun, this.getSelectedRunsFields()));
        return filteredRuns;
    }
