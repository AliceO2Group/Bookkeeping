...
export default class RunModel extends Observable {
    ...
    constructor(model) {
        ...
        this._eorReasonsFilterModel.observe(() => {
            /**
             * Workaround:
             * because EorReasonFilterModel calls reasons fetching during construction, it also calls notify() almost immedietaly.
             * So here, _applyFilters is called before method fetchAllRuns is overriden by sub-class
             */
            this._eorReasonsFilterModel.eorReasonTypes.match({
                Success: () => this._applyFilters(),
                NotAsked: () => {},
                Loading: () => {},
                Failure: () => {},
            });
        ...
        const updateDebounceTime = () => this._debouncedFetchAllRuns = debounce(() => this.fetchAllRuns(), model.inputDebounceTime);
