...
export default class Overview extends Observable {
    ...
    async fetchAllRuns() {
        const params = {
            ...
            ...this.trgStartFilterFrom && {
                'filter[trgStart][from]':
                    new Date(`${this.trgStartFilterFrom.replace(/\//g, '-')}T00:00:00.000`).getTime(),
            },
            ...this.trgStartFilterTo && {
                'filter[trgStart][to]':
                    new Date(`${this.trgStartFilterTo.replace(/\//g, '-')}T23:59:59.999`).getTime(),
            },
            ...this.trgEndFilterFrom && {
                'filter[trgEnd][from]':
                    new Date(`${this.trgEndFilterFrom.replace(/\//g, '-')}T00:00:00.000`).getTime(),
            },
            ...this.trgEndFilterTo && {
                'filter[trgEnd][to]':
                    new Date(`${this.trgEndFilterTo.replace(/\//g, '-')}T23:59:59.999`).getTime(),
            },
            ...
            ...this.ddFlpFilterValues && {
                'filter[ddflp]': this.ddFlpFilterValues,
            },
    ...
    resetRunsParams(fetch = true) {
        ...
        this.trgStartFilterFrom = '';
        this.trgStartFilterTo = '';
        this.trgStartFilterDebounce = null;

        this.trgEndFilterFrom = '';
        this.trgEndFilterTo = '';
        this.trgEndFilterDebounce = null;

        this.enviromentIdFilter = '';
        this.envFilterDebounce = null;

        this.nDetectorsFilterValues = null;
        this.nDetectorsDebounce = null;

        this.nFlpsFilterValues = null;
        this.nFlpsDebounce = null;

        this.ddflpFilterValues = null;
        this.ddflpDebounce = null;

        this.epnTopologyFilterValues = null;
        this.epnTopologyDebounce = null;

        this.detectorsFilterValues = null;
     ...
     */
      shouldShowMoreTags() {
         ...
         * @returns {Integer} The current minimum creation datetime
         */
          getTrgStartFilterFrom() {
            return this.trgStartFilterFrom;
        }
         ...
         * @returns {undefined}
         */
        setTrgStartFilter(key, date, valid) {
            if (valid) {
                this[`trgStartFilter${key}`] = date;
                this.fetchAllRuns();
            }
        }
         ...
         * @returns {undefined}
         */
        setTrgStartFilterWithDebounce(key, date, valid) {
            clearTimeout(this.trgStartFilterDebounce);
            this.trgStartFilterDebounce = setTimeout(() => this.setTrgStartFilter(key, date, valid), 200);
        }
         ...
         * @returns {undefined}
         */
        setTrgEndFilter(key, date, valid) {
            if (valid) {
                this[`trgEndFilter${key}`] = date;
                this.fetchAllRuns();
            }
        }
         ...
         * @returns {undefined}
         */
        setTrgEndFilterWithDebounce(key, date, valid) {
            clearTimeout(this.trgEndFilterDebounce);
            this.trgEndFilterDebounce = setTimeout(() => this.setTrgEndFilter(key, date, valid), 200);
        }
         ...
         * @returns {undefined}
         */
          setEnvironmentIdFilter(newTitle) {
            clearTimeout(this.envFilterDebounce);
            this.envFilterDebounce = setTimeout(() => {
                this.enviromentIdFilter = newTitle.trim();
                this.fetchAllRuns();
            }, 200);
        }
         ...
         * @returns {undefined}
         */
          setnDetectorsFilter(newTitle) {
            clearTimeout(this.envFilterDebounce);
            this.envFilterDebounce = setTimeout(() => {
                this.nDetectorsFilterValues = newTitle.trim();
                this.fetchAllRuns();
            }, 200);
        }
         ...
         */
         getEpnTopologyFilter() {
            return this.epnTopologyFilterValues;
        }
