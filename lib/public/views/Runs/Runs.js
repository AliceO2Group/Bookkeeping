...
export default class RunModel extends Observable {
    ...
    async fetchRunType() {
        const id = this.run.isSuccess() && this.run.payload.fillNumber
            ? this.run.payload.fillNumber : null;
        if (id) {
            this.runType = RemoteData.loading();

            const response = await fetchClient(`/api/runTypes/${id}`, { method: 'GET' });
            const result = await response.json();

            if (result.data) {
                this._runType = RemoteData.success(result.data);
            } else {
                this._runType = RemoteData.failure(result.errors || [
                    {
                        title: result.error,
                        detail: result.message,
                    },
            ...
            this._runType = RemoteData.success(null);
        }
        this.notify();
    }
