...
export default class Overview extends Observable {
    ...
    async getFilteredRuns() {
        ...
        // Gets all filled in run numbers and filters out empty inputs
        const runIds = this.getSelectedRunNumbers().split(',').filter((runId) => parseInt(runId, 10));
        const filteredRuns = [];
        ...
        return filteredRuns.map((selectedRun) => pick(selectedRun, this.getSelectedRunsFields()));
    }
