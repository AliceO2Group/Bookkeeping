...
export default class RunModel extends Observable {
    ...
    async fetchAllRuns() {
        ...
        const params = {
            ...
            'page[offset]': this._currentPageRuns.payload && this.isInfiniteScrollEnabled()
                ? this._currentPageRuns.payload.length
                : (this.selectedPage - 1) * this.runsPerPage,
        ...
        const { remoteData, pageCount, totalCount } = await parseFetchResponse(
            response,
            this.isInfiniteScrollEnabled() ? this._currentPageRuns.payload || [] : null,
        );
    ...
    async getFilteredRuns() {
        ...
        if (!result.data) {
            this._currentPageRuns = RemoteData.failure(result.errors || [
                {
                    title: result.error,
                    detail: result.message,
                },
        ...
        this._currentPageRuns = RemoteData.success(result.data);
        this.notify();
    ...
    get isAllRunsTruncated() {
        ...
        return this.allRuns.payload.length < this.totalRunsCount;
    }
