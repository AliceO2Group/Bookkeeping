...
import { createJSONExport, createCSVExport } from '../../utilities/export.js';
...
export default class Overview extends Observable {
     ...
     * @returns {undefined}
     */
    async createRunsExport(data, fileName) {
        this.getSelectedExportType() == ".csv" ?
            createCSVExport(data, fileName + ".csv", "text/csv;charset=utf-8;")
            :createJSONExport(data, fileName + ".json", "application/json")
    }
    ...
    setSelectedRunsFields(selectedOptions) {
        ...
        const filter = [...selectedOptions].map(( selectedOption ) => this.selectedRunsFields.push(selectedOption.value));
        this.notify();
    }
