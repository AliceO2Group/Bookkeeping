...
export default class RunModel extends Observable {
     ...
     * @param {boolean} add Selects if you want to add the runs to the existing list
     * @returns {undefined} Injects the data object with the response data
     */
    async fetchAllRuns(add = false) {
        const runs_c = this.runs;
        ...
        if (result.data) {
            if (this.isInfiniteScrollEnabled() || add) {
                const payload = runs_c && runs_c.payload ? [...runs_c.payload, ...result.data] : result.data;
     ...
     * @param {boolean} add selects if the runs should be combined
     * @return {Number} The chosen page number
     */
    setSelectedPage(page, add = false) {
        if (this.selectedPage !== page) {
            this.selectedPage = page;
            this.notify();
            this.fetchAllRuns(add);
        }
    }
