...
export const StatisticsPage = ({ statisticsModel }) => {
    ...
    if (periodLabel === null) {
        periodLabel = formatTimeRange(statisticsModel.timeRangeFilterModel.normalizedFilter);
    }
            ...
            },
            {
                ...
                [STATISTICS_PANELS_KEYS.EFFICIENCY_PER_DETECTOR]: (panelModel) => {
                    ...
                    return remoteDataDisplay(panelModel.data, {
                        Success: (efficiencyPerDetectors) => panelModel.detectors.match({
                            NotAsked: () => centeredComponent('No detectors available'),
                            Loading: () => spinner(),
                            Success: (detectors) => {
                                const activeDetectors = [];
                                for (const detector of detectors) {
                                    if (panelModel.getDetectorVisibility(detector)) {
                                ...
                                return [
                                    ...
                                    h('.flex-row.g2', detectors.map((detector, i) => switchInput(
                                    ...
                                    h('.flex-row.flex-grow.g5', [
                                        h(
                                            ...
                                            detectorsEfficienciesComponent(
                                                ...
                                                false,
                                                activeDetectors,
                                                detectorsColors.filter((_, i) => panelModel.getDetectorVisibility(detectors[i])),
                                                () => statisticsModel.notify(),
                                            ),
                                        ),
                                        h(
                            ...
                            Failure: (error) => centeredComponent(errorAlert(error)),
                        }),
                    });
                },
