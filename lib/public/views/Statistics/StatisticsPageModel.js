...
export class StatisticsPageModel extends Observable {
    ...
    constructor() {
        ...
        this._tabbedPanelModel.period = {
            from: new Date(startOfYearMidnight.getTime() - timezoneOffset * 60 * 1000).getTime(),
            to: new Date(tomorrowMidnight.getTime() - timezoneOffset * 60 * 1000).getTime(),
        };
    }
...
class DetectorsEfficienciesTabModel extends Observable {
    ...
    fetch() {
        this._remoteDataFetcher.fetch(buildUrl(
            ...
            { from: this._period.from, to: this._period.to },
        )).then((detectorEfficiency) => {
            this._detectors = detectorEfficiency.reduce((detectors, { efficiencies }) => {
     ...
     */
    showAll() {
        this._detectorsVisibility = new Map(this._detectors.map((detector) => [detector, true]));
        this.notify();
    }
    ...
    /**
     * Return the list of detector for which efficiency should be displayed
     *
     * @return {string[]} the list of detectors
     */
    get activeDetectors() {
        const ret = [];
        for (const detector of this._detectors) {
            if (this.getDetectorVisibility(detector)) {
                ret.push(detector);
            }
        }
        return ret;
