...
export default class LhcFills extends Observable {
     ...
     *
     * @param {boolean} hasStableBeams Indicates whether the LHC fills should have stable beams
     * @returns {void}
     */
    loadOverview(hasStableBeams = false) {
        this._overviewModel.load(hasStableBeams);
    }

    /**
     * Retrieve every stable LHC fill from the API
     *
     * @returns {void}
     */
    loadStableOverview() {
        this._overviewModel.fetchAllStableLhcFills();
    }
     ...
     * @param {string} queryParams.fillNumber the number of the fill for which model details must be instantiated
     * @param {string} [queryParams.panel] the tab panel key
     * @return {void}
     */
    loadDetails(queryParams) {
        const { fillNumber, panel } = queryParams;
        this._detailsModel.lhcFillNumber = parseInt(fillNumber, 10);
        this._detailsModel.tabbedPanelModel.currentPanelKey = panel;
    }
