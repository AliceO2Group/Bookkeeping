...
export default class Overview extends Observable {
    ...
    async fetchAllLhcFills() {
        ...
        const params = {
            'page[offset]': this._lhcFills.payload && this._lhcFillsPerPage === Infinity ?
                this._lhcFills.payload.length : (this._selectedPage - 1) * this._lhcFillsPerPage,
        ...
        if (result.data) {
            if (this.isInfiniteScrollEnabled()) {
                const payload = this._lhcFills && this._lhcFills.payload ? [...this._lhcFills.payload, ...result.data] : result.data;
