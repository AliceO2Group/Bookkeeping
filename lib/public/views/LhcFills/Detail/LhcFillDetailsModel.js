 ...
 */
import { Observable, RemoteData } from '@aliceo2/web-ui/Frontend/js/src/index.js';
...
class LhcFillDetailsTabbedPanelModel extends TabbedPanelModel {
     ...
     */
    set lhcFill(lhcFill) {
        this._lhcFillNumber = lhcFill.fillNumber;
    ...
    _fetchCurrentPanelData() {
        switch (this.currentPanelKey) {
            case LHC_FILL_DETAILS_PANELS_KEYS.RUNS:
                this.currentPanelData = this._runsTabModel.panelData;
                break;
            case LHC_FILL_DETAILS_PANELS_KEYS.LOGS:
    ...
    /**
     * Fetch all the logs of the current LHC fill
     *
    ...
    async _fetchLogsPanelData() {
        ...
        try {
            const { data: logsOfLhcFill } = await getRemoteData(`/api/lhcFills/${this._lhcFillNumber}/logs`);
            this.currentPanelData = RemoteData.success(logsOfLhcFill);
        } catch (error) {
