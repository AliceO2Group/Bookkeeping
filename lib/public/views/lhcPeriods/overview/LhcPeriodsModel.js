...
import { FilterigModel } from './FilteringModel.js';
...
export class LhcPeriodsModel extends Observable {
    ...
    constructor(model) {
        ...
        this._filteringModel.observe(() => {
            ...
            this._debouncedFetchLhcPeriods();
        });

        const updateDebounceTime = () => this._debouncedFetchLhcPeriods = debounce(this.fetchLhcPeriods.bind(this), model.inputDebounceTime);
    ...
    async fetchLhcPeriods() {
        ...
        // const filteringPhrase = 'filter[names][]=LHC22a';//this._filteringModel.toSeqrchQueryParamsString();
        const filteringPhrase = this._filteringModel.toSeqrchQueryParamsString();
        console.log(filteringPhrase);

        const endpoint = `/api/lhcPeriods?${[new URLSearchParams(params).toString(), filteringPhrase].join('&')}`;
