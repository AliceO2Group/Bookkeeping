...
export const lhcPeriodsActiveColumns = {
    ...
    associatedRuns: {
        ...
        format: (_, { name, runsCount }) =>
            runsCount === 0
                ? 'No runs'
                : frontLink(
                    h('.flex-row.g3', h('.f6.badge.bg-gray-light.black', runsCount)),
                    'runs-per-lhc-period',
                    { lhcPeriodName: name },
                ),
        classes: 'w-10',
    ...
    associatedDataPasses: {
        ...
        format: (_, { id, dataPassesCount }) =>
            dataPassesCount === 0
                ? 'No data passes'
                : frontLink(
                    h('.flex-row.g3', h('.f6.badge.bg-gray-light.black', dataPassesCount)),
    ...
    associatedSimulationPasses: {
        ...
        format: (_, { id, simulationPassesCount }) => simulationPassesCount === 0
            ? 'No MC'
            : frontLink(
                h('.flex-row.g3', h('.f6.badge.bg-gray-light.black', simulationPassesCount)),
                'simulation-passes-per-lhc-period-overview',
                { lhcPeriodId: id },
            ),
        classes: 'w-10',
