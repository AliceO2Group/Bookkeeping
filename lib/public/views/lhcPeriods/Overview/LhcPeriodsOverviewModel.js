...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    constructor() {
        ...
        this._dataSource = new RemoteDataSource()
            // eslint-disable-next-line no-return-assign
            .feeds((observableDataCurrent) => this._pagination.itemsCount = observableDataCurrent.payload?.meta?.page.totalCount)
            .feeds(new DataSourceTransfomer((remoteData) =>
                remoteData.isSuccess()
                    ? RemoteData.success(remoteData.payload.data.map((dataUnit) => this.flattenPeriodDataObject(dataUnit)))
                    : remoteData))
            .feeds(new PaginatedRemoteDataSource());
