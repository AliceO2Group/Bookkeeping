...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    async fetchLhcPeriods() {
        ...
        const endpoint = `/api/lhcPeriodsStatistics?${[paramsPhrase, this.getFilterQueryParamsString()].join('&')}`;
    ...
    getFilterQueryParamsString() {
        const namesFilterString = this._namesFilterModel.normalized
            .map((lhcPeriodName) => `filter[names][]=${lhcPeriodName}`).join('&');

        const yearsFilterString = this._yearsFilterModel.normalized
            .map((year) => `filter[years][]=${year}`);

        const beamTypesFilterString = this._beamTypesFilterModel.normalized
            .map((beamType) => `filter[beamTypes][]=${beamType}`);
        return [namesFilterString, yearsFilterString, beamTypesFilterString].join('&');
    }
