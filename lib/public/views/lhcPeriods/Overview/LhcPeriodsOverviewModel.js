...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    async fetchLhcPeriods(clear = false) {
        ...
        const paramsExpression = new URLSearchParams(queryParams).toString();
        const endpoint = `/api/lhcPeriodsStatistics?${[paramsExpression, this.getFilteringQueryParametersExpression()].join('&')}`;
    ...
    /**
     * Get filter related query params as single string
     * @return {string} filter search query phrase
     */
    getFilteringQueryParametersExpression() {
        const namesFilters = this._namesFilterModel.normalized
            .map((lhcPeriodName) => `filter[names][]=${lhcPeriodName}`);

        const yearsFilters = this._yearsFilterModel.normalized
            .map((year) => `filter[years][]=${year}`);

        const beamTypesFilters = this._beamTypesFilterModel.normalized
            .map((beamType) => `filter[beamTypes][]=${beamType}`);

        return [namesFilters, yearsFilters, beamTypesFilters].flat().join('&');
    }
