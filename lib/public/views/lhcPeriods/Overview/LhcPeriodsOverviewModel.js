...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    constructor() {
        super();
        this._remoteDataFetcher = new PaginatedRemoteDataSource({
            dataTransformer: (data) => data.map((dataUnit) => this.flattenPeriodDataObject(dataUnit)),
        });
        const { observableData } = this._remoteDataFetcher;
        observableData.bubbleTo(this);
        // eslint-disable-next-line no-return-assign
        observableData.observe(() => this._pagination.itemsCount = observableData.current.payload?.meta?.page.totalCount);
