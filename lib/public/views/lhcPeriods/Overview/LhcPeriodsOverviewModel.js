...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    constructor() {
        ...
        this._namesFilterModel = new TextTokensFilterModel(
            {
                exactLhcPeriodNames:
                    (token) => /^([ ,;]+)*([Ll][Hh][Cc]\d\d[A-Za-z]+)(([ ,;]+)([Ll][Hh][Cc]\d\d[A-Za-z]+))*([ ,;]*)$/.test(token),
            },
            /[ ,;]+/,
        );
    ...
    getFilterSearchQueryPhrase() {
        return this._namesFilterModel.normalized.exactLhcPeriodNames
            .map((lhcPeriodName) => {
                const uppercasedPrefix = lhcPeriodName.slice(0, 3).toUpperCase();
                const lowercaseLettersSuffix = lhcPeriodName.slice(3).toLocaleUpperCase();
                return `filter[names][]=${uppercasedPrefix}${lowercaseLettersSuffix}`;
            }).join('&');
    }
    ...
    /**
     * States wheter filters panel should be displayed
     * @return {boolean} true if filters panel should be displayed
     */
