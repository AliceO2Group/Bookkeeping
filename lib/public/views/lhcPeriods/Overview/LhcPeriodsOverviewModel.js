...
import { DataSourceBuilder } from '../../../utilities/fetch/DataSourceBuilder.js';
import { PaginatedDataSource } from '../../../utilities/fetch/PaginatedRemoteDataSource.js';
import { RemoteDataSource } from '../../../utilities/fetch/RemoteDataSource.js';
import { DataSourceTransfomer } from '../../../utilities/fetch/DataSourceTransformer.js';
...
export class LhcPeriodsOverviewModel extends Observable {
    ...
    constructor() {
        ...
        this._dsb = dataSourceBuilder;
        dataSourceBuilder
            .addDataSource(
                new RemoteDataSource(),
                // eslint-disable-next-line no-return-assign
                (observableDataCurrent) => this._pagination.itemsCount = observableDataCurrent.payload?.meta?.page.totalCount,
            ).addDataSource(new DataSourceTransfomer((remoteData) =>
                remoteData.isSuccess()
                    ? RemoteData.success(remoteData.payload.data.map((dataUnit) => this.flattenPeriodDataObject(dataUnit)))
                    : remoteData))
            .addDataSource(new PaginatedDataSource());

        this._dataSource = dataSourceBuilder.get();
        this._dataSource.observableData.bubbleTo(this);
     ...
     */
    get lhcPeriods() {
        return this._dataSource.observableData.current;
    }
     ...
     */
    reset() {
        this._dataSource.observableData.current = RemoteData.NotAsked();
