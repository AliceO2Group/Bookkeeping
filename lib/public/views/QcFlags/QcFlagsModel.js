...
export class QcFlagsModel extends Observable {
     ...
     * @returns {void}
     */
    loadCreationForDataPass({ dataPassId, runNumbers, dplDetectorIds }) {
        this._creationForDataPassModel = new QcFlagCreationForDataPassModel(
            {
                dataPassId: parseInt(dataPassId, 10),
                runNumbers: runNumbers ? runNumbers.split(',').map(Number) : null,
                dplDetectorIds: dplDetectorIds ? dplDetectorIds.split(',').map(Number) : null,
            },
            () => this.model.router.go(buildUrl(
                '/',
                { page: 'qc-flags-for-data-pass', dataPassId, runNumbers, dplDetectorIds },
            )),
        );
