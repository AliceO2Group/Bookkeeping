...
export class QcFlagsModel extends Observable {
    ...
    loadCreationForDataPass({ dataPassId, runNumberDetectorMap }) {
        const parsedRunNumberDetectorMap = runNumberDetectorMap ? this.parseRunNumberDetectorMap(runNumberDetectorMap) : null;
        const detectorIds = parsedRunNumberDetectorMap ? Object.values(parsedRunNumberDetectorMap).flat() : [];
        ...
        this._creationForDataPassModel = new QcFlagCreationForDataPassModel(
            ...
            () => {
                if (detectorIds.length > 1) {
                    ...
                    const [[runNumber, [dplDetectorId]]] = Object.entries(parsedRunNumberDetectorMap);
    ...
    parseRunNumberDetectorMap(runNumberDetectorMapString) {
        return runNumberDetectorMapString.split(';').reduce((acc, item) => {
            ...
            acc[parsedRunNumber] = detectorIds.split(',').map(Number);
            return acc;
        }, {});
    }
     ...
     * @returns {void}
     */
    loadCreationForSimulationPass({ simulationPassId, runNumberDetectorMap }) {
        const parsedRunNumberDetectorMap = runNumberDetectorMap ? this.parseRunNumberDetectorMap(runNumberDetectorMap) : null;
        const detectorIds = parsedRunNumberDetectorMap ? Object.values(parsedRunNumberDetectorMap).flat() : [];
