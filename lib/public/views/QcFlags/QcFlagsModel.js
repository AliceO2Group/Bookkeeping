 ...
 * @param {string} [params.runNumberDetectorMap] - String representation of run number to detector map
 * @returns {void}
 */
    loadCreationForDataPass({ dataPassId, runNumberDetectorMap }) {
        const parsedRunNumberDetectorMap = runNumberDetectorMap
            ? this.parseRunNumberDetectorMap(runNumberDetectorMap)
            : null;

        // Determine if more than one detector is selected
        const detectorIds = parsedRunNumberDetectorMap
            ? Object.values(parsedRunNumberDetectorMap).flat()
            : [];

        // Update the creation for data pass model with the new success handler
        this._creationForDataPassModel = new QcFlagCreationForDataPassModel(
            {
                dataPassId: parseInt(dataPassId, 10),
                runNumberDetectorMap: parsedRunNumberDetectorMap,
            },
            () => {
                if (detectorIds.length > 1) {
                    // Redirect to runs per data pass page
                    this.model.router.go(buildUrl(
                        '/',
                        { page: 'runs-per-data-pass', dataPassId },
                    ));
                } else if (detectorIds.length === 1) {
                    // Redirect to QC flags for data pass page with query params
                    const runNumber = Object.keys(parsedRunNumberDetectorMap)[0];
                    const dplDetectorId = detectorIds[0];
                    ...
                    throw new Error('No detectors available for redirection.');
                }
            }
        );
     ...
     * @param {string} runNumberDetectorMapString string with run number detector map
     * @return {Object<number, number[]>} object with run number as keys and detector ids as arrays
     */
    parseRunNumberDetectorMap(runNumberDetectorMapString) {
        return runNumberDetectorMapString.split(';').reduce((acc, item) => {
            const [runNumber, detectorIds] = item.split(':');
            acc[parseInt(runNumber, 10)] = detectorIds.split(',').map(Number);
            return acc;
        }, {});
    }
     ...
     * @returns {void}
     */
    loadDetailsForDataPass({ id, dataPassId, runNumber, dplDetectorId }) {
     ...
     * @returns {void}
     */
    loadDetailsForSimulationPass({ id, simulationPassId, runNumber, dplDetectorId }) {
     ...
     * @returns {void}
     */
    loadSynchronousOverview({ runNumber, dplDetectorId }) {
        this._synchronousOverviewModel.runNumber = parseInt(runNumber, 10);
        this._synchronousOverviewModel.dplDetectorId = parseInt(dplDetectorId, 10);
