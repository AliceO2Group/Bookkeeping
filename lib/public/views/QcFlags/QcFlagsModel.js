...
export class QcFlagsModel extends Observable {
    ...
    loadDetails({ id }) {
        this._detailsModel = new QcFlagDetailsModel(
            ...
            () => {
                /**
                 * Workaround, as there is no information about QC flag association to data/simulation pass,
                 * cannot simple go to proper overview page when flag is deleted
                 */
                if (this.model.router.history.length >= 3) {
                    this.model.router.history.back();
                } else {
                    this.model.router.go('/?page=home');
                }
            },
        );
