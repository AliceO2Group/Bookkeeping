...
const verifyButton = (qcFlagDetailsModel) => {
    ...
    return remoteQcFlag.match({
        Success: (qcFlag) => h(
            '.pv3',
            qcFlag.createdBy.externalId === sessionService.session.personid
                ? tooltip(
                    h('button#verify-qc-flag.btn.btn-primary', { disabled: true }, 'Verify'),
                    'You cannot verify flag created by you',
                )
                : h(
                    ...
                    qcFlag.createdBy.externalId !== sessionService.session.personid
                        ? 'Verify'
                        : tooltip('Verify', 'You cannot verify flag created by you'),
                ),
        ),
        Other: () => h('.pv3', h(
            'button#verify-qc-flag.btn.btn-primary',
...
export const qcFlagDetailsComponenet = (qcFlagDetailsModel, additionalDetailsConfiguration = {}) => {
    ...
    /**
     * Function called when QC flag delete button is clicked
     * @return {void}
     */
    const qcFlagDeleteOnclick = () => {
        ...
        h('.flex-row.justify-between.items-center', [
            ...
            h('.flex-row.g3', [
                ...
                sessionService.hasAccess(BkpRoles.ADMIN) && h('.pv3', deleteResult.match({
                    ...
                    Failure: () => deleteButton(qcFlagDeleteOnclick),
                    NotAsked: () => deleteButton(qcFlagDeleteOnclick),
                })),
            ]),
        ]),
