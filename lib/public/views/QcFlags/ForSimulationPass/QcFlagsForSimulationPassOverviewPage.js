    ...
    return h(
        ...
        [
            h('.flex-row.justify-between.items-center', [
                h('.flex-row.g1.items-center', qcFlagsBreadcrumbs({ remoteSimulationPass, remoteRun, remoteDplDetector })),
                remoteRun.match({
                    NotAsked: () => null,
                    Loading: () => null,
                    Failure: () => tooltip(h('.f3', iconWarning()), 'An error has occurred. Unable to determine whether run is subject for QC'),
                    Success: (run) => qcFlagCreationPanelLink(run, { simulationPassId }, dplDetectorId),
                }),
            ]),
            h('.w-100.flex-column', [
                table(
