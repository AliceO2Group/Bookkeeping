 ...
 */
export const QcFlagsForSimulationPassOverviewPage = (model) => {
    const { qcFlags: { forSimulationPassOverviewModel: qcFlagsForSimulationPassOverviewModel } } = model;
    const {
        run: remoteRun,
        ...
        h('.flex-row.justify-between.items-center', [
            h('.flex-row.g1.items-center', [
                remoteSimulationPass.match({
                    ...
                    Loading: () => [commonTitle, h('', spinner({ size: 2, absolute: false }))],
                    NotAsked: () => [commonTitle, tooltip(h('.f3', iconWarning()), 'No simulation pass data was asked for')],
                }),
            ]),
        ]),
        QcFlagsOverviewComponenet(qcFlagsForSimulationPassOverviewModel, model),
    ]);
};
