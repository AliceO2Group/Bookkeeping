...
export class QcFlagsForSimulationPassOverviewModel extends QcFlagsOverviewModel {
    ...
    async _fetchSimulationPass() {
        ...
        try {
            ...
            /**
             * Workadround to have simulationPassId available in activeColumns
             */
            this._observableItems
                .setCurrent(this.items.apply({
                    Success: (qcFlags) => qcFlags.map((qcFlag) => ({ ...qcFlag, simulationPassId: simulationPass.id })),
                }));
        } catch (error) {
