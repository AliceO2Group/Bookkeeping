...
const formatGeneralQuality = (contributingFlags, gaqDetectors) => {
    ...
    const missingDataMessage = [
        ... flagForSomeDetectorIsMissing ? [h('', 'No flag for some detector')] : [],
    ...
    if (allGood) {
        qualityDisplay = badge('good', { color: QC_SUMMARY_COLORS.ALL_GOOD });
    } else if (someBadNotReproducible) {
        qualityDisplay = badge('bad', { color: QC_SUMMARY_COLORS.ALL_BAD });
    } else {
        qualityDisplay = badge('MC.R', { color: QC_SUMMARY_COLORS.LIMITED_ACCEPTANCE_MC_REPRODUCIBLE });
    }
