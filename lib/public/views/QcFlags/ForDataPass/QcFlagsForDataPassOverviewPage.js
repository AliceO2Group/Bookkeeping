...
export const QcFlagsForDataPassOverviewPage = ({ qcFlags: { forDataPassOverviewModel: qcFlagsForDataPassOverviewModel } }) => {
    ...
    const runDetailsLink = frontLink(runNumber, 'run-detail', { runNumber });
    return h(
        '',
        ...
        [
            h('.flex-row.justify-between.items-center', [
                h('.flex-row.g1.items-center', breadcrumbs([
                    ...
                    remoteRun.match({
                        // Other: () => null,
                        Success: ({ runQuality }) => runQuality === RunQualities.BAD
                            ? tooltip(
                                h('h2.danger', runDetailsLink),
                ...
                remoteRun.match({
                    ...
                    Success: ({ runQuality }) => qcFlagCreationPanelLink({ dataPassId, runNumber, runQuality, dplDetectorId }),
                }),
            ]),
            h('.w-100.flex-column', [
                table(
