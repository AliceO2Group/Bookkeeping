...
import { pickerSelection } from '../../../components/common/selection/picker/pickerSelection.js';

/**
 * Render a form for GAQ detectors selection
 *
 * @param {GaqFlagsOverviewModel} gaqOverviewModel gaq overview model
 * @param {SelectionModel} gaqDetectorsSelectionModel gaq detectors selection model
 * @param {ModalHandler} modalHandler The modal handler, used to dismiss modal after successful operation
 *
 * @return {Component} the form component
 */
const selectionForm = (gaqOverviewModel, gaqDetectorsSelectionModel, modalHandler) => {
    const { setGaqDetectorsRequestResult: remoteSetGaqDetectorsRequestResult } = gaqOverviewModel;
    const sendingRequest = remoteSetGaqDetectorsRequestResult.match({
        Loading: () => true,
        Other: () => false,
    });

    return h('.flex-column.g3', [
        remoteSetGaqDetectorsRequestResult.match({
        ...
        h('.grid.columns-2-lg.columns-3-xl.g2', picker(
            gaqDetectorsSelectionModel,
            { selector: 'gaq-detectors', optionsAttributes: { class: 'f3', disabled: sendingRequest } },
        )),
        ...
        h('.flex-row.g3.justify-center ', [
            ...
            h(
                'button.shadow-level1.btn.mt2#revert',
                {
                    disabled: sendingRequest,
                    onclick: async () => {
                        gaqDetectorsSelectionModel.reset();
                        gaqOverviewModel.notify();
                    },
                },
                'Revert',
            ),
...
const gaqDetectorsSelectionModal = (gaqOverviewModel, modalHandler) => {
    ...
    return h('div#gaq-detector-selection-modal', [
        ...
        gaqDetectorPickerModel.selectionModel.match({
            NotAsked: () => errorDisplay(),
            Loading: () => spinner({ size: 3, absolute: false }),
            Success: (selectionModel) => selectionForm(gaqOverviewModel, selectionModel, modalHandler),
 ...
 */
export const gaqDetectorsSelectionModalTrigger = (gaqOverviewModel, modalModel, { autoMarginLeft = true } = {}) => h(
    `button.btn.btn-primary.w-15.h2${autoMarginLeft ? '.mlauto' : ''}#gaq-detectors-selection-trigger`,
    {
