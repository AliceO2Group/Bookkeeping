...
const gaqDetectorsSelectionModal = (gaqOverviewModel, modalHandler) => {
    const { gaqDetectors } = gaqOverviewModel;

    return h('div#export-runs-modal', [
        h('h2', 'Export Runs'),
        runsRemoteData.match({
            NotAsked: () => errorDisplay(),
            Loading: () => exportForm(gaqOverviewModel, null, modalHandler),
            Success: (payload) => exportForm(gaqOverviewModel, payload, modalHandler),
