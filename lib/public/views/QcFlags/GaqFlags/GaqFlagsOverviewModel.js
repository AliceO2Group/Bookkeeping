...
export class GaqFlagsOverviewModel extends OverviewPageModel {
    ...
    constructor() {
        ...
        this._submitResult = new ObservableData(RemoteData.notAsked());
        this._submitResult.bubbleTo(this);
    }
    ...
    async submitNewGaqDetectors() {
        this._submitResult.setCurrent(RemoteData.loading());

        try {
            await jsonPost('/api/dataPasses/gaqDetectors', {
                dataPassId: this._dataPassId,
                runNumbers: [this._runNumber],
                dplDetectorIds: this._gaqDetectorPickerModel.selected,
            });

            this._submitResult.setCurrent(RemoteData.success(null));
            this.load();
        } catch (errors) {
            this._submitResult.setCurrent(RemoteData.failure(errors));
        }
    }
     ...
     */
    get submitResult() {
        return this._submitResult.getCurrent();
    }
     ...
     */
    dismissSubmitResult() {
        this._submitResult.setCurrent(RemoteData.notAsked());
    }
}
