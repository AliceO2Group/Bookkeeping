...
export class GaqFlagsOverviewModel extends OverviewPageModel {
    ...
    updateDetectorSelectionModel() {
        this._gaqDetectors$.getCurrent().match({
            Success: (gaqDetectors) => this._detectorSelectionModel.options.match({
                Success: () => gaqDetectors.forEach(({ dplDetectorId }) => this._detectorSelectionModel.select(dplDetectorId)),
                Other: () => {},
            }),
            Other: () => {},
        });
