...
import { qcFlagCreationComponenet } from '../qcFlagCreationComponent.js';
...
/**
 * Render Quality Control Flags For Data Pass creation page
 * @param {Model} model The overall model object
 * @returns {Component} creation page
 */
export const QcFlagsForDataPassCreationPage = ({ qcFlags: { forDataPassCreationModel: qcFlagsForDataPassCreationModel } }) => {
    const {
        run: remoteRun,
        dplDetector: remoteDplDetector,
        dataPass: remoteDataPass,
    } = qcFlagsForDataPassCreationModel;

    const commonTitle = h('h2', 'QC');
    return h('', {
        onremove: () => qcFlagsForDataPassCreationModel.reset(),
    }, [
        h('.flex-row.justify-between.items-center', [
            ...
            frontLink(h('.flex-row.items-center.g1', 'QC'), 'qc-flags-for-data-pass', {
                runNumber: qcFlagsForDataPassCreationModel._runNumber,
                dataPassId: qcFlagsForDataPassCreationModel._dataPassId,
                dplDetectorId: qcFlagsForDataPassCreationModel._dplDetectorId,
            }, {
                class: 'btn btn-primary',
                title: 'Create a quality control flag',
            }),
        ]),
        qcFlagCreationComponenet(qcFlagsForDataPassCreationModel),
    ]);
};
