...
export const QcFlagCreationForDataPassPage = ({ qcFlags: { creationForDataPassModel: qcFlagCreationForDataPassModel } }) => {
    ...
    return [
        h('.flex-row.justify-between.items-center', [
            Array.isArray(dplDetectorIds) ? qcFlagsBreadcrumbs({ remoteDataPass }) :
                qcFlagsBreadcrumbs({ remoteDataPass, singleRun, singleDplDetector }),
            Array.isArray(dplDetectorIds) ?
                qcFlagOverviewPanelLink(
                    'QC',
                    { dataPassId, runNumber: runNumbers, dplDetectorId: dplDetectorIds },
