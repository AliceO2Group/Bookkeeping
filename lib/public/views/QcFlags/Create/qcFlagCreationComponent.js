...
const qcFlagCreationForm = (creationModel) => {
    ...
    const { _timeRangeModel: timeRangeFilterModel, useTimeBasedQcFlag, canSendTimeBasedQcFlag } = creationModel;
        ...
        Success: (run) => [
            h('.flex-row.g3', [
                ...
                canSendTimeBasedQcFlag ? h(PanelComponent, [
                    h(LabelPanelHeaderComponent, { for: 'time-based' }, 'Use time based'),
                    h(
                            ...
                            h('.switch#bad-toggle', [
                                h(
                                    'input',
                                    {
                                        ...
                                        onchange: () => creationModel.useTimeBasedQcFlag = !useTimeBasedQcFlag,
                                        type: 'checkbox',
                                    },
                                ),
                                h('span.slider.round'),
            ...
            canSendTimeBasedQcFlag
                ? useTimeBasedQcFlag
                    ? timeRangeInput(timeRangeFilterModel, { seconds: true, from: { min: run.startTime }, to: { max: run.endTime } })
                    : null
                : 'Can send run-based QC flag only as start or stop of run is missing',
        ],
        Other: () => null,
    });
...
export const qcFlagCreationComponenet = (creationModel) =>
    h('.w-100.flex-column', [qcFlagCreationForm(creationModel)]);
