 ...
 */
const selectedInfoTable = (creationModel) => {
    const runsData = creationModel.runs.match({
        Success: (runs) => runs,
        Other: () => [],
    });
    ...
    return h(PanelComponent, [
        ...
        h('table.table.table-sm', [
            ...
            h('tbody', Object.entries(creationModel.dplDetectors._payload).map(([runNumber, detectors]) => {
                const run = runsData.find((r) => r.runNumber === parseInt(runNumber, 10));
                return h('tr', [
                    h('td', runNumber),
        ...
        h('.flex-column.g3', [
            h('.flex-row.items-start.g3', [
                creationModel.runs.match({
                    Success: (runs) => runs.length > 1 ?
                        selectedInfoTable(creationModel) : singleSelectedTimeComponents(creationModel),
                    NotAsked: () => null,
