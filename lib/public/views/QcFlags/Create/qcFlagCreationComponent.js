...
const qcFlagCreationForm = (creationModel) => {
    const { timeRangeModel, isTimeBasedQcFlag, startTimings, endTimings, triggerValueIsOff } = creationModel;
    const { startTime, timeO2Start, timeTrgStart } = startTimings;
    const { endTime, timeO2End, timeTrgEnd } = endTimings;
        ...
        h('.flex-column.g3', [
            h('.flex-row.items-start.g3', [
                h(PanelComponent, [
                    ...
                    h('.p2', formatRunStart(timeO2Start, timeTrgStart, triggerValueIsOff, false)),
                ]),
                h(PanelComponent, [
                    h(LabelPanelHeaderComponent, 'Run End'),
                    h('.p2', formatRunEnd(timeO2End, timeTrgEnd, triggerValueIsOff, false)),
                ]),
