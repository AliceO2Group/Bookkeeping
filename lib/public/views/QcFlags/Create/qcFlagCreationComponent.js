...
const qcFlagCreationForm = (creationModel) => {
    ...
    const {startTime, startO2, startTrg, startTriggelVal} = startTimings
    const {endTime, endO2, endTrg, endTriggelVal} = endTimings
        ...
        h('.flex-column.g3', [
            h('.flex-row.items-start.g3', [
                h(PanelComponent, [
                    ...
                    h('.p2', formatRunStart(startO2, startTrg, startTriggelVal, false)),
                ]),
                h(PanelComponent, [
                    h(LabelPanelHeaderComponent, 'Run End'),
                    h('.p2', formatRunEnd(endO2, endTrg, endTriggelVal, false)),
                ]),
 ...
 */
export const qcFlagCreationComponent = (creationModel) => {
  const runs = Array.isArray(creationModel.runs) ? creationModel.runs : [creationModel.runs];

  return h(
    '.flex-column.g3.flex-grow',
    creationFormComponent(creationModel, [
      runs.map((runData) => runData.match({
        Success: (run) => {
          if (isRunNotSubjectToQc(run)) {
            return errorAlert([{ title: getRunNotSubjectToQcReason(run) }]);
          } else {
            return qcFlagCreationForm(creationModel); // Pass creationModel, not run
          }
        },
      ...
      // Check if ALL runs are not subject to QC
      runs.every(runData => runData.match({
        Success: (run) => isRunNotSubjectToQc(run),
        NotAsked: () => false, 
        Loading: () => false,
        Failure: () => false,
      })) ? errorAlert([{ title: 'All selected runs are not subject to QC.' }]) : null,
    ]),
  );
};
