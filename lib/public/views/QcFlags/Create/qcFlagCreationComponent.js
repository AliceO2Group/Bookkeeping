...
export const qcFlagCreationComponent = (creationModel) => h(
    ...
    creationFormComponent(creationModel, [
        creationModel.runs.map((runData) => runData.match({
            Success: (run) => {
                if (isRunNotSubjectToQc(run)) {
                    return errorAlert([{ title: getRunNotSubjectToQcReason(run) }]);
                } else {
                    return qcFlagCreationForm(run, creationModel);
                }
            },
        ...
        // Add a check to ensure at least one run is not subject to QC
        creationModel.runs.every(runData => runData.match({
