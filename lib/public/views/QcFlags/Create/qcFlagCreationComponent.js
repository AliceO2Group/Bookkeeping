 ...
 * @param {DplDetector[]} detectors detectors
 * @return {Component} table component
 */
const selectedInfoTable = (creationModel, detectors) => h(PanelComponent, [
    ...
    h('table.table.table-sm', [
        ...
        h('tbody', detectors.map((detector) => h('tr', [
            h('td', detector.name),
            h('td', creationModel.runNumbers),
            h('td', formatRunStart(creationModel, true)),
            h('td', formatRunEnd(creationModel, true)),
        ]))),
    ]),
]);

/**
 * Create components to display single run's start and end time
 * @param {QcFlagCreationModel} creationModel model
 * @return {Component[]} components
 */
const singleSelectedTimeComponents = (creationModel) => [
    h(PanelComponent, [
        h(LabelPanelHeaderComponent, 'Run Start'),
        h('.p2', formatRunStart(creationModel, false)),
    ]),
    h(PanelComponent, [
        h(LabelPanelHeaderComponent, 'Run End'),
        h('.p2', formatRunEnd(creationModel, false)),
    ]),
];
...
const qcFlagCreationForm = (creationModel) => {
    const { timeRangeModel, isTimeBasedQcFlag, startTimings, endTimings } = creationModel;
    const { startTime } = startTimings;
    const { endTime } = endTimings;
    ...
    return [
        h('.flex-column.g3', [
            h('.flex-row.items-start.g3', [
                creationModel.dplDetectors.match({
                    Success: (detectors) => detectors.length > 1 ?
                        selectedInfoTable(creationModel, detectors) : singleSelectedTimeComponents(creationModel),
                    NotAsked: () => null,
...
export const qcFlagCreationComponent = (creationModel) => h(
    ...
    creationFormComponent(creationModel, [
        creationModel.runs.match({
            Success: (runs) => creationModel.runs._payload.every(isRunNotSubjectToQc)
                ? errorAlert([{ title: getRunNotSubjectToQcReason(runs) }])
