...
export class QcFlagCreationModel extends CreationModel {
     ...
     * @param {number} [parameters.runNumbers] runNumbers of target run
     * @param {number} [parameters.dplDetectorIds] id of target dpl detector
    ...
    constructor({
        ...
        this._dplDetectorIds = dplDetectorIds;
    ...
    async _fetchRuns() {
        ...
        try {
            let runs;
            if (this._runNumbers.length > 1) {
                ({ data: runs } = await getRemoteData(`/api/runs/?filter[runNumbers]=${this._runNumbers}`));
            } else {
                ({ data: runs } = await getRemoteData(`/api/runs/${this._runNumbers[0]}`));
            }
    ...
    _getDetectors() {
        this._dplDetectors$.setCurrent(detectorsProvider.qc$.getCurrent().match({
            Success: (dplDetectors) => {
                const matchDetectors = dplDetectors.filter(({ id }) => this._dplDetectorIds.includes(id));
                return matchDetectors
                    ? RemoteData.success(matchDetectors)
                    : RemoteData.failure({ errors: [{ detail: `There is no dplDetector with given id (${this._dplDetectorIds})` }] });
            },
