...
export class QcFlagCreationModel extends CreationModel {
    ...
    isValid() {
        return this._flagTypeSelectionModel.selected.length > 0
            && (!this._useTimeBasedQcFlag || this._timeRangeModel.fromTimeInputModel.isValid)
            && (!this._useTimeBasedQcFlag || this._timeRangeModel.toTimeInputModel.isValid);
    }
    ...
    /**
     * States whether can mark run with time based QC flag - only if startTime and endTime of the run are not missing
