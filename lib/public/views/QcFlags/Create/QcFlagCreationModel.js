...
export class QcFlagCreationModel extends Observable {
     ...
     * @inheritDoc
     */
    set normalizedFilter({ comment, flagTypeId, from, to, runDetectors }) {
        this._comment = comment;
        this._flagTypeSelectionModel.select(flagTypeId);
        this._timeRangeModel.fromTimeInputModel.update(from);
        this._timeRangeModel.toTimeInputModel.update(to);

        this._runsAndDetectors = (runDetectors ?? []).map(({ runNumber, detectorIds }) => ({
            run: { runNumber },
            detectors: (detectorIds ?? []).map(id => ({ id })),
        }));
    }
