...
export class QcFlagCreationModel extends CreationModel {
    ...
    _getSerializableData() {
        return this._runNumbers.flatMap((runNumber) =>
            this._dplDetectorIds.map((dplDetectorId) => ({
                ...this.formData,
                runNumber,
                dplDetectorId,
    ...
    setTimings(runs) {
        for (const run of runs) {
            const { startTime, timeTrgEnd, timeO2End, triggerValue } = run;
    ...
    /**
     * Single run getter
     * @return {Run} selected run or current run
     */
    get singleRun() {
        return this._runs$.getCurrent().match({
            Success: (runs) => runs[0],
            Other: () => null,
        });
    }
     ...
     * @return {DplDetector} selected detector or current detector
     */
    get singleDplDetector() {
        return this._dplDetectors$.getCurrent().match({
            Success: (detectors) => detectors[Object.keys(detectors)[0]][0],
            Other: () => null,
        });
    }
