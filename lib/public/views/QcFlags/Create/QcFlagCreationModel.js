...
export class QcFlagCreationModel extends CreationModel {
    ...
    constructor({
        ...
        console.log(this._runNumberDetectorMap);
    ...
    _getDetectors() {
        this._dplDetectors$.setCurrent(detectorsProvider.qc$.getCurrent().match({
            Success: (dplDetectors) => {
                const detectorMap = new Map(dplDetectors.map(({ id, ...detector }) => [id, { id, ...detector }]));
                const errors = [];

                // Check if all detectors from runNumberDetectorMap are present in dplDetectors
                const matchedDetectors = Object.fromEntries(Object.entries(this._runNumberDetectorMap).flatMap(([key, value]) => {
                    const match = detectorMap.get(Number(value));
                    return match
                        ? [[key, match]]
                        : (errors.push({
                            title: `Detector with id ${value} not found`,
                            detail: `Detector with id ${value} is not found in the list of detectors user has access to`,
                        }), []);
                }));
