...
export class QcFlagCreationModel extends CreationModel {
    ...
    _getSerializableData() {
        const transformedRunDetectors = Object.entries(this._runNumberDetectorMap).map(([runNumber, detectorIds]) => ({
        ...
        return {
            ...
            runDetectors: transformedRunDetectors,
        };
    }
    ...
    /**
     * Single run getter
     * @return {RemoteData<Run>} first run
     */
    get singleRun() {
        return this.runs.match({
            Success: (runs) => RemoteData.success(runs[0]),
            Other: () => RemoteData.notAsked(),
        });
    }

    /**
     * Single detector getter
     * @return {RemoteData<DplDetector>} first dpl detector
     */
    get singleDplDetector() {
        return this.dplDetectors.match({
            Success: (detectors) => RemoteData.success(Object.values(detectors)[0][0]),
            Other: () => RemoteData.notAsked(),
        });
    }
