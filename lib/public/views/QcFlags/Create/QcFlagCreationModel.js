...
export class QcFlagCreationModel extends CreationModel {
    ...
    setTimings(runs) {
        ...
        for (const run of runs) {
            const { startTime, timeTrgStart, timeO2Start, timeTrgEnd, timeO2End } = run;
    ...
    async _fetchRuns() {
        ...
        try {
            const { data: runs } = await getRemoteData(`/api/runs/${this._runNumbers}`);
