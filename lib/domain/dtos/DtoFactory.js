...
/**
 * Return schema for sorting
 * @param {String|String[]} names list of names by which query results might be ordered
 * @returns {Joi.Schema} schema
 */
const createStdSortDto = (names) => {
    const orderSchema = Joi.string().valid('asc', 'desc', 'ASC', 'DESC');
    return Array.isArray(names) ?
        Joi.object(Object.fromEntries(names.map((name) => [name, orderSchema])))
        : Joi.object({
            [names]: orderSchema,
        });
};
...
/**
 * Return schema which accepts singular value of given type or array of such elements
 * @param {String} type valid Joi type, string, number, email, etc ...
 * @returns {Joi.Schema} schema
 */
const createSingleValueOrArrayDtoOfType = (type) => {
    try {
        return Joi.alternatives(Joi[type](), Joi.array().items(Joi[type]()));
    } catch (error) {
        throw new TypeError(`${type} is not valid Joi type`);
    }
};
...
exports.DtoFactory = {
    ...
    StdPaginationDto: require('./PaginationDto.js'),
    createStdSortDto,
    createSingleValueOrArrayDtoOfType,
};
