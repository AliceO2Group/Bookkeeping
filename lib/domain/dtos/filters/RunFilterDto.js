...
exports.RunFilterDto = Joi.object({
    ...
    detectorsQc: Joi.object({
        ...
        Joi.number().positive().integer().required(), // DplDetectorId
        FloatComparisonDto.when( // NotBadFraction
            Joi.object({
                dataPassIds: Joi.array().length(1),
                simulationPassIds: Joi.array().length(1),
                lhcPeriodIds: Joi.array().length(1),
            }).xor('dataPassIds', 'simulationPassIds', 'lhcPeriodIds'),
            {
                then: FloatComparisonDto,
                otherwise: Joi.forbidden().error(new Error('Filtering by detector not-bad fraction is enabled only when' +
                    'filtering with oexatcly one of following filters:' +
                    'dataPassIds, simulationPassIds, or lhcPeriodIds with exactly one id provided')),
            },
        ).required(),
    ),
});
