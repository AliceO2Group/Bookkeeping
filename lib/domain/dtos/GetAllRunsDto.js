...
const FilterDto = Joi.object({
    ...
    definitions: CustomJoi.stringArray().items(Joi.string().uppercase().trim().valid('PHYSICS', 'COSMIC', 'TECHNICAL', 'SYNTHETIC')),
    tags: TagsFilterDto,
...
const QueryDto = Joi.object({
    ...
    page: PaginationDto,
    sort: SortDto,
    fields: Joi.alternatives().try(Joi.object(), Joi.array()), // Support for sparse query notation
    token: Joi.string(),
});
