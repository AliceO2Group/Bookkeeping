...
class APILogAdapter {
    ...
    toAPIEntity(entityObject) {
        ...
        return {
            ...
            environments: (environments ?? []).map(this.environmentAdapter.toAPIEntity),
            lhcFills: (lhcFills ?? []).map(this.lhcFillAdapter.toAPIEntity),
        };
    }
    ...
    toDomain(apiEntityObject) {
        ...
        return {
            ...
            environments: (environments ?? []).map(this.environmentAdapter.toDomain),
            lhcFills: (lhcFills ?? []).map(this.lhcFillAdapter.toDomain),
        };
    }
}
