    ...
    toAPIEntity(entityObject) {
        ...
        return {
            ...
            environments: environments ?
                environments.map(this.environmentAdapter.toAPIEntity)
                : environments,
            lhcFills: lhcFills ?
                lhcFills.map(this.lhcFillAdapter.toAPIEntity)
                : lhcFills,
        };
    }

    /**
     * Converts the given api entity object to a database object.
     *
     * @param {APILog} apiEntityObject Object to convert.
     * @returns {Log} Converted database object.
     */
    toDomain(apiEntityObject) {
        ...
        return {
            ...
            runs: runs ?
                ...
                : runs,
            environments: environments ?
                environments.map(this.environmentAdapter.toDomain)
                : environments,
            lhcFills: lhcFills ?
                lhcFills.map(this.lhcFillAdapter.toDomain)
                : lhcFills,
        };
    }
}
