 ...
 *
 * @param origin the origin to change port from
 * @param port the new port to apply
 * @return {string} the new URL
 */
const changeOriginPort = (origin, port) => {
    return origin.replace(/(\/\/[a-zA-Z0-9-.]+:)\d+/, (_, match) => `${match}${port}`);
}
