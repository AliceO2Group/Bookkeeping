...
exports.services = {
    ...
    gaq: {
        enableRecalculation: process.env?.GAQ_ENABLE_RECALCULATION?.toLowerCase() === 'true' || true,
        recalculationPeriod: Number(process.env?.GAQ_RECALCULATION_PERIOD) || 10 * 1000, // 10s in milliseconds
        batchSize: Number(process.env?.GAQ_RECALCULATION_BATCH_SIZE) || 1,
    },
};
