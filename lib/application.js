...
const { gridUserCertificate, monalisa: monalisaConfig, ccdb: ccdbConfig } = ServicesConfig;
...
class BookkeepingApplication {
    ...
    async run() {
        ...
        try {
            ...
            if (monalisaConfig.enableSynchronization || ServicesConfig.enableHousekeeping) {
                const pfxCertificateBytes = await fs.readFile(gridUserCertificate.path);
                const certificatePassphrase = gridUserCertificate.passphrase;
