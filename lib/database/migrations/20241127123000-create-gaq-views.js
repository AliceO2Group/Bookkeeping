...
const SELECT_QCF_EFFECTIVE_PERIODS_TO_TIMESTAMPS_FOR_GAQ_PERIODS = `
    SELECT
        gaqd.data_pass_id,
        gaqd.run_number,
        COALESCE(qcfep.\`to\`, NOW(3)) AS ordering_timestamp,
...
LEFT JOIN (
    ...
    AND (qcfep.\`from\` IS NULL OR qcfep.\`from\` <= gaq_periods_timestamps.\`from\`)
    AND (qcfep.\`to\`   IS NULL OR gaq_periods_timestamps.\`to\` <= qcfep.\`to\`)
