        ...
        FROM (
                ...
                (
                    SELECT gaqd.data_pass_id,
                        gaqd.run_number,
                        COALESCE(UNIX_TIMESTAMP(time_start), 0) AS timestamp
                ...
                (
                    SELECT gaqd.data_pass_id,
                        gaqd.run_number,
                        UNIX_TIMESTAMP(COALESCE(time_end, NOW())) AS timestamp
                    FROM global_aggregated_quality_detectors AS gaqd
                    INNER JOIN runs as r
                        ON gaqd.run_number = r.run_number
                )
...
SELECT
    ...
    IF(gaq_periods_timestamps.\`to\` = UNIX_TIMESTAMP(NOW()), null, gaq_periods_timestamps.\`to\`) AS \`to\`,
...
GROUP BY
    ...
    IF(gaq_periods_timestamps.\`to\` = UNIX_TIMESTAMP(NOW()), null, gaq_periods_timestamps.\`to\`)
`;
