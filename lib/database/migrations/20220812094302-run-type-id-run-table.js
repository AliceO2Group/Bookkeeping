...
module.exports = {
    up: async (queryInterface, Sequelize) => queryInterface.sequelize.transaction((t) => Promise.all([
        ...
        queryInterface.removeColumn('runs', 'run_type', { transaction: t }),
    ])),
    down: async (queryInterface, _Sequelize) => queryInterface.sequelize.transaction((t) =>
        Promise.all([
            queryInterface.removeColumn('runs', 'run_type_id', { transaction: t }),
            queryInterface.addColumn('runs', 'run_type', { transaction: t }),
        ])),
};
