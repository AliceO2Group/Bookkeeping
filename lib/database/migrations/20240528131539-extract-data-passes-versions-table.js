...
module.exports = {
    up: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        await queryInterface.sequelize.query(`
            INSERT INTO data_pass_versions (data_pass_id, description, output_size, reconstructed_events_count, last_run_number)
            SELECT id, description, output_size, reconstructed_events_count, last_run_number
                FROM data_passes
        `, { transaction });
    ...
    down: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        await queryInterface.sequelize.query(`
            ...
            INNER JOIN (
                ...
                SELECT data_pass_id, description, output_size, reconstructed_events_count, last_run_number
                FROM lastwin
                WHERE rn = 1
            ) as recent_data_pass_versions

                ON recent_data_pass_versions.data_pass_id = data_passes.id

            SET data_passes.description = recent_data_pass_versions.description,
                data_passes.output_size = recent_data_pass_versions.output_size,
                data_passes.reconstructed_events_count = recent_data_pass_versions.reconstructed_events_count,
                data_passes.last_run_number = recent_data_pass_versions.last_run_number
