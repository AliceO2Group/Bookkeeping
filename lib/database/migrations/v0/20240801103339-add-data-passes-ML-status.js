...
module.exports = {
    up: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        `, { transaction, replacements: { status: DataPassVersionStatus.RUNNING }, type: QueryTypes.INSERT });
        await queryInterface.sequelize.query(`
            INSERT INTO data_pass_version_status_history(data_pass_version_id, status, created_at, updated_at)
                SELECT id, :status, updated_at, updated_at
                    FROM data_pass_versions
                    WHERE deleted_from_monalisa = true
        `, { transaction, replacements: { status: DataPassVersionStatus.DELETED }, type: QueryTypes.INSERT });
    ...
    down: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        `, { transaction, replacements: { statusDeleted: DataPassVersionStatus.DELETED }, type: QueryTypes.UPDATE });
