...
module.exports = {
    up: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        for (const { CONSTRAINT_NAME, TABLE_NAME } of constraints) {
            ...
            if (TABLE_NAME === 'dpl_processes_executions') {
            ...
            await queryInterface.sequelize.query(`
                UPDATE ${TABLE_NAME} AS u
                    INNER JOIN dpl_detectors AS dpl
                        ON dpl.id = u.${columnName}
                    INNER JOIN detectors AS d
                        ON d.name = dpl.name
                    SET u.${columnName} = d.id
            `);
            ...
            await queryInterface.sequelize.query(`ALTER TABLE ${TABLE_NAME} RENAME COLUMN ${columnName} TO detector_id`, { transaction });
        }
