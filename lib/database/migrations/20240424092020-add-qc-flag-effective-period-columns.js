...
const _updateEffectivePeriodForScopeAndPeriod = async (
    queryInterface,
    { from: intersectingPeriodFrom, to: intersectingPeriodTo },
    ...
    `
        INNER JOIN quality_control_flags as qcf
            ON qcfep.flag_id = qcf.id
                ...
                AND unix_timestamp(qcf.created_at) < ${createdAt}
        INNER JOIN ${
    ...
    await queryInterface.sequelize.query(
        `
            ...
            ${joinClause}
            WHERE unix_timestamp(qcfep.\`from\`) * 1000 >= ${intersectingPeriodFrom} 
                AND unix_timestamp(qcfep.\`to\`) * 1000 <= ${intersectingPeriodTo}
        `,
        { transaction },
    );
    ...
    await queryInterface.sequelize.query(`
            INSERT INTO quality_control_flag_effective_periods (\`from\`, \`to\`, flag_id)
                SELECT qcfep.\`from\`, '${timestampToMysql(intersectingPeriodFrom)}', qcfep.flag_id
                    FROM quality_control_flag_effective_periods as qcfep 
                    ${joinClause}
                    WHERE unix_timestamp(qcfep.\`from\`) * 1000 < ${intersectingPeriodFrom}
                        AND ${intersectingPeriodTo} < unix_timestamp(qcfep.\`to\`) * 1000;

            `, { transaction });
    await queryInterface.sequelize.query(`
            INSERT INTO quality_control_flag_effective_periods (\`from\`, \`to\`, flag_id)
                SELECT '${timestampToMysql(intersectingPeriodTo)}', qcfep.\`to\`, qcfep.flag_id
                    FROM quality_control_flag_effective_periods as qcfep 
                    ${joinClause}
                    WHERE unix_timestamp(qcfep.\`from\`) * 1000 < ${intersectingPeriodFrom}
                        AND ${intersectingPeriodTo} < unix_timestamp(qcfep.\`to\`) * 1000;
    ...
    await queryInterface.sequelize.query(`
        ...
        ${joinClause}
        SET qcfep.\`to\` = '${timestampToMysql(intersectingPeriodFrom)}'
        WHERE unix_timestamp(qcfep.\`from\`) * 1000 < ${intersectingPeriodFrom}
        ;
    `, { transaction });
    ...
    await queryInterface.sequelize.query(`
        UPDATE quality_control_flag_effective_periods as qcfep
        ${joinClause}
        SET qcfep.\`from\` = '${timestampToMysql(intersectingPeriodTo)}'
        WHERE unix_timestamp(qcfep.\`to\`) * 1000 > ${intersectingPeriodTo}
        ;
    `, { transaction });
};
