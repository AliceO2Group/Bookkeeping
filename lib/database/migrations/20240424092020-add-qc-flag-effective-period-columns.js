 ...
 * @param {number} scope.createdAt creation timestamp of discarding flag,
...
const _updateEffectivePeriodForScopeAndPeriod = async (
    queryInterface,
    { from: intersectingPeriodFromTimestamp, to: intersectingPeriodToTimestamp },
    ...
    intersectingPeriodFromTimestamp = timestampToMysql(intersectingPeriodFromTimestamp);
    intersectingPeriodToTimestamp = timestampToMysql(intersectingPeriodToTimestamp);
    ...
    await queryInterface.sequelize.query(
        `
            ...
            ${joinClause}
            WHERE qcfep.\`from\` >= ${intersectingPeriodFromTimestamp} 
                AND qcfep.\`to\` <= ${intersectingPeriodToTimestamp}
        `,
        { transaction },
    );
    ...
    await queryInterface.sequelize.query(`
            INSERT INTO quality_control_flag_effective_periods (\`from\`, \`to\`, flag_id)
                SELECT qcfep.\`from\`, '${timestampToMysql(intersectingPeriodFromTimestamp)}', qcfep.flag_id
                    FROM quality_control_flag_effective_periods as qcfep 
                    ${joinClause}
                    WHERE qcfep.\`from\` < ${intersectingPeriodFromTimestamp}
                        AND ${intersectingPeriodToTimestamp} < qcfep.\`to\`;

            `, { transaction });
    await queryInterface.sequelize.query(`
            INSERT INTO quality_control_flag_effective_periods (\`from\`, \`to\`, flag_id)
                SELECT '${timestampToMysql(intersectingPeriodToTimestamp)}', qcfep.\`to\`, qcfep.flag_id
                    FROM quality_control_flag_effective_periods as qcfep 
                    ${joinClause}
                    WHERE qcfep.\`from\` < ${intersectingPeriodFromTimestamp}
                        AND ${intersectingPeriodToTimestamp} < qcfep.\`to\`;
    ...
    await queryInterface.sequelize.query(`
        ...
        ${joinClause}
        SET qcfep.\`to\` = '${timestampToMysql(intersectingPeriodFromTimestamp)}'
        WHERE qcfep.\`from\` < ${intersectingPeriodFromTimestamp}
        ;
    `, { transaction });
    ...
    await queryInterface.sequelize.query(`
        UPDATE quality_control_flag_effective_periods as qcfep
        ${joinClause}
        SET qcfep.\`from\` = '${timestampToMysql(intersectingPeriodToTimestamp)}'
        WHERE qcfep.\`to\` > ${intersectingPeriodToTimestamp}
        ;
    `, { transaction });
};
