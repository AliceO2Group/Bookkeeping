...
const timestampSqlDate = (timestamp) => `'${new Date(timestamp).toISOString().replace(/T/, ' ').slice(0, -5)}'`;
...
const _reconsturctEffectivePeriodForOneFlag = async (queryInterface, newerFlag, flagToBePartiallyDiscarded, { transaction }) => {
    ...
    const [effectivePeriodsOfFlagToBePartiallyDiscarded] = await queryInterface.sequelize.query(`
        ...
        WHERE flag_id = ${flagToBePartiallyDiscardedId}
            AND (
                unix_timestamp(\`from\`) * 1000 >= ${newFlagFrom} AND unix_timestamp(\`from\`) * 1000 <= ${newFlagTo}
                OR
                unix_timestamp(\`to\`) * 1000 >= ${newFlagFrom} AND unix_timestamp(\`to\`) * 1000 <= ${newFlagTo}
            )
    ...
    for (const effectivePeriod of effectivePeriodsOfFlagToBePartiallyDiscarded) {
        ...
        if (newFlagFrom <= effectiveFrom && effectiveTo <= newFlagTo) { // Old flag is fully covered by new one
            ...
            await queryInterface.sequelize.query(`
                ...
                SET \`to\` = ${timestampSqlDate(newFlagFrom)}
                WHERE id = ${timestampSqlDate(effectivePeriodId)}
                ;
            `, { transaction });
            await queryInterface.sequelize(`
                INSERT INTO quality_control_flag_effective_periods(flag_id, \`from\`, \`to\`)
                VALUES (${flagToBePartiallyDiscardedId}, ${timestampSqlDate(newFlagTo)}, ${timestampSqlDate(effectiveTo)})
                ;
            `, { transaction });
            ...
            await queryInterface.sequelize.query(`
                UPDATE quality_control_flag_effective_periods
                SET \`from\` = ${timestampSqlDate(newFlagTo)}
                WHERE id = ${timestampSqlDate(effectivePeriodId)}
                ;
            `, { transaction });
        } else {
 ...
 * @return {Promise<void>} promise
 */
const _updateFlagsEffectivePeriods = async (queryInterface, { transaction }) => {
    ...
    // eslint-disable-next-line require-jsdoc
    const groupFlagsPerDataPassIdRunNumberDplDetectorId = (flags) => {
        const result = {};
        for (const flag of flags) {
            ...
            if (!result[data_pass_id]) {
                result[data_pass_id] = {};
            }
            if (!result[data_pass_id][run_number]) {
                result[data_pass_id][run_number] = {};
            }
            if (!result[data_pass_id][run_number][dpl_detector_id]) {
                result[data_pass_id][run_number][dpl_detector_id] = [];
            }
            result[data_pass_id][run_number][dpl_detector_id].push(flag);
        }
    };
    const groupedQcFlagsForDataPass = groupFlagsPerDataPassIdRunNumberDplDetectorId(qcFlagsPerDataPass);
    for (const perRunDetector of Object.values(groupedQcFlagsForDataPass)) {
        for (const perDetector of Object.values(perRunDetector)) {
            for (const qcFlags of Object.values(perDetector)) {
                await _reconsturctEffectivePeriodForFlagSInTheSameScope(qcFlags);
            }
        }
    }

    // eslint-disable-next-line require-jsdoc
    const groupFlagsPerSimulationPassIdRunNumberDplDetectorId = (flags) => {
        const result = {};
        for (const flag of flags) {
            const {
                simulation_pass_id,
            ...
            if (!result[simulation_pass_id]) {
                result[simulation_pass_id] = {};
            }
            if (!result[simulation_pass_id][run_number]) {
                result[simulation_pass_id][run_number] = {};
            }
            if (!result[simulation_pass_id][run_number][dpl_detector_id]) {
                result[simulation_pass_id][run_number][dpl_detector_id] = [];
            }
            result[simulation_pass_id][run_number][dpl_detector_id].push(flag);
        }
    };
    const groupedQcFlagsForSimulationPass = groupFlagsPerSimulationPassIdRunNumberDplDetectorId(qcFlagsPerSimulationPass);
...
module.exports = {
    up: async (queryInterface, Sequelize) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        await _updateFlagsEffectivePeriods(queryInterface, { transaction });
    }),
