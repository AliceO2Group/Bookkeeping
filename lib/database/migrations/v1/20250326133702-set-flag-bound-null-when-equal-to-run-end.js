...
module.exports = {
    up: async (queryInterface) => queryInterface.sequelize.transaction(async () => {
        await queryInterface.sequelize.query(`UPDATE quality_control_flags qcf INNER JOIN runs r ON qcf.run_number = r.run_number
                                              ...
                                              WHERE r.qc_time_start IS NOT NULL
                                                AND qcf.\`from\` = r.qc_time_start`, { type: QueryTypes.UPDATE });
        ...
        await queryInterface.sequelize.query(`UPDATE quality_control_flag_effective_periods qcfep
                                              ...
                                              SET qcfep.\`from\` = NULL
                                              WHERE r.qc_time_start IS NOT NULL
                                                AND qcfep.\`from\` = r.qc_time_start`, { type: QueryTypes.UPDATE });

        await queryInterface.sequelize.query(`UPDATE quality_control_flags qcf INNER JOIN runs r ON qcf.run_number = r.run_number
                                              SET qcf.\`to\` = NULL
                                              WHERE r.qc_time_end IS NOT NULL
                                                AND qcf.\`to\` = r.qc_time_end`, { type: QueryTypes.UPDATE });
        ...
        await queryInterface.sequelize.query(`UPDATE quality_control_flag_effective_periods qcfep 
                                              ...
                                              SET qcfep.\`to\` = NULL
                                              WHERE r.qc_time_end IS NOT NULL
                                                AND qcfep.\`to\` = r.qc_time_end`, { type: QueryTypes.UPDATE });
    }),
    ...
    down: async (queryInterface) => queryInterface.sequelize.transaction(async () => {
        await queryInterface.sequelize.query(`UPDATE quality_control_flags qcf INNER JOIN runs r ON qcf.run_number = r.run_number
                                              ...
                                              WHERE r.qc_time_start IS NOT NULL
                                                AND qcf.\`from\` IS NULL`, { type: QueryTypes.UPDATE });
        ...
        await queryInterface.sequelize.query(`UPDATE quality_control_flag_effective_periods qcfep
                                              ...
                                              WHERE r.qc_time_start IS NOT NULL
                                                AND qcfep.\`from\` IS NULL`, { type: QueryTypes.UPDATE });

        await queryInterface.sequelize.query(`UPDATE quality_control_flags qcf INNER JOIN runs r ON qcf.run_number = r.run_number
                                              SET qcf.\`to\` = r.qc_time_end
                                              WHERE r.qc_time_end IS NOT NULL
                                                AND qcf.\`to\` IS NULL`, { type: QueryTypes.UPDATE });
        ...
        await queryInterface.sequelize.query(`UPDATE quality_control_flag_effective_periods qcfep 
                                              ...
                                              WHERE r.qc_time_end IS NOT NULL
                                                AND qcfep.\`to\` IS NULL`, { type: QueryTypes.UPDATE });
    }),
};
