...
const CREATE_QC_FLAG_SIGNIFICANCE_AGGREGATE_FUNCTION = `
    CREATE OR REPLACE AGGREGATE FUNCTION qc_flag_significance(
        row_bad TINYINT(1),
...
const DROP_QC_FLAG_SIGNIFICANCE_AGGREGATE_FUNCTION = 'DROP FUNCTION qc_flag_significance';
...
const DROP_QC_FLAG_SIGNIFICANCE_COVERAGE_AGGREGATE_FUNCTION = 'DROP FUNCTION qc_flag_significance_coverage';

/** @type {import('sequelize-cli').Migration} */
module.exports = {
    up: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        await queryInterface.sequelize.query(CREATE_GAQ_PERIODS_VIEW, { transaction });
        await queryInterface.sequelize.query(CREATE_QC_FLAG_SIGNIFICANCE_AGGREGATE_FUNCTION, { transaction });
        await queryInterface.sequelize.query(CREATE_QC_FLAG_SIGNIFICANCE_COVERAGE_AGGREGATE_FUNCTION, { transaction });
    }),

    down: async (queryInterface) => queryInterface.sequelize.transaction(async (transaction) => {
        await queryInterface.sequelize.query(DROP_GAQ_PERIODS_VIEW, { transaction });
        await queryInterface.sequelize.query(DROP_GAQ_PERIODS_VIEW, { transaction });
        await queryInterface.sequelize.query(DROP_QC_FLAG_SIGNIFICANCE_AGGREGATE_FUNCTION, { transaction });
        await queryInterface.sequelize.query(DROP_QC_FLAG_SIGNIFICANCE_COVERAGE_AGGREGATE_FUNCTION, { transaction });
    }),
};
