...
module.exports = {
    up: async (queryInterface, Sequelize) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        const updateStmt = await prepareRunsUpdateStatement(
            await createAndGetPeriods({ queryInterface, transaction }),
            { queryInterface, transaction },
        );
        if (updateStmt) {
            await queryInterface.sequelize.query(updateStmt, { transaction });
        }
    ...
    down: async (queryInterface, Sequelize) => queryInterface.sequelize.transaction(async (transaction) => {
        ...
        const undoUpdateStmt = await prepareRunsUndoUpdateStatement({ queryInterface, transaction });
        if (undoUpdateStmt) {
