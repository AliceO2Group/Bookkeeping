...
class WhereAssociationQueryBuilder extends WhereQueryBuilder {
    ...
    constructor(sequelize, queryBuilder, association, column) {
        ...
        this.isItThroughJunctionTableExists = Boolean(sequelizeAssociation.toTarget);
        this.associatedModelName = this.isItThroughJunctionTableExists ?
    ...
    _op(operation) {
        this.queryBuilder.include({
            ...
            as: this.association,
            required: true,
            through: this.isItThroughJunctionTableExists ? {
                where: {
                    [`${this.associatedModelName}_${this.column}`]: operation,
                },
            } : undefined,
            where: !this.isItThroughJunctionTableExists ? {
