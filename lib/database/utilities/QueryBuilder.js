...
class QueryBuilder {
    ...
    constructor(sequelize) {
        this._sequelize = sequelize;
        this.options = { where: {}, replacements: {}, having: {} };
    }
     ...
     * @param {string} [attributeDefinition.query] sql definition of attribute, e.g. function or subquery
    ...
    /**
     * Add a litteral expression to the current HAVING condition ("AND")
     *
     * @param {string} expression the literal HAVING condition
     * @param {Object} replacements the replacement text for the literal (global to all the query builder). Placeholders in literal
     *     (prefixed by ":") will be replaced by the value at the corresponding key
     * @return {QueryBuilder} fluent interface
     */
    literalHaving(expression, replacements) {
        if (!this.options.having[Op.and]) {
            this.options.having[Op.and] = [];
        }

        this.options.having[Op.and].push(this._sequelize.literal(expression));
        for (const key in replacements) {
            this.options.replacements[key] = replacements[key];
        }
        return this;
