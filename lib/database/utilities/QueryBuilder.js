...
class QueryBuilder {
     ...
     *
     * @param {Object|string} model The database model or model identifier.
    ...
    toImplementation(model) {
        ...
        model = typeof model === 'string' ? this._sequelize.models[model] : model;

        if (!this._selectOnly && !hasExplicitColumns) {
            options.attributes = { include: options.attributes };
        } else {
            const primaryKey = model.primaryKeyAttribute;
            this.orderBy(primaryKey, 'asc'); // Array syntax is inconsistent with its sorting unless explicitly defined
        }
