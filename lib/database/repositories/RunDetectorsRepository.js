...
class RunDetectorsRepository extends Repository {
     ...
     * @param {number} runId  - run ID to delete detectors for
     * @return {Promise<void>} - delete result
     */
    async removeById(runId) {
        return RunDetectors.sequelize.getQueryInterface().bulkDelete('run_detectors', { runId: runId });
    }

    /**
     * Update run detectors by runNumber
     * @param {Array} entities - Detectors & runId to associate
