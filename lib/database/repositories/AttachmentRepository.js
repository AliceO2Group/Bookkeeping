...
class AttachmentRepository extends Repository {
    ...
    async bulkInsert(entities) {
        ...
        const results = await Attachment.bulkCreate(entities.map((entity) => {
            const fileData = fs.readFileSync(entity.path);
            const hash = crypto.createHash('md5').update(fileData).digest('hex');

            return AttachmentAdapter.toDatabase({ ...entity, hash });
        }), { returning: true });
