...
class QcFlagRepository extends Repository {
    ...
    async findGaqPeriods(dataPassId, runNumber) {
        const query = `
            ...
            INNER JOIN data_pass_quality_control_flag AS dpqcf ON dpqcf.quality_control_flag_id = qcf.id
            INNER JOIN gaq_periods_timestamps AS gaq_periods ON gaq_periods.data_pass_id = dpqcf.data_pass_id
            INNER JOIN global_aggregated_quality_detectors AS gaqd
    ...
    async getGaqCoverages(dataPassId) {
        const innerQuery = `
            SELECT 
                ...
                gp.run_number,
                gp.coverage_ratio,
                qc_flag_significance(qcft.bad, qcft.monte_carlo_reproducible) AS significance
            FROM gaq_periods_timestamps AS gp
            INNER JOIN data_pass_quality_control_flag AS dpqcf ON dpqcf.data_pass_id = gp.data_pass_id
            ...
            INNER JOIN quality_control_flag_types AS qcft ON qcf.flag_type_id = qcft.id
        ...
        const query = `
                SELECT data_pass_id,
                       run_number,
                       qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'bad')  AS bad_coverage,
                       qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'mcr')  AS mcr_coverage,
                       qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'good') AS good_coverage,
