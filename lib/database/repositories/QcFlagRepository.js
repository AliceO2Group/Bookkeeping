...
class QcFlagRepository extends Repository {
     ...
     *
     * @param {number} dataPassId id of data pass id
     * @return {Promise<GaqAggregation[]>} Resolves with the GAQ aggrgation
     */
    async getGaqSummary(dataPassId) {
        const effectivePeriodsWithTypeSubQuery = `
            SELECT
                ...
                GROUP_CONCAT( DISTINCT qcf.id ) AS missingVerificationsList
        ...
        const query = `
            SELECT
                ...
                GROUP_CONCAT(effectivePeriods.missingVerificationsList) AS missingVerificationsList,
        ...
        return rows.map(({
            ...
            missingVerificationsList,
            mcReproducible,
        }) => ({
            runNumber,
            bad,
            effectiveRunCoverage,
            missingVerificationsCount: [...new Set(missingVerificationsList.split(','))].length,
