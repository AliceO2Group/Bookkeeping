...
class QcFlagRepository extends Repository {
    ...
    async findGaqPeriods(dataPassId, runNumber) {
        ...
        const rows = await this.model.sequelize.query(query, { type: QueryTypes.SELECT });
        return rows.map(({
            dataPassId,
            runNumber,
            from,
    ...
    async getGaqCoverages(dataPassId, runNumber) {
        ...
        -- following LEFT JOIN causes nullification of significance of periods with missing QC flags (see condition in the IF statement),
        -- in consequence, lowering values of badCoverage, mcReproducibleCoverage and goodCoverage, e.g.: bad CPV + bad ITS + null ZDC => null
        -- when INNER JOIN is used, detectors with missing flags are rejected from results,
        -- therefore condition in the IF statement is met,
        -- following example above: bad CPV + bad ITS + (nothing because null ZDC is rejected) => bad)
            LEFT JOIN (
        ...
        const rows = await this.model.sequelize.query(summaryQuery, { replacements, type: QueryTypes.SELECT });
