    ...
    async findGaqPeriods(dataPassId, runNumber) {
        const query = `
            SELECT
                ...
                SUM(qcft.monte_carlo_reproducible) >= 1 AS mcReproducible,
                COUNT( DISTINCT qcf.id ) - COUNT( DISTINCT qcfv.flag_id ) AS missingVerificationsCount

            FROM quality_control_flags AS qcf
            INNER JOIN quality_control_flag_types AS qcft ON qcft.id = qcf.flag_type_id
            LEFT JOIN quality_control_flag_verifications as qcfv.flag_id = qcf.id
    ...
    async getGaqSummary(dataPassId) {
        const effectivePeriodsWithTypeSubQuery = `
            SELECT
                ...
                SUM(qcft.bad) >= 1 AS bad,
                IF(SUM(qcft.bad) = 0, SUM(qcft.monte_carlo_reproducible) >= 1, false) AS mcReproducible,
                COUNT( DISTINCT qcf.id ) - COUNT( DISTINCT qcfv.flag_id ) missingVerificationsCount
        ...
        const query = `
            SELECT
                ...
                SUM(effectivePeriods.missingVerificationsCount) AS missingVerificationsCount,
        ...
        return rows.map(({
            ...
            mcReproducible,
            bad,
            effectiveRunCoverage: parseFloat(effectiveRunCoverage),
        }));
    }
