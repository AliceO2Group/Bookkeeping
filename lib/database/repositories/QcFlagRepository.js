...
class QcFlagRepository extends Repository {
    ...
    async getRunGaqSubSummaries(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        // const [r] = await this.model.sequelize.query(effectivePeriodsWithTypeSubQuery, { replacements: { dataPassId, mcReproducibleAsNotBad } });
        // return r.map(r => ({ ...r, from: new Date(r.from * 1000), to: new Date(r.to * 1000) })).filter(r => r.runNumber === 56);
