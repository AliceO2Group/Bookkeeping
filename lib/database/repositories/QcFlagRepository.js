...
class QcFlagRepository extends Repository {
    ...
    async getGaqCoverages(dataPassId) {
        {
            const query = `
                SELECT gp.data_pass_id,
                       gp.run_number,
                       gp.coverage_ratio,
                       qc_flag_significance(qcft.bad, qcft.monte_carlo_reproducible) significance
                FROM gaq_periods gp
                         INNER JOIN data_pass_quality_control_flag dpqcf ON dpqcf.data_pass_id = gp.data_pass_id
                         INNER JOIN quality_control_flags qcf ON dpqcf.quality_control_flag_id = qcf.id
                         INNER JOIN quality_control_flag_effective_periods qcfep ON qcf.id = qcfep.flag_id AND
                                                                                    (qcfep.from IS NULL OR qcfep.\`from\` < gp.\`to\`) AND
                                                                                    (qcfep.to IS NULL OR qcfep.\`to\` > gp.\`from\`)
                         INNER JOIN quality_control_flag_types qcft ON qcf.flag_type_id = qcft.id
        ...
        const query = `
            SELECT data_pass_id,
                   run_number,
                   qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'bad')  bad_coverage,
                   qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'mcr')  mcr_coverage,
                   qc_flag_significance_coverage(gaq.significance, coverage_ratio, 'good') good_coverage,
                   SUM(coverage_ratio)                                                     total_coverage
            FROM (SELECT gp.data_pass_id,
                         gp.run_number,
                         gp.coverage_ratio,
                         qc_flag_significance(qcft.bad, qcft.monte_carlo_reproducible) significance
                  FROM gaq_periods gp
                           ...
                           INNER JOIN quality_control_flag_types qcft ON qcf.flag_type_id = qcft.id
                  WHERE gp.data_pass_id = :dataPassId
                  GROUP BY gp.data_pass_id, gp.run_number, gp.\`from\`, gp.to) AS gaq
            GROUP BY gaq.data_pass_id, gaq.run_number;
        `;
        const [rows] = await this.model.sequelize.query(query, { replacements: { dataPassId } });
        console.log(JSON.stringify(rows));
        return new Map(rows.map(({ run_number, bad_coverage, mcr_coverage, good_coverage, total_coverage }) => [
            run_number,
            {
    ...
    async getRunGaqSubSummaries(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        const query = `
            SELECT effectiveperiods.runnumber,
            ...
            GROUP BY effectiveperiods.datapassid,
