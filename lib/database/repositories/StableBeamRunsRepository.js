...
const getEorDistributionQuery = ({ from, to }) => `
    SELECT COUNT(*) count, category, title
    FROM (SELECT rt.category, rt.title
          FROM stable_beam_runs sbr
                   INNER JOIN runs r on sbr.run_number = r.run_number
          ...
          GROUP BY r.run_number, rt.category, rt.title) grouped
...
class StableBeamRunsRepository extends Repository {
    ...
    async getTimeBetweenRunsDistribution(period, timeWindow, restWindow) {
        const rows = await sequelize.query(
            getTimeBetweenRunsHistogramQuery(period, timeWindow, restWindow),
    ...
    async getEfficienciesPerFillPerDetector(period) {
        const rows = await sequelize.query(getEfficiencyPerDetectorsQuery(period), { raw: true, type: QueryTypes.SELECT });
        return rows;
    }
    ...
    async getEorDistribution(period) {
        const rows = await sequelize.query(getEorDistributionQuery(period), { raw: true, type: QueryTypes.SELECT });
        return rows;
    }
}
