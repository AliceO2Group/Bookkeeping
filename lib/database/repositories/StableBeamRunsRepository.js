...
const getEfficiencyPerDetectorsQuery = ({ from, to }) => `
    ...
    WHERE r.definition = 'PHYSICS'
      ...
      AND sbr.sb_run_end >= '${timestampToMysql(from)}'
      AND sbr.sb_run_start < '${timestampToMysql(to)}'
...
class StableBeamRunsRepository extends Repository {
     ...
     * @return {Promise<Array<{fillNumber: number, detectorName: string, efficiency: string, netEfficiency: string}>>} the efficiency statistics
     */
    async getEfficiencyPerFillPerDetector(period) {
