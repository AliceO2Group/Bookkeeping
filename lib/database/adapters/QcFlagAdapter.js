...
class QcFlagAdapter {
    ...
    toEntity(databaseObject) {
        ...
        return {
            ...
            ineffectiveFrom: ineffectiveFrom ? new Date(ineffectiveFrom).getTime() : null,
            ineffectiveTo: ineffectiveTo ? new Date(ineffectiveTo).getTime() : null,
            runNumber,
            dplDetectorId,
            flagTypeId,
            ...
            ineffectivenessPeriods: (ineffectivenessPeriods ?? []).map(({ from, to }) => ({ from, to })),
        };
    }
}
