/**
 * @license
 * Copyright CERN and copyright holders of ALICE O2. This software is
 * distributed under the terms of the GNU General Public License v3 (GPL
 * Version 3), copied verbatim in the file "COPYING".
 *
 * See http://alice-o2.web.cern.ch/license for full licensing information.
 *
 * In applying this license CERN does not waive the privileges and immunities
 * granted to it by virtue of its status as an Intergovernmental Organization
 * or submit itself to any jurisdiction.
 */

/**
 * LhcPeriodAdapter
 */
class LhcPeriodAdapter {
    /**
     * Constructor
     */
    constructor() {
        this.toEntity = this.toEntity.bind(this);
        this.toDatabase = this.toDatabase.bind(this);
    }

    /**
     * Converts the given database object to an entity object.
     *
     * @param {SequelizeLhcPeriod} databaseObject Object to convert.
     * @returns {LhcPeriod} Converted entity object.
     */
    toEntity(databaseObject) {
        const { id, name } = databaseObject;
        const { runs, dataPasses } = databaseObject;
        const runsCount = runs ? runs[0]?.get('count') ?? 0 : null;
        const distinctEnergies = runs ? runs[0]?.get('distinctEnergies')?.split(',').map((energy) => Number(energy)) ?? [] : null;
        const beamType = runs ? runs[0]?.get('beamType')?.split(',') ?? null : null;

        const dataPassesCount = dataPasses ? dataPasses[0]?.get('count') ?? 0 : null;
        return { id, name, runsCount, dataPassesCount, distinctEnergies, beamType };
    }

    /**
     * Converts the given entity object to a database object.
     *
     * @param {LhcPeriod} entityObject Object to convert.
     * @returns {SequelizeLhcPeriod} Converted database object.
     */
    toDatabase({ id, name }) {
        return { id, name };
    }
}

module.exports = LhcPeriodAdapter;
