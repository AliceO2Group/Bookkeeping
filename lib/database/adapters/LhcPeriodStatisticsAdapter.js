...
class LhcPeriodStatisticsAdapter {
    ...
    toEntity(databaseObject) {
        const { id, avgCenterOfMassEnergy, distinctEnergies, lhcPeriod } = databaseObject.dataValues;
        const entity = { id, avgCenterOfMassEnergy, distinctEnergies: distinctEnergies?.split(',').map((energy) => Number(energy)) ?? [] };
