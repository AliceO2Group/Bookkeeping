...
const LHC_REVOLUTION_CONSTANT = 11245; // The LHC revolution frequency: 11.245kHz (~ c/26.7km).
const COLLIDING_LHC_BUNCH_CROSSINGS_IN_FILL_SCHEMA_REGEX = /[A-Za-z0-9]+_[A-Za-z0-9]+_[0-9]+_([0-9]+)_.*/;

/**
 * Extract number of colliding LHC bunch crossing from LHC fill schema
 * @param {string} fillingSchema filling schema
 * @return {number} number of colliding LHC bunch crossing
 */
const extractCollidingLhcBunchCrossings = (fillingSchema) =>
    parseInt(fillingSchema?.match(COLLIDING_LHC_BUNCH_CROSSINGS_IN_FILL_SCHEMA_REGEX)?.[1], 10);
...
class RunAdapter {
    ...
    toEntity(databaseObject) {
        ...
        entityObject.avgInelasticInteractionRate = avgInelasticInteractionRate;
        ...
        if (lhcFill && avgInelasticInteractionRate !== null) {
            entityObject.muInelasticInteractionRate =
                avgInelasticInteractionRate / (extractCollidingLhcBunchCrossings(lhcFill.fillingSchemeName) * LHC_REVOLUTION_CONSTANT);
        }
    ...
    toDatabase(entityObject) {
        return {
            ...
            avgInelasticInteractionRate: entityObject.avgInelasticInteractionRate,
