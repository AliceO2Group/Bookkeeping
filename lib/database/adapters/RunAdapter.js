...
const filterObjectFields = (obj, args) => {
    ...
    if (!args || (args.length == 1 && args[0] === '') || args.length == 0 || !args['run']) return obj;

    return ({
        ...args['run'].split(',').reduce((res, key) => ({ ...res, [key]: obj[key] }), {})
    })
}
...
class RunAdapter {
     ...
     */
    static toEntity(databaseObject, fields) {
        const {
            bytesReadOut,
            dd_flp,
            dcs,
        ...
        return filterObjectFields(entityObject, fields);
    }
     ...
     * @param {Run} databaseObject run object
     * @returns {Object} minified version of the run entity
     */
