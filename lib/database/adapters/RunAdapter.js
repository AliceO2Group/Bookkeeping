...
class RunAdapter {
    ...
    _addDetectorsToObject(detectors, entityObject, addDetectorQuality = true) {
        ...
        if (addDetectorQuality) {
            entityObject.detectorsQualities =
                detectors.map((detector) => !detector.RunDetectors
                    ? null : { id: detector.id, name: detector.name, quality: detector.RunDetectors.quality }).filter((item) => Boolean(item));
        } else {
            entityObject.detectorsQualities = [];
        }
    }
    ...
    /**
     * Computes QCflags and adds them to an entityObject
     *
     * @param {QCFlag[]} [qcflags = []] the list of qcFlags
     * @param {object} entityObject the entity object that is to be modified
     * @param {boolean} [isSummary = true] if the qcFlags should be computed as summaries or entities
     * @returns {void}
     */
