...
class RunAdapter {
    ...
    toSummary(databaseObject) {
        const { detectors, eorReasons, runNumber, timeO2Start, timeO2End, timeTrgStart, timeTrgEnd, firstTfTimestamp, lastTfTimestamp } = databaseObject;
        const { inelasticInteractionRateAvg, inelasticInteractionRateAtStart, inelasticInteractionRateAtMid, inelasticInteractionRateAtEnd } = databaseObject;
        const { aliceL3Polarity, aliceL3Current, aliceDipolePolarity, aliceDipoleCurrent } = databaseObject;
    ...
    /**
     * Adapts the run entity to a summary for lhcFills.
     * @param {SequelizeRun} databaseObject run object
     * @returns {MinifiedRun} summary for lhcFills of the run entity
     */
    toLhcFillSummary(databaseObject) {
        const summary = {
            id: databaseObject.id,
            runNumber: databaseObject.runNumber,
            runQuality: databaseObject.runQuality,
            startTime: databaseObject.startTime,
            endTime: databaseObject.endTime,
            ctfFileSize: databaseObject.ctfFileSize,
            tfFileSize: databaseObject.tfFileSize,
        };

        this._addDetectorsToObject(databaseObject.detectors, summary, false);
