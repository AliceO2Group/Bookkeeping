...
const extractNumberOfCollidingLhcBunchCrossings = (fillingSchema) => {
    const [, matchedValueForAlice] = fillingSchema?.match(COLLIDING_LHC_BUNCH_CROSSINGS_IN_FILL_SCHEMA_REGEX) || [];
    ...
    toEntity(databaseObject) {
        ...
        if (lhcFill && inelasticInteractionRateAvg !== null) {
            ...
            entityObject.muInelasticInteractionRate = numberOfCollidingLhcBunchCrossings
                ? inelasticInteractionRateAvg / (numberOfCollidingLhcBunchCrossings * LHC_REVOLUTION)
                : null;
        } else {
