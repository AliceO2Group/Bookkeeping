...
/**
 * Remove legacy detectors for newer LHC periods.
 *
 ...
 */
const filterOutLegacyDetectorsForNewerPeriods = (detectors, label = '', legacyDetectorNames = ['CPV', 'PHS']) => {
    const shouldFilterOutLegacyDetectors = /LHC(2[5-9]|[3-9]\d)/.test(label ?? '');

    return detectors.filter(({ name }) => !shouldFilterOutLegacyDetectors || !legacyDetectorNames.includes(name));
};
...
exports.filterOutLegacyDetectorsForNewerPeriods = filterOutLegacyDetectorsForNewerPeriods;
