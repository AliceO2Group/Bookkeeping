...
const DATA_PASS_NAME_REGEX = /^LHC\d\d[a-zA-Z]+_[a-z]pass[^\s]*$/;
...
class MonAlisaClient {
     ...
     * @return {MonAlisaDataPassVersion[]} data passes list
     */
     ...
     * @return {string} payload raw data acquired from MonAlisa: csv with data passes proeprties
     * @private
     */
     ...
     * @param {string} payload raw data acquired from MonAlisa: csv with data passes proeprties
     * @return {MonAlisaDataPassVersion[]} adapted data
     * @private
     */
    _extractDataPassesFromMonAlisaPayload(payload) {
        return payload
            ...
            .map((row) => {
                ...
                const reconstructedEventsCount = Number(row[10]);
                const lastSeen = Number(row[14]);
                const outputSize = Number(row[16]);
            ...
            .filter((dataPass) => DATA_PASS_NAME_REGEX.test(dataPass.name) && extractLhcPeriod(dataPass.name)?.year >= this.yearLowerLimit);
    }
