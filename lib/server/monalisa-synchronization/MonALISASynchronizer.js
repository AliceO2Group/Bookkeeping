    ...
    async synchronizeSimulationPassesFromMonALISA() {
        try {
            ...
            const transactionablePipeline = async (simulationPassData) => {
                ...
                const simulationPassDB = await SimulationPassRepository.findOne(dataSource
                    .createQueryBuilder().where('name').is(simulationPassProperties.name));
                await simulationPassDB.addDataPass(associatedDataPasses, { ignoreDuplicates: true });

                const associatedRuns = await RunRepository.findAll(dataSource
