...
class MonALISASynchronizer {
    ...
    async synchronizeDataPassesFromMonALISA() {
        try {
            ...
            for (const dataPass of dataPasses) {
                await dataSource.transaction(() => this._updateOneDataPass(dataPass))
     ...
     * @private
     */
    async _updateOneDataPass(dataPass) {
        ...
        const runs = await RunRepository.findAll({ where: { runNumber: { [Op.in]: runNumbers } } });
