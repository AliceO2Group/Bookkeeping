...
const { setupMonALISAClient } = require('./MonALISAClient.js');
...
class MonALISASynchronizer {
    ...
    async synchronizeDataPassesFromMonALISA() {
        if (!this.monALISAInterface) {
            this.monALISAInterface = await setupMonALISAClient();
        }

        try {
            this.lastRunNumbers = await this._getLastRunNumbersInformation();
    ...
    _doesDataPassNeedUpdate(dataPass) {
        const isLastRunNumberSame = dataPass.lastRun && dataPass.lastRun === this.lastRunNumbers[dataPass.name];
        return !isLastRunNumberSame;
    }

    /**
     * Fetch last runs information telling whether some data pass must be updated
     * last run number of a data pass is the greatest run number associated with given data pass
     * @return {Object<string, number>} mapping data pass name -> its last run
     */
    async _getLastRunNumbersInformation() {
        ...
        const lastRunNumbersEntries = lastRunsData.map((lastRunData) => {
