...
class MonALISASynchronizer {
    ...
    async synchronizeSimulationPassesFromMonALISA() {
        try {
            const simulationPasses = await this.monALISAInterface.getDataPasses();

            // eslint-disable-next-line require-jsdoc
            const transactionablePipeline = async (simulationPass) => {
                const { lhcPeriods, dataPassesSuffixes, runNumbers } = simulationPass;
                const dataPassesNames = lhcPeriods.map((lhcPeriod) => dataPassesSuffixes.map((suffix) => `${lhcPeriod}_${suffix}`));
                const associatedDataPasses = await DataPassRepository.findAll(dataSource
                    .createQueryBuilder().where('name').oneOf(...dataPassesNames));
                if (associatedDataPasses.length === 0) {
                    throw new Error(`No data passes associated to ${simulationPass.name}`);
                }
                const associatedRuns = await RunRepository(dataSource
                    .createQueryBuilder().where('runNumber').oneOf(...runNumbers));
                const simulationPassDB = await SimulationPassRepository.upsert(simulationPass);
                await simulationPassDB.addDataPasses(associatedDataPasses, { ignoreDuplicates: true });
                if (associatedRuns.length > 0) {
                    simulationPassDB.addRuns(associatedRuns, { ignoreDuplicates: true });
                }
            };
