...
class MonAlisaSynchronizer {
     ...
     * @private
     */
    _doesDataPassVersionNeedUpdate({ description, lastSeen }, dataPassesLastSeen) {
        const isLastSeenSame = lastSeen && lastSeen === dataPassesLastSeen?.[description]?.lastSeen;
        return !isLastSeenSame;
    }

    /**
     * Fetch last_seen information telling whether some data pass must be updated
     * @return {Object<string, { lastSeen: number, id: number }>} mapping: data pass version description -> { lastSeen, id }
     * @private
     */
    async _getAllDataPassVersionsLastSeenAndId() {
        ...
        return Object.fromEntries(dataPassesLastSeen.map(({ id, description, lastSeen }) => [description, { lastSeen, id }]));
    }
    ...
    async _updateDeletedDataPassVersions(dataPassVersionsLastSeenAndId, allMonAlisaDataPassesVersions) {
        ...
        const deletedDataPassesVersionsDescriptions = allDBDataPassesVersionsDescriptions.filter((description) =>
            !allFechedDataPassesVersionsDescriptions.has(description));

        const deleteDataPassesVersionsIds = deletedDataPassesVersionsDescriptions.map((description) =>
            dataPassVersionsLastSeenAndId[description].id);
