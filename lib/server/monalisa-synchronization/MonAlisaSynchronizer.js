...
class MonAlisaSynchronizer {
    ...
    async _synchronizeDataPassesFromMonAlisa() {
        try {
            ...
            for (const monalisaDataPass of dataPasses) {
                try {
                    await dataSource.transaction(() => this._updateOneDataPass(monalisaDataPass));
                } catch (error) {
                    if (!errorMessageToDataPassesNames.has(error.message)) {
                        errorMessageToDataPassesNames[error.message] = [];
                    }
                    errorMessageToDataPassesNames[error.message].push(monalisaDataPass.name);
                }
            }
            if (errorMessageToDataPassesNames.size !== 0) {
                const stackedError = errorMessageToDataPassesNames.entries()
                    .map(([errorMessage, dataPassesList]) => `Error for data passes: ${dataPassesList.join(',')}\n${errorMessage}`)
                    .join('\n');

                this.logger.errorMessage(stackedError, {});
            }
     ...
     * @param {DataPass} monalisaDataPass data pass properties and its associations
     * @return {Promise<void>} promise
     * @private
     */
    async _updateOneDataPass(monalisaDataPass) {
