...
class MonAlisaSynchronizer {
    ...
    async _synchronizeDataPassesFromMonAlisa() {
        try {
            ...
            const monAlisaDataPassesVersions = (await this._monAlisaInterface.getDataPassesVersions())
                .filter((monalisaDataPass) => this._doesDataPassNeedUpdate(monalisaDataPass, dataPassVersionsLastSeen));
     ...
     * @private
     */
    _doesDataPassNeedUpdate({ name, description, lastSeen }, dataPassesLastSeen) {
