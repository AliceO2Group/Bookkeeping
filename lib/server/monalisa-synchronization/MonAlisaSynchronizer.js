...
class MonAlisaSynchronizer {
    ...
    async _synchronizeDataPassesFromMonAlisa() {
        try {
            const dataPassVersionsLastSeenAndId = await this._getAllDataPassVersionsLastSeenAndId();
            ...
            const monAlisaDataPassesVersionsToBeUpdated = allMonAlisaDataPassesVersions
                .filter((monalisaDataPassVersion) =>
                    this._doesDataPassVersionNeedUpdate(monalisaDataPassVersion, dataPassVersionsLastSeenAndId));
            ...
            await this._updateDeletedDataPassVersions(dataPassVersionsLastSeenAndId, allMonAlisaDataPassesVersions);
    ...
    _doesDataPassVersionNeedUpdate({ name, description, lastSeen }, dataPassesLastSeen) {
        const isLastSeenSame = lastSeen && lastSeen === dataPassesLastSeen?.[name]?.[description]?.lastSeen;
        return !isLastSeenSame;
    }

    /**
     * Fetch last_seen information telling whether some data pass must be updated
     * @return {Object<string, Object<string, { lastSeen: number, id: number }>>} mapping
     *  data pass name -> data pass version description -> { lastSeen, id }
     * @private
     */
    async _getAllDataPassVersionsLastSeenAndId() {
        ...
        return Object.fromEntries(dataPassesLastSeen
            .map(({ name, versions }) => [
                name,
                Object.fromEntries(versions.map(({ id, description, lastSeen }) => [description, { lastSeen, id }])),
            ]));
    }

    /**
     * Mark data passes versions deleted from MonALISA
     *
     * @param {object} dataPassVersionsLastSeenAndId mapping
     *  data pass name -> data pass version description -> { lastSeen, id }
     * @param {MonAlisaDataPassVersion[]} allMonAlisaDataPassesVersions all data pass versions fetched from MonALISA
     * @return {Promise<void>} promise
     */
    async _updateDeletedDataPassVersions(dataPassVersionsLastSeenAndId, allMonAlisaDataPassesVersions) {
        const allDBDataPassesVersionsDescriptions = Object.values(dataPassVersionsLastSeenAndId)
            .flatMap((descriptionToLastSeen) => Object.keys(descriptionToLastSeen));

        const allFechedDataPassesVersionsDescriptions = new Set(allMonAlisaDataPassesVersions.map(({ description }) => description));

        const deleteDataPassesVersionsDescriptions = allDBDataPassesVersionsDescriptions.filter((description) =>
            !allFechedDataPassesVersionsDescriptions.has(description));

        const descriptionToId = Object.fromEntries(Object.values(dataPassVersionsLastSeenAndId)
            .map((descriptionToId) => Object.entries(descriptionToId)).flatMap((_) => _));
        const deleteDataPassesVersionsIds = deleteDataPassesVersionsDescriptions.map((description) => descriptionToId[description].id);

        await DataPassVersionRepository.updateAll({ deletedFromMl: true }, { where: { id: { [Op.in]: deleteDataPassesVersionsIds } } });
    }
