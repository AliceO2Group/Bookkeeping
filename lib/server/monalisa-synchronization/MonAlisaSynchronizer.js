...
class MonAlisaSynchronizer {
    ...
    async _synchronizeDataPassesFromMonAlisa() {
        try {
            ...
            const monAlisaDataPassesVersions = (await this._monAlisaInterface.getDataPassesVersions())
                .filter((monalisaDataPassVersion) => this._doesDataPassVersionNeedsUpdate(monalisaDataPassVersion, dataPassVersionsLastSeen));
    ...
    async _updateOneDataPassVersion(monalisaDataPassVersion) {
        return dataSource.transaction(async () => {
            ...
            await DataPassRepository.upsert({ lhcPeriodId: lhcPeriodDB.id, name: dataPassName });

            const dataPass = await DataPassRepository.findOne({ where: { name: dataPassName } });
     ...
     * @private
     */
    _doesDataPassVersionNeedsUpdate({ name, description, lastSeen }, dataPassesLastSeen) {
