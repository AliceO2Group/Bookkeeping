 ...
 * @param {number[]} runList
 * @param {string} description
 * @param {string} PWG
 * @param {number} requestedEvents
 * @param {number} generatedEvents
 */
...
class MonALISAClient {
     ...
     * @return {SimulationPass[]} simulation passes list
     */
     ...
     * mapping of a simulation pass name to its parameters
     * @return {SimulationPass[]} adapted data
     * @private
     */
    _extractSimulationPassesFromPayload(payload) {
        return Object.entries(payload).map(([simulationPassName, simulationPassParameters]) => ({
            ...
            .filter(({ lhcPeriods, dataPassesSuffixes }) => lhcPeriods.length > 0 && dataPassesSuffixes.length > 0);
    }

    /**
     * Adapt simulation pass
     * @param {MonALISASimulationPass} simulationPass simulation pass
     * @return {SimulationPass} Convert MonALISA simulation pass to simulation pass entity
     * @private
     */
    _monALISASimulationPassToEntity(simulationPass) {
        return {
            name: simulationPass.name,
            jiraID: simulationPass.jiraID.trim() || null, // MonALISA returns empty string
            lhcPeriods: simulationPass.anchor_production.split(',').map((lhcPeriod) => lhcPeriod.trim()).filter((lhcPeriod) => lhcPeriod),
            dataPassesSuffixes: simulationPass.anchor_pass.split(',').map((suffix) => suffix.trim()).filter((suffix) => suffix),
            runList: simulationPass.runList.split(',').filter((runNumber) => runNumber.trim()).map((runNumber) => Number(runNumber)),
            description: simulationPass.generator.trim() || null,
            PWG: simulationPass.PWG.trim() || null,
            requestedEvents: Number(simulationPass.requested_events) || null, // 0 is not valid value so it's turned to null
            generatedEvents: Number(simulationPass.generated_events) || null,
        };
    }
}
