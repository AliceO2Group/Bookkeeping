...
export const objectFlattenReverseMarker = 'REVERSEMARKER';

/**
 * Flatten an object to an array of flat (no nesting) objects.
 ...
 */
export const objectsFlattener = (object, templateObject) => {
    const result = objectsFlattenerRecursive(object, templateObject);
    // Reverse child objects that are in the wrong order.
    const returnValue = reverseArraySelective(result, objectFlattenReverseMarker);
...
const objectsFlattenerRecursive = (object, templateObject, prefix = '', result = [], markReverse = false) => {
        ...
        if (Object.prototype.hasOwnProperty.call(object, property)) {
            ...
            if (typeof object[property] === 'object' && (Array.isArray(object[property])
                ...
                if (prefix === '') {
                    ...
                    objectToAdd[objectFlattenReverseMarker] = false;
                    propertiesAdded = true;
                } else {
                    objectToAdd[`${prefix}.${property}`] = object[property];
                    objectToAdd[objectFlattenReverseMarker] = markReverse;
                    propertiesAdded = true;
                }
            }
        }
    ...
    // Return result for final result (recursion ignores return value).
    return result;
};
 ...
 */
export const reverseArraySelective = (parentCollection, identifier = objectFlattenReverseMarker) => {
    const indexes = [];
    let startAddIndex = 0;
    let endAddIndex = 0;
