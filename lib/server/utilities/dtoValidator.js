 ...
 */
const dtoValidator = async (dto, { body, params, query, session, file = undefined, files = undefined }, response) => {
    let value = null;
    const request = { body, params, query };
    Object.assign(request, ...[file, files].filter((f) => f));

    try {
        value = await dto.validateAsync(request, {
            abortEarly: false,
        });
