 ...
 * @param {Number} maxPerPage limit with which the data were acquired
 * @returns {CountedDataView} adapted result
 */
function countedDataToHttpView({ count, rows }, maxPerPage) {
    count = Array.isArray(count) ? count.length : count;
    return {
        meta: {
            page: {
                totalCount: count,
                pageCount: maxPerPage ? Math.ceil(count / maxPerPage) : null,
            },
        },
        data: rows,
    };
