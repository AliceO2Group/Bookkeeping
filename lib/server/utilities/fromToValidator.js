...
function fromToValidator(text, value) {
    ...
    if (from && from > new Date()) {
        errors.push(attributeError(`${text}/from`, `Creation date must be today (${new Date().toISOString()}) or earlier`));
    }
    if (to) {
        to > endDay &&
            errors.push(attributeError(`${text}/to`, `Creation date must be today (${new Date(endDay).toISOString()}) or earlier`));
        if (from && from > to) {
