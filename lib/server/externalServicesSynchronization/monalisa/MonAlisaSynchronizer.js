...
class MonAlisaSynchronizer {
     ...
     *
     * If some of data passes in given batch are `skimmed`, what is indicated by having word `skimmed` in their names,
     * it is expected that those passes are updated with this method in incremental order coherent with their names
     * e.g. first update LHC24a_apass2_skimmed and only then LHC24a_apass3_skimmed
     *
    ...
    async _getSkimmingStage(dataPassName, lhcPeriodId) {
        ...
        if (SKIMMED_DATA_PASS_NAME_REGEX.test(dataPassName)) {
            const existingSkimmedPass = await DataPassRepository.findOne({
                where: {
                    lhcPeriodId,
                    skimmingStage: SkimmingStage.SKIMMED,
                    name: { [Op.not]: dataPassName },
                },
            });
    ...
    async _getAllDataPassVersionsLastSeenAndIdAndLastStatus() {
        ...
        return Object.fromEntries(dataPassesLastSeen.map(({ description, lastSeen, id, statusHistory }) => [
            description,
            { lastSeen, id, status: statusHistory[statusHistory.length - 1].status },
        ]));
    }
