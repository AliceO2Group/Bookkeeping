...
const createQcFlagHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            ...
            const qcFlag = { from, to, comment };
            const relations = {
                userIdentifier: { externalUserId: validatedDTO.session.externalId },
                flagTypeId,
                runNumber,
                dplDetectorId,
                dataPassId,
            ...
            const createdFlag = await qcFlagService.create(qcFlag, relations);
