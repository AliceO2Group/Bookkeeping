...
const getGaqSummaryHandler = async (request, response) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly(Joi.object({
            ...
            mcReproducibleAsNotBad: Joi.boolean().optional(),
            runNumber: Joi.number().positive().optional(),
        })),
        request,
        response,
    );
...
/**
 * Recalculate GAQ summary for given data pass and optionally run number
 */
const recalculateGaqSummaryHandler = async (request, response) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly(Joi.object({
            dataPassId: Joi.number().positive().required(),
    ...
    if (validatedDTO) {
        try {
            const { dataPassId, runNumbers, onlyInvalidated } = validatedDTO.query;

            const data = await gaqService.recalculateSummaries(dataPassId, runNumbers, onlyInvalidated);
            response.json({ data });
        } catch (error) {
            updateExpressResponseFromNativeError(response, error);
        }
