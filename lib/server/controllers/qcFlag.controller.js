    ...
    if (validatedDTO) {
        try {
            ...
            const { count, rows: items } = await qcFlagService.getAllPerDataPassAndRunAndDetector(
                { dataPassId, runNumber, detectorId: detectorId },
                { limit, offset },
                filter,
            );
    ...
    if (validatedDTO) {
        try {
            ...
            const { count, rows: items } = await qcFlagService.getAllPerSimulationPassAndRunAndDetector(
                { simulationPassId, runNumber, detectorId: detectorId },
                { limit, offset },
                filter,
            );
            res.json(countedItemsToHttpView({ count, items }, limit));
...
const createQcFlagsHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.bodyOnly(Joi.object({
            ...
            detectorId: Joi.number().when('runDetectors', {
                is: Joi.exist(),
                then: Joi.forbidden().messages({
                    'any.unknown': 'detectorId is not allowed when runDetectors is provided.',
                }),
                otherwise: Joi.required(),
            }),
        ...
        try {
            ...
            for (const flag of flags) {
                const scope = {
                    ...
                    detectorIdentifier: { detectorId: flag.detectorId },
    ...
    if (validatedDTO) {
        try {
            ...
            const data = await qcFlagSummaryService.getSummary(
                { dataPassId, simulationPassId, lhcPeriodId, detectorIds: detectorIds },
