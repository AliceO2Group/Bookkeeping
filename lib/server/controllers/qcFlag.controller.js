...
const createQcFlagHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            ...
            const relations = { userIdentifier: { externalUserId: validatedDTO.session.externalId } };
...
// eslint-disable-next-line valid-jsdoc
/**
 * Get QC flags summary for data pass
 */
const getDataPassQcFlagsSummaryHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
    ...
    if (validatedDTO) {
        try {
            ...
            const data = await qcFlagService.getQcFlagsSummariesForDataPass(dataPassId);
            res.json({ data });
        } catch (error) {
            updateExpressResponseFromNativeError(res, error);
        }
...
// eslint-disable-next-line valid-jsdoc
/**
 * Get QC flags summary for simulation pass
 */
const getSimulationPassQcFlagsSummaryHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
            simulationPassId: Joi.number().required(),
        }),
        req,
        res,
    );
    if (validatedDTO) {
        try {
            ...
            const data = await qcFlagService.getQcFlagsSummariesForSimulationPass(simulationPassId);
            res.json({ data });
        } catch (error) {
            updateExpressResponseFromNativeError(res, error);
        }
...
exports.QcFlagController = {
    ...
    verifyQcFlagHandler,
    getDataPassQcFlagsSummaryHandler,
    getSimulationPassQcFlagsSummaryHandler,
};
