...
const createQcFlagsHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.bodyOnly(Joi.alternatives().try(
            Joi.object({
    ...
    if (validatedDTO) {
        try {
            const qcFlags = (Array.isArray(validatedDTO.body) ? validatedDTO.body : [validatedDTO.body]).map((flag) => ({
                from: flag.from,
                to: flag.to,
            ...
            const scope = {
                runNumber: qcFlags[0].runNumber,
                detectorIdentifier: { detectorId: qcFlags[0].dplDetectorId },
                dataPassIdentifier: qcFlags[0].dataPassId ? { id: qcFlags[0].dataPassId } : null,
                simulationPassIdentifier: qcFlags[0].simulationPassId ? { id: qcFlags[0].simulationPassId } : null,
            };

            // Prepare user relations from the session
            const relations = {
                user: {
            ...
            // Create QC flags using the service
            const createdFlags = await qcFlagService.create(qcFlags, scope, relations);

            // Return response with created flags
