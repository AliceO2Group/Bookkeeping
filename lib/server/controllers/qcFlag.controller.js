...
const createQcFlagsHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            const qcFlags = validatedDTO.body.map((flag) => ({
                from: flag.from,
                to: flag.to,
            ...
            const [{ runNumber, dplDetectorId, dataPassId, simulationPassId }] = validatedDTO.body;
            const scope = { runNumber,
