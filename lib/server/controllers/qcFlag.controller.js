...
const createQcFlagsHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.bodyOnly(Joi.object({
            ...
            runDetectors: Joi.object().pattern(Joi.number(), Joi.array().items(Joi.number().required()).required()).optional(),
        })
    ...
    if (validatedDTO) {
        try {
            ...
            const flags = runDetectors ? Object.entries(runDetectors).map(([runNumber, detectorIds]) =>
