...
const { dtos: { CreateTagDto, GetAllTagsDto, GetTagDto, GetNotificationDto } } = require('../../domain');
...
/**
 * Get notification details (emails and mattermost channels) by tag name
 ...
 * @returns {undefined}
 */
const getNotificationByTagName = async (request, response, next) => {
    const value = await dtoValidator(GetNotificationDto, request, response);
    if (!value) {
        return;
    }

    const tag = await new GetNotificationUseCase()
        .execute(value);
    ...
    if (tag === null) {
        response.status(404).json({
            errors: [
                {
                    status: '404',
                    title: `Tag with this name (${value.params.name}) could not be found`,
                },
            ],
        });
    } else {
