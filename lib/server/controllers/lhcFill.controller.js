 ...
 * @returns {undefined}
 */
const createLhcFill = async (request, response, next) => {
 ...
 * @returns {undefined}
 */
const listLhcFills = async (request, response, next) => {
 ...
 * @returns {undefined}
 */
const updateLhcFills = async (request, response, next) => {
 ...
 * @returns {undefined}
 */
const getLhcFillById = async (request, response, next) => {
    ...
    if (lhcFill === null) {
        response.status(404).json({
            errors: [
                {
                    status: '404',
                    title: `LhcFill with this id (${value.params.lhcFillId}) could not be found`,
                },
            ],
        });
    } else {
...
const listRuns = async (request, response, next) => {
    ...
    console.log('pingthing')
    const { result, error } = await new GetLhcFillRunsUseCase()
        .execute(value);
 ...
 * @returns {undefined}
 */
const getRunByRunNumber = async (request, response, next) => {
    const value = await dtoValidator(GetLhcFillRunDto, request, response);
    if (!value) {
        return;
    }

    const { result, error } = await new GetLhcFillRunUseCase()
        .execute(value);
