    ...
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
            filter: {
                ...
                pdpBeamTypes: Joi.array().items(Joi.string()),
            },
            page: PaginationDto,
            sort: DtoFactory.order(['id', 'name', 'avgCenterOfMassEnergy', 'year', 'pdpBeamTypes']),
        }),
        req,
        res,
    );
 ...
 */
const getLhcPeriodStatisticsByIdHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
...
exports.LhcPeriodStatisticsController = {
    ...
    getLhcPeriodStatisticsByIdHandler,
};
