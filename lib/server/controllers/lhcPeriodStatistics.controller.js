 ...
 * @returns {undefined}
 */
const listPeriodStatisticsHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
            ...
            page: PaginationDto,
            sort: DtoFactory.sorting(['id', 'name', 'avgEnergy']),
        }),
        req,
        res,
    );
...
/**
 * Get one LHC Period with given id
 * @param {Object} req express HTTP request object
 * @param {Object} res express HTTP response object
 * @returns {undefined}
 */
const getLhcPeriodByIdHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
