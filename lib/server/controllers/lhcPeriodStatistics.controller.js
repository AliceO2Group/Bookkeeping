...
const listPeriodStatisticsHandler = async (req, res, next) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
            filter: {
                ids: DtoFactory.createSingleValueOrArrayDtoOfType('number'),
                names: DtoFactory.createSingleValueOrArrayDtoOfType('string'),
            },
    ...
    if (validatedDTO) {
        try {
            const { filter, page, sort } = validatedDTO.query;
            const periodsData = await lhcPeriodStatisticsService.getAllForPhysicsRuns({
                filter,
                ...page,
                sort,
            });
            res.json(countedDataToHttpView(periodsData, page?.limit ?? ApiConfig.pagination.limit));
        } catch (error) {
