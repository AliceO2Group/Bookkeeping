...
const { getAllQualityControlFlagTypes } = require('../services/qualityControlFlag/getAllQualityControlFlagTypes.js');
...
const listQualityControlFlagsHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({
            ...
            page: PaginationDto,
            sort: DtoFactory.order(['id', 'from', 'to']),
        }),
        req,
        res,
    );
 ...
 */
const listQualityControlFlagTypesHandler = async (req, res) => {
    const validatedDTO = await dtoValidator(
        DtoFactory.queryOnly({}),
        req,
        res,
    ...
    if (validatedDTO) {
        try {
            const flagReasons = await getAllQualityControlFlagTypes();
...
exports.QualityControlFlagsController = {
    ...
    listQualityControlFlagTypesHandler,
};
