...
const { getFullDataPassNameUsingRunPeriod } = require('../../services/dataPasses/getFullDataPassNameUsingRunPeriod.js');
...
class GRPCQcFlagController {
    ...
    async CreateForDataPass({ runNumber, detectorName, passName, flags }) {
        const qcFlags = await this.qcFlagService.create(
            ...
            {
                runNumber,
                dplDetectorIdentifier: { dplDetectorName: detectorName },
                dataPassIdentifier: { name: await getFullDataPassNameUsingRunPeriod(passName, runNumber) },
            },
