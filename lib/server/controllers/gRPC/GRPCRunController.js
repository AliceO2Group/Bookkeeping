...
class GRPCRunController {
    ...
    async Update(runPatchRequest) {
        ...
        return this.runToGRPC(await this.runService.update(
            { runNumber },
            { runPatch: run, relations: { runTypeName: relations.runType?.name } },
        ));
    }
    ...
    async gRPCToRunAndRelations(gRPCRunRequest) {
        ...
        if (gRPCRunRequest.eorReasons) {
            ...
            //TODO: what to do with runId?

            console.log(relations.eorReasons);

            // TODO: conversion working, but not saving to db yet.

            /*
             * Logger.info('Getting here!');
             * logger.info(gRPCRunRequest.eorReasons);
             */
        }
...
const convertEorReasonstoReasonTypeId = async (eorReasons) => {
    ...
    const convertedEorReasons = await Promise.all(eorReasons.map(async ({ id, category, title, description }) => {
        ...
        return null; // Or handle the case when tempReasonType is not found
    }));

    return convertedEorReasons.filter(Boolean); // Remove null entries
};
