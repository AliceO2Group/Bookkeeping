...
class GRPCRunController {
    ...
    async Create(newRunRequest) {
        // Run can only be updated or created with a valid user
        if (!await this.userService.validateUserFromGRPCRunRequest(newRunRequest.user_o2_start, newRunRequest.user_o2_stop)) {
            return;
        }
    ...
    async Update(runPatchRequest) {
        // Run can only be updated with a valid user
        if (!await this.userService.validateUserFromGRPCRunRequest(runPatchRequest.user_o2_start, runPatchRequest.user_o2_stop)) {
            return;
        }
