...
const getShiftDataHandler = async (request, response) => {
    ...
    if (requestDto) {
        ...
        try {
            const shift = await shiftService.getUserPendingShiftOrFail(userIdentifier);

            const issuesLogs = await shiftService.getShiftIssues(shift.start, userIdentifier);
            response.status(200).json({ data: { issuesLogs } });
        } catch (error) {
