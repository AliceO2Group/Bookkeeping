...
const { DATA_TAKING_DETECTOR_TYPES } = require('../../domain/enums/DetectorTypes.js');
 ...
 * @return {void}
 */
exports.listAllDetectors = async (_request, response) => {
    try {
        const detectors = await detectorService.getAll();
        response.status(200).json({ data: detectors.filter(({ type }) => DATA_TAKING_DETECTOR_TYPES.includes(type)) });
    } catch (error) {
