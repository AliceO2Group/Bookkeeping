...
const { status: { GetGuiStatusUseCase, GetDatabaseStatusUseCase } } = require('../../usecases');
const { webUiServer } = require('../index.js');
...
const clientCountHandler = (request, response) => {
    const {webUiServer} = require('../index.js');
    console.log(webUiServer.ws.server.clients.size);
    response.status(200).json({ data: webUiServer.ws.server.clients.size });
};
