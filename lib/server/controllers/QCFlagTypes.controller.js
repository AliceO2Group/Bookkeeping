...
const createQCFlagTypeHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            const createFlag = await qcFlagTypesService.create({ ...validatedDTO.body, externalUserId: validatedDTO.session.externalId });
...
const updateQCFlagTypeHandler = async (req, res) => {
    ...
    if (validatedDTO) {
        try {
            const updatedFlag = await qcFlagTypesService.update(
                validatedDTO.params.id,
                { ...validatedDTO.body, externalUserId: validatedDTO.session.externalId },
            );
