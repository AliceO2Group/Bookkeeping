...
const getChildLogsByParentIdHandler = async (request, response) => {
    const requestDto = await dtoValidator(GetLogDto, request, response);

    if (requestDto) {
        try {
            const { params: { logId } } = requestDto;
            const { count, childLogs } = await logService.getChildLogsByParentId(logId);
            ...
            if (count > 0) {
                ...
                await response.status(404).json({
                    errors: [
                        {
                            status: '404',
                            title: `Child logs of log (${request.params.logId}) could not be found`,
                        },
                    ],
                });
            }
