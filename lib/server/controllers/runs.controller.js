...
const noRunWithIdentifierResponse = (response, { runId, runNumber }) => {
    response.status(404).json({
        errors: [
            {
                status: '404',
                title: `Run with this ${runNumber ? `runNumber (${runNumber})` : `id (${runId})`} could not be found`,
            },
        ],
    });
};
 ...
 * @returns {undefined}
 */
const getRunByIdentifier = async (request, response, next) => {
 ...
 * @returns {undefined}
 */
const getLogsByRunIdentifier = async (request, response, next) => {
    const value = await dtoValidator(GetRunDto, request, response);
    if (!value) {
        return;
    }
 ...
 * @returns {undefined}
 */
const getFlpsByRunIdentifier = async (request, response, next) => {
    const value = await dtoValidator(GetRunDto, request, response);
    if (!value) {
        return;
    }
...
module.exports = {
    endRun,
    getRunByIdentifier,
    getLogsByRunIdentifier,
    getFlpsByRunIdentifier,
    listReasonTypes,
    listRuns,
    startRun,
