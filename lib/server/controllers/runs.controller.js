...
const CreateRunDto = require('../../domain/dtos/CreateRunDto');
const CreateRunUseCase = require('../../usecases/run/CreateRunUseCase');
 ...
 * @returns {undefined}
 */
const createRun = async (request, response, next) => {
    const value = await dtoValidator(CreateRunDto, request, response);
    if (!value) {
        return;
    }

    const { result, error } = await new CreateRunUseCase()
        .execute(value);
