...
const { apiRunAdapter } = require('../../domain/adapters/ApiRunAdapter.js');
...
const listRuns = async (request, response) => {
    ...
    response.status(200).json({
        data: runs.map(apiRunAdapter.toApiEntity),
        meta: {
            page: {
...
const getRunById = async (request, response, next) => {
    ...
    if (run === null) {
        ...
        response.status(200).json({
            data: apiRunAdapter.toApiEntity(run),
        });
    }
};
...
const startRun = async (request, response) => {
    ...
    if (error) {
        ...
        response.status(201).json({ data: apiRunAdapter.toApiEntity(run) });
    }
};
...
const updateRunByRunNumber = async (request, response) => {
    ...
    if (error) {
        ...
        response.status(200).json({ data: apiRunAdapter.toApiEntity(run) });
    }
};
