    ...
    if (requestDto) {
        try {
            const data = await dplProcessService.getAllExecutedProcessesByRunAndDetector(
                ...
                { detectorId: requestDto.query.detectorId },
            );
            response.status(200).json({ data });
        } catch (error) {
            updateExpressResponseFromNativeError(response, error);
 ...
 */
const getAllHostsWithExecutedProcessByRunAndDetectorHandler = async (request, response) => {
    const requestDto = await dtoValidator(
        DtoFactory.queryOnly({
            runNumber: Joi.number().required(),
            detectorId: Joi.number().required(),
 ...
 */
const getAllProcessExecutionByRunAndDetectorAndHostHandler = async (request, response) => {
    const requestDto = await dtoValidator(
        DtoFactory.queryOnly({
            runNumber: Joi.number().required(),
            detectorId: Joi.number().required(),
...
exports.DplProcessController = {
    ...
    getAllHostsWithExecutedProcessByRunAndDetectorHandler,
    getAllProcessExecutionByRunAndDetectorAndHostHandler,
};
