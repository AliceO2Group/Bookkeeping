...
const getAllEnvironments = async (request, response, next) => {
    ...
    const { count, environments } = await new GetAllEnvironmentsUseCase()
        .execute(value);
 ...
 * @returns {undefined}
 */
const getEnvironmentById = async (request, response, next) => {
    const value = await dtoValidator(GetEnvironmentDto, request, response);
    if (!value) {
        return;
    }

    const { result, error } = await new GetEnvironmentUseCase()
        .execute(value);
