...
const { convertRunLhcPeriod } = require('../utilities/convertRunLhcPeriod.js');
...
const getAllEnvironments = async (request, response, next) => {
    ...
    environments.map((env) => env?.runs.map(convertRunLhcPeriod));
...
const getEnvironmentById = async (request, response) => {
    ...
    try {
        ...
        environment.runs?.map(convertRunLhcPeriod);
...
const getAllEnvironmentLogsHandler = async (request, response) => {
    ...
    if (requestDto) {
        try {
            ...
            data.forEach((l) => l.environments?.forEach((e) => e.runs?.forEach(convertRunLhcPeriod)));
