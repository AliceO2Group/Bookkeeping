 ...
 */
const getWeeklyRunDataSize = async (request, response) => {
    const requestDto = await dtoValidator(DtoFactory.queryOnly({
        from: Joi.date().required(),
        to: Joi.date().required(),
    }), request, response);
 ...
 */
const getTimeBetweenRunsDistribution = async (request, response) => {
    const requestDto = await dtoValidator(DtoFactory.queryOnly({
        from: Joi.date().required(),
        to: Joi.date().required(),
    }), request, response);
...
exports.StatisticsController = {
    ...
    getWeeklyRunDataSize,
    getTimeBetweenRunsDistribution,
};
