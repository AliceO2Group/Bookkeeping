...
const { longToBigInt, bigIntToLong, numberToLong } = require('../../../utilities/bigIntConverter.js');
 ...
 *     path will be `['a', 'b'] and the converter will store how to convert from this enum value to js and the other way around
 *
 * @property {string[]} path array containing the list of fields from the root message up to the field name
 ...
 * @param {object} typeDefinition.field the field to look for
 * @param {object} typeDefinition.name the name of the type
 * @param {Map<string, object>} absoluteMessageDefinitions the absolute messages definitions
            ...
            switch (type) {
                    ...
                    fieldConverters.push({
                        ...
                        fromJs: (value) => {
                            if (typeof value === 'bigint') {
                                return bigIntToLong(value, true);
                            }
                            if (typeof value === 'string') {
                                return Long.fromString(value, true, 10);
                            }
                            return value;
                        },
                    });
                ...
                case 'TYPE_INT64':
                    fieldConverters.push({
                        ...
                        fromJs: (value) => {
                            if (typeof value === 'bigint') {
                                return bigIntToLong(value, false);
                            }
                            if (typeof value === 'string') {
                                return Long.fromString(value, false, 10);
                            }
                            if (typeof value === 'number') {
                                return numberToLong(value, false);
                            }
                            return value;
                        },
                    });
                ...
                case 'TYPE_MESSAGE':
                    ...
                    extractInnerFieldsConverters(definition.field, fullPath, [...context, definition.name]);
                    break;
            }
        }
    };
