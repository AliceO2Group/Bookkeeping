...
const { longToBigInt, bigIntToLong, longToNumber, numberToLong } = require('../../../utilities/bigIntConverter.js');
 ...
 */
const getCloserMessageDefinition = (namespace, messageName, context, absoluteMessageDefinitions) => {
    // First, look for message directly
    if (absoluteMessageDefinitions.has(messageName)) {
        return absoluteMessageDefinitions.get(messageName);
    }
    ...
    for (let depth = context.length; depth >= 0; depth--) {
        ...
        const absoluteMessageName = [namespace, prefix, messageName].filter((item) => Boolean(item)).join('.');
 ...
 */
const extractFieldsConverters = (namespace, { field, name }, absoluteMessageDefinitions) => {
    /**
     * @type {FieldConverter[]}
     */
            ...
            switch (type) {
                ...
                case 'TYPE_INT64':
                    fieldConverters.push({
                        path,
                        name,
                        toJs: (value) => (value ?? null) !== null ? longToNumber(value) : value,
                        fromJs: (value) => typeof value === 'number'
                            ? numberToLong(value, false)
                            : typeof value === 'string' ? Long.fromString(value, false, 10) : value,
                    });
                    break;
                case 'TYPE_MESSAGE':
                    // eslint-disable-next-line no-case-declarations
                    const definition = getCloserMessageDefinition(namespace, typeName, context, absoluteMessageDefinitions);
                    ...
                    if (typeName === 'google.protobuf.Timestamp') {
                        fieldConverters.push({
                            ...
                            toJs: (value) => {
                                ...
                                return new Date(Math.round(1000 * (longToNumber(seconds) + nanos / 1e9)));
                            },
                            fromJs: (value) => {
                                if ((value ?? null) === null) {
                                    return null;
                                ...
                                return {
                                    seconds: numberToLong((time - milliseconds) / 1e3, false),
                                    nanos: milliseconds * 1e6,
                                };
                            },
                        });
                        continue;
