 ...
 */
const bindGRPCController = (namespace, serviceDefinition, implementation, absoluteMessagesDefinitions) => {
    const adapter = {};

    for (const [methodName, { requestType, responseType, path }] of Object.entries(serviceDefinition)) {
        const requestEnumsPaths = extractFieldsConverters(namespace, requestType.type, absoluteMessagesDefinitions);
        const responseEnumsPaths = extractFieldsConverters(namespace, responseType.type, absoluteMessagesDefinitions);
