...
const { QcFlagRepository,
    GaqSummaryRepository,
    GaqSummaryInvalidationRepository,
...
class GaqService {
    ...
    async getSummary(dataPassId, { mcReproducibleAsNotBad = false, runNumber } = {}) {
        ...
        const invalidatedSummaries = await GaqSummaryInvalidationRepository.findAll({
            where,
        });

        const combinedSummaries = [];
        for (const summary of summaries) {
            const invalidation = invalidatedSummaries.find((s) => s.runNumber === summary.runNumber);
            if (invalidation) {
                const formattedSummary = { ...this._formatSummary(summary.get(), mcReproducibleAsNotBad), invalidated: true };
                combinedSummaries.push([summary.runNumber, formattedSummary]);
            } else {
                const formattedSummary = { ...this._formatSummary(summary.get(), mcReproducibleAsNotBad), invalidated: false };
                combinedSummaries.push([summary.runNumber, formattedSummary]);
            }
        }

        /*
         * Add summaries which are invalidated but not yet recalculated
         * (e.g. no summary record in the database, but an invalidation record exists)
         */
        for (const invalidation of invalidatedSummaries) {
            if (!combinedSummaries.find(([runNumber]) => runNumber === invalidation.runNumber)) {
                const invalidSummary = {
                    invalidated: true,
                };
                combinedSummaries.push([invalidation.runNumber, invalidSummary]);
            }
        }

        return runNumber ? combinedSummaries.find(([rn]) => rn === runNumber)?.[1] ?? {} : Object.fromEntries(combinedSummaries);
    }
    ...
    _formatSummary(summary, mcReproducibleAsNotBad) {
        return {
            ...
            [QcSummarProperties.CALCULATION_FAILED]: summary.calculationFailed,
        };
    }
    ...
    async calculateAndStoreGaqSummary(dataPassId, runNumber) {
        ...
        await GaqSummaryRepository.upsert({ dataPassId, runNumber, ...summary, calculationFailed: summary === null });
    }
    ...
    async recalculateSummaries(dataPassId, runNumbers) {
        ...
        for (const summary of summariesToRecalculate) {
            await GaqSummaryInvalidationRepository.upsert({ dataPassId: summary.dataPassId, runNumber: summary.runNumber });
        }
