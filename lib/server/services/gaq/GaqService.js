...
class GaqService {
    ...
    async recalculateSummaries(dataPassId, runNumbers) {
        ...
        const now = new Date();
        await GaqSummaryRepository.insertAll(
            summariesToRecalculate.map(({ dataPassId, runNumber }) => ({ dataPassId, runNumber, invalidatedAt: now })),
            { updateOnDuplicate: ['invalidatedAt'] },
        );
