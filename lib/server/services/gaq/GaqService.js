...
class GaqService {
     ...
     * @return {Promise<Record<number, RunGaqSummary>|RunGaqSummary|null>} when runNumber is given, the run's summary or null if no row exists yet; otherwise a map of runNumber to summary
     */
    async getSummary(dataPassId, { mcReproducibleAsNotBad = false, runNumber } = {}) {
        ...
        if (runNumber !== undefined) {
            return formattedSummaries.find((s) => s.runNumber === runNumber)?.summary ?? null;
        }
