...
class GaqService {
    ...
    async popNInvalidSummaryAndRecalculate(batchSize = 1) {
        ...
        if (rows.length > 0) {
            this._logger.infoMessage(`Processing ${rows.length} out of ${count} invalidated GAQ summaries (batch size: ${batchSize})`);
        }

        await Promise.all(rows.map(({ dataPassId, runNumber }) =>
            this.calculateAndStoreGaqSummary(dataPassId, runNumber)));
    }
