...
class GaqService {
    ...
    async getSummary(dataPassId, { mcReproducibleAsNotBad = false, runNumber } = {}) {
        ...
        const summaries = await GaqSummaryRepository.findAll({
            ...
            attributes: {
                include: [
                    [
                        Sequelize.literal(
                            'EXISTS (SELECT 1 FROM `gaq_summary_invalidations` AS `gsi`'
                            + ' WHERE `gsi`.`data_pass_id` = `GaqSummary`.`data_pass_id`'
                            + ' AND `gsi`.`run_number` = `GaqSummary`.`run_number`)',
                        ),
                        'invalidated',
                    ],
                ],
        ...
        const summaryByRun = Object.fromEntries(summaries.map((s) => {
            const formatted = this._formatSummary(s, mcReproducibleAsNotBad);
            if (s.get('invalidated')) {
                formatted.invalidated = true;
            }
            return [s.runNumber, formatted];
        }));
    ...
    _formatSummary(summary, mcReproducibleAsNotBad) {
        return {
            [QcSummarProperties.BAD_EFFECTIVE_RUN_COVERAGE]:
                summary.badEffectiveRunCoverage + (mcReproducibleAsNotBad ? 0 : summary.mcReproducibleCoverage),
            [QcSummarProperties.EXPLICITELY_NOT_BAD_EFFECTIVE_RUN_COVERAGE]:
                summary.explicitlyNotBadEffectiveRunCoverage + (mcReproducibleAsNotBad ? summary.mcReproducibleCoverage : 0),
    ...
    async calculateAndStoreGaqSummary(dataPassId, runNumber) {
        ...
        await GaqSummaryRepository.upsert({ dataPassId, runNumber, ...summary, updatedAt: new Date() });
    }
