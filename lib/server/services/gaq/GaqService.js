 ...
 * @property {number} undefinedQualityPeriodsCount - number of GAQ periods with no contributing QC flag
 */
...
class GaqService {
     ...
     * @return {Promise<Record<number, RunGaqSummary>|RunGaqSummary|null>} when runNumber is given, the run's summary or
     * null if no row exists; otherwise a map of runNumber to summary. Rows with no coverage values yet (notComputable,
     * or invalidated without ever having been computed) are returned with all coverage fields set to null.
     */
    async getSummary(dataPassId, { mcReproducibleAsNotBad = false, runNumber } = {}) {
        ...
        if (runNumber !== undefined) {
            ...
            const row = await GaqSummaryRepository.findOne({ where });
            return row ? this._formatSummary(row, mcReproducibleAsNotBad) : null;
        }

        const rows = await GaqSummaryRepository.findAll({ where });
        return Object.fromEntries(rows.map((row) => [row.runNumber, this._formatSummary(row, mcReproducibleAsNotBad)]));
    }

    /**
     * Format a raw GAQ summary record into its API representation. Rows without coverage values (notComputable, or
     * invalidated before any compute) return null coverage fields so callers can distinguish "no data" from real 0%.
    ...
    _formatSummary(summary, mcReproducibleAsNotBad) {
        if (summary.badRunCoverage === null) {
            return {
                [QcSummarProperties.BAD_EFFECTIVE_RUN_COVERAGE]: null,
                [QcSummarProperties.EXPLICITELY_NOT_BAD_EFFECTIVE_RUN_COVERAGE]: null,
                [QcSummarProperties.MC_REPRODUCIBLE]: false,
                [QcSummarProperties.MISSING_VERIFICATIONS]: null,
                [QcSummarProperties.UNDEFINED_QUALITY_PERIODS_COUNT]: null,
            };
        }
        return {
    ...
    async calculateAndStoreGaqSummary(dataPassId, runNumber, { expectedInvalidatedAt } = {}) {
        ...
        console.log('[GAQ DEBUG]', { dataPassId, runNumber, expectedInvalidatedAt: expectedInvalidatedAt?.toISOString(), rows });
    ...
    async popNInvalidSummaryAndRecalculate(batchSize = 1) {
        ...
        console.log('[GAQ POP]', rows.map((r) => ({ dataPassId: r.dataPassId, runNumber: r.runNumber, invalidatedAt: r.invalidatedAt?.toISOString() })));
