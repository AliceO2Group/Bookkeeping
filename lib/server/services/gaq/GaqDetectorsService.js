...
const { GaqDetectorRepository, RunRepository, DetectorRepository, GaqSummaryInvalidationRepository } = require('../../../database/repositories');
...
class GaqDetectorService {
    ...
    setGaqDetectors(dataPassId, runNumbers, detectorIds) {
        return dataSource.transaction(async () => {
            ...
            // Invalidate GAQ summaries for all affected runs
            await Promise.all(runNumbers.map((runNumber) => GaqSummaryInvalidationRepository.upsert({
                dataPassId,
                runNumber,
            })));
