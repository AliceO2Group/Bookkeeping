...
const { GaqDetectorRepository, RunRepository, DetectorRepository, GaqSummaryRepository } = require('../../../database/repositories');
...
class GaqDetectorService {
    ...
    setGaqDetectors(dataPassId, runNumbers, detectorIds) {
        return dataSource.transaction(async () => {
            ...
            await Promise.all(runNumbers.map((runNumber) => GaqSummaryRepository.upsert({
                dataPassId,
                runNumber,
