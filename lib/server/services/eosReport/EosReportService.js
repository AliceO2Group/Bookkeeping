...
class EosReportService {
    ...
    async createLogEntry(reportType, reportCreationRequest, shifterUserIdentifier) {
         ...
         */
        const issuesLogEntries = await getLogsByTagsInPeriod(this._issuesLogEntriesTags, shift.start, shift.end);
        ...
        const report = await formatEosReport({
            ...
            issues: reportCreationRequest.issues,
            issuesLogEntries,
        ...
        return logService.create(
            {
                title: report.substring(0, report.indexOf('\n')),
                text: report,
                subtype: 'comment',
                origin: 'process',
            },
