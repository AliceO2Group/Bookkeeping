...
const { getTagsByReportType } = require('./eosTypeSpecificFormatter/getTypeSpecificTags.js');
    ...
    async createLogEntry(reportType, reportCreationRequest, shifterUserIdentifier) {
        ...
        switch (reportType) {
            case ShiftTypes.ECS:
                ...
                typeSpecificTags.push(...getTagsByReportType(reportType));
                break;
        }
        ...
        return logService.create(
            ...
            (await getTagsByText([...typeSpecificTags])).map(({ text }) => text),
        );
    }
}
