    ...
    async createLogEntry(reportType, reportCreationRequest, currentUserIdentifier) {
        ...
        switch (reportType) {
            case ShiftTypes.ECS:
                ...
                logEnvironmentIds = eosReport.typeSpecific.environments.map(({ id }) => id);
                break;
            case ShiftTypes.QC_PDP:
                ...
                logEnvironmentIds = Object.values(eosReport.typeSpecific.runs).map((runs) => runs
                    .map(({ envId }) => envId))
                    .flat()
                    .filter((id) => id);
                break;
