...
const formatEnvironment = async (environment, environmentComments, runComments) => {
    ...
    let ret = `- (${formatShiftDate(environment.createdAt)}) [${environment.id}](${envUrl})`;
    if (environment.runs.length > 0) {
        ret += `\n${(await Promise.all(environment.runs.map(async (run) => await formatRun(
            run,
...
const formatRun = async (run, comment) => {
    ...
    let ret = `    * (${start ? formatShiftDate(start) : '-'})`
