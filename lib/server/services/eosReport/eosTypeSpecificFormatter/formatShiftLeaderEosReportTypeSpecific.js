 ...
 *
 * @param {number} timestamp the time of the magnet configuration
 ...
 */
const formatMagnetsConfiguration = (timestamp, dipole, solenoid) => {
    const time = formatShiftDate(timestamp, { time: true });
    return `- ${time} - Dipole ${dipole} - Solenoid ${solenoid}`;
};
...
const formatMagnetsSection = ({ start, end }, magnets) => {
    ...
    sortableMagnets.sort(({ timestamp: a }, { timestamp: b }) => a - b);

    const intermediates = sortableMagnets.map(({
        timestamp,
        magnetsConfiguration: { dipole, solenoid },
    }) => formatMagnetsConfiguration(timestamp, dipole, solenoid));

    return [
        formatMagnetsConfiguration(start, magnets.start.dipole, magnets.start.solenoid),
        ...intermediates,
        formatMagnetsConfiguration(end, magnets.end.dipole, magnets.end.solenoid),
    ].join('\n');
};
 ...
 */
const formatRun = async (run) => `- [${run.runNumber}](${frontUrl({ page: 'run-detail', runNumber: run.runNumber })})`;
