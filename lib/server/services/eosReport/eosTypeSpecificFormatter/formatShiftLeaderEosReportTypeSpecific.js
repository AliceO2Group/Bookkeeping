 ...
 */
const formatMagnetsConfiguration = (timestamp, solenoid, dipole) => {
    const time = formatShiftDate(timestamp, { time: true });
    return `- ${time} - Solenoid ${solenoid} - Dipole ${dipole}`;
};
...
const formatMagnetsSection = ({ start, end }, magnets) => {
    ...
    }) => formatMagnetsConfiguration(timestamp, solenoid, dipole));
