...
exports.formatEosReport = async (eosReport) => {
    ...
    const formattedIssuesLogs = eosReport.issuesLogEntries.length > 0
        ? eosReport.issuesLogEntries.map((log) => {
            ...
            return `- \\[${formattedTags}\\] - [${log.title}](${'#'})`;
        }).join('\n')
        : '-';
    ...
    // For now, do not handle specific report part
    const specificReportPart = null;
 ...
 * @param {string} issues the issues summary provided by the shifter
 * @param {string} formattedIssuesLogs the formatted logs list considered as related to issues
 * @param {string} shiftFlow the shift flow provided by the shifter (as markdown)
 * @return {string} the start of EOS report
 */
const formatCommonEOSReportStart = (title, shifterName, shifterTraineeName, issues, formattedIssuesLogs, shiftFlow) => `\
# ${title}
- shifter:  ${shifterName}
