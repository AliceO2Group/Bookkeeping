...
exports.getOrCreateAllPhysicalDetectorsByName = async (detectorNames) => {
    ...
    const nonPhyscial = detectors.filter(({ isPhysical }) => !isPhysical);

    if (nonPhyscial.length === 1) {
        throw new Error(`Detector ${nonPhyscial[0].name} exists, but it is not physical`);
    } else if (nonPhyscial.length > 1) {
        throw new Error(`Detectors ${nonPhyscial.map(({ name }) => name)} exist, but they are not physical`);
    }
    ...
    for (const detectorName of missingDetectors) {
        const id = await createDetector({ name: detectorName });
