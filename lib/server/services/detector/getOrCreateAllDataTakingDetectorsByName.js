...
exports.getOrCreateAllDataTakingDetectorsByName = async (detectorNames) => {
    ...
    const dataTakingDetectors = detectors.filter(({ type }) => DATA_TAKING_DETECTOR_TYPES.includes(type));
    const nonDataTakingDetectors = detectors.filter(({ type }) => !DATA_TAKING_DETECTOR_TYPES.includes(type));
