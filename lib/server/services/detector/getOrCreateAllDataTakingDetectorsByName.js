...
exports.getOrCreateAllDataTakingDetectorsByName = async (detectorNames) => {
    ...
    const dataTakingDetectors = detectors.filter(({ type }) => type === [DetectorType.PHYSICAL, DetectorType.VIRTUAL].includes(type));
    const nonDataTakingDetectors = detectors.filter(({ type }) => type !== ![DetectorType.PHYSICAL, DetectorType.VIRTUAL].includes(type));
