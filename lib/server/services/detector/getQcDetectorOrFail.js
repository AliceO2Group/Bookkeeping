 ...
 *
 * @param {DetectorIdentifier} identifier the identifier of the DPL detector to fetch
 * @return {Promise<Detector>} the found physical detector
 */
exports.getQcDetectorOrFail = async ({ detectorId, detectorName }) => {
    ...
    const detector = await DetectorRepository.findOne({
        where: {
            [Op.and]: [
                criteria,
                { type: { [Op.in]: [DetectorType.PHYSICAL, DetectorType.QC] } },
            ],
        },
    });
