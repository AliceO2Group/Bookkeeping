...
const { lhcFillStatisticsAdapter } = require('../../../database/adapters');
...
class LhcPeriodService {
    ...
    async getById({ id, name }) {
        return lhcFillStatisticsAdapter.toEntity(await LhcPeriodStatisticsRepository.findOne({ id, name }));
    }

    /**
     * Get all lhc periods with statistics
     * @param {Object} dto definition of request
    ...
    async getAllForPhysicsRuns(dto = {}) {
        ...
        return {
            count,
            rows: rows.map(lhcFillStatisticsAdapter.toEntity),
        };
    }
}
