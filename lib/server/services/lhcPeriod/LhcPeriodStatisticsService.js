...
const DISTINCT_ENERGIES_IN_STATISTICS_ATTRIBUTE = {
    ...
    /**
     * Prepare SQL statement for filtering by distinct energies.
     * It is used to put constrain to retrive only records which have list of distinctEnergies intersecting
     ...
     * @return {string} sql statement to be used in WHERE clause - use :distinctEnergies replacements
     */
    prepareFilterQuery_intersectionOfDistinctEnergies: (precision = 2) => `
    EXISTS (
        SELECT *
        FROM runs AS r
...
class LhcPeriodStatisticsService {
    ...
    async getAllForPhysicsRuns({
        ...
        if (filter) {
            ...
            if (distinctEnergies) {
                queryBuilder.literalWhere(
                    DISTINCT_ENERGIES_IN_STATISTICS_ATTRIBUTE.prepareFilterQuery_intersectionOfDistinctEnergies(),
                    { distinctEnergies },
                );
            }
        }
