...
class LhcPeriodStatisticsService {
    ...
    prepareQueryBuilder() {
        return dataSource.createQueryBuilder()
            ...
            .literalWhere('(`run`.`run_quality` AND `run`.`definition` = :definition) OR (`run`.`run_quality` is NULL)', { definition: 'PHYSICS', quailty: 'good' })

            // .literalWhere('(`run`.`run_quality` = :quailty OR `run`.`run_quality` IS NULL) AND (`run`.`definition` = :definition OR `run`.`definition` IS NULL)', { definition: 'PHYSICS', quailty: 'good' })

            // .literalWhere('(`run`.`run_quality` = :quailty OR `run`.`run_quality` IS NULL)', { quailty: 'good' })
            // .literalWhere('(`run`.`definition` = :definition OR `run`.`definition` IS NULL)', { definition: 'PHYSICS' })
