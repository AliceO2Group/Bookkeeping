...
class LhcPeriodStatisticsService {
    ...
    async getByIdentifier({ id, name }) {
        ...
        queryBuilder.include({ association: 'lhcPeriodStatistics' });

        const lhcPeriod = await LhcPeriodRepository.findOne(queryBuilder);
        return lhcPeriod ? lhcPeriodAdapter.toEntity(lhcPeriod) : null;
    }
    ...
    async getAllForPhysicsRuns({ filter, page = {}, sort = { name: 'desc' } } = {}) {
        ...
        if (filter) {
            ...
            if (names) {
                queryBuilder.where('name').oneOf(...Array.isArray(names) ? names : [names]);
            }
        }
        queryBuilder.include({ association: 'lhcPeriodStatistics' });
