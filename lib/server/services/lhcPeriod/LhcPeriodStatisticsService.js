...
const { repositories: { LhcPeriodRepository, LhcPeriodStatisticsRepository } } = require('../../../database');
...
class LhcPeriodStatisticsService {
    ...
    async getByIdentifier({ id, name }) {
        ...
        queryBuilder.include({ association: 'lhcPeriods' });
    ...
    async getAllForPhysicsRuns({ filter, page = {}, sort = { name: 'desc' } } = {}) {
        ...
        if (filter) {
            ...
            if (names) {
                queryBuilder.whereAssociation('lhcPeriods', 'name').oneOf(...Array.isArray(names) ? names : [names]);
            }
        }
        queryBuilder.include({ association: 'lhcPeriods' });
