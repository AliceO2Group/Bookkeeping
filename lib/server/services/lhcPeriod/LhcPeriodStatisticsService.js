...
const PDP_BEAM_TYPES_IN_STATISTICS_ATTRIBUTE = {
    query: `(SELECT r.pdp_beam_type
        FROM runs AS r
        WHERE r.lhc_period_id = lhcPeriod.id 
            AND r.definition = 'PHYSICS'
...
class LhcPeriodStatisticsService {
    ...
    async getByIdentifier({ id, name }) {
        ...
        queryBuilder.includeAttribute(PDP_BEAM_TYPES_IN_STATISTICS_ATTRIBUTE);
    ...
    async getAllForPhysicsRuns({
        ...
        if (sort) {
            for (const property in sort) {
                ...
                } else if (property === 'beamType') {
                    expression = (sequelize) => sequelize.literal(PDP_BEAM_TYPES_IN_STATISTICS_ATTRIBUTE.query);
                }
        ...
        if (filter) {
            ...
            if (beamTypes) {
                queryBuilder.literalWhere(
                    `${PDP_BEAM_TYPES_IN_STATISTICS_ATTRIBUTE.query} IN (:beamTypes)`,
                    { beamTypes },
                );
            }
        }
