...
class SimulationPassService {
    ...
    async getAll({
        ...
        return {
            count: count.length, // With group cluase, findAndCountAll method returns list of counts per each grouping
    ...
    prepareQueryBuilder() {
        return dataSource.createQueryBuilder()
            ...
            .include((sequelize) => ({
                association: 'dataPasses',
                through: { attributes: [] },
                attributes: [[sequelize.fn('count', sequelize.fn('distinct', sequelize.col('dataPasses.id'))), 'count']],
            }))
            .groupBy('id');
    }
}
