...
class DataPassService {
    ...
    async getAll({
        ...
        const byName = filter?.include?.byName?.split(',').filter(({ length }) => length > 0) ?? [];
        if (!byName.includes(NonPhysicsProductionsNamesWords.TEST)) {
            queryBuilder.where('name').not().substring(NonPhysicsProductionsNamesWords.TEST);
        }
        if (!byName.includes(NonPhysicsProductionsNamesWords.DEBUG)) {
            queryBuilder.where('name').not().substring(NonPhysicsProductionsNamesWords.DEBUG);
        }
