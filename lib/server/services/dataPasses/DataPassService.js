...
class DataPassService {
    ...
    async getAll({
         ...
         */
        const { ids, names, includeByName = [], lhcPeriodIds, simulationPassIds } = filter;
        ...
        if (!includeByName.includes(NonPhysicsProductionsNamesWords.TEST)) {
            queryBuilder.where('name').not().substring(`\\_${NonPhysicsProductionsNamesWords.TEST}`);
        }

        if (!includeByName.includes(NonPhysicsProductionsNamesWords.DEBUG)) {
