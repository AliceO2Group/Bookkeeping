...
class DataPassService {
    ...
    async getSkimmableRuns(identifier) {
        ...
        if (dataPass.skimmingStage !== SkimmingStage.SKIMMABLE) {
            throw new BadParameterError(`DataPass ${dataPass.name} is not marked as skimmable, but (${dataPass.skimmingStage})`);
        }
     ...
     */
    async updateReadyForSkimmingRuns(identifier, runsList) {
        return dataSource.transaction(async () => {
            const dataPass = await this.getOneOrFail(identifier);
            if (dataPass.skimmingStage !== SkimmingStage.SKIMMABLE) {
            ...
            for (const { runNumber, readyForSkimming } of runsList) {
                await DataPassRepository.updateAll({ readyForSkimming }, { where: { runNumber, dataPassId: dataPass.id } });
            }
        });
    }
