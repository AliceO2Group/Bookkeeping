...
class DataPassService {
     ...
     * @param {boolean} frozen if true, data pass will be frozen, else it will be unfrozen (independently of the current state)
     * @return {Promise<void>} resolves once the data pass freeze state has been updated
     */
    async setFrozenState(dataPassIdentifier, frozen) {
        return dataSource.transaction(async () => {
            ...
            await DataPassRepository.update(dataPass, { frozen });
        });
    }
