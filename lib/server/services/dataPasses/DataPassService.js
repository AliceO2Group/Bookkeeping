...
class DataPassService {
    ...
    setGaqDetectors(dataPassId, runNumbers, dplDetectorIds) {
        return dataSource.transaction(async () => {
            ...
            await this._verifyRunsAndDplDetectorsAssociations(runNumbers, dplDetectorIds);
     ...
     * @throws {BadParameterError} if some association does not exist
     */
    async _verifyRunsAndDplDetectorsAssociations(runNumbers, dplDetectorIds) {
    ...
    async getAll({
        ...
        if (filter) {
            /**
             * @param {number[]} [filter.lhcPeriodIds] lhcPeriod identifier to filter with
             * @param {number[]} [filter.simulationPassIds] simulationPass identifier to filter with
             * @param {number[]} [filter.ids] data passes identifier to filter with
             * @param {string[]} [filter.names] data passes names to filter with
             */
