...
class DataPassService {
    ...
    async getAll({
        ...
        if (sort) {
            const { outputSize: outputSizeOrder, reconstructedEventsCount: reconstructedEventsCountOrder, ...otherSortProperties } = sort;
            if (outputSizeOrder) {
                queryBuilder.orderBy((sequelize) => sequelize.literal('sum(versions.output_size)'), outputSizeOrder);
            }
            if (reconstructedEventsCountOrder) {
                queryBuilder.orderBy((sequelize) =>
                    sequelize.literal('sum(versions.reconstructed_events_count)'), reconstructedEventsCountOrder);
            }
