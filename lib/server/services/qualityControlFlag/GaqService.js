...
class GaqService {
    ...
    async getSummary(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        const gaqSummary = Object.entries(gaqCoverages).map(([
            ...
            runNumber,
            {
                [SUMMARY_PROPERTIES.badEffectiveRunCoverage]: badCoverage + (mcReproducibleAsNotBad ? 0 : mcReproducibleCoverage),
                [SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage]: goodCoverage + (mcReproducibleAsNotBad ? mcReproducibleCoverage : 0),
                [SUMMARY_PROPERTIES.mcReproducible]: mcReproducibleCoverage > 0,
                [SUMMARY_PROPERTIES.missingVerificationsCount]: flagsIds.length - verifiedFlagsIds.length,
            },
        ]);
