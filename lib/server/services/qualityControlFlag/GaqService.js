 ...
 * @property {number} badCoverage - fraction of run's data, which aggregated quality is bad
 * @property {number} mcReproducibleCoverage - fraction of run's data, which aggregated quality is MC reproducible
 * @property {number} goodCoverage - fraction of run's data, which aggregated quality is good
 * @property {number} totalCoverage - fraction of run's data covered by flags
 */
...
class GaqService {
     ...
     * Get GAQ summary
     *
     * @param {number} dataPassId id of data pass for which GAQ needs to be computed
     * @return {Promise<Map<number, RunGaqSummary>>} Resolves with the GAQ Summary
     */
    async getSummary(dataPassId) {
        return QcFlagRepository.getGaqCoverages(dataPassId);
    }
