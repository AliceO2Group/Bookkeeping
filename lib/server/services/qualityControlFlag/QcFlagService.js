...
class QcFlagService {
     ...
     * @param {QcFlag} flagToBePartiallyDiscarded flag to be updated
     * @return {Promise<void>} promise
     */
    async _updateOneFlagEffectiveness(createdFlag, flagToBePartiallyDiscarded) {
        ...
        const ineffectivenesPeriodsOfFlagToBePartiallyDiscarded = await QcFlagIneffectivenessPeriodRepository.findAll({
            where: {
        ...
        await QcFlagIneffectivenessPeriodRepository.removeAll({ where: { id: { [Op.in]: overlappingPeriodIds } } });
    }

    /**
     * Make existing flags ineffective for certain period dependent on newly created one
     * @param {QcFlag} createdFlag newly created flag
     * @param {QcFlag[]} flagsToBePartiallyDiscarded flags to be updated
     * @return {Promise<void>} promise
     */
    async _updateExistingFlagsEffectiveness(createdFlag, flagsToBePartiallyDiscarded) {
        return Promise.all(flagsToBePartiallyDiscarded
            .map((flagToBePartiallyDiscarded) => this._updateOneFlagEffectiveness(createdFlag, flagToBePartiallyDiscarded)));
    }
    ...
    async createForDataPass(parameters, relations = {}) {
        ...
        return dataSource.transaction(async () => {
            ...
            const adaptedFlagsToBePartiallyDiscarded = flagsToBePartiallyDiscarded.map(qcFlagAdapter.toEntity);
            await this._updateExistingFlagsEffectiveness(
                adaptedCreatedQcFlag,
                adaptedFlagsToBePartiallyDiscarded,
            );
    ...
    async delete(id, relations) {
        return dataSource.transaction(async () => {
            ...
            await QcFlagIneffectivenessPeriodRepository.removeAll({ where: { flagId: qcFlag.id } });
            ...
            for (const olderFlags of flagsCreatedBeforeRemovedFlag) {
                const periodsToBeDeleted = await QcFlagIneffectivenessPeriodRepository.findAll({ where: { flagId: olderFlags.id } });
                for (const p of periodsToBeDeleted) {
                    await QcFlagIneffectivenessPeriodRepository.removeOne({ where: { id: p.id } });
                }
            }
            for (const newerFlag of flagsCreatedAfterRemovedFlag) {
                await this._updateExistingFlagsEffectiveness(newerFlag, flagsCreatedBeforeRemovedFlag);
            }
        ...
        const queryBuilder = dataSource.createQueryBuilder()
            ...
            .include({
                ...
                include: [
                    ...
                    { association: 'ineffectivenessPeriods' },
                ],
                where: {
                    runNumber,
                    dplDetectorId,
    ...
    prepareQueryBuilder() {
        return dataSource.createQueryBuilder()
            ...
            .include({ association: 'ineffectivenessPeriods' })
            .groupBy('id');
    }
}
