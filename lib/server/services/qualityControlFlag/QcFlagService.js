    ...
    setAgqDetectors({ dataPassId, runNumbers, dplDetectorIds }) {
        return dataSource.transaction(async () => {
            ...
            const validRunNumbers = await RunRepository.findAll({
                ...
                include: [{ association: 'dataPass', where: { id: dataPassId } }],
            });
            if (validRunNumbers.length !== runNumbers.length) {
                ...
                if (invalidRunNumbers.length === 0) {
                    ...
                    const errorMessage = `Following runs are not assoicated with data pass with id (${dataPassId}): ${invalidRunNumbers}`;
