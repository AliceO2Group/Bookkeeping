...
class QcFlagService {
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            for (const i in flagsCreatedBeforeRemovedFlag) {
                const flagToBeUpdated = flagsCreatedBeforeRemovedFlag[i];
                const otherNewerFlagsOfOlder = flagsCreatedBeforeRemovedFlag.slice(Number(i) + 1);
