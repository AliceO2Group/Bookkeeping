...
class QcFlagService {
    ...
    async getQcFlagsSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runSummaries of Object.values(summary)) {
            for (const runDetectorSummary of Object.values(runSummaries)) {
                this._fillSummaryMissingValues(runDetectorSummary);
            }
        }
    ...
    async getGaqSummary(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runSummary of Object.values(summary)) {
            this._fillSummaryMissingValues(runSummary);
        }
    ...
    /**
     * Set default summary properties which were not set when merging them, so each expected property has no 'undefined' value
     *
     * @param {RunDetectorQcSummary|RunGaqSummary} summaryUnit RunDetectorQcSummary or RunGaqSummary
     * @return {void}
     */
    _fillSummaryMissingValues(summaryUnit) {
