    ...
    async getGaqQcFlags(dataPassId, runNumber) {
        ...
        const alterationPoints = effectivePeriods
            .flatMap(({ from, to, flag }) => [
                { timestamp: from.getTime(), type: PERIOD_START, flag },
                { timestamp: to.getTime(), type: PERIOD_END, flag },
            ])
            .sort((
                { timestamp: timestamp1, type: type1 },
                { timestamp: timestamp2, type: type2 },
            ) => timestamp1 - timestamp2 + (type1 - type2));

        const currentPeriod = { from: alterationPoints[0].timestamp, to: null };
        const gaqPeriods = [];
        let contributingFlagsStack = [];
        let index = 0;

        while (index < alterationPoints.length) {
            while (alterationPoints[index].type === PERIOD_START && alterationPoints[index].timestamp === currentPeriod.from) {
                contributingFlagsStack.push(alterationPoints[index].flag);
                index++;
            }
            ...
            gaqPeriods.push({
                ...
                to: currentPeriod.to,
                contributingFlags: contributingFlagsStack.map(qcFlagAdapter.toEntity),
            });

            while (alterationPoints[index]?.type === PERIOD_END && alterationPoints[index].timestamp === currentPeriod.to) {
                alterationPoints[index].flag.dropped = true;
                index++;
            }
            contributingFlagsStack = contributingFlagsStack.filter(({ dropped }) => !dropped);
