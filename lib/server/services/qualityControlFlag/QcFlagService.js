...
class QcFlagService {
     ...
     * @param {detectorIdentifiers} scope.detectorIdentifiers associated dplDetector's identifier
    ...
    async create(qcFlags, scope, relations) {
        ...
        return dataSource.transaction(async () => {
            ...
            const detectors = await Promise.all(detectorIdentifiers.map((detectorIdentifier) => getQcDetectorOrFail(detectorIdentifier)));
            ...
            detectors.forEach((detector) => validateUserDetectorAccess(userRoles, detector.name));

            const createdFlags = [];

            for (const qcFlag of qcFlags) {
                const detector = detectors.find((d) => d.id === qcFlag.dplDetectorId);

                if (!detector) {
                    throw new NotFoundError(`Detector with ID ${qcFlag.dplDetectorId} not found`);
                }
                ...
                const effectivePeriodsToBeUpdated = await QcFlagEffectivePeriodRepository.findOverlappingPeriodsCreatedNotAfter(
                    ...
                    { dataPassId: dataPass?.id, simulationPassId: simulationPass?.id, runNumber: qcFlag.runNumber, detectorId: detector.id },
                );
