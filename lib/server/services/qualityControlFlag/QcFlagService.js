...
class QcFlagService {
            ...
            for (const qcFlag of qcFlags) {
                ...
                try {
                    ...
                    if (dataPass) {
                        ...
                        await GaqSummaryInvalidationRepository.upsert({
                            dataPassId: dataPassIdentifier,
                            runNumber,
                        });
                    }
    ...
    async deleteAllForDataPass(dataPassId) {
        return dataSource.transaction(async () => {
            ...
            qcFlagIdsToRunNumbers.map(({ runNumber }) => runNumbers.add(runNumber));
            ...
            await Promise.all(Array.from(runNumbers).map((runNumber) => GaqSummaryInvalidationRepository.upsert({
                dataPassId,
                runNumber,
            })));
    ...
    async verifyFlag({ flagId, comment }, relations, { transaction } = {}) {
        return dataSource.transaction(async () => {
            ...
            if (dataPassId && (!qcFlag.verifications || qcFlag.verifications.length === 0)) {
                await GaqSummaryInvalidationRepository.upsert({
                    dataPassId,
                    runNumber: updatedQcFlag.runNumber,
                });
            }
