...
class QcFlagService {
        ...
        return dataSource.transaction(async (transaction) => {
            ...
            for (const qcFlag of qcFlags) {
                ...
                try {
                    ...
                    /**
                     * Invalidate GAQ summary for the dataPass and runNumber of the created flag.
                     * Skip when `verify` is true: verifyFlag() above already invalidated the same (dataPassId, runNumber).
                     */
                    if (dataPass && !verify) {
                        await GaqSummaryRepository.invalidate(dataPass.id, runNumber);
                    }
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
...

            {
                ...
                if (dataPassId) {
                    // Invalidate GAQ summary for the dataPass and runNumber of the deleted flag
    ...
    async deleteAllForDataPass(dataPassId) {
        return dataSource.transaction(async () => {
            ...
            const qcFlagIdsToRunNumbers = (await QcFlagRepository.findAll({
                attributes: ['id', 'runNumber'],
                where: { deleted: false },
                include: {
                    association: 'dataPasses',
                    attributes: [],
                ...
                raw: true,
            })).map(({ id, runNumber }) => ({ id, runNumber }));

            const qcFlagIds = qcFlagIdsToRunNumbers.map(({ id }) => id);
            const runNumbers = new Set(qcFlagIdsToRunNumbers.map(({ runNumber }) => runNumber));
            ...
            // Invalidate GAQ summary for the dataPass and all runNumbers of the deleted flags
            await GaqSummaryRepository.invalidateMany(Array.from(runNumbers, (runNumber) => ({ dataPassId, runNumber })));
    ...
    async verifyFlag({ flagId, comment }, relations, { transaction } = {}) {
        return dataSource.transaction(async () => {
            ...
            const updatedQcFlag = await this.getOneOrFail(flagId);

            // Get data pass id for the flag (if exists) to invalidate only related GAQ summaries
            const dataPassQcFlag = await DataPassQcFlagRepository.findOne({ where: { qualityControlFlagId: flagId } });
            const dataPassId = dataPassQcFlag?.dataPassId;

            // Invalidate GAQ summary if it's the first verification
            if (dataPassId && (!qcFlag.verifications || qcFlag.verifications.length === 0)) {
                await GaqSummaryRepository.invalidate(dataPassId, updatedQcFlag.runNumber);
            }

            return updatedQcFlag;
        }, { transaction });
    }
