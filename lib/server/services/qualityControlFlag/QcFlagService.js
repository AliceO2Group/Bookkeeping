...
const { qcFlagAdapter, dataPassQcFlagAdapter, simulationPassQcFlagAdapter, gaqDetectorAdapter } = require('../../../database/adapters/index.js');
...
class QcFlagService {
    ...
    async getGaqQcFlags({ dataPassId, runNumber }) {
        ...
        const eP = await QcFlagEffectivePeriodRepository.findAll({
            include: [
                {
                    association: 'flag',
                    where: { runNumber, dplDetectorId: { [Op.in]: dplDetectorIds } },
                    include: [{ association: 'dataPasses', where: { id: dataPassId } }],
                },
            ],
        ...
        let contributing = [];
        let index = 0;
        let newstPeriodContributingId;
        while (index < eP.length) {
            newstPeriodContributingId = index;
            while (eP[newstPeriodContributingId] && eP[newstPeriodContributingId].from <= p.from) {
                contributing.push(eP[newstPeriodContributingId]);
                newstPeriodContributingId++;
                // if (eP[newstPeriodContributingId].to < p.to) {
                //     p.to = eP[newstPeriodContributingId].to;
                // }
            }
            // if (eP[oldestActiveFlagId + 1] && eP[oldestActiveFlagId + 1].from < p.to) {
            //     p.to = eP[oldestActiveFlagId + 1].from;
            // }

            /*
             * If (oldestActiveFlagId === eP.length - 1 || eP[oldestActiveFlagId].to < eP[oldestActiveFlagId + 1].from) {
             *     p.to = eP[oldestActiveFlagId].to;
             *     console.log(1, 'TOBEC')
             * } else {
             *     p.to = eP[oldestActiveFlagId + 1].from;
             *     console.log(2, 'TOBEC')
             * }
             */
            gaqPeriods.push([p, contributing]);
            p.from = p.to;
            p.to = !eP[index + 1] || eP[index + 1].from === eP[index].from
                ? eP[index].to
                : eP[index].from;

            let dropByIndex = 0;
            while (dropByIndex < contributing.length && contributing[dropByIndex].to <= p.from) {
                dropByIndex++;
            }
            contributing = contributing.slice(dropByIndex);
            index++;
            break;
        }
