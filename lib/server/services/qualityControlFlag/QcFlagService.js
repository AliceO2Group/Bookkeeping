 ...
 * @property {number} explicitlyNotBadEffectiveRunCoverage - fraction of run's data, which aggregated quality is explicitely good
 * @property {number} missingVerificationsCount - number of not verified QC flags which are not discarded
...
class QcFlagService {
    ...
    async getGaqSummary(dataPassId) {
        const rows = await QcFlagRepository.getGaqAggregations(dataPassId);

        const summary = {};
        const verifications = {};
        // Fold list of summaries into nested object
        for (const subSummary of rows) {
            const {
                runNumber,
                bad,
                effectiveRunCoverage,
            ...
            if (!verifications[runNumber]) {
                verifications[runNumber] = {};
            }

            const runSummary = summary[runNumber];
            const runVerifications = verifications[runNumber];

            runVerifications.flagsIds = new Set([...runVerifications.flagsIds ?? [], ...flagsIds]);
            runVerifications.verifiedFlagsIds = new Set([...runVerifications.verifiedFlagsIds ?? [], ...verifiedFlagsIds]);
        ...
        for (const [runNumber, { flagsIds, verifiedFlagsIds }] of Object.entries(verifications)) {
