...
class QcFlagService {
    ...
    async _updatePreviousFlagsEffectiveness(createdFlag, flagsToBePartiallyDiscarded) {
        ...
        for (const partiallyDiscardFlag of flagsToBePartiallyDiscarded) {
            const { from, to, uneffectiveFrom, uneffectiveTo } = partiallyDiscardFlag;
            if (newFlagEffectiveTo <= from || to <= newFlagEffectiveFrom) {
                continue;
            }

            await QcFlagRepository.update(partiallyDiscardFlag, {
                uneffectiveFrom: Math.max(
                    from,
                    Math.min([uneffectiveFrom, newFlagEffectiveFrom].filter((_) => _)),
                ),
                uneffectiveTo: Math.min(
                    to,
                    Math.max(...[uneffectiveTo, newFlagEffectiveTo].filter((_) => _)),
                ),
            });
        }
    }
