...
const AgqDetectorRepository = require('../../../database/repositories/AgqDetectorRepository.js');
...
class QcFlagService {
    ...
    /**
     * Set list of DPL detectors whih contribute to GQD (global aggregated quality)
     ...
     */
    setAgqDetectors({ dataPassId, runNumbers, dplDetectorIds }) {
        return dataSource.transaction(async () => {
            ...
            if (runsNotAssociatedToDataPass.length > 0) {
                const errorMessage = `Following runs are not assoicated with data pass with id (${dataPassId}): ${runsNotAssociatedToDataPass}`;
                throw new BadParameterError(errorMessage);
            }

            // With detectors
            const dplDetectors = await DplDetectorRepository.findAll({ where: { id: { [Op.in]: dplDetectorIds } } });
            ...
            if (validDetectorIdsSet.size !== dplDetectorIds.length) {
                ...
                throw new BadParameterError(`No DPL detectors for IDs: (${invalidIds})`);
            }
            ...
            const totalRetrivedRecords = runAndDetector.reduce((sum, [_, detectors]) => sum + detectors.length, 0);
            ...
            if (totalRetrivedRecords !== runNumbers.length * dplDetectorIds.length) {
                ...
                const errorMessage = `No association between runs and detectors: ${missingRunDetectorAssociations}`;
                throw new BadParameterError(errorMessage);
            }

            await AgqDetectorRepository.removeAll({ where: {
                dataPassId,
                runNumber: { [Op.in]: runNumbers },
            } });
            const entities = runNumbers
                .flatMap((runNumber) => dplDetectorIds
                    .map((dplDetectorId) => ({ dataPassId, runNumber, dplDetectorId })));
            await AgqDetectorRepository.insertAll(entities);
        });
    }
