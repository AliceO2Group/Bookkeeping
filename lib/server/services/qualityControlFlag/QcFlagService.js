...
const { getPhysicalDetectorOrFail } = require('../dpl/getPhysicalDetectorOrFail.js');
 ...
 * @type {Object<number, RunDetectorQcSummary>} detectorID to RunDetectorQcSummary mappings
 */
...
class QcFlagService {
     ...
     * @param {DetectorIdentifier} scope.detectorIdentifier associated detector's identifier
    ...
    async create(qcFlags, scope, relations) {
        ...
        return dataSource.transaction(async () => {
            ...
            const detector = await getPhysicalDetectorOrFail(detectorIdentifier);
    ...
    async verifyFlag({ flagId, comment }, relations) {
        return dataSource.transaction(async () => {
            ...
            const detector = await getPhysicalDetectorOrFail({ detectorId: qcFlag.detectorId });
            validateUserDetectorAccess(userRoles, detector.name);
    ...
    /**
     * Return a paginated list of QC flags related to a given data pass, run and detector
     *
     ...
     * @param {number} criteria.detectorId the id of the detector to which QC flag should release
    ...
    /**
     * Return a paginated list of QC flags related to a given simulation pass, run and detector
     *
     * @param {object} criteria the QC flag criteria
     ...
     * @param {string} detectorName the name of the detector that the run must include
     ...
     * @private
     */
    async _getQcTargetRun(runNumber, detectorName, { dataPassId, simulationPassId }) {
        const { VIRTUAL_DETECTOR_NAME } = await import('../../../public/domain/enums/detectorsNames.mjs');
        const runInclude = [
            {
                association: 'detectors',
                where: { name: detectorName },
                through: { attributes: [] },
                attributes: [],
                required: detectorName !== VIRTUAL_DETECTOR_NAME.GLO,
            },
        ];
        ...
        if (!run) {
            const criteria = [`run with this number (${runNumber})`, `detector with this name (${detectorName})`];
