 ...
 */
const getQcDetectorExpertRoleName = (detectorName) => `det-${detectorName.toLowerCase()}`;
...
const checkIfUserCanManageFlagsForDetector = (userRoles, detectorName) => {
    if (!userRoles.includes(BkpRoles.ADMIN) && !userRoles.includes(getQcDetectorExpertRoleName(detectorName))) {
