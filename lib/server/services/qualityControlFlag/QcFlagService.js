...
const {
    repositories: {
        ...
        QcFlagDiscardedPeriodRepository,
    },
...
class QcFlagService {
     ...
     */
    async _updateQcOneFlagDiscardedPeriods(createdFlag, flagToBePartiallyDiscarded) {
        ...
        const ineffectivenesPeriodsOfFlagToBePartiallyDiscarded = await QcFlagDiscardedPeriodRepository.findAll({
            where: {
        ...
        await QcFlagDiscardedPeriodRepository.insert({
            flagId: flagToBePartiallyDiscardedId,
            from: ineffectiveFrom,
            to: ineffectiveTo,
        });
        // Remove periods included in newly created one
        await QcFlagDiscardedPeriodRepository.removeAll({ where: { id: { [Op.in]: overlappingPeriodIds } } });
    }
     ...
     * @return {Promise<void>} promise
     */
    async _updateFlagsDiscardedPeriod(createdFlag, flagsToBePartiallyDiscarded) {
        return Promise.all(flagsToBePartiallyDiscarded
            .map((flagToBePartiallyDiscarded) => this._updateQcOneFlagDiscardedPeriods(createdFlag, flagToBePartiallyDiscarded)));
    }
    ...
    async createForDataPass(parameters, relations = {}) {
        ...
        return dataSource.transaction(async () => {
            ...
            await this._updateFlagsDiscardedPeriod(
                createdFlag,
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            await QcFlagDiscardedPeriodRepository.removeAll({ where: { flagId: qcFlag.id } });
            ...
            await QcFlagDiscardedPeriodRepository.removeAll({
                where: {
            ...
            if (flagsCreatedAfterRemovedFlag.length === 0) {
                if (flagsCreatedBeforeRemovedFlag.length >= 2) {
                    ...
                    await this._updateFlagsDiscardedPeriod(newestFlag, olderFlags);
                }
                ...
                for (const newerFlag of flagsCreatedAfterRemovedFlag) {
                    await this._updateFlagsDiscardedPeriod(newerFlag, flagsCreatedBeforeRemovedFlag);
                }
            }
    ...
    prepareQueryBuilder() {
        return dataSource.createQueryBuilder()
            ...
            .include({ association: 'discardedPeriods' })
            .groupBy('id');
    }
}
