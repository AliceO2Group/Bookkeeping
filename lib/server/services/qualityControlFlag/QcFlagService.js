...
const GaqDetectorRepository = require('../../../database/repositories/GaqDetectorRepository.js');
...
class QcFlagService {
    ...
    /**
     * Set list of DPL detectors which contribute to GAQ (global aggregated quality)
     * for given runs and data pass
     *
     * @param {object} scope of definition
     * @param {number} scope.dataPassId data pass id
     * @param {number[]} scope.runNumbers list of unique run numbers
     * @param {number[]} scope.dplDetectorIds list of unique DPL detector ids
     * @return {Promise<void>} promise
     */
    setGaqDetectors({ dataPassId, runNumbers, dplDetectorIds }) {
        return dataSource.transaction(async () => {
            await getOneDataPassOrFail({ id: dataPassId });
            await this._validateDataPassAndRunsAssociation(dataPassId, runNumbers);
            await this._validateRunsAndDplDetectorsAssociations(runNumbers, dplDetectorIds);
            ...
            const gaqEntries = runNumbers
                .flatMap((runNumber) => dplDetectorIds
                    .map((dplDetectorId) => ({ dataPassId, runNumber, dplDetectorId })));
    ...
    /**
     * Validate association between data pass and runs exists
     *
     ...
     */
    async _validateDataPassAndRunsAssociation(dataPassId, runNumbers) {
        const validRunNumbers = await RunRepository.findAll({
            attributes: ['runNumber'],
            where: { runNumber: { [Op.in]: runNumbers } },
            include: [{ association: 'dataPass', where: { id: dataPassId }, required: false }],
        });
        if (validRunNumbers.length !== runNumbers.length) {
            ...
            const errorMessage = `No runs for run numbers: (${invalidRunNumbers})`;
            throw new BadParameterError(errorMessage);
        }
        const runsNotAssociatedToDataPass = validRunNumbers
            .filter(({ dataPass }) => dataPass.length === 0)
            .map(({ runNumber }) => runNumber);
        if (runsNotAssociatedToDataPass.length > 0) {
            const errorMessage = `No association between data pass with id ${dataPassId} and following runs: ${runsNotAssociatedToDataPass}`;
            throw new BadParameterError(errorMessage);
        }
    }

    /**
     * Validate association between runs and DPL detectors.
     ...
     *
     * @param {number[]} runNumbers list of run numbers
     * @param {number[]} dplDetectorIds list of IDs of DPL detectors
     * @return {Promise<void>} promise resolved once associations are validated
     * @throws {BadParameterError} if some association does not exist
     */
    async _validateRunsAndDplDetectorsAssociations(runNumbers, dplDetectorIds) {
        const dplDetectors = await DplDetectorRepository.findAll({
            attributes: ['id', 'name'],
            where: { id: { [Op.in]: dplDetectorIds } },
        });
        const validDetectorIdsSet = new Set(dplDetectors.map(({ id }) => id));
        if (validDetectorIdsSet.size !== dplDetectorIds.length) {
            const invalidIds = dplDetectorIds.filter((dplDetectorId) => !validDetectorIdsSet.has(dplDetectorId));
        ...
        const totalRetrivedRecords = runAndDetector.reduce((count, [_, detectors]) => count + detectors.length, 0);

        if (totalRetrivedRecords !== runNumbers.length * dplDetectorIds.length) {
        ...
        return dataSource.transaction(async () => {
            ...
            for (const qcFlag of qcFlags) {
                ...
                const effectivePeriodsToBeUpdated = await QcFlagEffectivePeriodRepository.findOverlappingPeriodsCreatedNotAfter(
                    ...
                    { dataPassId: dataPass?.id, simulationPassId: simulationPass?.id, runNumber, dplDetectorId: dplDetector.id },
                );
    ...
    /**
     * Get periods with sets of QC flags which apply in them
     *
     * @param {number} dataPassId id od data pass
     * @param {number} runNumber id od data pass
     * @return {Promise<GaqQcFlags>} promise of aggregated QC flags
     */
    async getGaqQcFlags(dataPassId, runNumber) {
        const gaqDetectors = await GaqDetectorRepository.findAll({ where: { dataPassId, runNumber } });
        const dplDetectorIds = gaqDetectors.map(({ dplDetectorId }) => dplDetectorId);
        const effectivePeriods = await QcFlagEffectivePeriodRepository.findAll({
            include: [
                {
                    association: 'flag',
                    where: { runNumber, dplDetectorId: { [Op.in]: dplDetectorIds } },
                    include: [
                        { association: 'dataPasses', where: { id: dataPassId } },
                        { association: 'flagType' },
                        { association: 'dplDetector' },
                    ],
                },
            ],
        ...
        const END = 1;
        const START = 2;
        const alterationPoints = effectivePeriods
            .flatMap(({ from, to, flag }) => [
                { timestamp: from, type: START, flag },
                { timestamp: to, type: END, flag },
            ])
            .sort((
                { timestamp: timestamp1, type: type1 },
                { timestamp: timestamp2, type: type2 },
            ) => timestamp1.getTime() - timestamp2.getTime() + (type1 - type2));

        const currentPeriod = { from: alterationPoints[0].timestamp, to: null };
        const gaqPeriods = [];
        let contributingFlags = [];
        let index = 0;

        while (index < alterationPoints.length) {
            while (alterationPoints[index].type === START && alterationPoints[index].timestamp.getTime() === currentPeriod.from.getTime()) {
                contributingFlags.push(alterationPoints[index].flag);
                index++;
            }
            currentPeriod.to = alterationPoints[index].timestamp;
            if (contributingFlags.length > 0) {
                gaqPeriods.push([
                    { ...currentPeriod },
                    contributingFlags.map(({
                        flagType,
                        dplDetector,
                    }) => [
                        qcFlagTypeAdapter.toMinifiedEntity(flagType),
                        dplDetectorAdapter.toMinifiedEntity(dplDetector),
                    ]),
                ]);
            }
            contributingFlags = [...contributingFlags];

            while (alterationPoints[index]?.type === END && alterationPoints[index].timestamp.getTime() === currentPeriod.to.getTime()) {
                alterationPoints[index].flag.dropped = true;
                index++;
            }
            contributingFlags = contributingFlags.filter(({ dropped }) => !dropped);
