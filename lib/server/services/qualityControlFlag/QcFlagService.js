...
class QcFlagService {
    ...
    async getQcFlagsSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runSummaries of Object.values(summary)) {
            for (const runDetectorSummary of Object.values(runSummaries)) {
                this._fillQcSummaryUnitMissingValues(runDetectorSummary);
            }
        }
    ...
    async getGaqSummary(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runSummary of Object.values(summary)) {
            this._fillQcSummaryUnitMissingValues(runSummary);
        }
    ...
    _mergeIntoSummaryUnit(summaryUnit, partialSummaryUnit) {
        ...
        if (bad === null) {
            summaryUnit[QC_SUMMARY_PROPERTIES.qualityNotDefinedEffectiveRunCoverage] = effectiveRunCoverage;
        } else if (bad) {
     ...
     * @return {void}
     */
    _fillQcSummaryUnitMissingValues(summaryUnit) {
        ...
        if (summaryUnit[QC_SUMMARY_PROPERTIES.qualityNotDefinedEffectiveRunCoverage] === undefined) {
            const badEffectiveRunCoverage = summaryUnit[QC_SUMMARY_PROPERTIES.badEffectiveRunCoverage];
            const explicitlyNotBadEffectiveRunCoverage = summaryUnit[QC_SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage];
            const definedCoverage = badEffectiveRunCoverage + explicitlyNotBadEffectiveRunCoverage;

            summaryUnit[QC_SUMMARY_PROPERTIES.qualityNotDefinedEffectiveRunCoverage] =
