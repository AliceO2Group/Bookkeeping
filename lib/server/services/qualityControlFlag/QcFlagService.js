...
class QcFlagService {
    ...
    /**
     * Update effective periods ... TODO
     * @param {QcFlag} qcFlags QC flag
     * @param {QcFlagEffectivePeriod} effectivePeriodsOfFlagToBePartiallyDiscarded periods
     * @return {Promise<void>} promise
     */
    async _updateEffectivePeriods({ from: newerPeriodFrom, to: newerPeriodTo }, effectivePeriodsOfFlagToBePartiallyDiscarded) {
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            for (const index in flagsCreatedBeforeRemovedFlag) {
                await this._resetQcFlagEffectivePeriod(flagsCreatedBeforeRemovedFlag[index]);

                const olderUpdatingFlags = flagsCreatedBeforeRemovedFlag.slice(Number(index) + 1);
                for (const updatingFlag of [...olderUpdatingFlags, ...flagsCreatedAfterRemovedFlag]) {
                    const effectivePeriods = await QcFlagEffectivePeriodRepository.findAll({
                        where: {
