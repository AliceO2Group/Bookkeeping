...
class QcFlagService {
     ...
     */
    async getAllPerDataPassAndRunAndDetector({ dataPassId, runNumber, dplDetectorId }, { limit, offset }) {
        ...
        const { count, rows } = await DataPassQcFlagRepository.findAndCountAll(queryBuilder);
     ...
     */
    async getAllPerSimulationPassAndRunAndDetector({ simulationPassId, runNumber, dplDetectorId }, { limit, offset }) {
        const queryBuilder = dataSource.createQueryBuilder()
        ...
        const { count, rows } = await SimulationPassQcFlagRepository.findAndCountAll(queryBuilder);
