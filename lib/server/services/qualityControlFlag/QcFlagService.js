...
class QcFlagService {
        ...
        return dataSource.transaction(async () => {
            ...
            for (const qcFlag of qcFlags) {
                ...
                if (dataPass) {
                    ...
                    flagInclude.push({ association: 'simulationPasses', where: { id: null } });
                } else {
     ...
     * if it is for asyncronous QC, also, linked to a given data/simulation pass
     * No data/simulation pass is required if it is for synchronous QC
     ...
     * @param {number} [monalisaProduction] MonALISA production
    ...
    async _getQcTargetRun(runNumber, dplDetectorName, { dataPassId, simulationPassId }) {
        ...
        // Else fething information for synchronous QC flags
