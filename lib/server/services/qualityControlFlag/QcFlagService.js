...
class QcFlagService {
     ...
     * @param {*} createdFlag newly created flag
     * @param {*} flagsToBePartiallyDiscarded flags to be updated
     * @return {Promise<void>} promise
     */
    async _updatePreviousFlagsEffectiveness(createdFlag, flagsToBePartiallyDiscarded) {
        const { from: newFlagEffectiveFrom, to: newFlagEffectiveTo } = createdFlag;

        for (const partiallyDiscardFlag of flagsToBePartiallyDiscarded) {
            const { from, to, ineffectiveFrom, ineffectiveTo } = partiallyDiscardFlag;
            if (newFlagEffectiveTo <= from || to <= newFlagEffectiveFrom) {
                continue;
            }

            await QcFlagRepository.update(partiallyDiscardFlag, {
                ineffectiveFrom: Math.max(
                    from,
                    Math.min([ineffectiveFrom, newFlagEffectiveFrom].filter((_) => _)),
                ),
                ineffectiveTo: Math.min(
                    to,
                    Math.max(...[ineffectiveTo, newFlagEffectiveTo].filter((_) => _)),
                ),
            });
        }
    }
