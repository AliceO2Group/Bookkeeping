...
class QcFlagService {
    ...
    async getGaqSummary(dataPassId) {
        const rows = await QcFlagRepository.getGaqSummary(dataPassId);
        ...
        for (const subSummary of rows) {
            ...
            runSummary[QC_SUMMARY_PROPERTIES.missingVerificationsCount] =
                (runSummary[QC_SUMMARY_PROPERTIES.missingVerificationsCount] ?? 0) + missingVerificationsCount;
