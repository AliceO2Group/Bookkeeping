...
class QcFlagService {
     ...
     */
    async getQcFlagsSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad }) {
    ...
    _mergeIntoSummaryUnit(summaryUnit, partialSummaryUnit) {
        ...
        if (bad) {
            summaryUnit[QC_SUMMARY_PROPERTIES.badEffectiveRunCoverage] = parseFloat(effectiveRunCoverage);
            summaryUnit[QC_SUMMARY_PROPERTIES.mcReproducible] =
                mcReproducible || summaryUnit[QC_SUMMARY_PROPERTIES.mcReproducible];
        } else {
            summaryUnit[QC_SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage] = parseFloat(effectiveRunCoverage);
            summaryUnit[QC_SUMMARY_PROPERTIES.mcReproducible] =
                mcReproducible || summaryUnit[QC_SUMMARY_PROPERTIES.mcReproducible];
        }
    ...
    async _getQcTargetRun(runNumber, dplDetectorName, { dataPassId, simulationPassId }) {
        const { NonPhysicalDetector } = await import('../../../public/domain/enums/detectorsNames.mjs');
        const runInclude = [
            {
                ...
                through: { attributes: [] },
                attributes: [],
                required: dplDetectorName !== NonPhysicalDetector.GLO,
            },
        ];
