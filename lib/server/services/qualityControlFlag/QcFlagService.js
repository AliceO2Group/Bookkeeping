 ...
 * @property {string} operator : one of ['or', 'none']
 */
...
class QcFlagService {
    ...
    getCreatedByFilterClause(filter = {}) {
        const { createdBy: { names: createdNyNames, operator: createdByOperator } = {} } = filter;

        if (createdNyNames && createdByOperator) {
            if (createdByOperator === 'or') {
                return { name: { [Op.or]: createdNyNames } };
            } else if (createdByOperator === 'none') {
                return { name: { [Op.not]: { [Op.or]: createdNyNames } } };
            } else {
