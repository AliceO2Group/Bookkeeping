...
class QcFlagService {
    ...
    async getGaqFlags(dataPassId, runNumber) {
        const flagsInGaqPeriods = await QcFlagRepository.findFlagsInGaqEffectivePeriods({ dataPassId, runNumbers: [runNumber] });
        const qcFlags = (await QcFlagRepository.findAll({
            where: { id: { [Op.in]: flagsInGaqPeriods.flatMap(({ contributingFlagIds }) => contributingFlagIds) } },
        })).map(qcFlagAdapter.toEntity);

        const idToFlag = Object.fromEntries(qcFlags.map((flag) => [flag.id, flag]));

        return flagsInGaqPeriods.map(({
            contributingFlagIds,
            from,
            to,
        }) => ({
