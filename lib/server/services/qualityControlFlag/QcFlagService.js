...
class QcFlagService {
     ...
     * for given in runs and data pass
     *
     * @param {object} scope of definition
     * @param {number} scope.dataPassId data pass id
     * @param {number[]} scope.runNumbers run numbers
     * @param {number[]} scope.dplDetectorIds DPL detector ids
     * @return {Promise<void>} promise
     */
    setGaqDetectors({ dataPassId, runNumbers, dplDetectorIds }) {
        return dataSource.transaction(async () => {
            // Check data integrity
            if (runNumbers.length !== new Set(runNumbers).size) {
                throw new BadParameterError('Run Numbers cannot be duplicated');
            }
            if (dplDetectorIds.length !== new Set(dplDetectorIds).size) {
                throw new BadParameterError('DPL detector IDs cannot be duplicated');
            }
    ...
    /**
     * Validate association between data pass and set of runs exsits
     *
