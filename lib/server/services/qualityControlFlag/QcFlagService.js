 ...
 * @type {Object<number, {'bad': QcSummary, 'notBad': QcSummary }>} dplDetectorID to QcSummary mappings
 */
...
class QcFlagService {
    ...
    async _getQcFlagsSummaries(queryBuilder) {
        ...
        for (const runDetectorSunmmary of runDetectorSummaries) {
            const { runNumber, dplDetectorId, bad, ...summary } = runDetectorSunmmary;
            ...
            if (bad) {
                result[runNumber][dplDetectorId]['bad'] = summary;
            } else {
                result[runNumber][dplDetectorId]['notBad'] = summary;
            }
        }
