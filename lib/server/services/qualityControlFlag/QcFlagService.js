...
class QcFlagService {
    ...
    _prepareQcFlagPeriod(timestamps, targetRun) {
        ...
        const runStartTime = firstTfTimestamp?.getTime() ?? timeTrgStart?.getTime() ?? timeO2Start?.getTime() ?? null;
        const runEndTime = lastTfTimestamp?.getTime() ?? timeTrgEnd?.getTime() ?? timeO2End?.getTime() ?? null;
