...
class QcFlagService {
    ...
    async getGaqFlags(dataPassId, runNumber) {
        const gaqFlagsIndentifiers = await QcFlagRepository.findGaqFlagsIdentifiers({ dataPassId, runNumbers: [runNumber] });
        const qcFlags = (await QcFlagRepository.findAll({
            where: { id: { [Op.in]: gaqFlagsIndentifiers.flatMap(({ contributingFlagIds }) => contributingFlagIds) } },
        })).map(qcFlagAdapter.toEntity);

        const idToFlag = Object.fromEntries(qcFlags.map((flag) => [flag.id, flag]));

        return gaqFlagsIndentifiers.map(({
            contributingFlagIds,
            from,
            to,
        }) => ({
