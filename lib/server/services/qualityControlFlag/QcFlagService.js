...
class QcFlagService {
    ...
    async getGaqQcFlags({ dataPassId, runNumber }) {
        ...
        const p = {
            from: eP[0].from,
            to: !eP[1] || eP[1].from.getTime() === eP[0].from.getTime()
                ? eP[0].to
                : eP[1].from,
        };
        ...
        let contributing = [eP[0]];
        let index = 0;
        let nextContributingFlagIndex;
        while (index < eP.length) {
            nextContributingFlagIndex = index + 1;
            while (eP[nextContributingFlagIndex] && eP[nextContributingFlagIndex].from <= p.from) {
                contributing.push(eP[nextContributingFlagIndex]);
                nextContributingFlagIndex++;
            }
            ...
            if (eP[index]) {
                p.to = !eP[index + 1] || eP[index + 1].from.getTime() === p.from.getTime()
                    ? minDate(eP[index].to, eP[index + 1]?.to ?? eP[index].to)
                    : minDate(eP[index + 1].from, eP[index].to);
            }

            console.log(index, eP.length, 'processed TOBEC')
            // if (index === 2) {
            //     break;
            // }
        }
        return gaqPeriods;
        return { ep: eP, gaqPeriods };
    }
