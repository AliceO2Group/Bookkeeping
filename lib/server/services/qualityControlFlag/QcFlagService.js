...
class QcFlagService {
     ...
     * @param {SequelizeQcFlag} flagToBePartiallyDiscarded flag to be updated
     * @return {Promise<void>} promise
     */
    async _updateOneFlagEffectiveness(createdFlag, flagToBePartiallyDiscarded) {
        const { from: newFlagFrom, to: newFlagTo } = qcFlagAdapter.toEntity(createdFlag);

        const { id: discardedFlagId, from, to } = qcFlagAdapter.toEntity(flagToBePartiallyDiscarded);
        if (newFlagTo <= from || to <= newFlagFrom) {
            return;
        }

        const ineffectiveFrom = Math.max(from, newFlagFrom);
        const ineffectiveTo = Math.min(to, newFlagTo);
    ...
    async createForDataPass(parameters, relations = {}) {
        ...
        return dataSource.transaction(async () => {
            ...
            await this._updateExistingFlagsEffectiveness(adaptedCreatedQcFlag, flagsToBePartiallyDiscarded);
    ...
    async createForSimulationPass(parameters, relations = {}) {
        ...
        return dataSource.transaction(async () => {
            ...
            await this._updateExistingFlagsEffectiveness(createdFlag, flagsToBePartiallyDiscarded);
