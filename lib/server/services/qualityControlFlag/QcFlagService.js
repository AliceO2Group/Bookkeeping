...
const { getPhysicalDetectorOrFail } = require('../detector/getPhysicalDetectorOrFail.js');
...
class QcFlagService {
    ...
    async getQcFlagsSummary({ dataPassId, simulationPassId }) {
        ...
        queryBuilder
            ...
            .groupBy('runNumber')
            .groupBy('detectorId')
            .groupBy((sequelize) => sequelize.col('`flagType`.bad'));
    ...
    async verifyFlag({ flagId, comment }, relations) {
        return dataSource.transaction(async () => {
            ...
            const detector = await getPhysicalDetectorOrFail({ detectorId: qcFlag.dplDetectorId });
            validateUserDetectorAccess(userRoles, detector.name);
