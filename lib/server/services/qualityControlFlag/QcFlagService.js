...
class QcFlagService {
    ...
    async _updateEffectivePeriods({ from: newerPeriodFrom, to: newerPeriodTo }, periods) {
         ...
         * so  `from` == null is extended to -1, which is interpreted as run.startTime
         * and `to`   == null is extended to MAX_DATE, which is interpreted as run.endTime
         */
        ...
        for (const effectivePeriod of periods) {
            ...
            } else if (evalEffectiveFrom < newerFromEval && newerToEval < evalEffectiveTo) {
                /*
