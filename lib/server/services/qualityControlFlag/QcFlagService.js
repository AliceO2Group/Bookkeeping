    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            for (const index in flagsCreatedBeforeRemovedFlag) {
                ...
                const olderFlagsAfterFlagToBeUpdated = flagsCreatedBeforeRemovedFlag.slice(Number(index) + 1);
                for (const updatingFlag of [...olderFlagsAfterFlagToBeUpdated, ...flagsCreatedAfterRemovedFlag]) {
