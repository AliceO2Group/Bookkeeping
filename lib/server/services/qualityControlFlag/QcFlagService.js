...
class QcFlagService {
    ...
    /**
     * Returns all the QC flags attached to a given run
     *
     * @param {number} runNumber the run number of run for which QC flags should be returned
     * @return {Promise<QcFlag[]>} the list of attached QC flags
     */
    getAllPerRun(runNumber) {
        return QcFlagRepository.findAll({
            where: { runNumber },
            include: [
                { association: 'dataPasses' },
                { association: 'simulationPasses' },
