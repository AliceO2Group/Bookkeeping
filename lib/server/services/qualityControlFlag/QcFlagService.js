...
class QcFlagService {
     ...
     * @param {object} parameters flag instance parameters
     * @param {number} [parameters.fromTime] flag start timestamp
     * @param {number} [parameters.toTime] flag start timestamp
     * @param {number} [parameters.comment] associated comment
    ...
    async createForDataPass(parameters, relations = {}) {
        ...
        if (!(fromTime < toTime)) {
            throw new BadParameterError('Parameter `toTime` must be greater than `fromTime`');
        }
        return dataSource.transaction(async () => {
            ...
            if (!dplDetector) {
                throw new BadParameterError(`No DPL Detector with given id (${dplDetectorId})`);
            }
            if (!dplDetector.name || dplDetector.name === 'TST') {
                throw new BadParameterError(`Bad DPL Detector selection, QC flag creation for "${dplDetector.name} is impossible"`);
            }

            const targetEntitiesTuple = await RunRepository.findOne({
                subQuery: false,
            ...
            if (fromTime < targetEntitiesTuple.timeTrgStart.getTime() || targetEntitiesTuple.timeTrgEnd.getTime() < toTime) {
                // eslint-disable-next-line max-len
                throw new BadParameterError(`Given QC flag period (${fromTime} ${toTime}) is beyond run trigger period (${targetEntitiesTuple.timeTrgStart.getTime()}, ${targetEntitiesTuple.timeTrgEnd.getTime()})`);
            }
            ...
            await createdFlag.addDataPasses(targetEntitiesTuple.dataPass);
     ...
     * @param {number} [parameters.flagTypeId] flag type id
     * @param {number} [parameters.runNumber] associated run's number
     * @param {number} [parameters.simulationPassId] associated dataPass' id
     * @param {number} [parameters.dplDetectorId] associated dplDetector's id
     * @return {Promise<QcFlag>} promise
     * @throws {BadParameterError, NotFoundError}
     */
    async createForSimulationPass(parameters, relations = {}) {
        ...
        // Check user
            ...
            await createdFlag.addSimulationPasses(targetEntitiesTuple.simulationPasses);
