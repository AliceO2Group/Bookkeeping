...
class QcFlagService {
    ...
    _prepareQcFlagPeriod(timestamps, targetRun) {
        const { startTime: runStartTime, endTime: runEndTime } = targetRun;
    ...
    async _updateEffectivePeriods({ from: newerPeriodFrom, to: newerPeriodTo }, periods) {
         ...
         * so  `from` == null is extended to -1, which is interpreted as run.start
         * and `to`   == null is extended to MAX_DATE
         */
        const MAX_DATE = new Date('100000-01-01').getTime();
        const newerFromEval = newerPeriodFrom ?? -1;
        const newerToEval = newerPeriodTo ?? MAX_DATE;

        for (const effectivePeriod of periods) {
            const { id: effectivePeriodId, from: effectiveFrom, to: effectiveTo } = effectivePeriod;

            const evalEffectiveFrom = effectiveFrom ?? -1;
            const evalEffectiveTo = effectiveTo ?? MAX_DATE;

            if (newerFromEval <= evalEffectiveFrom && evalEffectiveTo <= newerToEval) { // Old flag is fully covered by new one
                await QcFlagEffectivePeriodRepository.removeOne({ where: { id: effectivePeriodId } });
            } else if (evalEffectiveFrom < newerFromEval
                && newerToEval < evalEffectiveTo) {
                /*
                 * New flag's period is included in the old one's period.
                 * Possible only when both timestamps of newer period are not null
                 */
            ...
            } else if (evalEffectiveFrom < newerFromEval) { // Possible only when `newerPeriodFrom` is not null
                await QcFlagEffectivePeriodRepository.update(effectivePeriod, { to: newerPeriodFrom });
            } else if (newerToEval < evalEffectiveTo) { // Possible only when `newerPeriodTo` is not null
        ...
        return dataSource.transaction(async () => {
            ...
            for (const qcFlag of qcFlags) {
                ...
                if (dataPass) {
                    ...
                    flagIncludes.push({ association: 'dataPasses', where: { id: dataPass.id }, required: true });
                } else if (simulationPass) {
                    await createdFlag.addSimulationPass(simulationPass);
                    flagIncludes.push({ association: 'simulationPasses', where: { id: simulationPass.id }, required: true });
                } else {
                        ...
                        [Op.and]: [
                            {
                                [Op.or]: [
                                    { from: null },
                                    to ? { from: { [Op.lt]: to } } : { from: { [Op.not]: null } },
                                ],
                            },
                            {
                                [Op.or]: [
                                    { to: null },
                                    from ? { to: { [Op.gt]: from } } : { to: { [Op.not]: null } },
                                ],
                            },
                        ],
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            if (qcFlag.dataPasses?.[0]?.id) {
                flagIncludes.push({ association: 'dataPasses', where: { id: qcFlag.dataPasses[0].id }, required: true });
            } else if (qcFlag.simulationPasses?.[0]?.id) {
                flagIncludes.push({ association: 'simulationPasses', where: { id: qcFlag.simulationPasses[0].id }, required: true });
            } else {
                flagIncludes.push({ association: 'dataPasses', required: false });
                flagIncludes.push({ association: 'simulationPasses', required: false });
                synchronousQcWhereClause = { '$flag->dataPasses.id$': null, '$flag->simulationPasses.id$': null };
            ...
            for (const newestFlagToBeUpdatedIndex in flagsCreatedBeforeRemovedFlag) {
                ...
                for (const updatingFlag of [...updatingFlagsCreatedBefore, ...flagsCreatedAfterRemovedFlag]) {
                    const { from: updatingFrom, to: updatingTo } = updatingFlag;
                    const effectivePeriods = await QcFlagEffectivePeriodRepository.findAll({
                        [Op.and]: [
                            {
                                [Op.or]: [
                                    { from: null },
                                    updatingTo ? { from: { [Op.lt]: updatingTo } } : { from: { [Op.not]: null } },
                                ],
                            },
                            {
                                [Op.or]: [
                                    { to: null },
                                    updatingFrom ? { to: { [Op.gt]: updatingFrom } } : { to: { [Op.not]: null } },
                                ],
                            },
                        ],
                        include: [
