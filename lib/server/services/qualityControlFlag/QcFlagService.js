 ...
 * @property {number} qualityNotDefinedEffectiveRunCoverage - fraction of run's data NOT marked with any QC flag
...
/**
 * @typedef QcSubSummaryUnit
 * @property {boolean} bad - true when coverage is bad, false otherwise
 * @property {number} effectiveRunCoverage - fraction of run's data covered with flags
 * @property {number} missingVerificationsCount - number of not verified QC flags which are not discarded
 * @property {boolean} mcReproducible - states whether some Limited Acceptance MC Reproducible flag was assigned
 */

/**
 * Wrapper class for QcSummaryUnit
 */
class QcSummaryUnit {
    /**
     * Constructor
     */
    constructor() {
        this.badEffectiveRunCoverage = undefined;
        this.explicitlyNotBadEffectiveRunCoverage = undefined;
        this.qualityNotDefinedEffectiveRunCoverage = undefined;
        this.missingVerificationsCount = 0;
        this.mcReproducible = false;
    }

    /**
     * Update self with partial QC summary unit
     *
     * @param {Partial<QcSubSummaryUnit>} partialSubSummaryUnit new properties to be applied to the self
     * @return {void}
     */
    updateWithSubSummary(partialSubSummaryUnit) {
        const {
            bad,
            effectiveRunCoverage,
            mcReproducible,
            missingVerificationsCount,
        } = partialSubSummaryUnit;

        if (bad !== undefined && effectiveRunCoverage !== undefined) {
            if (bad === null) {
                this.qualityNotDefinedEffectiveRunCoverage = effectiveRunCoverage;
            } else if (bad) {
                this.badEffectiveRunCoverage = effectiveRunCoverage;
            } else {
                this.explicitlyNotBadEffectiveRunCoverage = effectiveRunCoverage;
            }
        }

        if (missingVerificationsCount) {
            this.missingVerificationsCount += missingVerificationsCount;
        }

        this.mcReproducible = mcReproducible || this.mcReproducible;
    }

    /**
     * Set default summary properties which were not set when merging sub-summaries, so each expected property has no 'undefined' value
     *
     * @return {void}
     */
    fillMissingValues() {
        if (this.explicitlyNotBadEffectiveRunCoverage === undefined) {
            this.explicitlyNotBadEffectiveRunCoverage = 0;
        }
        if (this.badEffectiveRunCoverage === undefined) {
            this.badEffectiveRunCoverage = 0;
        }
        if (this.qualityNotDefinedEffectiveRunCoverage === undefined) {
            const { badEffectiveRunCoverage, explicitlyNotBadEffectiveRunCoverage } = this;
            const definedCoverage = badEffectiveRunCoverage + explicitlyNotBadEffectiveRunCoverage;

            this.qualityNotDefinedEffectiveRunCoverage =
                badEffectiveRunCoverage === null || explicitlyNotBadEffectiveRunCoverage === null
                    ? null
                    : 1 - definedCoverage;
        }
    }
...
class QcFlagService {
    ...
    async getQcFlagsSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runDetectorSummaryForFlagTypesClass of runDetectorSummaryList) {
            ...
            if (!summary[runNumber][detectorId]) {
                summary[runNumber][detectorId] = new QcSummaryUnit();
            }
            ...
            runDetectorSummary.updateWithSubSummary({
                ...runDetectorSummaryForFlagTypesClass,
                missingVerificationsCount,
            });
        }
        ...
        for (const runSummaries of Object.values(summary)) {
            for (const runDetectorSummary of Object.values(runSummaries)) {
                runDetectorSummary.fillMissingValues();
            }
        }
    ...
    async getGaqSummary(dataPassId, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const subSummary of runGaqSubSummaries) {
            ...
            if (!summary[runNumber]) {
                summary[runNumber] = new QcSummaryUnit();
            }
            ...
            runSummary.updateWithSubSummary(subSummary);
        }

        for (const [runNumber, { distinctFlagsIds, distinctVerifiedFlagsIds }] of Object.entries(flagsAndVerifications)) {
            summary[runNumber].updateWithSubSummary({ missingVerificationsCount: distinctFlagsIds.size - distinctVerifiedFlagsIds.size });
        }
        for (const runSummary of Object.values(summary)) {
            runSummary.fillMissingValues();
        }
