 ...
 * @type {Object<number, QcSummary>} dplDetectorID to QcSummary mapping
 */
...
class QcFlagService {
     ...
     * @return {Promise<void>} promise
     */
    async _updateFlagsEffectivePeriods(createdFlag, flagsToBePartiallyDiscarded) {
    ...
    async _getQcFlagsSummaries(queryBuilder) {
        queryBuilder
            ...
            .set('attributes', (seq) => [
                ...
                'dplDetectorId',
                [
                    seq.literal('SUM(`effectivePeriods`.`to` - `effectivePeriods`.`from`) \
                        / (COALESCE(run.time_trg_end, run.time_o2_end) - COALESCE(run.time_trg_start, run.time_o2_start)) '),
                    'effectiveness',
                ],
                [seq.col('`flagType`.bad'), 'bad'],
                [
                    seq.literal('count(distinct `QcFlag`.id) - count(distinct `verifications`.flag_id)'),
        ...
        const runDetectorSummaries = (await QcFlagRepository.findAll(queryBuilder))
            .map(({ dataValues: { runNumber, dplDetectorId, effectiveness, bad, missingVerificationsCount } }) =>
                ({ runNumber, dplDetectorId, effectiveness, bad: Boolean(bad), missingVerificationsCount }));

        const result = {};
        for (const runDetectorSunmmary of runDetectorSummaries) {
            const { runNumber, dplDetectorId, ...summary } = runDetectorSunmmary;
            if (!result[runNumber]) {
                result[runNumber] = {};
            }
            if (!result[dplDetectorId]) {
                result[runNumber][dplDetectorId] = summary;
            }
        }
    ...
    async createForDataPass(parameters, relations = {}) {
        ...
        return dataSource.transaction(async () => {
            ...
            await this._resetQcFlagEffectivePeriod(createdFlag);
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            if (flagsCreatedAfterRemovedFlag.length === 0) {
                if (flagsCreatedBeforeRemovedFlag.length >= 1) {
                    ...
                    await this._updateFlagsEffectivePeriods(newestFlag, olderFlags);
                }
            } else if (flagsCreatedBeforeRemovedFlag.length > 0) {
                // Must iterate from oldest to newest flag
                for (const newerFlag of flagsCreatedAfterRemovedFlag) {
                    await this._updateFlagsEffectivePeriods(newerFlag, flagsCreatedBeforeRemovedFlag);
                }
            }
