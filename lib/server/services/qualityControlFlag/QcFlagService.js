...
class QcFlagService {
    ...
    async delete(id) {
        return dataSource.transaction(async () => {
            ...
            {
                const { id, from, to, origin, createdById, runNumber, detectorId, flagTypeId, createdAt } = qcFlag;
    ...
    async verifyFlag({ flagId, comment }, relations, { transaction } = {}) {
        return dataSource.transaction(async () => {
            ...
            const detector = await getQcDetectorOrFail({ detectorId: qcFlag.detectorId });
            validateUserDetectorAccess(userRoles, detector.name);
