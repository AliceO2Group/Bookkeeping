...
class QcFlagService {
     ...
     */
    async getAllSynchronousPerAndRunAndDetector({ runNumber, detectorId }, pagination) {
        const { limit, offset } = pagination || {};

        const { count, rows } = await QcFlagRepository.findAndCountAll({
            where: {
                runNumber,
                detectorId,
                '$dataPasses.id$': null,
            ...
            include: [
                ...
                { association: 'createdBy' },
                { association: 'verifications', include: [{ association: 'createdBy' }] },
                { association: 'dataPass', required: false },
                { association: 'simulationPass', required: false },
            ],
            limit,
            offset,
        });
