...
class QcFlagService {
     ...
     * @param {object} [relations] QC Flag entity relations
     * @param {UserIdentifier+IsAdminOption} [relations.user] user
     * @return {Promise<QcFlag>} promise
     */
    async delete(id, relations) {
        return dataSource.transaction(async () => {
            ...
            const { user: { userId, externalUserId, isAdmin } } = relations;
            const user = await getUserOrFail({ userId, externalUserId });

            if (qcFlag.createdById !== user.id && !isAdmin) {
                throw new Error('You are not allowed to remove this QC flag');
            }
