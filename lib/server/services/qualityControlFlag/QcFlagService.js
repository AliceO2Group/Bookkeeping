...
class QcFlagService {
    ...
    _prepareQcFlagPeriod(timestamps, targetRun) {
        const from = targetRun.startTime
            ? timestamps.from ?? targetRun.startTime
            : null;
        const to = targetRun.endTime
            ? timestamps.to ?? targetRun.endTime
            : null;

        if (!targetRun.startTime && timestamps.from) {
            throw new BadParameterError('Only null `from` timestamp is accepted as run.startTime is missing');
        }
        if (!targetRun.endTime && timestamps.to) {
            throw new BadParameterError('Only null `to` timestamp is accepted as run.endTime is missing');
        }

        if (from && to && from >= to) {
            throw new BadParameterError('Parameter "to" timestamp must be greater than "from" timestamp');
        }
        const isFromOutOfRange = from && targetRun.startTime && from < targetRun.startTime;
        const isToOutOfRange = to && targetRun.endTime && targetRun.endTime < to;
