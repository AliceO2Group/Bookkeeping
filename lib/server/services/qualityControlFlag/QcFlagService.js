 ...
 */
const getDetectorExpertRoleName = (detectorName) => `det-${detectorName.toLowerCase()}`;
...
const checkIfUserCanManageFlagsForDetector = (userRoles, detectorName) => {
    if (!userRoles.includes(BkpRoles.ADMIN) && !userRoles.includes(getDetectorExpertRoleName(detectorName))) {
