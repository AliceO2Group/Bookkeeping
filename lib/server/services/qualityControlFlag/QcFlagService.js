...
class QcFlagService {
     ...
     * @param {Partial<QcFlagVerification>} parameters flag verification parameters
     * @param {object} relations QC Flag entity relations
     * @param {UserWithRoles} relations.userWithRoles usepr with roles
     * @return {Promise<QcFlag>} promise
     * @throws {NotFoundError|AccessDeniedError}
     */
    async verifyFlag(parameters, relations) {
        return dataSource.transaction(async () => {
            const { flagId, comment } = parameters;

            const qcFlag = await this.getOneOrFail(flagId);

            const { userWithRoles: { userId, externalUserId } } = relations;
            const user = await getUserOrFail({ userId, externalUserId });
