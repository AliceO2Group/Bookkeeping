...
class QcFlagSummaryService {
     ...
     * @param {number[]} [scope.detectorIds] ids of dpl detectors
     ...
     */
    async getSummary({ dataPassId, simulationPassId, lhcPeriodId, detectorIds }, { mcReproducibleAsNotBad = false } = {}, filter = {}) {
        ...
        if (detectorIds) {
            queryBuilder.where('detectorId').oneOf(...detectorIds);
        }
