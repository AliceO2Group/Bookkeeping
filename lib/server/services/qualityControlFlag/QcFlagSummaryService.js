...
const { QcSummarProperties } = require('./QcSummaryProperties.js');
...
class QcFlagSummaryService {
    ...
    static mergeIntoSummaryUnit(summaryUnit, partialSummaryUnit) {
        ...
        if (bad) {
            summaryUnit[QcSummarProperties.badEffectiveRunCoverage] = effectiveRunCoverage;
            summaryUnit[QcSummarProperties.mcReproducible] =
                mcReproducible || summaryUnit[QcSummarProperties.mcReproducible];
        } else {
            summaryUnit[QcSummarProperties.explicitlyNotBadEffectiveRunCoverage] = effectiveRunCoverage;
            summaryUnit[QcSummarProperties.mcReproducible] =
                mcReproducible || summaryUnit[QcSummarProperties.mcReproducible];
        }
        if (summaryUnit[QcSummarProperties.badEffectiveRunCoverage] === undefined) {
            summaryUnit[QcSummarProperties.badEffectiveRunCoverage] = 0;
        }
        if (summaryUnit[QcSummarProperties.explicitlyNotBadEffectiveRunCoverage] === undefined) {
            summaryUnit[QcSummarProperties.explicitlyNotBadEffectiveRunCoverage] = 0;
        }
    }
    ...
    async getSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runDetectorSummaryForFlagTypesClass of runDetectorSummaryList) {
            ...
            if (!summary[runNumber][detectorId]) {
                summary[runNumber][detectorId] = { [QcSummarProperties.mcReproducible]: false };
            }

            const runDetectorSummary = summary[runNumber][detectorId];

            runDetectorSummary[QcSummarProperties.missingVerificationsCount] =
                (runDetectorSummary[QcSummarProperties.missingVerificationsCount] ?? 0) + missingVerificationsCount;
