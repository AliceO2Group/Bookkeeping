...
const { RUN_DETECTOR_QC_SUMMARY_PROPERTIES: SUMMARY_PROPERTIES } = require('./RunDetectorQcSummaryProperties.js');
...
class QcFlagSummaryService {
    ...
    static mergeIntoSummaryUnit(summaryUnit, partialSummaryUnit) {
        ...
        if (bad) {
            summaryUnit[SUMMARY_PROPERTIES.badEffectiveRunCoverage] = effectiveRunCoverage;
            summaryUnit[SUMMARY_PROPERTIES.mcReproducible] =
                mcReproducible || summaryUnit[SUMMARY_PROPERTIES.mcReproducible];
        } else {
            summaryUnit[SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage] = effectiveRunCoverage;
            summaryUnit[SUMMARY_PROPERTIES.mcReproducible] =
                mcReproducible || summaryUnit[SUMMARY_PROPERTIES.mcReproducible];
        }
        if (summaryUnit[SUMMARY_PROPERTIES.badEffectiveRunCoverage] === undefined) {
            summaryUnit[SUMMARY_PROPERTIES.badEffectiveRunCoverage] = 0;
        }
        if (summaryUnit[SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage] === undefined) {
            summaryUnit[SUMMARY_PROPERTIES.explicitlyNotBadEffectiveRunCoverage] = 0;
        }
    }
    ...
    async getSummary({ dataPassId, simulationPassId, lhcPeriodId }, { mcReproducibleAsNotBad = false } = {}) {
        ...
        for (const runDetectorSummaryForFlagTypesClass of runDetectorSummaryList) {
            ...
            if (!summary[runNumber][detectorId]) {
                summary[runNumber][detectorId] = { [SUMMARY_PROPERTIES.mcReproducible]: false };
            }

            const runDetectorSummary = summary[runNumber][detectorId];

            runDetectorSummary[SUMMARY_PROPERTIES.missingVerificationsCount] =
                (runDetectorSummary[SUMMARY_PROPERTIES.missingVerificationsCount] ?? 0) + missingVerificationsCount;
