...
class QualityControlFlagService {
     ...
     * @param {QualityControlFlagVerification+UserIdentifier} parameters flag verification instance parameters
     * @return {Promise<QualityControlFlagVerification>} promise
     * @throws {NotFoundError}
     */
    async createVerification({ qualityControlFlagId, comment, userId, externalUserId }) {
        // Check user
    ...
    async create(parameters) {
        ...
        if (!timeStart < timeEnd) {
            throw new BadParameterError('Parameter `timeTrgEnd` must be greater than `timeTrgStart`');
        }
        ...
        if (timeStart < result.timeTrgStart || result.timeTrgEnd < timeEnd) {
            throw new BadParameterError(`
                Given QC flag period (${timeStart} ${timeEnd}) is beyond run trigger period (${result.timeTrgStart}, ${result.timeTrgEnd})
            `);
        }
        ...
        return qualityControlFlagAdapter.toEntity(newInstance);
    }
