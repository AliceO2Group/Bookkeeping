...
class QualityControlFlagService {
     ...
     * @param {number[]} [options.filter.simulationPassIds] filtering defintion
    ...
    async getAll({ filter, sort, limit, offset } = {}) {
        ...
        if (filter) {
            const { ids, dataPassIds, runNumbers, detectorIds, externalUserIds, userNames, simulationPassIds } = filter;
            if (ids) {
