...
class QcFlagTypeService {
    ...
    async create(parameters) {
        ...
        let conflictingEntity = await QcFlagTypeRepository.findOne({
            where: { name },
        });
        if (conflictingEntity) {
            throw new ConflictError('name must be unique');
        }
        conflictingEntity = await QcFlagTypeRepository.findOne({
            where: { method },
        });
        if (conflictingEntity) {
            throw new ConflictError('method must be unique');
        }
