...
class QcFlagTypeService {
     ...
     * @param {Partial<{userId: number, externalUserId: number}>} [userIdentifiers] user identifiers
     * @return {Promise<QcFlagType>} promise
     */
    async create(parameters, userIdentifiers) {
        const {
            name,
            method,
            bad,
        ...
        const { userId, externalUserId } = userIdentifiers;
