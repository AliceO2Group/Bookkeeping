...
class LogService {
     ...
     * @return {Promise<{count: number, childLogs: Log[]}>} A promise that resolves with an object containing the total count of all child logs,
     * and an array of the child logs for the given parent log
     */
    async getChildLogsByParentId(logId) {
        const { count, childLogIds } = await this._getChildLogIds(logId);
        const childLogs = await this._getAllByIds(childLogIds);
    ...
    /**
     * Fetches the IDs of all child logs of the parent log and the total count of all child log IDs.
     * @param {number} logId The id of the log of which the child logs are retrieved
     * @return {Promise<{count: number, childLogIds: number[]}>} A promise that resolves with an object containing an array of child log IDs
     * of the given log and the total count of all child logs
     */
    async _getChildLogIds(logId) {
        const { rows, count } = await LogRepository.findAndCountAll({
        ...
        const childLogIds = rows.map((row) => row.id);

        return { count, childLogIds };
    }
