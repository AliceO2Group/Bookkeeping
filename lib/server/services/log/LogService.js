...
const { logAdapter, attachmentAdapter, tagAdapter } = require('../../../database/adapters/index.js');
...
class LogService {
     ...
     *
     * @param {{include: (string[]|null), exclude: string[]}} tagsFilter the tags used to filter issues. If include is null, no include criteria
     *     is applied, all tags are matched
     ...
     * @return {Promise<Tag[]>} resolves with the resulting logs list
     */
    async getAllByTagsInPeriod(tagsFilter, period, options) {
        return getLogsByTagsInPeriod(tagsFilter, period, options).map(tagAdapter.toEntity);
    }
    ...
    _createFindQueryBuilder() {
        return dataSource.createQueryBuilder()
            ...
            .include({ association: 'tags', attributes: ['text'] });
    }
     ...
     * @param {string} template the template name used to create the log
     * @param {string} description the description of the log
     * @param {string} system the system of the log
     * @return {Promise<Log>} resolve with the created log instance
     */
    async create(newLog, runNumbers = [], tagsTexts = [], environments = [], lhcFills = [], attachments = [], template = '', description = '', system = '') {
