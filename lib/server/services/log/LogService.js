...
const { getRootLogIds } = require('./getRootLogIds');
...
class LogService {
    ...
    /**
     * Find and return all root logs
     * @param {Partial<GetAllLogsDto>} dto The GetAllLogs DTO which contains all request data.
     * @return {Promise<[Log]|null>} resolve with the logs found or null
     */
    async getAllRootLogs(dto = {}) {
        const { count, logIds } = await getRootLogIds((queryBuilder) => {
            const { query = {} } = dto;
            const { page = {} } = query;
        ...
        const detailedLogs = await this._getAllByIds(logIds);
