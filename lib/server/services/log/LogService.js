...
class LogService {
    ...
    async getAllRootLogs(limit, offset) {
        const { count, logIds } = await this.getRootLogIds(limit, offset);
     ...
     */
    async getRootLogIds(limit, offset) {
        const queryBuilder = new QueryBuilder()
            .where('parentLogId').is(null)
            .limit(limit).offset(offset);

        const { rows, count } = await LogRepository.findAndCountAll(queryBuilder);
        const logIds = rows.map(({ id }) => id);
