...
class LogService {
     ...
     * @return {Promise<{childLogs: Log[]}>} A promise that resolves with an object containing
     * an array of the child logs for the given parent log
     */
    async getChildLogsByParentId(logId) {
        const childLogQueryBuilder = this._createFindQueryBuilder();
        childLogQueryBuilder.where('parentLogId').is(logId);
     ...
     *
     * @param {logs[]} logs the list of logs to fetch
