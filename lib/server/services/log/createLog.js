...
const { getAllEnvironmentsByTextOrFail } = require('../environment/getAllEnvironmentsByTextOrFail.js');
 ...
 * @param {string[]} environmentsText the texts of environment IDs to attach to log
 ...
 */
exports.createLog = async (log, runNumbers, tagsTexts, environmentsText, attachments, transaction) => {
    const runIds = await getRunIdsFromRunNumbersOrFail(runNumbers);
    const tags = await getAllTagsByTextOrFail(tagsTexts);
    const environments = await getAllEnvironmentsByTextOrFail(environmentsText);
    ...
    if (environments.length > 0) {
        await TransactionHelper.provide(
            () => LogEnvironmentsRepository.bulkInsert(environments.map((env) => ({ environmentId: env.id, logId: newLogId }))),
            { transaction },
        );
    }
