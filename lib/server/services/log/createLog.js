 ...
 * @param {string[]} environments the environment IDs to attach to log
 ...
 */
exports.createLog = async (log, runNumbers, tagsTexts, environments, attachments, transaction) => {
    const runIds = await getRunIdsFromRunNumbersOrFail(runNumbers);
    const tags = await getAllTagsByTextOrFail(tagsTexts);
    const envs = await getAllEnvironmentsOrFail(environments);
    ...
    if (envs.length > 0) {
        await TransactionHelper.provide(
            () => LogEnvironmentsRepository.bulkInsert(envs.map((env) => ({ environmentId: env.id, logId: newLogId }))),
            { transaction },
        );
    }
