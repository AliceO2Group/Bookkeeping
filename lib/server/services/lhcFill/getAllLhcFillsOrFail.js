...
exports.getLhcFillNumbersOrFail = async (lhcFills) => {
    ...
    /**
     * Check if, for each lhcFillNumber, there is a lhcFill present in the DB with that lhcFillNumber
     * Add the lhcFill's id to lhcFillIds if there is.
     */
    const retrievedLhcFills = []
    for (let possibleFillNumber of lhcFills) {
        let { fillNumber } = await getLhcFillByNumberOrFail(possibleFillNumber);
