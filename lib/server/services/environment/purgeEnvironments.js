 ...
 *
 * @param {number[]} environmentIdsToKeep the list of ids of environments to keep
 ...
 * @deprecated
 */
exports.purgeEnvironments = async (environmentIdsToKeep, modificationTimeWindow) => sequelize.query(`
    ...
    HAVING e.updated_at > date_sub(now(), INTERVAL ${modificationTimeWindow} MINUTE)
       ...
       AND e.id NOT IN (${environmentIdsToKeep.map((id) => `'${id}'`).join(',')})
`);
