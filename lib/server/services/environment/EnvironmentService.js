...
class EnvironmentService {
        ...
        const environmentId = await dataSource.transaction(async () => {
            ...
            if (!isAnEmptyShell(currentState)) {
                ...
                await createEnvironmentHistoryItem(environmentHistoryItemAdapter.toDatabase({
                    ...
                    timestampNano: currentState.timestampNano,
                }));
            }
    ...
    async update(environmentId, environmentPatch, newState) {
        await dataSource.transaction(async () => {
            ...
            const currentState = await EnvironmentHistoryItemRepository.findOne({
                where: { environmentId },
                order: [['timestamp', 'DESC'], ['timestampNano', 'DESC']],
            });
            ...
            if (!isAnEmptyShell(newState) && newState.status !== currentState?.status) {
                ...
                await createEnvironmentHistoryItem(environmentHistoryItemAdapter.toDatabase({
                    ...
                    timestampNano: newState.timestampNano,
                }));
            }
        });
        return this.get(environmentId);
     ...
     * @param {Object<string, string>} configurationVariables updated environment configuration variables
     * @return {Promise<void>} resolves once the state has been saved
     */
    async createOrUpdateEnvironment(environmentId, state, configurationVariables) {
        await dataSource.transaction(async () => {
                ...
                await this.create(
                    {
                        ...
                        rawConfiguration: JSON.stringify(configurationVariables),
                    },
                    state,
                );
            } else {
    ...
    _getEnvironmentQbConfiguration(queryBuilder) {
        ...
        queryBuilder.orderBy('timestamp', 'ASC', 'historyItems');
        queryBuilder.orderBy('timestampNano', 'ASC', 'historyItems');
    }
}
