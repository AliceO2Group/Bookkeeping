...
const { getAllEnvironmentsByLogId } = require('./getAllEnvironmentsByLogId.js');
const { getLogOrFail } = require('../log/getLogOrFail.js');
...
class EnvironmentService {
    ...
    /**
     * Find environments by their associated log id
     *
     * @param {number} logId the log to find environments by
     * @returns {Promise<Environment[]>} resolve with the resulting environments
     */
    async getAllEnvironmentsByLogId(logId) {
        const { id } = await getLogOrFail(logId);
        const environments = await getAllEnvironmentsByLogId(id);
        return environments ? environments.map((env) => environmentAdapter.toEntity(env, false)) : null;
    }
