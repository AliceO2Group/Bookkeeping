...
exports.getEnvironmentsInPeriod = async ({ from, to }, relations) => {
    ...
    const [rows] = await sequelize.query(`
        ...
        HAVING e.updated_at >= '${timestampToMysql(from)}'
            OR e.updated_at > '${timestampToMysql(lostIfNoNewsSince)}'
            AND GROUP_CONCAT(ehi.status) NOT LIKE '%ERROR%'
            AND GROUP_CONCAT(ehi.status) NOT LIKE '%DESTROYED%'
            AND GROUP_CONCAT(ehi.status) NOT LIKE '%DONE%'
    `);
    const ids = rows.map(({ id }) => id);

    return EnvironmentRepository.findAll({
        where: { id: { [Op.in]: ids } },
        include: environmentIncludeClauseFromRelations(relations),
        order: ['createdAt'],
    });
};
