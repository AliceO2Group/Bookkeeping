...
const { getDetectorOrFail } = require('./getDetectorOrFail.js');
...
const { getOrCreateDetectorByName } = require('./getOrCreateDetector.js');
...
const cachedGetOrCreateDetectorByName = cacheAsyncFunction((detectorName) => getOrCreateDetectorByName(detectorName)
    .then(({ id }) => id));
...
class DplProcessService {
    /**
     * Returns all the detectors that have at least one executed process in the given run
     *
     * @param {RunIdentifier} runIdentifier the identifier of the run that must have at least one executed process
     * @return {Promise<Detector[]>} the resulting detectors
     */
    async getAllDetectorsWithExecutedProcessesByRun(runIdentifier) {
        const run = await getRunOrFail(runIdentifier);

        // Do the requests in two steps because some tests randomly fail, maybe because of a bug in sequelize?
        ...
        const detectorIds = detectors.map(({ id }) => id);

        const sortedDetectors = await DetectorRepository.findAll({
            where: { id: { [Op.in]: detectorIds } },
            order: [['name', 'ASC']],
        });

        return sortedDetectors.map(detectorAdapter.toEntity);
    }
     ...
     * @param {DetectorIdentifier} detectorIdentifier the identifier of the detector for which the process must have been executed
    ...
    async getAllExecutedProcessesByRunAndDetector(runIdentifier, detectorIdentifier) {
        const run = await getRunOrFail(runIdentifier);
        const detector = await getDetectorOrFail(detectorIdentifier);

        // Do the requests in two steps because some tests randomly fail, maybe because of a bug in sequelize?
        const processes = await DplProcessRepository.findAll({
            include: {
                association: 'processesExecutions',
                where: {
                    runNumber: run.runNumber,
        ...
        const sortedProcesses = await DplProcessRepository.findAll({
            where: { id: { [Op.in]: processIds } },
            include: 'type',
            order: [['name', 'ASC']],
        });

        return sortedProcesses.map(dplProcessAdapter.toEntity);
    }
    ...
    async getAllHostWithExecutedProcessByRunAndDetector(runIdentifier, detectorIdentifier, processIdentifier) {
        ...
        // Do the requests in two steps because some tests randomly fail, maybe because of a bug in sequelize?
        const hosts = await HostRepository.findAll({
            include: {
                association: 'processesExecutions',
                where: {
                    runNumber: run.runNumber,
        ...
        const sortedHosts = await HostRepository.findAll({
            where: { id: { [Op.in]: hostIds } },
            order: [['hostname', 'ASC']],
        });

        return sortedHosts.map(hostAdapter.toEntity);
    }
    ...
    async getAllProcessExecutionByRunAndDetectorAndHost(runIdentifier, detectorIdentifier, processIdentifier, hostIdentifier) {
        const [run, detector, process, host] = await Promise.all([
            ...
            getDetectorOrFail(detectorIdentifier),
            getDplProcessOrFail(processIdentifier),
            getHostOrFail(hostIdentifier),
        ]);

        const processExecutions = await DplProcessExecutionRepository.findAll({
            where: {
                runNumber: run.runNumber,
                detectorId: detector.id,
                processId: process.id,
        ...
        return processExecutions.map(dplProcessExecutionAdapter.toEntity);
    }
    ...
    async createProcessExecution(newProcessExecution, relations) {
        ...
        const [runNumber, detectorId, processId, hostId] = await Promise.all([
            ...
            cachedGetOrCreateDetectorByName(relations.detectorName),
