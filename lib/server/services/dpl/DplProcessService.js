 ...
 * @param {function} argsToCacheKey method applied on arguments to obtain a unique cache key
 * @return {function} the cached function
 */
const cacheFunction = (toCache, argsToCacheKey = (...args) => JSON.stringify({ ...args })) => {
    const cache = new Map();
    return async (...args) => {
...
const cachedGetRunOrFail = cacheFunction((runIdentifier) => getRunOrFail(runIdentifier)
    .then(({ runNumber }) => runNumber));

const cachedGetOrCreateDplDetectorByName = cacheFunction((detectorName) => getOrCreateDplDetectorByName(detectorName)
    .then(({ id }) => id));

const cachedGetOrCreateDplProcessByNameAndTypeId = cacheFunction(async (processName, processTypeLabel) => {
...
const cachedGetOrCreateHostByHostname = cacheFunction((hostname) => getOrCreateHostByHostname(hostname)
    .then(({ id }) => id));
