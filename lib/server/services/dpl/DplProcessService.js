...
class DplProcessService {
    ...
    async getAllDetectorsWithExecutedProcessesByRun(runIdentifier) {
        ...
        const detectors = await DplDetectorRepository.findAll({ where: { id: { [Op.in]: ids } } });
        const sortedDetectors = detectors
    ...
    async getAllHostWithExecutedProcessByRunAndDetector(runIdentifier, detectorIdentifier, processIdentifier) {
        ...
        const hosts = await HostRepository.findAll({ where: { id: { [Op.in]: ids } } });

        const sortedHosts = hosts
            .map((host) => ({ ...hostAdapter.toEntity(host), name: host.name || '' }))
            .sort((a, b) => a.name.localeCompare(b.name));
