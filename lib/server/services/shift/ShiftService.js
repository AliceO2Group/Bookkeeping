...
class ShiftService {
    ...
    async getShiftData(shifterUserIdentifier, shiftType) {
        ...
        const prevShiftIssues = await getShiftIssuesByTitle(title);

        if (prevShiftIssues.length === 0) {
            ret.infoFromPreviousShifter.errorMessage = `No ${shiftType} EoS reports found from the previous shift`;
        } else if (prevShiftIssues.length > 1) {
            ...
            if (!eosReportText) {
                ...
                if (!infoFromPreviousShifterAllMatches) {
                    ret.infoFromPreviousShifter.errorMessage = 'EoS report from the previous shift is missing the information transfer field';
                } else if (infoFromPreviousShifterAllMatches.length > 1) {
                    ret.infoFromPreviousShifter.errorMessage = 'Previous EoS report contains multiple information transfer fields';
                } else {
                    const [previousInfoMatch] = infoFromPreviousShifterAllMatches;
                    if (previousInfoMatch === '' || previousInfoMatch === '-') {
                        ret.infoFromPreviousShifter.errorMessage = 'Previous EoS report contains no information for next shifter';
                    } else {
                        ret.infoFromPreviousShifter.value = previousInfoMatch;
                    }
                }
            }
        }
