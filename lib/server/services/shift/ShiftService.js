...
class ShiftService {
    ...
    async getShiftData(shifterUserIdentifier, shiftType) {
        ...
        const shiftTag = `${shiftType} Shifter`;
        const reportFormat = `End of shift report - ${shiftType} - `;

        const prevShift = getShiftFromTimestamp(shift.start - SHIFT_DURATION);
        const prevShiftIssues = await getShiftIssues(prevShift, { include: [shiftTag], exclude: [] }) || null;

        if (prevShiftIssues.length > 0) {
            const [eosReport] = prevShiftIssues.filter((report) => report.title.startsWith(reportFormat))
            ...
            if (infoFromPreviousShifterAllMatches
                ...
                ret.previousInfo = infoFromPreviousShifterAllMatches[0];
            } else {
                ret.previousInfo = '';
            }
        } else {
