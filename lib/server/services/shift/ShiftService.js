    ...
    async getShiftData(shifterUserIdentifier, shiftType) {
        ...
        if (previousShiftReports.length === 0) {
            ...
            if (!eosReportText) {
                ...
                const infoFromPreviousShifterAllMatches = [...eosReportText.matchAll(infoFromPreviousShifterRegex)].map((match) => match[1]);
                if (infoFromPreviousShifterAllMatches.length === 0) {
