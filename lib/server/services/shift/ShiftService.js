...
const { getShiftIssuesByTitle } = require('../eosReport/getShiftIssuesByTitle.js');
...
class ShiftService {
     ...
     */
    async getShiftData(shifterUserIdentifier, shiftType, shiftStart = null) {
        ...
        const timestamp = shiftStart || shift.start;
        const pastShift = getShiftFromTimestamp(timestamp - SHIFT_DURATION);
        const title = formatEosReportTitle(pastShift, shiftType);

        const prevShiftIssues = await getShiftIssuesByTitle(pastShift, title);

        if (prevShiftIssues.length !== 1) {
            ret.previousInfo.errorMessage = `No ${shiftType} EoS reports found from the previous shift`;
        } else {
            const eosReportText = prevShiftIssues[0].text;
            if (!eosReportText) {
                ret.previousInfo.errorMessage = 'EoS report from the previous shift is missing the text field';
            } else {
                const infoFromPreviousShifterRegex = /(?<=\n\n### For next shifter\n)([\S\s]*)(?=\n\n###)/g;
                const infoFromPreviousShifterAllMatches = eosReportText.match(infoFromPreviousShifterRegex);
                if (infoFromPreviousShifterAllMatches.length !== 1) {
                    ret.previousInfo.errorMessage = 'EoS report from the previous shift is missing the information transfer field';
                } else {
                    const [previousInfoMatch] = infoFromPreviousShifterAllMatches;
                    if (previousInfoMatch === '-') {
                        ret.previousInfo.errorMessage = 'Previous EoS report contains no information for next shifter';
                    } else {
                        ret.previousInfo.value = previousInfoMatch;
                    }
                }
            }
        }
