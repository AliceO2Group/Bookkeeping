...
class ShiftService {
    ...
    async getShiftData(shifterUserIdentifier, shiftType) {
        ...
        // Finds all EoS reports from the last 16 hours and sorts by most recent first
        const pastShift = { start: new Date() - SHIFT_DURATION * 2, end: new Date() };
        const prevShiftIssues = (await getShiftIssues(pastShift, { include: ['EoS'], exclude: [] }))
            .reverse()
            .sort((a, b) => {
        ...
        ret.previousInfo = '';
        if (prevShiftIssues && prevShiftIssues.length > 0) {
            const reportFormat = `End of shift report - ${shiftType} - `;
            const tags = getEosReportTagsByType(shiftType);

            // Filters for the correct shift type, correct tags, and correct EoS title
            const [eosReport] = prevShiftIssues.filter((report) =>
                tags.every((tag) => report.tags.map((x) => x.text).includes(tag))
            ...
            if (infoFromPreviousShifterAllMatches
                    && infoFromPreviousShifterAllMatches.length === 1
                    && infoFromPreviousShifterAllMatches[0] !== '-') {
