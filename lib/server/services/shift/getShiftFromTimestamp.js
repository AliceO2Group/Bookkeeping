...
exports.getShiftFromTimestamp = (timestamp) => {
    // Remove the milliseconds part of the timestamp
    ...
    const shifterLocaleDate = date.toLocaleDateString(parseableDateFormat, { timeZone: SHIFTER_TIMEZONE })
        + ' ' + date.toLocaleTimeString(parseableDateFormat, { timeZone: SHIFTER_TIMEZONE });
    ...
    const shiftIndex = Math.floor(shiftedShifterLocalTimestamp / SHIFT_DURATION);

    // Timestamp of the shift start in the shifter's timezone WITH OFFSET
    const shiftedShiftStart = shiftIndex * SHIFT_DURATION;

    return {
        start: shiftedShiftStart - timeOffset - shifterToUTCOffset,
        // One shift lasts for 8 hours
        end: shiftedShiftStart - timeOffset - shifterToUTCOffset + SHIFT_DURATION,
