...
const GaqSummaryInvalidationRepository = require('../../../database/repositories/GaqSummaryInvalidationRepository.js');
...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        ...
        // Need to reload the runModel as qcTimeStart and qcTimeEnd are virtual columns not in the patch so will stay stale after update
        ...
        if (previousRun.qcTimeStart?.getTime() !== runModel.qcTimeStart?.getTime()
            ...
            for (const { dataPassId } of dataPassRuns) {
                await GaqSummaryInvalidationRepository.upsert({
                    dataPassId,
                    runNumber: runModel.runNumber,
                });
            }
        }
