...
const GaqSummaryRepository = require('../../../database/repositories/GaqSummaryRepository.js');
const DataPassRunRepository = require('../../../database/repositories/DataPassRunRepository.js');
...
/**
 * Run patch fields whose values feed the qc_time_start / qc_time_end virtual columns
 *
 * @type {string[]}
 */
const QC_TIME_SOURCE_FIELDS = [
    'firstTfTimestamp',
    'lastTfTimestamp',
    'timeTrgStart',
    'timeTrgEnd',
...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        ...
        const previousRun = {
            runQuality: runModel?.runQuality,
            calibrationStatus: runModel?.calibrationStatus,
            qcTimeStart: runModel?.qcTimeStart,
            qcTimeEnd: runModel?.qcTimeEnd,
        };
        ...
        // Only check for invalidation when the patch could have changed them
        if (QC_TIME_SOURCE_FIELDS.some((field) => field in runPatch)) {
            // Reload because qcTimeStart/qcTimeEnd are virtual columns not in the patch and stay stale after update
            await runModel.reload();
            if (previousRun.qcTimeStart?.getTime() !== runModel.qcTimeStart?.getTime()
                || previousRun.qcTimeEnd?.getTime() !== runModel.qcTimeEnd?.getTime()) {
                const dataPassRuns = await DataPassRunRepository.findAll({
                    attributes: ['dataPassId'],
                    where: { runNumber: runModel.runNumber },
                });
                await GaqSummaryRepository.invalidateMany(dataPassRuns.map(({ dataPassId }) => ({ dataPassId, runNumber: runModel.runNumber })));
            }
        }
