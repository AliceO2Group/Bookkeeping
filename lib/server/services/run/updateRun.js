...
const { getOrCreateLhcPeriod } = require('../period/getOrCreateLhcPeriod');
...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        ...
        if (previousRun.runQuality !== runModel.runQuality) {
            ...
            const lhcPeriod = await getOrCreateLhcPeriod(lhcPeriodId);
            const lhcPeriodName = lhcPeriod.name;
