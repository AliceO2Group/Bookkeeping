...
exports.updateRun = async (identifier, payload, transaction) => {
    ...
    return TransactionHelper.provide(async (transaction) => {
        ...
        if (previousRun.runQuality !== runModel.runQuality) {
            await logRunQualityChange(runModel.runNumber, previousRun.runQuality, runModel.runQuality, transaction, user);
        }

        if (previousRun.calibrationStatus !== runModel.calibrationStatus) {
            const { calibrationStatusChangeReason } = metadata;

            // Do not allow calibration status change reason if we were not in failed sattus
            if (previousRun.calibrationStatus !== RunCalibrationStatus.FAILED && calibrationStatusChangeReason) {
            ...
            if (previousRun.calibrationStatus === RunCalibrationStatus.FAILED || runModel.calibrationStatus === RunCalibrationStatus.FAILED) {
                if (previousRun.calibrationStatus === RunCalibrationStatus.FAILED && !calibrationStatusChangeReason) {
                    throw new BadParameterError(`Calibration status change require a reason when changing from ${RunCalibrationStatus.FAILED}`);
                }
