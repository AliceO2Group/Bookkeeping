...
class RunService {
    ...
    async create(newRun, relations) {
        const { runTypeName = null, user_o2_start = null, user_o2_stop = null } = relations || {};
        ...
        if (user_o2_start?.externalId) {
            ...
            const user = await userService.getOrCreate({ externalUserId: user_o2_start.externalId }, user_o2_start.name);
            newRun.user_o2_start = user.id;
        }
        if (user_o2_stop?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: user_o2_stop.externalId }, user_o2_stop.name);
            newRun.user_o2_stop = user.id;
        }
    ...
    async update(identifier, payload) {
        ...
        const { tagsTexts = null, eorReasons = null, runTypeName = null,
            ...
            user_o2_start = null, user_o2_stop = null } = relations;
        ...
        if (user_o2_start?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: user_o2_start.externalId }, user_o2_start.name);
            runPatch.user_o2_start = user.id;
        }
        if (user_o2_stop?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: user_o2_stop.externalId }, user_o2_stop.name);
            runPatch.user_o2_stop = user.id;
        }
