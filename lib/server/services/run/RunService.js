...
const { getAllDetectorsByTextOrFail } = require('../detector/getAllDetectorsByTextOrFail');
...
class RunService {
     ...
     */
    async create(newRun) {
        // Save the old detector values
        newRun.concatenatedDetectors = newRun.detectors;
        const detectors = newRun.detectors ? await getAllDetectorsByTextOrFail(newRun.detectors.split(',')) : null;
    ...
    async update(identifier, runPatch, relations) {
        const { tagsTexts = null, eorReasons = null, runTypeName = null } = relations || {};
