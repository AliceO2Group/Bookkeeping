...
const { getOrCreateLhcPeriod } = require('../period/getOrCreateLhcPeriod.js');
...
class RunService {
    ...
    async update(identifier, payload) {
        ...
        const { tagsTexts = null, eorReasons = null, runTypeName = null,
            userIdentifier = {}, detectorsQualities = [], lhcPeriodName = null } = relations;
...
const updateEorReasonsOnRun = async (runId, userName, eorReasonPatches) => {
    ...
    // Determine the format of the eorReasonPatches (can be reasonTypeId || category and title)
    if (eorReasonPatches[0]?.reasonTypeId) {
        const reasonTypeIds = reasonTypes.map((reason) => reason.id);
        const allReceivedReasonsExists = eorReasonPatches.every((eorReasonPatch) => reasonTypeIds.includes(eorReasonPatch.reasonTypeId));

        if (!allReceivedReasonsExists) {
            throw new Error('Provided reason types do not exist');
        }
    } else if (eorReasonPatches[0]?.category && eorReasonPatches[0]?.title) {
        // Create a map of the category + title (stringified to avoid conflicts) with id as value
        const reasonTypesMap = new Map();
        reasonTypes.forEach((reason) => {
            reasonTypesMap.set(JSON.stringify(reason.category + reason.title), reason.id);
        });

        // Check if every EorReason is valid and update to reasonTypeId
        for (let i = 0; i < eorReasonPatches.length; i++) {
            // Get the reasonTypeId from the created map
            const reasonTypeId = reasonTypesMap.get(JSON.stringify(eorReasonPatches[i].category + eorReasonPatches[i].title));

            if (!reasonTypeId) {
                throw new Error('Provided reason types do not exist');
            } else {
                // Update the eorReason with the reasonTypeId for saving to the db later
                eorReasonPatches[i] = {
                    id: eorReasonPatches[i].id,
                    reasonTypeId,
                    description: eorReasonPatches[i].description,
                };
            }
        }
    }
