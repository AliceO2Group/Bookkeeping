...
const { models: { ReasonType }, logger } = require('../../../database');
...
const updateEorReasonsOnRun = async (runId, userName, eorReasonPatches) => {
    ...
    const reasonTypesMap = new Map(reasonTypes
        ...
        // Create a new map with all sub-maps
        .flat());

    const validPatches = [];
    for (const patch of eorReasonPatches) {
        // Get the reasonTypeId from the map with reasonTypeId and category+title keys
        ...
        if (reasonTypeId !== null) {
            // Set the reasonTypeId for adding to the db later
