...
class RunService {
    ...
    async create(newRun, relations) {
        ...
        newRun.userIdO2Start = await getOrCreateUser(userO2Start);
        newRun.userIdO2Stop = await getOrCreateUser(userO2Stop);
    ...
    async update(identifier, payload) {
        ...
        // Update the users for the run
        runPatch.userIdO2Start = await getOrCreateUser(userO2Start);
        runPatch.userIdO2Stop = await getOrCreateUser(userO2Stop);
 ...
 *
 * @param {User|null} userData - user object, either userO2Start or userO2Stop
...
const getOrCreateUser = async (userData) => {
    if (userData?.externalId) {
        ...
        const user = await userService.getOrCreate({ externalUserId: userData.externalId }, userData.name);
        return user.id;
    }
    return null;
};
