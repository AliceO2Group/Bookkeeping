...
/**
 * @typedef {import('@aliceo2/web-ui').Logger} Logger a logger instance
 */
...
class RunService {
            ...
            if (existingRun) {
                ...
                if (rawConfiguration) {
                    try {
                        ...
                        runDataFromConfiguration = this._extractRunDataFromEnvironmentConfiguration(
                            configuration,
    ...
    async create(newRun, relations) {
        ...
        if (userO2Start) {
            ...
            console.log('Set start ', userO2Start, newRun.userIdO2Start);
        }
        if (userO2Stop) {
            newRun.userIdO2Stop = (await getOrCreateUser(userO2Stop))?.id;
            console.log('Set stop ', userO2Stop, newRun.userIdO2Stop);
        }
    ...
    async update(identifier, payload) {
        ...
        if (userO2Start) {
            ...
            console.log('Applied user start in update', userO2Start, runPatch.userIdO2Start);
        }
        if (userO2Stop) {
            runPatch.userIdO2Stop = (await getOrCreateUser(userO2Stop))?.id;
            console.log('Applied user stop in update', userO2Stop, runPatch.userIdO2Stop);
        }
        ...
        await dataSource.transaction(async (transaction) => {
            ...
            if (detectorsQualities.length > 0) {
                ...
                await this._logRunDetectorQualityChange(run.runNumber, updatedRunDetectors, transaction, user, detectorsQualitiesChangeReason);
            }
        });
     ...
     * @private
     */
    _logRunDetectorQualityChange = async (runNumber, runDetectors, transaction, user, justification) => {
     ...
     * @return {Partial<Run>} the partial run
     * @private
     */
    _extractRunDataFromEnvironmentConfiguration(configuration, onError) {
        ...
        let detectors = [];
        try { detectors = configuration.getArray('detectors'); } catch (e) { onError(e); }
        let hosts = [];
        try { hosts = configuration.getArray('hosts'); } catch (e) { onError(e); }

        let ctp = false;
        try { ctp = configuration.getBool('trg_global_run_enabled'); } catch (e) { onError(e); }
        let triggerEnabled = false;
        try { triggerEnabled = configuration.getBool('trg_enabled'); } catch (e) { onError(e); }
        ...
        partialRun.runType = configuration.getString('run_type');

        try { partialRun.dd_flp = configuration.getBool('dd_enabled'); } catch (e) { onError(e); }
        try { partialRun.dcs = configuration.getBool('dcs_enabled'); } catch (e) { onError(e); }
        try { partialRun.epn = configuration.getBool('odc_enabled'); } catch (e) { onError(e); }
        ...
        partialRun.lhcPeriod = configuration.getString('lhc_period');
        ...
        partialRun.odcTopologyFullName = configuration.getString('odc_n_epns');
