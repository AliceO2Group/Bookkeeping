...
class RunService {
            ...
            if (existingRun) {
                ...
                if (rawConfiguration) {
                    try {
                        ...
                        const { partialRun, relations } = this._extractRunDataFromEnvironmentConfiguration(
                            configuration,
                            (e) => this._logger.errorMessage(e.message, { environmentId, runNumber }),
                        );
                        runDataFromConfiguration = partialRun;
                        flpRoles = relations.hosts.map((host) => ({ name: host, hostname: host, runNumber }));
                    } catch (e) {
    ...
    async create(newRun, relations) {
        const { runTypeName = null, userO2Start = null, userO2Stop = null, flpRoles } = relations || {};
