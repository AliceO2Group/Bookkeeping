 ...
 * @private
 */
const logRunDetectorQualityChange = async (runNumber, runDetectors, transaction, user) => {
...
class RunService {
    ...
    async update(identifier, payload) {
        const { runPatch = {}, relations = {} } = payload;
        ...
        await TransactionHelper.provide(async (transaction) => {
            const run = await updateRun(
                identifier,
                { runPatch: runAdapter.toDatabase(runPatch), relations: { tags, user } },
                transaction,
            );
            ...
            if (detectorsQualities.length > 0) {
                ...
                await logRunDetectorQualityChange(run.runNumber, updatedRunDetectors, transaction, user);
            }
        });
