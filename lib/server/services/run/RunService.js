...
class RunService {
    ...
    async create(newRun, relations) {
        ...
        if (userO2Start?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: userO2Start.externalId }, userO2Start.name);
            newRun.userIdO2Start = user.id;
        }
        if (userO2Stop?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: userO2Stop.externalId }, userO2Stop.name);
            newRun.userIdO2Stop = user.id;
        }
    ...
    async update(identifier, payload) {
        ...
        if (userO2Start?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: userO2Start.externalId }, userO2Start.name);
            runPatch.userIdO2Start = user.id;
        }
        if (userO2Stop?.externalId) {
            // Create the user if it does not yet exist
            const user = await userService.getOrCreate({ externalUserId: userO2Stop.externalId }, userO2Stop.name);
            runPatch.userIdO2Stop = user.id;
        }
