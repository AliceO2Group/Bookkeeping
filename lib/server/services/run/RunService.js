...
class RunService {
    ...
    async create(newRun, relations) {
        ...
        newRun.userIdO2Start = await getUserIdFromExternalId(userO2Start?.externalId, userO2Start?.name);
        newRun.userIdO2Stop = await getUserIdFromExternalId(userO2Stop?.externalId, userO2Stop?.name);
    ...
    async update(identifier, payload) {
        ...
        // Update the users for the run
        runPatch.userIdO2Start = await getUserIdFromExternalId(userO2Start?.externalId, userO2Start?.name);
        runPatch.userIdO2Stop = await getUserIdFromExternalId(userO2Stop?.externalId, userO2Stop?.name);
 ...
 *
 * @param {number} externalId - external identifier of the user
 * @param {string|null} name - name of the user or empty
 * @returns {Promise<number|null>} - returns the id of the user or null if the userData is null
 */
const getUserIdFromExternalId = async (externalId, name) => {
    if (externalId) {
