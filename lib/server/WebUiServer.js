...
class WebUiServer {
    ...
    constructor() {
        ...
        [this.http.router, this.http.routerPublic, this.http.routerStatics].forEach(function a(router) {
            // eslint-disable-next-line prefer-arrow-callback
            router.use(async function b(request, response, next) {
                ...
                } else if (process.env.NODE_ENV === 'test access') {
                    request.session = {
                        personid: 0,
                        id: 0,
                        name: 'Anonymous',
