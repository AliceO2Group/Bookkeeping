...
class WebUiServer {
    ...
    constructor() {
        ...
        console.log(`Token for signature ${this.http.o2TokenService._secret}`);
        console.log(this.http.o2TokenService.generateToken(1, 'Martin', 'martin', 'admin'));
        ...
        [this.http.router, this.http.routerPublic, this.http.routerStatics].forEach((router) => {
            router.use(async (request, response, next) => {
                ...
                if (process.env.NODE_ENV === 'test' && request.query?.token) {
                    user.access = request.query.token === 'admin' ? ['admin'] : ['guest'];
                }
