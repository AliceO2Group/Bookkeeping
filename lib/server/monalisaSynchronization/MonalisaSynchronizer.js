...
const { repositories: { DataPassRepository, LhcPeriodRepository, RunRepository } } = require('../../database/index.js');
    ...
    async synchronizeDataPassesFromMonalisa() {
        ...
        try {
            ...
            const transactionablePipeline = async (dataPass) => {
                const sequelizeLhcPeriod = await LhcPeriodRepository.findOne({ where: { name: extractLhcPeriod(dataPass.name).name } });
                const [dataPassDB] = await DataPassRepository.upsert({ lhcPeriodId: sequelizeLhcPeriod.id, ...dataPass });
                ...
                runs.forEach((run) => run.addDataPass(dataPassDB.id, { ignoreDuplicates: true }));
            };
