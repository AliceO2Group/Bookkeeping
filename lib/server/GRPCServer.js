...
class GRPCServer {
    ...
    constructor() {
        ...
        for (const [protoName, implementations] of Object.entries(servicesImplementationsMappings)) {
            ...
            implementations.forEach(({ service, implementation }) => {
                // Context is the namespace parts that lead to the service definition
                const absoluteServiceKey = service.split('.');
                absoluteServiceKey.pop();
                this._addServiceImplementation(
                    absoluteServiceKey.join('.'),
                    proto[service],
                    implementation,
     ...
     * @param {Map<string, Object>} absoluteMessagesDefinitions the map of each message definitions and their nested definitions recursively
     * @return {void}
     * @private
     */
    _addServiceImplementation(namespace, definition, implementation, absoluteMessagesDefinitions) {
        this._server.addService(
            definition,
            bindGRPCController(namespace, definition, implementation, absoluteMessagesDefinitions),
        );
    }
}
