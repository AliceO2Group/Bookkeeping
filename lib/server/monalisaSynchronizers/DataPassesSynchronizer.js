...
class DataPassesSynchronizer {
    ...
    async sync() {
        ...
        this.context.lastRuns = await this._getLastRunsInformation();
        ...
        try {
            ...
            this.context.lastRuns = null;
        }
    }

    /**
     * Fetch lastRun information telling whether some data pass must be updated
     * lastRun is the greatest run number associated with given data pass
     * @return {{lastRuns: Object<string, number>}} last runs
     */
    ...
    _doesDataPassNeedUpdate(dataPass, lhcPeriod) {
        const isYearOutOfBound = lhcPeriod.year < this.context.yearLowerLimit;
        const isLastRunNumberSame = dataPass.lastRun && dataPass.lastRun === this.context.lastRuns[dataPass.name];
