...
class DataPassesSynchronizer {
     ...
     * @param {{lastRuns: Object<string, number>}} context holding information about last runs associated with data passes
     * @return {boolean} true if it is needed to update data pass
     */
    _doesDataPassNeedUpdate(dataPass, lhcPeriod, context) {
        const isYearOutOfBound = lhcPeriod.year < context.yearLowerLimit;
        const isLastRunNumberSame = dataPass.lastRun && dataPass.lastRun === context.lastRuns[dataPass.name];
    ...
    /**
     * Is used to process server response payload to database entites, remove unnecessary data
     ...
     */
    async _processRawResponse(payload, context) {
