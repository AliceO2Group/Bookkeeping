 ...
 *
 * @param {MagnetsConfigurationFormModel} magnetsConfigurationFormModel the magnets configuration form model
 * @return {Component} the intermediate magnet configuration form
 */
const intermediatesMagnetConfigurationForm = (magnetsConfigurationFormModel) => {
    const entries = Object.entries(magnetsConfigurationFormModel.intermediates)
        .filter(([, value]) => value !== null);
    entries.sort((
        [, { timestamp: a }],
        [, { timestamp: b }],
    ) => (a || '99:99:99').localeCompare(b || '99:99:99'));
    ...
    return entries.map(([key, magnetConfigurationSnapshot]) => h(
        ...
        [
            ...
            // eslint-disable-next-line no-return-assign
            h('.btn.btn-danger', { onclick: () => magnetsConfigurationFormModel.dropIntermediate(Number(key)) }, iconTrash()),
        ],
    ));
};
 ...
 * @param {{magnets: MagnetsConfigurationFormModel}} shiftData.typeSpecific SL type-specific shift data
    ...
    return h('#type-specific.flex-column.g3', [
        h('.panel', [
            ...
            h('.flex-column.p2.pv3.g3', [
                ...
                intermediatesMagnetConfigurationForm(formData.typeSpecific.magnets),
                h('.flex-row.items-center.g2', [
                    h(
                        '#magnets-add.button.btn.btn-primary',
                        {
                            onclick: () => formData.typeSpecific.magnets.addIntermediate(),
                        },
                        'Add',
                    ),
                ]),
