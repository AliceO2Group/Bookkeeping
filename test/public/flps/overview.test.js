...
const {
    ...
    waitForTableLength,
    goToPage,
    getInnerText, fillInput, takeScreenshot,
} = require('../defaults.js');
...
module.exports = () => {
    ...
    it('dynamically switches between visible pages in the page selector', async () => {
        await pressElement(page, amountSelectorButtonSelector);
        await fillInput(page, `${amountSelectorSelector} input`, '1');
        await takeScreenshot(page, 'step1');
        console.log(await page.evaluate(async () => {
            const first = model.flps.pagination.customItemsPerPage;
            const middle = document.querySelector('#amountSelector input').value;
            document.querySelector('#amountSelector input').blur();
            await (new Promise(res => setTimeout(() => res(), 2500)));
            const second = model.flps.pagination.itemsPerPage;
            return `${first} - ${middle} - ${second}`;
        }));
        console.log(await page.evaluate(() => {
            return model.flps.pagination.itemsPerPage;
        }));
        await takeScreenshot(page, 'step2');
    ...
    it('notifies if table loading returned an error', async () => {
        await pressElement(page, amountSelectorButtonSelector);
        await fillInput(page, `${amountSelectorSelector} input`, '200');
