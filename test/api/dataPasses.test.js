...
module.exports = () => {
    ...
    describe('GET /api/dataPasses', () => {
        ...
        it('should successfully include TEST productions', async () => {
            const response = await request(server).get('/api/dataPasses?filter[lhcPeriodIds][]=2&filter[includeByName]=test');
        ...
        it('should successfully include DEBUG productions', async () => {
            const response = await request(server).get('/api/dataPasses?filter[lhcPeriodIds][]=2&filter[includeByName]=debug');
            expect(response.status).to.be.equal(200);
            const { data } = await response.body;
